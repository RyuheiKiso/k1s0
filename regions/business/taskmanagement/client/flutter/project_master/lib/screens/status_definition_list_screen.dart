...
/// ステータス定義バージョン履歴画面
/// ステータス定義の変更履歴を時系列で表示する監査証跡画面
class StatusDefinitionListScreen extends ConsumerWidget {
  /// 対象ステータス定義のID
  final String statusDefinitionId;

  const StatusDefinitionListScreen({
    super.key,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    /// バージョン履歴の状態を監視する
    final versionsAsync = ref.watch(versionListProvider(statusDefinitionId));

    return Scaffold(
      appBar: AppBar(
        title: Text('バージョン履歴: $statusDefinitionId'),
        actions: [
          /// 履歴を再読み込みするボタン
          IconButton(
            ...
            tooltip: '更新',
            onPressed: () => ref
                .read(versionListProvider(statusDefinitionId).notifier)
                .load(),
          ),
        ],
      ),
