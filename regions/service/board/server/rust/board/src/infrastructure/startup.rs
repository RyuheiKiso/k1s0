...
pub async fn run() -> anyhow::Result<()> {
...

    {
        ...
        loop {
            let locked: (bool,) = sqlx::query_as("SELECT pg_try_advisory_lock(1000000011)")
            ...
            if start.elapsed() > lock_timeout {
                anyhow::bail!("advisory lock timeout after 30s (lock ID: 1000000011)");
            }
    ...
    let auth_state_opt = cfg.auth
        ...
        .map(|auth_cfg| -> anyhow::Result<AuthState> {
            ...
            info!(jwks_url = %jwks_url, "initializing JWKS verifier for board-server");
