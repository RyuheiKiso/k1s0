-- board_service の RLS ポリシーに FORCE + AS RESTRICTIVE + WITH CHECK を追加する。
-- HIGH-BIZ-002 監査対応: 003 マイグレーションで RLS は設定されているが、
-- FORCE ROW LEVEL SECURITY・AS RESTRICTIVE・WITH CHECK が欠落していた。
-- FORCE により テーブルオーナーにも RLS を適用し、AS RESTRICTIVE で他ポリシーと AND 結合する。
-- WITH CHECK により INSERT/UPDATE 時のテナント検証を強制する。

BEGIN;

SET LOCAL search_path TO board_service, public;

-- board_columns テーブルに FORCE ROW LEVEL SECURITY を追加する
ALTER TABLE board_service.board_columns FORCE ROW LEVEL SECURITY;

-- board_columns の tenant_isolation ポリシーを AS RESTRICTIVE + WITH CHECK 付きで再作成する
DROP POLICY IF EXISTS tenant_isolation ON board_service.board_columns;
CREATE POLICY tenant_isolation ON board_service.board_columns
    AS RESTRICTIVE
    USING (tenant_id = current_setting('app.current_tenant_id', true))
    WITH CHECK (tenant_id = current_setting('app.current_tenant_id', true));

COMMIT;
