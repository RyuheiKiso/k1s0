fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    {
        Ok(()) => {
            println!("cargo:warning=tonic-build succeeded for inventory proto");
        }
        Err(e) => {
            println!(
    ...
    let event_proto_file =
        "../../../../../../api/proto/k1s0/event/service/inventory/v1/inventory_events.proto";
    ...
    {
        Ok(()) => {
            println!("cargo:warning=tonic-build succeeded for inventory event proto");
        }
        Err(e) => {
            println!(
