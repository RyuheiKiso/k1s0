-- マルチテナント対応: inventory_items / inventory_reservations テーブルに tenant_id カラムを追加し、RLS ポリシーを設定する。
-- 設計根拠: docs/architecture/multi-tenancy.md Phase 1 対応。
-- 既存データは tenant_id = 'system' でバックフィルし、NOT NULL 制約を維持する。
-- RLS ポリシーにより app.current_tenant_id セッション変数でテナントを分離する。

SET search_path TO inventory_service;

-- inventory_items テーブルに tenant_id カラムを追加する（既存データのバックフィルとして 'system' をデフォルト値とする）
ALTER TABLE inventory_items
    ADD COLUMN IF NOT EXISTS tenant_id TEXT NOT NULL DEFAULT 'system';

-- inventory_reservations テーブルにも tenant_id を追加する（親テーブル inventory_items と整合性を保つ）
ALTER TABLE inventory_reservations
    ADD COLUMN IF NOT EXISTS tenant_id TEXT NOT NULL DEFAULT 'system';

-- tenant_id のインデックスを追加してクエリ性能を確保する
CREATE INDEX IF NOT EXISTS idx_inventory_tenant_id ON inventory_items (tenant_id);
CREATE INDEX IF NOT EXISTS idx_inventory_tenant_product ON inventory_items (tenant_id, product_id);
CREATE INDEX IF NOT EXISTS idx_reservations_tenant_id ON inventory_reservations (tenant_id);

-- inventory_items テーブルの RLS を有効化する
ALTER TABLE inventory_items ENABLE ROW LEVEL SECURITY;

-- テナント分離ポリシーを設定する（app.current_tenant_id セッション変数でフィルタリング）
-- current_setting の第 2 引数 true = 変数未設定時に NULL を返すことでエラーを回避する
DROP POLICY IF EXISTS tenant_isolation ON inventory_items;
CREATE POLICY tenant_isolation ON inventory_items
    USING (tenant_id = current_setting('app.current_tenant_id', true)::TEXT);

-- inventory_reservations テーブルの RLS を有効化する
ALTER TABLE inventory_reservations ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS tenant_isolation ON inventory_reservations;
CREATE POLICY tenant_isolation ON inventory_reservations
    USING (tenant_id = current_setting('app.current_tenant_id', true)::TEXT);
