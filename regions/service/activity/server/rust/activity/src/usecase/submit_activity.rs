...
impl SubmitActivityUseCase {
    ...
    pub async fn execute(&self, tenant_id: &str, id: Uuid, actor_id: &str) -> anyhow::Result<Activity> {
        let activity = self
            .repo
...
mod tests {
    ...
    async fn test_submit_success_active_to_submitted() {
        ...
        mock.expect_update_status()
            .times(1)
            .returning(move |_, _, _, _| Ok(submitted_clone.clone()));

        let uc = SubmitActivityUseCase::new(Arc::new(mock));
        let result = uc.execute("tenant1", activity_id, "user1").await;
        assert!(result.is_ok());
    ...
    async fn test_submit_not_found() {
        ...
        let uc = SubmitActivityUseCase::new(Arc::new(mock));
        let result = uc.execute("tenant1", Uuid::new_v4(), "user1").await;
        assert!(result.is_err());
