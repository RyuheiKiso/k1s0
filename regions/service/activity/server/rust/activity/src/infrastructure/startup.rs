...
pub async fn run() -> anyhow::Result<()> {
...

    {
        ...
        // Advisory Lock ID: 1000000012 (activity-service)
        ...
        loop {
            let locked: (bool,) = sqlx::query_as("SELECT pg_try_advisory_lock(1000000012)")
            ...
            if start.elapsed() > lock_timeout {
                anyhow::bail!("advisory lock timeout after 30s (lock ID: 1000000012)");
            }
    ...
    let auth_state_opt = cfg.auth
        ...
        .map(|auth_cfg| -> anyhow::Result<AuthState> {
            info!(jwks_url = %auth_cfg.jwks_url, "initializing JWKS verifier for activity-server");
