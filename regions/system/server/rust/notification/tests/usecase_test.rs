...
mod get_template {
    ...
    async fn success_returns_existing_template() {
        ...
        let result = uc.execute(&template_id, "system").await;
        assert!(result.is_ok());
    ...
    async fn not_found_for_missing_template() {
        ...
        let result = uc.execute("tpl_nonexistent", "system").await;
        assert!(result.is_err());
        match result.unwrap_err() {
...
mod list_templates {
    ...
    async fn success_returns_empty_list() {
        ...
        let result = uc.execute("system").await;
        assert!(result.is_ok());
        assert!(result.unwrap().is_empty());
    }
    ...
    async fn paginated_filters_by_channel_type() {
        ...
        let (items, total) = uc
            .execute_paginated("system", 1, 10, Some("email".to_string()))
            .await
            .unwrap();
        assert_eq!(total, 2);
    ...
    async fn paginated_respects_page_size() {
        ...
        let (items, total) = uc.execute_paginated("system", 1, 2, None).await.unwrap();
        assert_eq!(total, 3);
        assert_eq!(items.len(), 2);

        let (items, _) = uc.execute_paginated("system", 2, 2, None).await.unwrap();
        assert_eq!(items.len(), 1);
    }
}
...
mod delete_template {
    ...
    async fn internal_error_on_repo_failure() {
        ...
        let result = uc.execute("tpl_any", "system").await;
        assert!(result.is_err());
        match result.unwrap_err() {
