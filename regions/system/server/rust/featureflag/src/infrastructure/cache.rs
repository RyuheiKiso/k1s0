...
impl FlagCache {
    ...
    fn cache_key(tenant_id: Uuid, flag_key: &str) -> String {
        format!("{tenant_id}:{flag_key}")
    }

    /// tenant_id + flag_key に対応するフラグを取得する。
    /// キャッシュミスの場合は None を返す。
    pub async fn get(&self, tenant_id: Uuid, flag_key: &str) -> Option<Arc<FeatureFlag>> {
    ...
    pub async fn invalidate(&self, tenant_id: Uuid, flag_key: &str) {
        self.inner
...
mod tests {
    ...
    fn make_flag(tenant_id: Uuid, flag_key: &str, enabled: bool) -> Arc<FeatureFlag> {
        Arc::new(FeatureFlag {
            id: Uuid::new_v4(),
            tenant_id,
    ...
    async fn test_tenant_isolation_in_cache() {
        let cache = FlagCache::new(100, 60);
        let tid1 = Uuid::parse_str("00000000-0000-0000-0000-000000000001").unwrap();
        let tid2 = Uuid::parse_str("00000000-0000-0000-0000-000000000002").unwrap();
