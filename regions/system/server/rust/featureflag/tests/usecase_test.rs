...
async fn create_flag_success() {
    ...
    let input = CreateFlagInput {
        tenant_id: system_tenant().to_owned(),
...
async fn get_flag_found() {
    ...
    let result = uc
        .execute(system_tenant(), "feature.dark-mode")
        .await
        .unwrap();
    assert_eq!(result.flag_key, "feature.dark-mode");
    assert!(result.enabled);
...
async fn get_flag_not_found() {
    ...
    let err = uc
        .execute(system_tenant(), "nonexistent")
        .await
        .unwrap_err();
...
async fn delete_flag_repo_error() {
    ...
    let err = uc
        .execute(system_tenant(), &Uuid::new_v4())
        .await
        .unwrap_err();
...
async fn flag_crud_workflow() {
    ...
    let fetched = get_uc
        .execute(system_tenant(), "feature.workflow")
        .await
        .unwrap();
