// AI Gateway認証ミドルウェア。
// BearerトークンをJWKSで検証し、Claims をリクエストエクステンションに格納する。
...
/// AI Gateway認証ミドルウェアが使用する共有状態。
#[derive(Clone)]
pub struct AiGatewayAuthState {
    pub verifier: Arc<JwksVerifier>,
}

/// Bearerトークンを検証し、Claimsをリクエストエクステンションに格納する認証ミドルウェア。
pub async fn auth_middleware(
    State(state): State<AiGatewayAuthState>,
