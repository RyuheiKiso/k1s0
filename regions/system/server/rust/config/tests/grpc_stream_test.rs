...
async fn test_watch_config_multiple_clients_receive_same_event() {
    ...
    let _ = uc.sender().send(make_event("system.auth", "timeout", 42));
...
async fn test_watch_config_multiple_clients_independent_namespace_filters() {
    ...
    let _ = uc.sender().send(make_event("system.config", "key1", 1));
    let _ = uc.sender().send(make_event("business.billing", "rate", 2));
...
async fn test_watch_config_subscriber_receives_only_post_subscription_events() {
    ...
    let _ = uc.sender().send(make_event("system.auth", "old-key", 1));
    ...
    // 購読後に新しいイベントを発行
    let _ = uc.sender().send(make_event("system.auth", "new-key", 2));
