...
use adapter::grpc::EventMonitorGrpcService;
use adapter::repository::event_record_postgres::EventRecordPostgresRepository;
use adapter::repository::flow_definition_postgres::FlowDefinitionPostgresRepository;
use adapter::repository::flow_instance_postgres::FlowInstancePostgresRepository;
use domain::entity::event_record::EventRecord;
use domain::entity::flow_definition::FlowDefinition;
use domain::entity::flow_instance::{FlowInstance, FlowInstanceStatus};
use domain::repository::{EventRecordRepository, FlowDefinitionRepository, FlowInstanceRepository};
use infrastructure::config::Config;
use infrastructure::dlq_client::{DlqManagerClient, NoopDlqClient};
...
async fn main() -> anyhow::Result<()> {
    ...
    let auth_state = k1s0_server_common::require_auth_state(
        ...
        cfg.auth.as_ref().map(|auth_cfg| {
            ...
            adapter::middleware::auth::EventMonitorAuthState {
    ...
    // gRPC server
    use proto::k1s0::system::event_monitor::v1::event_monitor_service_server::EventMonitorServiceServer;

    let grpc_tonic = adapter::grpc::EventMonitorServiceTonic::new(grpc_svc);
    ...
    let kafka_future = async {
        if let Some(ref kafka_cfg) = cfg.kafka {
            ...
            match infrastructure::kafka_consumer::EventKafkaConsumer::new(
                kafka_cfg,
                event_repo,
                flow_def_repo,
                flow_inst_repo,
