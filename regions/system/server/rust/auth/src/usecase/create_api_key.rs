...
fn generate_random_key() -> String {
    ...
    // OsRng は OS のエントロピーソース（Linux: getrandom/urandom, macOS: SecRandomCopyBytes）を
    // 直接使用するため暗号論的に安全な 192bit ランダムキーを生成できる。
...
fn hash_key(raw_key: &str) -> Result<String, CreateApiKeyError> {
    ...
    let pepper =
        std::env::var("API_KEY_PEPPER").map_err(|_| CreateApiKeyError::PepperNotConfigured)?;
...
fn compute_hmac_hex(raw_key: &str, pepper: &str) -> String {
    ...
    let mut mac = HmacSha256::new_from_slice(pepper.as_bytes())
        .expect("HMAC は任意長のキーを受け入れるためエラーにならない");
