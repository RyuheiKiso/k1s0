...
pub async fn run() -> anyhow::Result<()> {
    ...
    let dlq_repo: Arc<dyn crate::domain::repository::DlqMessageRepository> =
    ...
    let auth_state = k1s0_server_common::require_auth_state(
        ...
        cfg.auth.as_ref().map(|auth_cfg| {
            ...
            crate::adapter::middleware::auth::DlqAuthState {
...
struct InMemoryDlqMessageRepository {
    messages: tokio::sync::RwLock<Vec<crate::domain::entity::DlqMessage>>,
}
...
impl crate::domain::repository::DlqMessageRepository for InMemoryDlqMessageRepository {
    ...
    async fn create(&self, message: &crate::domain::entity::DlqMessage) -> anyhow::Result<()> {
        self.messages.write().await.push(message.clone());
        Ok(())
    }

    async fn update(&self, message: &crate::domain::entity::DlqMessage) -> anyhow::Result<()> {
