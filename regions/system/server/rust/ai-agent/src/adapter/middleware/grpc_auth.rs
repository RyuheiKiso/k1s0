// gRPC認証ミドルウェア
// gRPCリクエストのBearerトークンを検証し、RBAC権限チェックを行う
...
/// Claimsからアクションの権限を確認する
fn authorize_claims(claims: &Claims, action: &str) -> bool {
    check_system_permission(claims.realm_roles(), "agents", action)
}
