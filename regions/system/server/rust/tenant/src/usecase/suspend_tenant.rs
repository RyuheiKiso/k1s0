use std::sync::Arc;
use uuid::Uuid;

use crate::domain::entity::{Tenant, TenantStatus};
use crate::domain::repository::TenantRepository;
use crate::infrastructure::kafka_producer::{NoopTenantEventPublisher, TenantEventPublisher};
use crate::usecase::watch_tenant::TenantChangeEvent;

#[derive(Debug, thiserror::Error)]
pub enum SuspendTenantError {
    #[error("tenant not found: {0}")]
    NotFound(String),
    #[error("invalid status: {0}")]
    InvalidStatus(String),
    #[error("internal error: {0}")]
    Internal(String),
}

pub struct SuspendTenantUseCase {
    tenant_repo: Arc<dyn TenantRepository>,
    event_publisher: Arc<dyn TenantEventPublisher>,
    watch_sender: Option<tokio::sync::broadcast::Sender<TenantChangeEvent>>,
}

impl SuspendTenantUseCase {
    pub fn new(tenant_repo: Arc<dyn TenantRepository>) -> Self {
        Self {
            tenant_repo,
            event_publisher: Arc::new(NoopTenantEventPublisher),
            watch_sender: None,
        }
    }

    pub fn with_event_publisher(mut self, event_publisher: Arc<dyn TenantEventPublisher>) -> Self {
        self.event_publisher = event_publisher;
        self
    }

    pub fn with_watch_sender(
        mut self,
        sender: tokio::sync::broadcast::Sender<TenantChangeEvent>,
    ) -> Self {
        self.watch_sender = Some(sender);
        self
    }

    pub async fn execute(&self, tenant_id: Uuid) -> Result<Tenant, SuspendTenantError> {
        let mut tenant = self
            .tenant_repo
            .find_by_id(&tenant_id)
            .await
            .map_err(|e| SuspendTenantError::Internal(e.to_string()))?
            .ok_or_else(|| SuspendTenantError::NotFound(tenant_id.to_string()))?;

        if tenant.status != TenantStatus::Active {
            return Err(SuspendTenantError::InvalidStatus(format!(
                "tenant must be active to suspend, current status: {}",
                tenant.status.as_str()
            )));
        }

        tenant.status = TenantStatus::Suspended;

        self.tenant_repo
            .update(&tenant)
            .await
            .map_err(|e| SuspendTenantError::Internal(e.to_string()))?;

        if let Err(e) = self.event_publisher.publish_tenant_suspended(&tenant).await {
            tracing::warn!(
                tenant_id = %tenant.id,
                error = %e,
                "failed to publish tenant_suspended event"
            );
        }

        if let Some(sender) = &self.watch_sender {
            let _ = sender.send(TenantChangeEvent {
                tenant_id: tenant.id.to_string(),
                change_type: "SUSPENDED".to_string(),
                tenant_name: tenant.name.clone(),
                tenant_display_name: tenant.display_name.clone(),
                tenant_status: tenant.status.as_str().to_string(),
                tenant_plan: tenant.plan.as_str().to_string(),
            });
        }

        Ok(tenant)
    }
}

#[cfg(test)]
#[allow(clippy::unwrap_used)]
mod tests {
    use super::*;
    use crate::domain::entity::Plan;
    use crate::domain::repository::tenant_repository::MockTenantRepository;

    #[tokio::test]
    async fn test_suspend_tenant_success() {
        let mut mock = MockTenantRepository::new();
        let tenant_id = Uuid::new_v4();
        let tid = tenant_id;
        mock.expect_find_by_id()
            .withf(move |id| *id == tid)
            .returning(move |_| {
                Ok(Some(Tenant {
                    id: tenant_id,
                    name: "acme-corp".to_string(),
                    display_name: "ACME Corporation".to_string(),
                    status: TenantStatus::Active,
                    plan: Plan::Free,
                    owner_id: None,
                    settings: serde_json::json!({}),
                    keycloak_realm: None,
                    db_schema: None,
                    created_at: chrono::Utc::now(),
                    updated_at: chrono::Utc::now(),
                }))
            });
        mock.expect_update().returning(|_| Ok(()));

        let uc = SuspendTenantUseCase::new(Arc::new(mock));
        let tenant = uc.execute(tenant_id).await.unwrap();
        assert_eq!(tenant.status, TenantStatus::Suspended);
    }

    #[tokio::test]
    async fn test_suspend_tenant_not_active() {
        let mut mock = MockTenantRepository::new();
        let tenant_id = Uuid::new_v4();
        let tid = tenant_id;
        mock.expect_find_by_id()
            .withf(move |id| *id == tid)
            .returning(move |_| {
                Ok(Some(Tenant {
                    id: tenant_id,
                    name: "acme-corp".to_string(),
                    display_name: "ACME Corporation".to_string(),
                    status: TenantStatus::Provisioning,
                    plan: Plan::Free,
                    owner_id: None,
                    settings: serde_json::json!({}),
                    keycloak_realm: None,
                    db_schema: None,
                    created_at: chrono::Utc::now(),
                    updated_at: chrono::Utc::now(),
                }))
            });

        let uc = SuspendTenantUseCase::new(Arc::new(mock));
        let result = uc.execute(tenant_id).await;
        assert!(result.is_err());
        match result.unwrap_err() {
            SuspendTenantError::InvalidStatus(_) => {}
            e => panic!("unexpected error: {:?}", e),
        }
    }

    #[tokio::test]
    async fn test_suspend_tenant_not_found() {
        let mut mock = MockTenantRepository::new();
        mock.expect_find_by_id().returning(|_| Ok(None));

        let uc = SuspendTenantUseCase::new(Arc::new(mock));
        let result = uc.execute(Uuid::new_v4()).await;
        assert!(result.is_err());
        match result.unwrap_err() {
            SuspendTenantError::NotFound(_) => {}
            e => panic!("unexpected error: {:?}", e),
        }
    }
}
