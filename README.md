...
- Go 1.23+ / Node.js 22+ / Dart 3.4+（各言語で開発する場合）
...
```bash
# 10 の system サーバーを一括起動（初回はビルドに数分かかります）
...
# JWT 付きリクエスト → auth サーバーでトークン検証
curl -H "Authorization: Bearer ${TOKEN}" http://localhost:8083/api/v1/auth/introspect

# JWT なし → 401 Unauthorized
curl http://localhost:8083/api/v1/auth/introspect
```
...
| Kafka | 9092 | — | infra |
| Keycloak | 8180 | — | infra |
| auth | 8083 | 9083 | system |
| config | 8084 | 9084 | system |
| saga | 8085 | 9085 | system |
| dlq-manager | 8086 | 9086 | system |
| featureflag | 8087 | 9087 | system |
| ratelimit | 8088 | 9088 | system |
| tenant | 8089 | 9089 | system |
| vault | 8091 | 9091 | system |
