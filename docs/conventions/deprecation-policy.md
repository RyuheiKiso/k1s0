...
```json
{
  ...
  "deprecated": {
    ...
    "deadline": "2026-12-31",
    "reason": "新しい production-v2 domain に機能を統合しました"
  }
}
```
...
```bash
# domain を非推奨化
k1s0 domain deprecate --name production \
  --migrate-to production-v2 \
  --deadline 2026-12-31 \
  --reason "新しい production-v2 domain に機能を統合しました"

# 非推奨化を解除
k1s0 domain undeprecate --name production
```
...
{
  "deprecated": {
    "migrate_to": ["production-core", "production-scheduling"],
    "migration_guide": "https://docs.example.com/migration/production-split"
  }
}
```
...
```
[K044] WARNING: domain 'production' は非推奨です
...
```rust
// domain 内のコード
#[deprecated(since = "1.5.0", note = "Use production_v2::WorkOrder instead")]
pub struct WorkOrder { ... }
```

使用側での警告:
```
warning: use of deprecated struct `production::WorkOrder`: Use production_v2::WorkOrder instead
```
...
```bash
# 非推奨 domain の依存者を確認
k1s0 domain dependents --name production --deprecated-only

# 出力例
WARNING: domain 'production' is deprecated since 1.5.0

Features depending on deprecated 'production':
...
# 依存者がいないことを確認
k1s0 domain dependents --name production
# => No features depend on 'production'

# domain を削除（手動）
rm -rf domain/backend/rust/production
```
...
/// # Deprecated
///
/// `1.5.0` から非推奨です。代わりに [`production_v2::WorkOrder`] を使用してください。
#[deprecated(since = "1.5.0", note = "Use production_v2::WorkOrder instead")]
pub struct WorkOrder {
    // ...
}
...
impl WorkOrder {
    ...
    /// # Deprecated
    ///
    /// `1.5.0` から非推奨です。代わりに [`production_v2::WorkOrder::start`] を使用してください。
    #[deprecated(since = "1.5.0", note = "Use production_v2::WorkOrder::start instead")]
...
//
// Deprecated: Use production_v2.WorkOrder instead (since 1.5.0).
...
//
// Deprecated: Use production_v2.WorkOrder.Start instead (since 1.5.0).
...
/**
 * Work order entity.
 * @deprecated Since 1.5.0. Use WorkOrder from production-v2 instead.
 */
 ...
 * @deprecated Since 1.5.0. Use createWorkOrder from production-v2 instead.
 */
...
```markdown
# production から production-v2 への移行ガイド

## 概要

production domain は 1.5.0 で非推奨となり、2026-12-31 に削除されます。
このガイドでは production-v2 への移行手順を説明します。

## 主な変更点

1. WorkOrder.quantity の型が u32 から Quantity 値オブジェクトに変更
2. SchedulingService が別 domain (production-scheduling) に分離
...
+ use production_v2::domain::entities::WorkOrder;
\`\`\`
...
└── migration/
    ├── README.md                         # 移行ガイド一覧
    └── production-to-production-v2.md    # 個別の移行ガイド
```
