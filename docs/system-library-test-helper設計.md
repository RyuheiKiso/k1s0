...
**配置先**: `regions/system/library/csharp/test-helper/`
...
│   ├── TestHelper.csproj
│   ├── Containers/
│   │   ├── TestContainerBuilder.cs     # コンテナビルダー
│   │   ├── PostgresContainer.cs        # PostgreSQL コンテナ
│   │   ├── RedisContainer.cs           # Redis コンテナ
│   │   ├── KafkaContainer.cs           # Kafka コンテナ
│   │   └── KeycloakContainer.cs        # Keycloak コンテナ
│   ├── Jwt/
│   │   ├── JwtTestHelper.cs            # JWT トークン生成
│   │   └── TestClaims.cs              # テスト用クレーム
│   ├── Mocks/
│   │   ├── MockServerBuilder.cs        # モックサーバービルダー
│   │   └── SystemServerMocks.cs        # 各サーバーのモック定義
│   ├── Fixtures/
│   │   └── FixtureBuilder.cs           # フィクスチャビルダー
│   └── Assertions/
│       └── AssertionHelper.cs          # アサーションヘルパー
├── tests/
│   ├── TestHelper.Tests.csproj
│   └── Unit/
│       ├── JwtTestHelperTests.cs
│       └── AssertionHelperTests.cs
├── .editorconfig
└── README.md
```
...
| パッケージ | 用途 |
|-----------|------|
| Testcontainers.PostgreSql 4.2 | PostgreSQL コンテナ管理 |
| Testcontainers.Redis 4.2 | Redis コンテナ管理 |
| Testcontainers.Kafka 4.2 | Kafka コンテナ管理 |
| WireMock.Net 1.6 | HTTP モックサーバー |
| System.IdentityModel.Tokens.Jwt 8.0 | JWT トークン生成 |
| Bogus 35.6 | ランダムフィクスチャ生成 |

**名前空間**: `K1s0.System.TestHelper`

**主要クラス・インターフェース**:

| 型 | 種別 | 説明 |
|---|------|------|
| `TestContainerBuilder` | static class | コンテナ起動ヘルパー |
| `PostgresContainer` | class | PostgreSQL コンテナ（IAsyncDisposable） |
| `RedisContainer` | class | Redis コンテナ（IAsyncDisposable） |
| `JwtTestHelper` | class | テスト用 JWT トークン生成 |
| `TestClaims` | record | テスト用 JWT クレーム |
| `MockServerBuilder` | static class | system tier モックサーバー構築 |
| `FixtureBuilder` | static class | テスト用エンティティ生成 |
| `AssertionHelper` | static class | JSON 部分一致・イベントアサーション |

**主要 API**:

```csharp
namespace K1s0.System.TestHelper;

public static class TestContainerBuilder
{
    public static Task<PostgresContainer> PostgresAsync(
        string db = "testdb", string user = "testuser",
        CancellationToken ct = default);

    public static Task<RedisContainer> RedisAsync(CancellationToken ct = default);

    public static Task<KafkaContainer> KafkaAsync(CancellationToken ct = default);

    public static Task<KeycloakContainer> KeycloakAsync(
        string realm = "k1s0-test",
        CancellationToken ct = default);
}
...
{
    public static IWireMockServer NotificationServer();
    public static IWireMockServer RatelimitServer();
    public static IWireMockServer TenantServer();
}

public static class AssertionHelper
{
    public static void AssertJsonContains(string actual, string expected);
    public static void AssertEventEmitted(IEnumerable<object> events, string eventType);
}
```
...
```swift
// JWT ヘルパー
public struct TestClaims: Sendable {
...
public struct JwtTestHelper: Sendable {
    public init(secret: String, algorithm: JwtAlgorithm = .hs256)
...
public enum JwtAlgorithm: Sendable { case hs256, rs256 }

// モックサーバービルダー
public struct MockServerBuilder: Sendable {
    public static func notificationServer() -> MockServerHelper
    public static func ratelimitServer() -> MockServerHelper
    public static func tenantServer() -> MockServerHelper
}

public actor MockServerHelper {
    public func withSuccessResponse() -> Self
    public func withErrorResponse(status: Int, body: [String: Any]?) -> Self
    public func start() async throws -> MockServerSession
}

public struct MockServerSession: Sendable {
...
public struct FixtureBuilder: Sendable {
    ...
    public static func int(min: Int = 0, max: Int = .max) -> Int
}

// アサーションヘルパー
public struct AssertionHelper: Sendable {
    public static func assertJsonContains(_ actual: Any, _ expected: Any) throws
    public static func assertEventEmitted(in events: [Any], type eventType: String) throws
}
```
...
public enum TestHelperError: Error, Sendable {
    ...
    case mockServerFailed(underlying: Error)
...
**配置先**: `regions/system/library/python/test_helper/`
...
│       ├── containers.py     # TestContainerBuilder・各コンテナ型
│       ├── jwt.py            # JwtTestHelper・TestClaims dataclass
│       ├── mock_server.py    # MockServerBuilder・MockServer
│       ├── fixture.py        # FixtureBuilder
│       ├── assertion.py      # AssertionHelper
│       └── py.typed
└── tests/
    ├── test_containers.py
    └── test_jwt.py
...
| 型 | 種別 | 説明 |
|---|------|------|
| `TestContainerBuilder` | class | PostgreSQL・Redis・Kafka・Keycloak のコンテナ起動ヘルパー |
| `JwtTestHelper` | class | HS256/RS256 署名の JWT テストトークン生成 |
| `TestClaims` | dataclass | テスト用 JWT クレーム（sub・roles・tenant_id 等） |
| `MockServerBuilder` | class | system tier サーバーの HTTP モック構築 |
| `FixtureBuilder` | class | テスト用エンティティ生成・ランダム値注入 |
| `AssertionHelper` | class | JSON 部分一致・イベントアサーション |
...
from k1s0_test_helper import (
    TestContainerBuilder, JwtTestHelper, TestClaims,
    MockServerBuilder, FixtureBuilder, AssertionHelper,
)

# pytest fixture でコンテナを起動
@pytest.fixture(scope="session")
async def postgres():
    async with TestContainerBuilder.postgres(db="testdb") as container:
        yield container

@pytest.fixture(scope="session")
async def redis():
    async with TestContainerBuilder.redis() as container:
        yield container

# JWT テストトークン生成
jwt_helper = JwtTestHelper(secret="test-secret")

admin_token = jwt_helper.create_admin_token()
user_token = jwt_helper.create_user_token("user-123", roles=["user"])
custom_token = jwt_helper.create_token(TestClaims(
    sub="service-account",
    roles=["service"],
...
# system tier サーバーのモック
@pytest.fixture
async def notification_mock():
    async with MockServerBuilder.notification_server().with_success_response() as mock:
        yield mock

# JSON 部分一致アサーション
actual = {"id": "123", "status": "ok", "extra": "ignored"}
AssertionHelper.assert_json_contains(actual, {"id": "123", "status": "ok"})

# フィクスチャビルダー
user_id = FixtureBuilder.uuid()
...
| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| testcontainers[postgres,redis,kafka] | >=0.7 | コンテナ起動管理 |
| pyjwt[cryptography] | >=2.8 | JWT トークン生成 |
| respx | >=0.21 | httpx ベース HTTP モック |
| faker | >=33.0 | ランダムフィクスチャ生成 |
