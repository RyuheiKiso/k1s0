...
[認証認可設計.md](認証認可設計.md) および [RBAC設計.md](RBAC設計.md) に基づき、以下の方針で実装する。
...
全エンドポイントは [API設計.md](API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_POLICY_` とする。
...
[helm設計.md](helm設計.md) のサーバー用 Helm Chart を使用する。policy 固有の values は以下の通り。
