...
├── docker/                         # 共通 Docker 設定
│   └── base-images/
└── istio/                          # サービスメッシュ設定（Terraform Helm Provider で管理。詳細は terraform設計.md を参照）
    ├── gateway.yaml
...
├── devcontainer設計.md             # DevContainer 設定
...
- **依存方向は下位→上位の一方向のみ** — service → business → system。逆方向・同階層間の直接依存は禁止。ただし、tier-architecture.md に定義された例外ルール（メッセージング経由・API Gateway経由の間接通信）は許容される
