...
public struct StreamId: Hashable, Sendable {
    ...
    public init(aggregateType: String, aggregateId: String)
}
...
public struct EventEnvelope: Sendable {
    ...
    public let payload: Data        // JSON エンコード済みイベント本体
    public let metadata: Data       // JSON エンコード済みメタデータ
...
public protocol EventStore: Sendable {
    ...
    func exists(streamId: StreamId) async throws -> Bool
}
...
public struct Snapshot: Sendable {
    public let streamId: StreamId
    public let version: Int64
    public let stateData: Data      // JSON エンコード済み集約状態
    public let takenAt: Date
}

// スナップショットストアプロトコル
public protocol SnapshotStore: Sendable {
    func save(streamId: StreamId, version: Int64, stateData: Data) async throws
    func loadLatest(streamId: StreamId) async throws -> Snapshot?
}
```
...
public enum EventStoreError: Error, Sendable {
    case concurrencyConflict(expectedVersion: Int64, actualVersion: Int64)
    case streamNotFound(StreamId)
...
- `InMemoryEventStore` によるユニットテスト
- testcontainers（Docker）を使った PostgreSQL 統合テスト
- カバレッジ目標: 85%以上
