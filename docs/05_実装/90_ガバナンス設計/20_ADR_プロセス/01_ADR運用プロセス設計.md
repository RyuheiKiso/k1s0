...
本ファイルは k1s0 における Architecture Decision Record（ADR）の起票・レビュー・承認・改訂・廃止の運用プロセスを実装段階確定版として確定する。90 章方針の IMP-POL-POL-002（全技術決定の ADR 化）と IMP-POL-POL-007（緊急例外の 72 時間事後 ADR）を、`docs/02_構想設計/adr/` の物理配置、ADR 接頭辞体系、CODEOWNERS 強制、MADR テンプレート、関係性リンク（Supersedes / Superseded-by / Related-to）で具体化する。

![ADR ライフサイクル: 起票 → レビュー → 承認 → Accepted → 改訂 / Superseded](img/20_ADR_ライフサイクル.svg)
...
- リリース時点: ADR テンプレート確定、接頭辞体系、CODEOWNERS 強制、20 本規模で定常運用
- リリース時点: D-ADR（事後 ADR）プロセス確立、Radar との相互リンク自動化
- リリース時点: ADR と IMP-* ID の双方向リンク整合チェック自動化、廃止 ADR の参照検出
...
ADR 自体の merge には architecture-team（Security + SRE + DX の 3 名で構成）のうち 複数名以上の approve が必須となる。1 名の独断で technical decision が通る経路を構造的に塞ぐ。
...
- `Accepted`: 複数名以上の approve で merge された後、本状態に更新。IMP-* 実装が追従する
...
| 役割 | 責務 |
|---|---|
| Architecture team（Security + SRE + DX 3 名、共同 A） | ADR レビュー、複数名以上 approve、メタ ADR（ADR-0XXX）の更新 |
...
| ID | 主題 | 適用段階 |
|---|---|---|
| IMP-POL-ADR-020 | ADR 接頭辞体系 12 種の固定と予約 ID 一覧維持 | 採用初期 |
| IMP-POL-ADR-021 | ID 不変原則（接頭辞変更の改名禁止） | 採用初期 |
| IMP-POL-ADR-022 | MADR v3.0 + k1s0 拡張テンプレートと必須項目 9 個 | 採用初期 |
| IMP-POL-ADR-023 | CODEOWNERS 強制と ADR なし merge 拒否の CI ゲート | 採用初期 |
| IMP-POL-ADR-024 | D-ADR 72 時間ルールと追認プロセス | 採用初期 |
| IMP-POL-ADR-025 | ライフサイクル 4 状態（Proposed / Accepted / Deprecated / Superseded） | 採用初期 |
| IMP-POL-ADR-026 | ADR 削除禁止、系譜保持による試行錯誤の可視化 | 採用初期 |
| IMP-POL-ADR-027 | Technology Radar との双方向リンク運用 | 採用初期 |

## 対応 ADR / DS-SW-COMP / NFR

- [ADR-0001](../../../02_構想設計/adr/ADR-0001-istio-ambient-vs-sidecar.md)（Istio Ambient 選定）/ [ADR-0002](../../../02_構想設計/adr/ADR-0002-diagram-layer-convention.md)（図解レイヤ規約）/ [ADR-CICD-003](../../../02_構想設計/adr/ADR-CICD-003-kyverno.md)（Kyverno）
