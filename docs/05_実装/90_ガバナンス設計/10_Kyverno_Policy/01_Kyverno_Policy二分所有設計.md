...
- Phase 1a: validate / mutate / generate 3 類型ディレクトリ分離、CODEOWNERS 分離、audit 観測モードで初期セット 10 本を deploy
- Phase 1b: enforce モード昇格（観測 2 週間以上経た policy から順次）、policy exception の運用ルール確立
- Phase 1c: policy lifecycle 管理の自動化（違反率が閾値下回った policy の自動 enforce 提案）
...
validate ポリシーは Phase 1a で 10 本を初期セットとして deploy する（IMP-POL-KYV-012）。各 policy は 80 章 / 85 章 / 60 章の実装方針から演繹され、根拠 ADR と対応 IMP ID を policy の annotations に記録する。
...
mutate ポリシーは「開発者が意識せずとも統制要件を満たす」ための補助で、Platform/SRE が運用速度で判断する（IMP-POL-KYV-013）。初期 6 本を Phase 1a で deploy する。
...
generate ポリシーは「namespace 作成時に NetworkPolicy / ResourceQuota / LimitRange を自動生成する」類の補助で、Phase 1a で 4 本を deploy する（IMP-POL-KYV-014）。
...
- validate 10 本 / mutate 6 本 / generate 4 本の初期セットが Phase 1a 完了時点で deploy 済
...
| ID | 主題 | Phase |
|---|---|---|
| IMP-POL-KYV-010 | `deploy/kyverno/` を validate / mutate / generate の 3 類型で分離 | 1a |
| IMP-POL-KYV-011 | CODEOWNERS で所有権分離（validate=Security、mutate/generate=Platform/SRE） | 1a |
| IMP-POL-KYV-012 | validate 10 本の初期セット、80 章 / 85 章 / 60 章と構造結合 | 1a |
| IMP-POL-KYV-013 | mutate 6 本で補助系、開発者の書き忘れ救済 | 1a |
| IMP-POL-KYV-014 | generate 4 本で namespace 作成時の最低限統制自動付与 | 1a |
| IMP-POL-KYV-015 | audit 観測 2 週間 → enforce 昇格の段階運用 | 1a |
| IMP-POL-KYV-016 | PolicyException による例外適用、ADR 必須 | 1a |
| IMP-POL-KYV-017 | Kyverno 本体の HA 3 replica 構成と Renovate バージョン追従 | 1a |
| IMP-POL-KYV-018 | PolicyReport の OTel 連携、violation 件数の SLO 指標化 | 1a / 1b |
| IMP-POL-KYV-019 | enforce rollback Runbook（1 時間以内に audit 復帰） | 1a |
| IMP-POL-KYV-020 | validate / generate セットによる「付かず拒否」二重構造 | 1a |
| IMP-POL-KYV-021 | major 昇格の staging soak 2 週間、ADR-CICD-003 改訂での方針記録 | 1b |
| IMP-POL-KYV-022 | 違反率低下 policy の自動 enforce 提案（Phase 1c） | 1c |
