...
本ファイルは k1s0 における STRIDE 脅威モデルの物理配置・作成粒度・更新プロセス・緩和策リンクを実装フェーズ確定版として確定する。90 章方針の IMP-POL-POL-004（tier1 公開 11 API / 外部連携ごとの STRIDE）を、`docs/05_実装/90_ガバナンス設計/40_脅威モデル_STRIDE/` 配下の API 別ファイル、STRIDE 6 区分テンプレート、API 追加時の PR チェックリスト、85 章 / 80 章 / 60 章との双方向リンクで具体化する。

![STRIDE 6 区分と tier1 11 API の対応マトリクス](img/stride_tier1_11api_対応マトリクス.svg)
...
- Phase 1b: tier1 公開 11 API 分の STRIDE 脅威モデル初版作成、85 章 / 80 章 / 60 章との双方向リンク
- Phase 1c: 外部連携面（SAP / SaaS 等）5-8 件分の追加、年次レビュー定着
- Phase 2: 脅威モデルの自動検証（CI で「この API に tenant_id 検証 IMP が無い」を検出）
...
STRIDE の主要脅威を API ごとに事前に予測しておくと、初版作成の出発点になる（IMP-POL-STR-043）。以下は Phase 1b 初版の想定脅威一覧で、実装レビューで精緻化する。
...
これらは出発点で、実装レビューで Phase 1b 時点の精緻化を行う。各脅威は対応する NFR / IMP-* ID へ双方向リンクを張り、緩和策の実装状況をトレーサビリティマトリクスで追跡する。
...
- tier1 公開 11 API 分の STRIDE ファイル 11 個が Phase 1b 完了時点で存在
...
| ID | 主題 | Phase |
|---|---|---|
| IMP-POL-STR-040 | 作成単位（tier1 11 API / 外部連携 / tier 境界 / データ層） | 1b |
| IMP-POL-STR-041 | STRIDE 6 区分テンプレートと必須 5 項目 | 1b |
| IMP-POL-STR-042 | Microsoft STRIDE 原典準拠と全 6 区分網羅要求 | 1b |
| IMP-POL-STR-043 | tier1 API 別主要脅威例と出発点の定義 | 1b |
| IMP-POL-STR-044 | API 追加 / 変更 PR の STRIDE 更新必須チェックリスト | 1b |
| IMP-POL-STR-045 | 緩和策なし脅威の `受容` 明示記述 | 1b |
| IMP-POL-STR-046 | 年次全件レビューと diff ADR 起票 | 1c |
| IMP-POL-STR-047 | 外部連携面の追加脅威 5 種（なりすまし / replay / 改竄 / 漏洩 / DoS 転嫁） | 1c |
