...
本ファイルは k1s0 のガバナンス運用（Kyverno ポリシー / ADR プロセス / Technology Radar / 脅威モデル / 監査ログ / コンプライアンス）を実装する際に常に参照する 7 本の原則を定義する。採用検討通過後の「意思決定の可監査性」を 10 年維持するため、誰が・いつ・どの根拠で採用したかを ID 付きで残す運用を構造レベルで規定する。

![ガバナンス原則俯瞰](../img/90_Kyverno二分所有_ADR_Radar.svg)

## 原則が必要な理由

採用側組織のガバナンスは、実装者（B / D）と承認者（D）が異なる前提で機能する。この前提を実装層で維持しないと、以下の破綻が常態化する。
...
Radar 未運用状態では、個別プロジェクトの採用判断が組織記憶から切り離され、同じ失敗を別チームで繰り返す。半期更新のサイクルは 日本企業に多い人事異動サイクル（4 月 / 10 月）に合わせ、新規参画者が Radar を読めば「採用候補」「避けるべき技術」を 30 分で把握できる状態を維持する。
...
認証取得を目指さない場合でも、統制相当の実装は 採用側組織の内部監査・取引先監査で要求される。後から統制を遡及実装するのは破綻的なコストとなるため、リリース時点 までに 80% 以上をカバーする設計で進める。優先対応は以下の統制領域とする。
...
詳細な意思決定フロー図は [img/ガバナンス原則俯瞰.drawio](../img/90_Kyverno二分所有_ADR_Radar.drawio)（リリース時点点で svg 作成）を参照。
