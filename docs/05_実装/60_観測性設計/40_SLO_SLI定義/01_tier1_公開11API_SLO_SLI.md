...
tier1 公開 11 API は [`03_要件定義/20_機能要件/10_tier1_API要件/`](../../../03_要件定義/20_機能要件/10_tier1_API要件/) で一次的に列挙される。これを以下の SLO で固定する。リリース時点 時点の値で / で実測値に応じた再調整を行う（IMP-OBS-SLO-042）。

| API | 適用段階 | Availability 目標 | p99 目標 | Correctness / Freshness | Error Budget（月次） |
|---|---|---|---|---|---|
| Service Invoke | 採用初期 | 99.9% | 500 ms | — | 43 分 12 秒 |
| State | 採用初期 | 99.95% | 50 ms | — | 21 分 36 秒 |
| PubSub | 採用初期 | 99.9% | 100 ms（publish） | Freshness: 5 秒以内到達率 99% | 43 分 12 秒 |
| Secrets | 採用初期 | 99.95% | 200 ms | — | 21 分 36 秒 |
| Binding | 採用初期 | 99.5% | 2 s | — | 3 時間 36 分 |
| Workflow | 採用初期 | 99.9% | 1 s（state 遷移） | Correctness: 99.99%（Saga 補償漏れ 0） | 43 分 12 秒 |
| Log | 採用初期 | 99.9% | 20 ms | Freshness: 1 分以内到達率 99% | 43 分 12 秒 |
| Telemetry | 採用初期 | 99.9% | 20 ms | — | 43 分 12 秒 |
| Decision | 採用初期 | 99.9% | 100 ms | Correctness: 99.99%（ZEN Engine 評価誤差 0） | 43 分 12 秒 |
| Audit / PII | 採用初期 | 99.95% | 50 ms | Correctness: 99.999%（改ざん検知率）| 21 分 36 秒 |
| Feature | 採用初期 | 99.9% | 50 ms | Freshness: flag 反映 10 秒以内率 99% | 43 分 12 秒 |
...
SLO 値は「一度決めたら動かさない」運用は不健全である。実測値との乖離が大きい値を放置すると、Burn Rate Alert が鳴りすぎて疲弊する（SLO が厳しすぎる）か、逆に実害が出ても何も鳴らない（緩すぎる）状態に陥る。本節の SLO は リリース時点 時点で第 1 回改訂を行い、以降は四半期ごとに見直す（IMP-OBS-SLO-046）。
...
- IMP-OBS-SLO-042: tier1 公開 11 API の SLO 初期値固定と リリース時点 再調整
