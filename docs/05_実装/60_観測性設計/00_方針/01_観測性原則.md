...
観測性設計は「収集できるデータをとりあえず集める」運用に流れやすく、統制を失うと JTC の 10 年保守フェーズで破綻する。放置した場合の典型的な破綻は以下として顕在化する。
...
本原則は、これらの破綻を「計測・評価・対応」の各層で構造的に防止する 7 軸である。Google SRE Book の用語体系を採用し、JTC 特有のセキュリティ統合を Incident Taxonomy の統合分類として反映することが本章の核となる。
...
詳細なテレメトリ経路は [img/観測性7原則マップ.drawio](../img/60_LGTM_Pyroscope_OTel配置.drawio)（Phase B 完了時点で svg 作成）を参照。
