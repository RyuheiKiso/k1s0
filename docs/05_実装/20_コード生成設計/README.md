...
本章は k1s0 の Protobuf / OpenAPI / Scaffold CLI によるコード生成パイプラインを実装フェーズ確定版として固定する。ADR-DIR-001 で昇格した `src/contracts/` を単一真実源とし、ADR-TIER1-002 で統一した Protobuf gRPC 契約から 4 言語 SDK（Rust / Go / C# / TypeScript）および tier2 雛形を一貫した生成経路で再現可能に保つ。
...
- Phase 1a: OpenAPI 経由の外部連携層（BFF）
- Phase 1b: Scaffold テンプレートの Backstage catalog-info.yaml 自動生成
