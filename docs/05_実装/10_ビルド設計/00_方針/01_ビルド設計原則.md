...
k1s0 は ADR-TIER1-001（Go + Rust ハイブリッド）と ADR-TIER1-003（内部言語不可視）により、tier1 内部だけで 2 言語が並走する。加えて tier2 は C# / Go、tier3 は TypeScript / C# / Go、SDK は 4 言語同時配布という構造になっており、ビルド機構も自然と 4 つに増える。ここで「統一ビルドツールを入れたい」という誘惑は常に存在するが、採用側の小規模運用の初期段階で Bazel を導入すると学習コストが申請を通過した予算の多くを食い潰す。
...
本原則は、これらの破綻を構造レベルで回避し、かつ リリース時点 時点で Bazel 再評価が正しく行える計測点を設けるための 7 軸である。

## 原則 1: 言語ネイティブビルドを優先する（IMP-BUILD-POL-001）

**Cargo / go build / pnpm / dotnet の各言語ネイティブビルド機構を第一選択とし、Bazel / Buck2 / Pants 等の統一ビルドツールは 採用初期 では採用しない。**

構想設計 `00_ディレクトリ設計/00_設計方針/02_世界トップ企業事例比較.md` で確定したとおり、Google / Meta / Uber 規模では Bazel / Buck2 が選択ビルドのほぼ唯一解だが、k1s0 は 採用側の小規模運用から始まる。Bazel は BUILD ファイル・`rules_rust` / `rules_go` / `rules_nodejs` の習熟コストが 2 人月以上かかることが各社事例で報告されており、リリース時点 で投資すると機能提供が後退する。

リリース時点 のビルド時間計測（原則 6）で単言語ビルドが 30 分を超えた場合に限り、当該言語について Bazel 導入を新 ADR で再評価する。それ以外のタイミングで Bazel を導入する提案は、本原則により却下される。
...
- Go: `src/tier1/go/go.mod` / `src/tier2/go/services/<svc>/go.mod`（サービス単位）/ `src/sdk/go/go.mod` / `src/tier3/bff/go.mod` / `tests/e2e/owner/go.mod` / `tests/e2e/user/go.mod` の複数分離（e2e 2 module 分離は ADR-TEST-008 で確定）
...
- 第 2 層（CI）: GitHub Actions `actions/cache` で lockfile ハッシュをキーにした PR 間共有。リリース時点の主力
- 第 3 層（リモート）: sccache（Rust / C/C++） / Turbo Remote Cache（TypeScript）を リリース時点 で導入。self-hosted runner 間で共有

Go は公式のリモートキャッシュが無いため第 2 層止まり、dotnet も同様。これは リリース時点に再評価する。
...
計測が無いと Bazel 再評価の判断基準が感覚論になる。本原則は リリース時点 から計測点を設置し / での判断を可能にする。
