...
- **replica 数**: 3（Phase 0 から本番）
- **DataStore**: CloudNativePG（ADR-DATA-001）の `spire-db` cluster
- **署名 CA**: 自己署名（Phase 0）→ Phase 1b で intermediate CA として外部 root CA から派生
...
- **trust domain**: `k1s0.local`（Phase 0 単一）。Phase 1b で JTC 顧客オンプレと federation する際に `k1s0-customer-<id>.local` を追加
...
JTC 顧客オンプレに k1s0 のサブシステムを deploy するケース（Phase 1b）では、顧客側 SPIRE Server と k1s0 中央 SPIRE Server の相互信頼を Federation で構築する（IMP-SEC-SP-029）。trust domain を跨いだ SVID 検証が可能になり、k1s0 中央から顧客側 workload に mTLS で到達できる。
...
Federation は Phase 1b の重要決定であり、運用負荷が跳ねる。Phase 0 / 1a は単一 trust domain で閉じる。
...
ztunnel は Node ごとに 1 Pod で動作し、同 Node 上の全 Pod の L4 mTLS を終端する。追加の hop が生まれるため、latency 影響を Phase 0 段階で計測する（IMP-SEC-SP-034）。
...
- IMP-SEC-SP-029: Phase 1b Federation による trust domain 跨ぎ
