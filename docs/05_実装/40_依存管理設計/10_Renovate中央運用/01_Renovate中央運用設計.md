...
- Phase 0: GitHub App 経由で Renovate 稼働、週次 PR グループ化、手動マージ必須、AGPL 6 件の import path detector 稼働
- Phase 1a: patch レベル自動マージ開始、Dependency Dashboard 稼働、週次レビュー会定着
- Phase 1b: minor グループ PR の Argo Rollouts カナリア連動、Renovate self-host 検討（現行は GitHub App）
...
- `tools/ci/renovate/automerge.json5` : patch 自動マージ条件（Phase 1a 以降有効化）
...
Phase 0 は GitHub App `renovate[bot]` を採用し、self-host しない（IMP-DEP-REN-012）。self-host 案も検討したが、2 名運用で Renovate 自身の運用保守（DB / Redis / scheduler）を背負う余力はなく、Phase 1b までは GitHub App 依存で進める。AGPL ライセンスの Renovate 本体は GitHub App 経由で外部サービスとして利用するため、ADR-0003 の「別プロセス / 別ネットワーク境界で運用」条件を満たしており、自社コードへの AGPL 汚染は発生しない。

実行頻度は `schedule: ["before 6am on monday"]`（日本時間）を base 設定とする。毎日実行では PR 量が過大で、月次では rebase コスト過多となる。月曜早朝実行 → 日中レビュー → 火曜マージの週次サイクルを 2 名運用の現実的な吸収帯とする。セキュリティアラート経路（Dependabot 相当）は即時扱いで、CVE が CVSS 7.0 以上の依存は `schedule` を無視して即 PR を上げる `vulnerabilityAlerts` 設定を有効化する（IMP-DEP-REN-013）。
...
- **エコシステム別 minor**: 同様に 4 グループ、週 1 PR、人手レビュー（Phase 1a）
- **patch 統合**: 全 ecosystem 横断で 1 PR、Phase 1a 以降 quality gate 全 pass で自動マージ
...
自動マージは Phase 1a から patch レベルに限定して解禁する（IMP-DEP-REN-016）。30 章 `_reusable-quality-gate.yml` の全 check pass を必須条件とし、以下 6 条件を AND で満たす PR のみ bot 自身が merge する。
...
- Rust: `tokio` / `tonic` / `sqlx` などは major 更新時に必ず `labels: rust-major-review` を付与し、ZEN Engine / rdkafka など JTC 固有依存は個別レビュアを指定
...
Phase 0 / 1a 完了時点で以下を満たすことを本節の受け入れ基準とする。
...
- Phase 1a 以降、patch 自動マージが quality gate + 6 条件 AND で稼働
...
| IMP-DEP-REN-012 | Phase 0 は GitHub App 採用、self-host は Phase 1b 以降判断 | 0 |
...
| IMP-DEP-REN-016 | patch 自動マージを quality gate + 6 条件 AND で稼働 | 1a |
