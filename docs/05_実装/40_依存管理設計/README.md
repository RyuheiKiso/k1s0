...
本章は k1s0 の外部依存（crates.io / Go modules / npm / NuGet / サードパーティ vendoring）をどう取り込み、どう更新し、どう監査するかを実装フェーズ確定版として固定する。構想設計で選定された Renovate を軸に、lockfile 運用・`third_party/` vendoring・ライセンス監査を統合的に規定する。ADR-0003（AGPL 分離アーキテクチャ）と連動し、AGPL OSS 6 件（Grafana / Loki / Tempo / Pyroscope / MinIO / Renovate）は依存グラフ上で分離が維持されていることを恒常的に検証する。

## 本章の位置付け

2 名運用の JTC では、日々発生する依存更新 PR を人手で 1 件ずつレビューすると業務が破綻する。したがって「自動マージ可能な範囲」と「人手レビュー必須の範囲」を明確に線引きし、前者を Renovate + quality gate で吸収、後者だけに人手を集中させる運用を確定する。
...
- Phase 1a: 自動マージ範囲の拡張（patch レベルのみ）、SBOM 差分監視（80 章連携）
- Phase 1b: 依存グラフの可視化（脆弱性経路）
