...
本ファイルは k1s0 モノレポにおける外部依存の SPDX ライセンス判定と CI ゲートを実装段階確定版として確定する。40 章方針の IMP-DEP-POL-004（SPDX 明示 / BUSL / SSPL 自動拒否）および IMP-DEP-POL-005（AGPL 6 件の分離境界恒常検証）を、4 ecosystem（Rust / Go / TypeScript / .NET）の各ツール設定と CI workflow、および ADR 承認フローで実装する。ADR-0003（AGPL 分離アーキテクチャ）と連動し、非互換ライセンスの混入を構造的に拒否する設計である。

![ライセンス判定マトリクス × 使用形態 × AGPL 分離接続](img/30_ライセンス判定マトリクス.svg)

ライセンス判定を PR merge 時に人手で行う運用は、採用側組織では即座に破綻する。「このライブラリのライセンスは何か」を都度 google で調べる運用は、1 件の見落としが商用配布全体を法務リスクに晒す。本節は「全依存は必ず SPDX 識別子で宣言される / CI が許可リストに照合する / リスト外は自動拒否」という 3 条件で人間の判断を排除する構造を固定する。
...
- リリース時点: 4 ecosystem の license checker を CI に組込、許可 / 条件付き / 拒否の 3 分類でゲート、AGPL 6 件の import path check
- リリース時点: 条件付きライセンス（LGPL / MPL-2.0）の動的リンク検証、非 SPDX ライセンスの ADR 承認プロセス
- リリース時点: 依存グラフのライセンス可視化（Dependency-Track の license feature 拡張）、SBOM の license field 監査
...
Path B は 採用側の小規模運用の現実的逃げ道だが、override 件数を `ops/audit/license-overrides-count.json` で月次 tracking し、10 件を超えたら Technology Radar（90 章）で依存戦略見直しの議論起点とする。override は 6 か月ごとに再検証し、上流で SPDX 付与されていれば削除する運用規律を IMP-DEP-LIC-034 として固定する。
...
これらの代替採用判断は リリース時点で確定済（構想設計で選定）であり、将来の BSL / SSPL 化に備えて Renovate のプリセットで「上流が BSL 転換した際の alert」を設定する。転換検知時は Radar 更新 + ADR 起票を 1 週間以内に完了するサイクルを定める。
...
レポートは MinIO WORM bucket に 7 年保管し、外部監査（NFR-H-AUD-002）に対応する。採用側組織の顧客の取引先監査で license compliance を問われた際、本レポート群が一次証跡となる。
