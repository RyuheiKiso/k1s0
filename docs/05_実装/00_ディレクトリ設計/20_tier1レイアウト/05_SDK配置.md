...
│   │       └── src/
│   └── examples/
└── rust/                   # Cargo workspace（採用後の運用拡大時 骨組み）
    ├── Cargo.toml
    ├── rust-toolchain.toml
    └── crates/
        ├── k1s0-sdk/
...
採用後の運用拡大時 骨組みとして最小実装のみ（`k1s0-sdk` crate の `lib.rs` に TODO コメント）。採用後の運用拡大時 で Dapr Rust SDK の stable 化状況を見て本実装に進む。
...
- `rust/crates/k1s0-sdk-proto/src/gen/v1/` : Rust generated .rs（採用後の運用拡大時）

CI で `buf generate` 実行後、diff が 0 であることを検証。

## 公開（publish）戦略

リリース時点: 社内限定。GitHub Packages / 社内 NuGet / 社内 npm registry に publish。外部公開しない。

リリース時点: 必要に応じて public publish を検討。ただし OSS ライセンス・商標確認が必要。

リリース時点: 公式 SDK として nuget.org / npmjs.com / pkg.go.dev / crates.io に publish 検討（採用検討時点では未決定）。
...
`src/sdk/rust/` は 採用後の運用拡大時 骨組みで tier1-rust 本体とは独立運用のため、`tier1-rust-dev` cone には含めない。Rust SDK の改修が必要な場合は `sdk-dev` cone を兼任し、`02_役割別cone定義.md` の「複数役割兼任の推奨」に従って合成する。採用後の運用拡大時 で Rust SDK が本実装に移行した時点で tier1-rust-dev に含めるかを再評価する。
...
- DS-SW-COMP-122 / DS-SW-COMP-139（採用後の運用拡大時 拡張）
