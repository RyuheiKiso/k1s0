...
### 2. proto/v1/internal/ の追加

ADR-DIR-001 の昇格により、contracts は tier1 公開 11 API（`src/contracts/tier1/v1/`）と tier1 内部（`src/contracts/internal/v1/`）の 2 種類に分離した。Go 側の生成先も `internal/proto/v1/` 直下（tier1 公開）と `internal/proto/v1/internal/` 配下（tier1 内部）に分離する。
...
CI で `buf generate` 実行後に `git diff --exit-code internal/proto/v1/` を実行し、drift 検出を自動化する。
