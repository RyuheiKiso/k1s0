...
> 注: Wave -5 で展開する `infra/dapr/` は Dapr Control Plane と Component CRD の「宣言」（operator へ CRD を教える段階）までを指し、この時点で backing store（Postgres / Kafka / Valkey / MinIO）が稼働している保証はない。実際の Component がアプリ Pod から利用可能になるのは Wave 0（`infra/data`）を経て Wave 10 の tier1 Pod 起動時である。Dapr sidecar が起動時に `secrets` / `state` store の接続不能を検出する場合は、Wave 0 の依存先が Ready であることを ArgoCD の `SyncOperation.resources[].hookDeletePolicy` + `PreSync` の Job で検証するパターンも検討する。
