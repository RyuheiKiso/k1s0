...
**確定フェーズ**: Phase 1b。**対応要件**: DX-CICD-\*、NFR-SUP-\*。**上流**: DS-OPS-ENV-\*、DS-DEVX-CICD-\*。
...
1 Component は 1 YAML に分離し、環境固有値（接続文字列、Secret 参照）は `{{ }}` で placeholder 化して GitOps 側で `envsubst` または Kustomize で注入する。`runtime/` を `delivery/` から独立させたのは、Dapr Component は Pod 再デプロイと独立に Kubernetes に apply でき、更新頻度・更新主体が Pod と異なるためである。

**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-STATE-\*、FR-T1-PUBSUB-\*、FR-T1-SECRETS-\*、FR-T1-BINDING-\*。**上流**: DS-SW-COMP-024〜029。
...
各 OSS 配下に Cluster 定義・Topic / User / Database / Backup などの周辺 CR を置く。CR の apply 順序は `sync-wave` annotation で制御し、Namespace → Operator → Cluster → Topic/User の順で段階的に反映する。operators と backends を機能グループで対称にすることで、「messaging 領域を運用するのは `platform/operators/messaging/` と `platform/backends/messaging/` の 2 箇所」という対応が視覚的に明示される。

**確定フェーズ**: Phase 1b。**対応要件**: NFR-A-FT-\*、NFR-A-REC-\*、NFR-C-NOP-002。**上流**: DS-OPS-ENV-\*、DS-SW-COMP-054。
...
- `valueFrom.secretKeyRef` — Kubernetes Secret 参照
- External Secrets の `secretStoreRef` — OpenBao 参照
- `envFrom.secretRef` — Secret 全体の env 注入

「開発用の仮パスワード」を values に書くことも禁止する（commit 履歴に残り、後で本番に漏れるリスク）。dev 環境用の仮 Secret は GitOps リポジトリの dev overlay で External Secrets 経由で OpenBao Dev Mode から取得する。

**確定フェーズ**: Phase 1a。**対応要件**: NFR-E-ENC-\*、NFR-H-KEY-\*、NFR-SUP-\*。**上流**: DS-SW-COMP-031、DS-IMPL-DIR-129。
