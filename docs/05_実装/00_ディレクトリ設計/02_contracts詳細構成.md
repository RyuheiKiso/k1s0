...
Rust 側生成コードは `src/tier1/rust/crates/shared/proto-gen/src/generated/` 配下に `k1s0.state.v1.rs` / `k1s0.common.v1.rs` などの flat なファイルとして生成される（prost の仕様で `.` を区切りとするフラットファイル命名になる）。`proto-gen/src/lib.rs` で module 階層を手書き復元する。

**Go と Rust の公開 API の論理的対称化**: Go 側は `paths=source_relative` により `.proto` のディレクトリ構造（`v1/state/`）が生成コードにそのまま反映され、consumer 側の import は `github.com/.../shared/proto/v1/state` という階層付き path になる。一方 Rust は prost の仕様上 flat ファイル（`k1s0.state.v1.rs`）が強制されるため**物理配置は非対称**になるが、**consumer 側から見た module 階層は対称**に揃える。具体的には `proto-gen/src/lib.rs` で次の module 階層を必ず露出させる。

```rust
// src/tier1/rust/crates/shared/proto-gen/src/lib.rs
// Go の proto/v1/<api> と同一の階層（v1::<api>）を公開する
pub mod v1 {
    pub mod common  { include!("generated/k1s0.common.v1.rs"); }
    pub mod errors  { include!("generated/k1s0.errors.v1.rs"); }
    pub mod state   { include!("generated/k1s0.state.v1.rs"); }
    pub mod pubsub  { include!("generated/k1s0.pubsub.v1.rs"); }
    // ... 以下 11 API
}
```

これにより Rust 側 consumer は `use k1s0_proto::v1::state::StateService;`、Go 側は `statev1 "github.com/.../shared/proto/v1/state"` と書くことになり、**両言語で「v1 + API 名」の 2 階層を持つ**という契約が揃う。`k1s0.state.v2` を追加する時の更新箇所（Rust: `lib.rs` の `pub mod v2` 追加、Go: `proto/v2/` ディレクトリ追加）も対称になる。この階層契約が崩れる変更は DS-IMPL-DIR-036（破壊的変更手続）の対象とする。

**手動 include の自動化**: Phase 1a 時点では `lib.rs` の module 宣言は手書き（11 API + common + errors で約 15 行）で、`.proto` の API 追加時に開発者が更新する。Phase 1b で `tools/gen-proto-lib-rs/` を追加し、`src/generated/` 配下のファイル一覧から `lib.rs` の module 宣言を自動生成する（Go 側が `paths=source_relative` で自動化される体験に合わせる）。将来的に [tonic-build の `FileDescriptorSet` 機能](https://docs.rs/tonic-build/latest/tonic_build/) で `generated/` ごと生成する経路も Phase 1b で評価する。

**確定フェーズ**: Phase 1a（手書き include）、Phase 1b（gen-proto-lib-rs 導入）。**対応要件**: DX-CICD-\*、NFR-C-NOP-002。**上流**: DS-SW-COMP-122、DS-SW-COMP-132、DS-IMPL-DIR-029。
...
└── src/
    ├── lib.rs                        # pub mod v1 { pub mod <api> ... } の階層 include（DS-IMPL-DIR-030）
...
**Go / Rust 対称性の CI 検証**: `.proto` の追加・削除時に Go と Rust 両方の生成コードが揃っていることを機械的に保証するため、Phase 1b で `tools/check-proto-symmetry/` を追加する。lint は次を検証する。(a) `contracts/v1/<api>/v1.proto` に定義されている API 一覧と、Go 側 `internal/shared/proto/v1/<api>/` のディレクトリ一覧が一致する、(b) 同 API 一覧と Rust 側 `crates/shared/proto-gen/src/lib.rs` の `pub mod v1::<api>` 宣言が一致する、(c) 生成された Rust の `generated/k1s0.<api>.v1.rs` にも対応ファイルが存在する。片側だけ追加・削除されたら PR を fail させ、consumer（SDK / Pod）が一方の言語でだけ新 API を参照できる非対称状態を防ぐ。

**確定フェーズ**: Phase 1a（proto-gen crate 本体）、Phase 1b（check-proto-symmetry lint 追加）。**対応要件**: DX-CICD-\*、ADR-TIER1-002。**上流**: DS-SW-COMP-132、DS-IMPL-DIR-029、DS-IMPL-DIR-030。
