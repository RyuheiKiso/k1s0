...
**2026-04 プロフェッショナル改訂 — 長期運用耐性の強化**: Phase 1a 着手直前の全章レビューを受け、以下の方針で既存 ID を拡張する改訂を実施した（新 ID 追加ではなく既存 ID 拡張を選んだ理由は、全章の ID 番号を安定させて外部参照リンクの破壊を避けるため）。

- **契約の跨言語同期**（[09 章 DS-IMPL-DIR-231](09_tier1全体配置とSDK境界.md)）: 6 言語 SDK を同一 SemVer + Phase で partial release 禁止とし、`sdk-only-release.yml` で PATCH を全言語同時 bump する契約を追加。
- **SDK Security Update Protocol**（[09 章 DS-IMPL-DIR-234](09_tier1全体配置とSDK境界.md)）: CVE 検出から tier2/3 repo への自動 PR まで 6 ステップの応答フロー（Critical/High 48h、Medium 7d）を追加。
- **Secret 参照の脆弱性排除**（[05 章 DS-IMPL-DIR-130 / DS-IMPL-DIR-158](05_infra詳細構成.md)）: envsubst `${ENV_VAR}` 展開を禁止し `secretKeyRef` 統一へ移行。`tools/check-dapr-components/` と双方向 hygiene lint を CI に組込み、OpenBao role と Component の drift を機械的に防ぐ。
- **values SoT 階層の明示**（[05 章 DS-IMPL-DIR-129](05_infra詳細構成.md)）: 静的デフォルト / 環境構造差分 / 環境動的値 / Secret の 4 階層で SoT を 1 箇所に固定し、`tools/check-values-sot/` で drift を検出。
- **GitOps ツール非依存化**（[05 章 DS-IMPL-DIR-145](05_infra詳細構成.md)）: `sync-wave` は Argo CD 特異であるため CR 本体から分離し、overlay 層で注入する設計に整理。Flux 切替時の影響を封じ込める。
- **Go / Rust 生成コード論理対称化**（[02 章 DS-IMPL-DIR-030 / 031](02_contracts詳細構成.md)、[03 章 DS-IMPL-DIR-053](03_Goモジュール詳細構成.md)、[04 章 DS-IMPL-DIR-089](04_Rustワークスペース詳細構成.md)）: prost 仕様で flat となる Rust 物理配置を `lib.rs` の `pub mod v1::<api>` 階層で吸収し、両言語の consumer 側 module 階層を対称化。`tools/check-proto-symmetry/` で片側追加を CI ブロック。
- **shared/policy 内部依存方向の 3 層規定**（[03 章 DS-IMPL-DIR-051](03_Goモジュール詳細構成.md)）: sub-module 相互 import 禁止、Layer 2 で `shared/dapr` への依存禁止、合成は Layer 3 の `policy.go` のみ、という 3 層規約を `go-cleanarch` で強制。
- **Template Drift Detection の 3 層防御**（[10 章 DS-IMPL-DIR-258](10_tier2サービス配置とテンプレート.md)）: 生成時 fingerprint 埋込・Kyverno admission・定期 drift detector の 3 層と、Phase 1c で Template 改版追従の `k1s0-template-migrate-bot` を追加。
- **BFF 責務境界の allowlist/denylist**（[11 章 DS-IMPL-DIR-267](11_tier3アプリ配置とテンプレート.md)）: 許容 6 項目と禁止 6 項目を具体列挙し、`tools/bff-boundary-lint/` で検出。BFF が隠れ tier2 化する drift を防ぐ。
- **Auth Flow Matrix**（[11 章 DS-IMPL-DIR-272](11_tier3アプリ配置とテンプレート.md)）: Web + BFF / Web 直接 / Mobile + BFF / Mobile 直接の 4 構成で token 保管・tier1 呼び出し identity・refresh 経路・脅威対策を 1 表に集約。
- **300 行制限の adaptive guideline**（[08 章 DS-IMPL-DIR-209](08_命名規約と配置ルール.md)）: green/yellow/red/black の 4 段階ゲートと allowlist カテゴリ（5 種）、`valid-until` 付き時限例外で、機械判定と人手判断を両立。

**2026-04 プロフェッショナル改訂 第 2 次 — 長期運用での衰弱をヘッジする 7 項目**: 第 1 次改訂後の再レビューで「Phase 2 で tier2/tier3 が 10〜15+ に拡大した時点で、CI lint の自重・Template drift bot の notification fatigue・SDK atomic publish の未実装・shared/ 肥大化の定性判断が同時に顕在化する」構造リスクが抽出された。以下 7 項目で既存 ID を拡張する改訂を実施し、2027 年以降の sustainability を構造的に担保する。第 1 次と同じく ID 番号は増やさず既存 ID の本体を拡張する方式を採る。

- **CI lint の critical / optional レーン分類と四半期生存判定**（[07 章 DS-IMPL-DIR-190](07_開発ツールとCICD配置.md)）: 13 ジョブを 8 critical + 5 optional に明示分類し、Phase 1b で追加される 7 種の追加 lint は導入 1 四半期を optional レーンで運用。四半期ごとに起動率・fail 率・false positive 率・平均対処時間の 4 指標で生存判定し、死骸化した lint は ADR 起票で削除する。OWNERS ファイルによる保守オーナー宣言を必須化。
- **shared/ 追加・撤収の数値判定基準**（[03 章 DS-IMPL-DIR-050](03_Goモジュール詳細構成.md)、[04 章 DS-IMPL-DIR-088](04_Rustワークスペース詳細構成.md)）: 昇格は「2 Pod 以上で実質同一ロジック（diff 20% 以内・50 行超）検出で候補、3 Pod で ADR 必須」、分割は「Go 800 行・Rust 1000 行 + 責務明確な 2 分割可能時」、撤収は「利用 Pod が 1 に縮退して 2 四半期連続時」の 3 基準で定量化。Go / Rust 両言語を同じ基準で揃え drift を予防する。
- **Rust の crate 内層間依存 CI 検証**（[04 章 DS-IMPL-DIR-114](04_Rustワークスペース詳細構成.md)）: `tools/check-rust-deps/` を 2 モード化し、crate-mode（Phase 1a、crate 間 path 依存）に加えて layer-mode（Phase 1b、`syn` による AST 解析で `grpc → service → {domain, adapter}` の 1 方向を強制）を追加。Go の `go-cleanarch` との対称性を確保する。
- **雛形 CLI による横断ファイルの atomic 改訂**（[07 章 DS-IMPL-DIR-183](07_開発ツールとCICD配置.md)）: `k1s0 new pod-go / pod-rust / shared-crate / api / tier2-service` の 5 サブコマンドが、ソース生成だけでなく CODEOWNERS・`.github/workflows/pr.yml` の paths filter・`Cargo.toml` `[workspace] members`・ADR スタブまでを 1 transaction で改訂。途中 fail 時は全改訂を rollback し、半端状態を commit させない。生成結果は `tests/golden/k1s0-cli/` でゴールデンテスト対象とする。
- **Template Drift Detection の backpressure 仕様**（[10 章 DS-IMPL-DIR-258](10_tier2サービス配置とテンプレート.md)）: Layer C の自動 PR 作成を per-repo 週 1 件 + 複数 drift の batch 化 + 3 sprint decline で autoclose + 15+ repo 到達時の digest mode 切替 の 4 段 backpressure で spam 化を防止。`// kisocheck: drift-exception=<理由> valid-until=<日付>` による時限的除外と、生成ファイルマーカー（DS-IMPL-DIR-210）・formatter 結果（whitespace-only）の除外フィルタを追加。migrate-bot も同じ backpressure に従う。
- **SDK atomic publish orchestrator の実装確定**（[09 章 DS-IMPL-DIR-233](09_tier1全体配置とSDK境界.md)）: partial release 禁止（DS-IMPL-DIR-231）を実体化する 3-phase runner（staging publish → production promote → rollback）を `.github/workflows/reusable/sdk-atomic-publish.yml` と `tools/sdk-promote/` / `tools/sdk-rollback/` に実装。Phase B 失敗時 15 分以内 rollback を SLO とし、失敗ログを nightly で集計して四半期末に統計評価する。Phase 1a 時点で Go 単独でも atomic runner 経由で publish する（後付け atomic 化を避けるため）。
- **docs ↔ 実体の双方向整合検証（DS-mapping 検査）**（[07 章 DS-IMPL-DIR-182](07_開発ツールとCICD配置.md)）: `tools/check-deps/` に責務 4 として追加。`docs/05_実装/00_ディレクトリ設計/**/*.md` 内の DS-IMPL-DIR-NNN 宣言と実ディレクトリを `goldmark` AST で双方向照合し、言及ファイル不存在・未参照ディレクトリ・ID 飛び番を週次 digest で通知する。PR ブロックではなく digest 通知のみとして false positive 耐性を確保し、5 年超の運用で docs が陳腐化する経路を監視する。

**未解決の検証項目**（Phase 1b 着手前に解消）: (a) 03 章・04 章・05 章は目安 400 行を超えている（03 章: ~600 行、04 章: ~680 行、05 章: ~770 行。2026-04 改訂で各章が拡張されたため増分あり）。内容の論理分割が可能なサブディレクトリ（例: `03_Go/01_cmd.md` / `03_Go/02_internal.md` / ...）への再編は Phase 1a 着手時点では読み手が章番号を追いやすいことを優先して見送った。Phase 1b 以降で章数が増えて視認性が落ちた時点で再編する。(b) Go 4 層依存方向図・CI 4 系統フロー図・SDK Security Update Protocol の 6 ステップフロー図・Template Drift Detection の 3 層防御図・Auth Flow Matrix の 4 構成図の drawio は本章未掲載で、読者は散文と表から関係を再構成する必要がある。Phase 1b で追加を検討する。(c) tier2 / tier3 の Backstage Template ソース配置（`tools/backstage-templates/tier2/go/` など）は本章で設計 ID を確定したが、実際の skeleton ファイルは Phase 1b 初頭の実装タスクとして別途コミットする。Phase 1b 末までに Template ソース実体の初版を用意する。(d) 2026-04 改訂で追加した 6 種の CI lint（`check-dapr-components/` / `check-values-sot/` / `check-proto-symmetry/` / `bff-boundary-lint/` / `check-file-size/` / `template-drift-detector/`）は設計のみ確定しており、実装は Phase 1b の前半タスクで着手する。
