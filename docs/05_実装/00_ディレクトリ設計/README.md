...
本章は k1s0 モノレポ全体のディレクトリ配置を実装フェーズ確定版として固定する。`src/` 配下のソースコード一次ディレクトリに加え、`infra/` / `deploy/` / `ops/` / `tools/` / `tests/` / `examples/` / `third_party/` / `docs/` を含むリポジトリルート全体のレイアウトを規定し、スパースチェックアウト cone mode による役割別運用、CODEOWNERS による path-pattern ベースの責任分界、依存方向の一方向化を同時に保証する。
...
3. [20_tier1レイアウト/](20_tier1レイアウト/) — tier1 層（Phase 1a 確定範囲）
4. [30_tier2レイアウト/](30_tier2レイアウト/) / [40_tier3レイアウト/](40_tier3レイアウト/) — tier2 / tier3 層（Phase 1b / Phase 2 骨組み）
...
- Phase 0 稟議承認時点: 本章全体のディレクトリレイアウトを確定
- Phase 1a: tier1 実装対応範囲の IMP-DIR を全採番完了
- Phase 1b: tier2 / tier3 実装対応範囲の IMP-DIR を順次採番
- Phase 1c: スパースチェックアウト必須化可否を再評価
