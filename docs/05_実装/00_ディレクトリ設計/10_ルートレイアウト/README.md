...
本章は k1s0 リポジトリの最上位（ルート）に直接置かれるディレクトリ・ファイルの配置を確定する。実装フェーズの開発者が `git clone` して最初に目にする階層であり、本章の構成が開発者体験の第一印象を決める。
...
原則レベル IMP-DIR-ROOT-001 〜 007 は [../00_設計方針/01_ディレクトリ設計原則.md](../00_設計方針/01_ディレクトリ設計原則.md) で採番済み。
