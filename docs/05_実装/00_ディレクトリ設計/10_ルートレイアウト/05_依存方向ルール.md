...
- **tier1** は contracts に依存する。sdk / tier2 / tier3 を参照してはいけない。infra の Dapr Components に対しては「実行時の Sidecar 連携」であり、コンパイル依存ではないが、以下の **設定依存** は存在する:
  - tier1 Go は `infra/dapr/components/*.yaml` で定義された `metadata.name`（state store / pubsub topic / secret store のコンポーネント名）と `spec.type` を設定値として参照する
  - したがって `infra/dapr/components/` のスキーマ変更（name の rename、topic 追加、type 変更）は tier1 の統合テスト再実行を必要とする
  - CI では `tools/ci/infra-schema-diff.sh` が PR で `infra/dapr/components/**` の変更を検知した際に tier1 の `tests/integration/` を必ずトリガする（path-filter 依存関係を `.github/workflows/ci-tier1.yaml` に記述）
...
`replace` directive は依存方向違反（例: SDK → tier1 実装の逆向き参照）を隠蔽する用途では使わない。ただし go.work 不採用（IMP-BUILD-GM-026）の代替として、**順方向依存の範囲内** での local path 参照には許容する（例: SDK が未 publish の Phase 1a で `tier2/go` が `sdk/go` を `replace` で参照）。詳細は [`10_ビルド設計/20_Go_module分離戦略/01_Go_module分離戦略.md`](../../10_ビルド設計/20_Go_module分離戦略/01_Go_module分離戦略.md) を参照。各 go.mod は `src/sdk/go/` / `src/contracts/` 生成の Go 型のみ import し、他 tier の internal は参照しない。

#### module path に `src/` を含める設計判断

上記 4 module path はいずれも `github.com/k1s0/k1s0/src/<tier>/...` の形式で、リポジトリ内のディレクトリ構造をそのまま反映している。短縮形（例 `github.com/k1s0/k1s0/tier1`）を採用しない理由は以下のとおり。

- **import パスとファイルパスの 1:1 対応**: `github.com/k1s0/k1s0/src/tier1/go/internal/...` を見れば直ちに `src/tier1/go/internal/...` を読める。短縮すると IDE のジャンプ先がぶれる
- **`replace` 指示を排除する設計**: short path だと `go.mod` で `replace github.com/k1s0/k1s0/tier1 => ./src/tier1/go` が必要になり、CI / ビルド毎に書き換えリスクが発生する。Go モジュール解決を `src/` 配下のレイアウトに直結させることで `replace` を全廃できる
- **monorepo 内 module 検索の安全性**: `grep -r "github.com/k1s0/k1s0/src/tier2"` のような完全一致 grep が、同名 module（仮に他リポジトリで `tier2` を切った場合）と衝突しない

短縮の議論は将来 `src/` 配下が安定し、`go.work` の use 句で abstraction を吸収できる目処が立った時点で別 ADR として扱う。Phase 0〜1c では現行の `src/` を含む長 path を維持する（CLAUDE.md 「プロジェクト構造」節と同期）。
...
### .NET: NetArchTest + Roslyn Analyzer

.NET 系の依存方向違反は、C# プロジェクト間の `<ProjectReference>` と個別 `using` 文の 2 レベルで検出する。

まず `<ProjectReference>` レベルでは、`src/sdk/dotnet/Sdk.sln` / `src/tier2/dotnet/Tier2.sln` / `src/tier3/native/Tier3Native.sln` / `src/tier3/legacy-wrap/LegacyWrap.sln` の各ソリューション配下に `tools/ci/dotnet-arch-test/` 相当の NetArchTest ベースの検証テストを配置する。NetArchTest は「特定のアセンブリが特定の名前空間を参照していないこと」を xUnit テストとして記述できるため、「`K1s0.Sdk.*` は `K1s0.Tier2.*` / `K1s0.Tier3.*` を一切参照しない」「`K1s0.Tier2.*` は `K1s0.Tier3.*` を参照しない」といった方向性をテストで強制できる。テストは通常の `dotnet test` フェーズで実行し、違反があれば CI 失敗とする。

次に個別 `using` レベルでは、`src/platform/analyzer/` 配下に横断 Roslyn Analyzer を配置し、NuGet package として各 .sln に配布する。Analyzer は個別 `using K1s0.Tier2.*` を SDK アセンブリから禁止する等、`<ProjectReference>` をすり抜ける動的型解決や reflection 経由の参照も build 警告として検出する。診断 ID は `K1S0DEPDIR0001`〜 を予約する。

両者の併用で「プロジェクト構造としての違反」と「ソースレベルでの違反」の両方を build/CI 時に捕捉する。
