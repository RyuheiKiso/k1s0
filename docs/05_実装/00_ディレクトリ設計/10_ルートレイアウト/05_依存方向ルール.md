...
`replace` directive は依存方向違反（例: SDK → tier1 実装の逆向き参照）を隠蔽する用途では使わない。ただし go.work 不採用（IMP-BUILD-GM-026）の代替として、**順方向依存の範囲内** での local path 参照には許容する（例: SDK が未 publish の リリース時点 で `tier2/go` が `sdk/go` を `replace` で参照）。詳細は [`10_ビルド設計/20_Go_module分離戦略/01_Go_module分離戦略.md`](../../10_ビルド設計/20_Go_module分離戦略/01_Go_module分離戦略.md) を参照。各 go.mod は `src/sdk/go/` / `src/contracts/` 生成の Go 型のみ import し、他 tier の internal は参照しない。
...
短縮の議論は将来 `src/` 配下が安定した時点で別 ADR として扱う。リリース時点〜採用初期 では現行の `src/` を含む長 path を維持する（CLAUDE.md 「プロジェクト構造」節と同期）。
