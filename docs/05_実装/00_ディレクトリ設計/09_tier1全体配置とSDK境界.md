...
│   ├── internal/pods/<pod>/          # Pod 別 4 層実装（03 章 DS-IMPL-DIR-043）
│   ├── internal/shared/              # 横断共有層（03 章 DS-IMPL-DIR-042）
│   ├── internal/shared/proto/        # tier1 内部用 Protobuf 生成物（02 章）
│   └── pkg/k1s0sdk/                  # Go 公開 SDK の wrapper（本章 DS-IMPL-DIR-226）
├── rust/                             # 変更頻度 ★★★★（Rust 3 Pod 実装）
│   ├── crates/pods/<crate>/          # 3 bin crate（04 章 DS-IMPL-DIR-082）
│   ├── crates/shared/<crate>/        # 横断共有 crate（04 章 DS-IMPL-DIR-083、proto-gen 含む）
│   └── crates/sdk/k1s0-sdk/          # Rust 公開 SDK の wrapper（本章 DS-IMPL-DIR-230 / 04 章 DS-IMPL-DIR-084）
└── infra/                            # 変更頻度 ★★（delivery / runtime / governance / platform の 4 カテゴリ）
    └── ...（05 章 DS-IMPL-DIR-121〜160）
```

`contracts/sdk-out/` と `go/pkg/k1s0sdk/` / `rust/crates/sdk/k1s0-sdk/` は本章で新設する配置で、それ以外は 02〜05 章で既に確定した配置を再掲している。本章が追加するのは「tier1 の物理境界の内側で SDK をどこに置くか」の指定に集中する。
...
- `contracts/`（最も変更頻度が高い、API オーナー主導）: 新 API 追加・既存 API の field 追加は `contracts/v1/*.proto` の変更で起きる。変更は `go/internal/shared/proto/` と `rust/crates/shared/proto-gen/` に生成コードとして波及し、さらに `sdk-out/` にも波及する。レビューは `@k1s0/api-leads` と `@k1s0/tier1-architects` のダブル必須。
- `go/` / `rust/`（次点、言語チーム主導）: ビジネスロジックの実装変更は `internal/pods/<pod>/` / `crates/pods/<crate>/src/` で起きる。契約変更を伴わない修正は言語チーム単独でマージ可能。`contracts/` と同 PR で変更する場合は言語チーム + API リード の承認が必要。横断共有層（`internal/shared/` / `crates/shared/`）の変更はアーキテクト必須で、これが 03 / 04 章の深化によって物理配置として担保されている。
- `infra/`（最も変更頻度が低い、infra / devex チーム主導）: Helm / Kustomize / Dapr Component の変更は実装リリースに追従するのみで、単独の infra-only PR は四半期に数回程度。05 章の 4 カテゴリ深化（`delivery/` / `runtime/` / `governance/` / `platform/`）により、Kyverno ポリシーや Namespace などの統治系変更は `governance/` 配下に集約され、セキュリティチーム承認のトリガが path で自動的に決まる。
...
Rust 公開 SDK は `src/tier1/rust/crates/sdk/k1s0-sdk/` に新設する bin crate ではない lib crate として配置する（04 章 DS-IMPL-DIR-084 の `crates/sdk/` 下に属する唯一の crate として、Phase 2 で workspace members に追加される）。tier1 内部用の `src/tier1/rust/crates/shared/proto-gen/`（tier1 自身の Rust 3 Pod が `use k1s0_proto_gen::...` で利用するもの）とは別 crate で、公開 SDK 用に `proto-gen` の stub を再 export し、wrapper 層（tenant_id / trace_id 自動付与）を追加する。
...
**跨言語のバージョン同期契約**: tier1 が提供する全言語 SDK（Go / C# / TypeScript / Rust / Java / Python の 6 言語）は**同一の SemVer + Phase ラベルを共有**する。ある tier1 release で Go SDK が `1.2.0-phase1b` を publish した場合、C# / TypeScript 等の SDK も同じ時刻に `1.2.0-phase1b` を publish する（DS-IMPL-DIR-233 の GitHub Actions ワークフローで同時実行）。tier2 / tier3 が「Go SDK は 1.2.0、C# SDK は 1.0.0」という drift した状態を観測することは構造的に不可能にする。言語別に publish タイミングがずれる状況（例: C# SDK が NuGet 側の障害で publish 失敗）が発生した場合は、**全言語の publish を rollback し、tier1 release 自体を一時的に失敗扱いにする**（partial release を禁止）。これは tier2 / tier3 の SDK 参照を「tier1 release バージョン」1 変数で解決できるようにするための規約で、Phase 1b の SDK version 一覧 API（Backstage Software Catalog 上で自動集約）もこの前提に立つ。言語別 SDK に独立した wrapper 層バグ修正が必要で同期を崩す場合は、DS-IMPL-DIR-233 の `sdk-only-release.yml` を使い、かつ PATCH バージョンを全言語で同時に bump する（一方の言語だけ `1.2.1` に上がる運用は禁止）。

**確定フェーズ**: Phase 0（ルール）、各 Phase（適用）。**対応要件**: DX-CICD-\*、ADR-TIER1-003、NFR-SUP-\*。**上流**: DS-IMPL-DIR-214、DS-IMPL-DIR-233。
...
**SDK Security Update Protocol**: 保証期間中の全 MAJOR（N、N-1、N-2 の 3 系統）は独立に PATCH release を受ける必要がある。SDK が依存する外部ライブラリ（`grpc-go` / `tonic` / `Grpc.Net.Client` 等）に CVE が検出された場合の対応フローを次の 6 段階で固定する。(1) CVE 検出（Dependabot + Trivy + OSV-Scanner の GitHub Advanced Security 通知、Phase 1b 以降は Nexus IQ 併用）、(2) severity 判定（Critical / High は 48 時間以内に対応、Medium は 7 日以内、Low は次 MINOR に同梱）、(3) tier1 repo で保証期間中の全 MAJOR 系統（最大 3 本）の release branch に同時 PATCH PR を自動生成、(4) `sdk-only-release.yml`（DS-IMPL-DIR-233）で全言語 SDK を同時 publish、(5) tier2 / tier3 repo に対し GitHub Organization 機能の `security advisory` で通知し、`k1s0-sdk-security-bot`（Phase 1b で導入）が全 tier2 / tier3 repo に SDK version bump PR を自動生成、(6) Critical / High の場合は 7 日後に自動 merge（tier2 / tier3 側の CI が pass する場合のみ、失敗時は owner に escalate）。この仕組みにより tier2 repo が 10 個、tier3 repo が 5 個に達した時点でも、CVE 対応を 2 名運用で管理可能な体制を担保する。手動介入なしで全 15 repo に到達できないフローは設計違反とみなし、本 protocol に追記する。

**Phase ごとの導入**: Phase 1a では手動 protocol として文書化のみ（実 tier2 repo が存在しないため）。Phase 1b のパイロット tier2 サービス着手時に `k1s0-sdk-security-bot` の実装と Dependabot / Trivy 統合を開始。Phase 1c までに自動 PR 生成 + CI 連携 + 自動 merge までを整備し、Phase 2 開始時点で全段階自動化を完了する。

**確定フェーズ**: Phase 0（ルール）、Phase 1b（bot 実装）、Phase 1c（自動化完了）、各 MAJOR / PATCH release（適用）。**対応要件**: ADR-TIER1-003、NFR-SUP-\*、NFR-H-\*（完整性）、DX-CICD-\*。**上流**: DS-SW-COMP-138、DS-IMPL-DIR-233。
...
| DS-IMPL-DIR-234 | SDK 互換性保証期間と Security Update Protocol | Phase 0/1b/1c |
