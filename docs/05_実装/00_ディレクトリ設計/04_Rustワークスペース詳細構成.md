...
**shared/ crate の追加・撤収の数値判定基準**（本 ID は `shared/common/` 単体ではなく、DS-IMPL-DIR-088〜091 の全 `crates/shared/<crate>/` に共通適用し、Go 側 DS-IMPL-DIR-050 の判定基準と対称に運用する）: Rust は新規 crate 追加が `Cargo.toml` `[workspace]` `members` への明示追記と ADR 起票を要する（DS-IMPL-DIR-120）ため、Go の package 追加よりも手続き重量が 1 段高い。この非対称を放置すると、**Go 側だけ `shared/` が肥大化し、Rust 側は本来 shared にすべきコードを pods に残す** drift が生じる。両言語の shared 判定を同じ数値基準で揃えることで、この drift を予防する。

- **`pods/<pod>/` から `shared/<crate>/` への昇格基準**: 2 Pod 以上（自作 Rust 3 Pod は `audit` / `decision` / `pii`）が実質同一のロジック（diff 20% 以内、50 行超）を持つことを `tools/check-rust-deps/` の類似度検査（Phase 1b 追加）で検出した時点で「昇格候補」扱い。3 Pod 全てが同一ロジックを持つ状態に至った場合は ADR 起票を**必須**とし、新 `crates/shared/<crate>/` を 2 スプリント以内に切り出す。切り出し時は path 依存を更新した PR を同時に出し、元の pods/ 側のコードは削除する（フォーク運用を禁止）。
- **`crates/shared/<crate>/` 内のモジュール分割基準**: 1 crate の総行数（`src/**/*.rs` 合計）が 1000 行（目安 5 ファイル × 200 行）を超え、かつ責務が明確に 2 分割以上できる場合に**新 crate 化**を検討する（Rust は crate 追加コストが高いため Go よりやや緩い 1000 行しきい値）。1000 行未満の場合は `src/<module>.rs` のモジュール分割にとどめ、crate は分割しない。`crates/shared/policy/` はこの基準に従い、Phase 1a では 1 crate 内の 6 ファイル構成（DS-IMPL-DIR-091）を維持する。
- **`crates/shared/<crate>/` から `crates/pods/<pod>/` への撤収基準**: 利用 Pod 数が 1 に縮退した状態が 2 四半期連続した場合、対応 Pod crate の `src/<module>.rs` に吸収する PR を起票する。crate の delete は `Cargo.toml` `[workspace]` `members` 編集と ADR 起票を要するが、これは drift 除去のコストとして必要。「過去に横断利用されていたが現在は 1 Pod のみ」という負債 crate を残さない。
- **機械検査の対称性**: 上記 3 基準は `tools/check-rust-deps/internal/shared_residency.rs`（Phase 1b）で週次検査し、Go 側の `tools/check-deps/internal/rules/shared_residency.go` と同一フォーマットで digest 通知する。両言語の shared 候補・撤収候補を `@k1s0/tier1-architects` が同じレポートで判断できるようにする。

**確定フェーズ**: Phase 1a（`shared/common/` crate 配置）、Phase 1b（数値基準の機械検査ツール導入）。**対応要件**: DX-GP-\*、NFR-C-NOP-001、NFR-C-NOP-002。**上流**: DS-SW-COMP-108、DS-IMPL-DIR-050、DS-IMPL-DIR-120、DS-IMPL-DIR-182。
...
`crates/` 配下の path 依存と crate 内部の層間依存は以下の 3 階層ルールで厳格に管理する。Rust は `Cargo.toml` の `[dependencies]` に `path = "../../pods/other"` を書けば namespace 衝突なく参照できるため、Go の `internal/` 可視性相当の compiler 強制が効かない。この弱みを CI lint の二段構え（crate 間 + crate 内）で補完する。

- **ルール 1（crate 間の path 依存、ディレクトリ横断）**: `pods/<pod>/` の `[dependencies]` に書ける path は `shared/<lib>` と `sdk/<crate>` のみ。`pods/<other>/` への path 依存は禁止する。
- **ルール 2（shared の閉鎖性）**: `shared/<lib>` の `[dependencies]` に書ける path は `shared/<other lib>` のみ。`pods/` や `sdk/` への path 依存は禁止する（shared は Pod に依存しない）。
- **ルール 3（crate 内の層間依存、Phase 1b 追加）**: 各 bin crate（`pods/<pod>/`）の `src/` 配下 4 層（`grpc/` / `service/` / `domain/` / `adapter/`、DS-IMPL-DIR-092）の依存方向を以下 1 方向に固定する。`grpc` → `service` → `{domain, adapter}`、および `adapter` → `domain`。逆流（`domain` → `adapter` や `service` → `grpc`）と、層スキップ（`grpc` → `adapter` 直接、`service` → domain を経由せず `grpc` 型の直接利用）は禁止する。これは Go 側の `go-cleanarch`（DS-IMPL-DIR-074）と対称の規定で、従来は `check-rust-deps` が crate 間 path 依存のみ検証し、crate 内の層間依存は実質的に野放しになっていた。

**ルール違反の機械検査**: CI の `tools/check-rust-deps/` を 2 モードで運用する。

1. **crate-mode（Phase 1a）**: `cargo metadata --format-version=1` の JSON 出力を parse し、各 crate の `dependencies[].path` を列挙。ルール 1・2 の違反があれば exit code 1。実装は Rust 製 200 行以内。
2. **layer-mode（Phase 1b）**: `syn` crate で各 `.rs` ファイルを AST parse し、`use` 文と型参照の module path を抽出して層間の依存グラフを構築、ルール 3 の違反を検出する。各 bin crate 配下の `src/grpc/` / `src/service/` / `src/domain/` / `src/adapter/` をルート module として扱い、module path の接頭辞で層を判定する。実装は Rust 製 400 行程度を想定し、`tools/check-rust-deps/src/layer_check.rs` に分離する。検出は PR 時の `check-rust-deps` ジョブ（07 章 DS-IMPL-DIR-190 の critical レーン）で実行する。

`workspace` 内 crate を git や crates-io 経由で取得することも禁止し、workspace の完結性を保つ（`cargo publish` 時の意図せぬ公開を防ぐ）。Phase 2 以降で tools/ と crates/ の間で依存が必要になる場合も path 依存（相対パス）で接続する（DS-IMPL-DIR-008 参照）。

**Phase 1b での strict 化**: 層間依存違反は Phase 1a 時点の既存コードに遡及しても違反ゼロであるべきだが、層 naming の重複命名（例: `pods/audit/src/service/audit.rs` と `pods/audit/src/domain/audit.rs` のような名前衝突）で false positive が出る可能性がある。Phase 1b 前半は **ratchet** モード（既存違反は例外リストで凍結、新規違反のみ fail）で運用し、四半期末に strict（全違反 fail）へ移行する。ratchet 期間が長期化した場合は ADR で理由を明示する。

**Go 側との対称性**: Go の 4 層（handler / service / domain / repository、DS-IMPL-DIR-054）と Rust の 4 層（grpc / service / domain / adapter、DS-IMPL-DIR-092）は命名こそ異なるが、依存方向は同じ 1 方向（外層 → 内層 + domain/adapter 並列）で揃えている。`tools/check-deps/` と `tools/check-rust-deps/` は出力フォーマットを揃え、layer 違反の digest を両言語横串で確認できるようにする。

**確定フェーズ**: Phase 1a（crate-mode 実装）、Phase 1b（layer-mode 追加、ratchet 運用開始）、Phase 1b 末（strict 移行）。**対応要件**: NFR-C-NOP-002、DX-CICD-\*、DX-TEST-\*。**上流**: DS-SW-COMP-112、DS-SW-COMP-113、DS-SW-COMP-131、DS-IMPL-DIR-074、DS-IMPL-DIR-092。
