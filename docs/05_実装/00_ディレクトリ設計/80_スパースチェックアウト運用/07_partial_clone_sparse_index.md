...
`core.sparseCheckoutCone` は `init --cone` で自動的に true に設定されるため、明示 `git config` は冗長（Git 2.40+）。`git update-index --split-index` は split-index という別機能（sparse index 以前の古い index 縮小機構）であり、sparse index とは無関係なので触らない。
