...
**Layer C の backpressure 仕様（notification fatigue 防止）**: tier2 repo 数が 10+、tier3 を含めて 15+ に達する Phase 2 中盤で、drift detector が per-repo × per-file で PR を量産すると、1 日あたり数十件の自動 PR が tier2 サービスチームに流入し、本当に対応すべき PR が埋没する。これを防ぐため、Layer C の PR 作成ロジックに以下 4 段階の backpressure を強制する。

1. **per-repo rate limit**: 同一 tier2 repo に対する自動復元 PR は **週あたり最大 1 件**。同一週に複数の drift が検出された場合、後述の batch 化で 1 PR にまとめる。週次カウンタは日本時間の月曜 0:00 にリセット。`high` severity は rate limit の対象外とし、検出時に即 lifecycle 更新 + 1 PR 作成する（業務影響のため backpressure を緩和）。
2. **batch 化**: 同一 repo の `medium` / `low` drift は 1 PR に束ねる。PR タイトルは `[drift] restore N files to template baseline (T1-arch-nnn)`、本文には drift ファイルごとの「現在 SHA256 / 期待 SHA256 / Template 版での元ファイル / 推奨アクション」の表を添付する。個別 drift に対する個別 PR は作らない。
3. **3 sprint decline で autoclose + escalate**: 同一内容の drift PR が tier2 サービスチームに 3 sprint 連続で decline（close without merge）された場合、detector は以降の自動 PR 作成を停止し、`@k1s0/tier1-architects` に GitHub Issue 1 本で escalate する。Issue には「このチームが Template 準拠を拒否している構造的理由」を tier2 サービスチーム自身が記述する責務があり、未記述のまま 1 sprint 経過した repo は lifecycle を `deprecated` へ自動遷移させる（`high` severity と同等扱い）。
4. **digest 配信モード**: Backstage Catalog 上の tier2 repo 数が 15 を超えた時点で、detector は自動で `digest mode` に切り替わる。digest mode では個別 PR を作成せず、週次の digest Issue 1 本を `@k1s0/tier1-architects` に対して発行し、tier2 サービスチームには通知しない（Layer A / Layer B の静的防御で実害を抑制する前提）。digest Issue は drift の傾向（repo あたりの drift 数分布、ファイル種別分布、severity 分布）のみを含め、個別 repo 名の列挙は付録に回す。

**false positive の除外フィルタ**: Layer A の SHA256 照合は「意図した改訂（例: `renovate-bot` による依存更新の結果、Dockerfile の base image バージョン部分のみが変わる）」と「意図しない drift」を区別できない。これを補うため、detector は次のパターンを除外する。(a) ファイル末尾に `// kisocheck: drift-exception=<理由> valid-until=<日付>` コメントがある場合、指定期日まで当該ファイルを除外（`valid-until` は最大 6 か月、08 章 DS-IMPL-DIR-209 の adaptive guideline と同じ時限性）、(b) 生成ファイル識別マーカー（08 章 DS-IMPL-DIR-210 の `// Code generated` / `// @generated`）を含むファイルは除外、(c) 自動 formatter（gofmt / rustfmt / prettier）の結果と等価な whitespace-only diff は除外（`go fmt -d` / `cargo fmt --check` で再確認）。

**生存判定との連動**: `tools/template-drift-detector/` 自身も 07 章 DS-IMPL-DIR-190 の四半期生存判定の対象で、false positive 率 5% 超または autoclose 率 50% 超（= 対応されない PR を量産しているだけ）の四半期は、ADR で「backpressure 仕様の再設計」を起票する。

**Template 改版への追従**: Template 自体が改版（例: CI 6 段→7 段の段追加、adapter ラッパのインターフェイス変更）された時、既存 tier2 repo を自動で追従させる経路が必要になる。Phase 1c で `k1s0-template-migrate-bot` を追加し、Template 改版 PR が main にマージされた時点で、Layer A の `template-version` が古い全 tier2 repo に対して自動でマイグレーション PR を作成する。マイグレーションは差分適用可能な範囲（ci.yml の段追加、Dockerfile の baseimage バージョン更新など）に限定し、破壊的変更（adapter インターフェイス変更）は必ず人手レビューを通す。migrate-bot は drift-detector と同じ backpressure（per-repo 週 1 件、batch 化、3 sprint decline で escalate）に従う。Template 改版時は DS-IMPL-DIR-260 の ADR 起票に「マイグレーション戦略」節の記述を必須とする。

**確定フェーズ**: Phase 1b（Layer A / B / C の 3 層防御確立 + backpressure 初期実装）、Phase 1c（Template 改版マイグレーション bot 追加 + migrate-bot の backpressure 適用）、Phase 2（tier2 repo 15+ 到達時の digest mode 有効化と運用検証）。**対応要件**: NFR-E-AC-\*、NFR-H-INT-\*、NFR-C-NOP-001、NFR-SUP-\*、ADR-CICD-003（Kyverno）。**上流**: DS-SW-DOC-002、DS-IMPL-DIR-140、DS-IMPL-DIR-190（四半期生存判定）、DS-IMPL-DIR-209（例外コメントと valid-until）、DS-IMPL-DIR-210（生成ファイルマーカー）、DS-IMPL-DIR-244、DS-IMPL-DIR-259、DS-IMPL-DIR-260。
