...
└── tools/backstage-templates/
    ├── tier2-microservice-go/           # Go tier2 用 Template
    ...
    │   │   └── README.md
    │   └── docs/                        # Template 利用ガイド
    └── tier2-microservice-csharp/       # C# tier2 用 Template（Phase 1b）
        ├── template.yaml
        └── skeleton/
...
tier2 用 Backstage Software Template のソース一式は、k1s0 repo 内の `tools/backstage-templates/` 配下に配置する。サブディレクトリは**言語別に分ける**（`tier2-microservice-go/` / `tier2-microservice-csharp/`）。言語別に分ける理由は (a) Template ごとに `template.yaml` の `parameters` セクションが異なる（Go は `go_module_path`、C# は `dotnet_target_framework` など）、(b) skeleton の拡張子やビルドファイル（`go.mod` vs `*.csproj`）が排他的、(c) Template 更新を言語別に独立管理したい、の 3 点である。

`tools/backstage-templates/` は 07 章 DS-IMPL-DIR-183（`tools/` 配下全般）の下位として配置される。07 章では Phase 1c で `tools/backstage/` に TechDocs / Catalog 設定を置くと決定済みだが、Software Template は独立したサブディレクトリ `tools/backstage-templates/` に分ける（Backstage の TechDocs 設定と Software Template は異なる Backstage プラグインで管理され、変更頻度とオーナーも異なるため）。

Template ソースの Phase 導入は次のとおり。Phase 1a では `tier2-microservice-go/` のみ配置し、C# 版は骨格のみ（`README.md` に Phase 1b で正式化する旨を明記）。Phase 1b で C# 版を完全実装。Phase 2 で Java / Python 用 Template を追加する可能性があるが、追加には ADR を要する。
...
C# 版は Phase 1b で導入するため、Template 自身も Phase 1b で完全版化する。Phase 1a 時点では `tools/backstage-templates/tier2-microservice-csharp/` は README のみを配置し、「Phase 1b 公開予定」と明記する。
...
## DS-IMPL-DIR-258 tier2 repo の生成後の禁止変更
...
禁止改変の検出は (a) k1s0-gitops 側で Kyverno ClusterPolicy による admission 検査（Dockerfile 由来の image が distroless 以外なら admission reject）、(b) PR レビュー時の目視確認、の 2 経路で行う。Backstage Catalog の「ヘルスチェック」機能で定期的に tier2 repo の状態を検査する仕組みは Phase 1c で導入する。

**確定フェーズ**: Phase 1b。**対応要件**: NFR-E-AC-\*、NFR-H-INT-\*、NFR-C-NOP-001、ADR-CICD-003（Kyverno）。**上流**: DS-SW-DOC-002、DS-IMPL-DIR-140。
...
| DS-IMPL-DIR-258 | tier2 repo 生成後の禁止変更 | Phase 1b |
