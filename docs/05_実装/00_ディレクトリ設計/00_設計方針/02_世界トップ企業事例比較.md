...
k1s0 は 採用側の小規模運用を前提とする 採用側組織（NFR-C-NOP-001）であり、Google / Meta のような 10000 人超の開発者数は想定外である。一方で 10 年保守を目指すため、小規模時点の判断が将来の大規模化を阻害しないことが重要となる。本ファイルでは 9 社の事例を、規模・採用スタック・スパースチェックアウト対応・特筆点の 4 観点で俯瞰し、k1s0 に取り入れる判断と取り入れない判断を明示する。
...
取り入れない判断は、Sapling / Buck2 の採用。採用側の小規模運用で扱える OSS スタックを増やすリスクが大きい。
...
取り入れない判断は、Bazel の採用。k1s0 の規模では cargo / go / pnpm の各ネイティブビルドで十分対応できる。Bazel 学習コストは 採用側の小規模運用では回収困難。
...
k1s0 に取り入れるべき知見は、**LFS 判定の遅延**。Twitter は初期に LFS を積極採用した結果、後年の運用負荷に直面した。k1s0 ではリリース時点では LFS を使わず、採用後の運用拡大時に LFS 判定を行う（ADR-DIR-004 として将来起票）方針を取る。
...
3. **スパースチェックアウトは Day 1 から**: 2 名時点で必要性は薄いが、リリース時点 / 採用後の運用拡大時 で必須化する前提で Day 1 から習慣化
