...
Phase 0 稟議時点では実在の人員は 2 名だが、Phase 1b 以降のチーム拡大を見据えて以下の GitHub team を予約する。GitHub org は `k1s0`（Phase 1b 以降で本設立）を想定。
...
Phase 0 時点では全チームが同一の 2 名に割り当てられる状態で開始する。Phase 1a 以降の採用進捗に応じて段階的に分離する。
...
- 契約変更（`src/contracts/` 配下）: `@k1s0/contract-reviewers` から 2 名以上の承認（Phase 1b 以降、チーム 3 名以上時）
...
- NFR-C-NOP-001（2 名運用）/ NFR-H-AUD-\*（監査）/ DX-CICD-\* / 制約 8
