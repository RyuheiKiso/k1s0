...
本ファイルは実装段階のディレクトリ設計で採番する ID 体系 `IMP-DIR-<サブ>-<通番>` の命名規則と双方向トレース運用を定める。ID 体系は [../../../04_概要設計/00_設計方針/03_設計ID体系とトレーサビリティ.md](../../../04_概要設計/00_設計方針/03_設計ID体系とトレーサビリティ.md) の「実装 ID 体系」節で予約された `IMP-DIR-*` を具体化するものである。

## ID 体系が必要な理由

概要設計の設計 ID（`DS-SW-COMP-120` など）は論理レベルの配置を規定する。一方で実装段階では、CODEOWNERS の path-pattern・cone 定義への追記・CI ワークフローの path-filter 設定・ArgoCD ApplicationSet の検索パスなど、物理配置に起因する複数資産の同時更新が発生する。これらを個別の章内参照のみで管理すると、ある章の配置変更が他章に波及した際に追跡が困難になる。
...
- `IMP-DIR-ROOT-013` 〜 `IMP-DIR-ROOT-020` : 運用蓄積後の予約
...
- `IMP-DIR-T1-027` 〜 `IMP-DIR-T1-040` : 運用蓄積後の予約
...
- `IMP-DIR-T2-047` 〜 `IMP-DIR-T2-055` : 運用蓄積後の予約
...
- `IMP-DIR-T3-061` 〜 `IMP-DIR-T3-070` : 運用蓄積後の予約
...
- `IMP-DIR-INFRA-079` 〜 `IMP-DIR-INFRA-090` : 運用蓄積後の予約
...
- `IMP-DIR-OPS-098` 〜 `IMP-DIR-OPS-110` : 運用蓄積後の予約
...
- `IMP-DIR-COMM-117` 〜 `IMP-DIR-COMM-125` : 運用蓄積後の予約
...
- `IMP-DIR-SPARSE-133` 〜 `IMP-DIR-SPARSE-145` : 運用蓄積後の予約
...
合計 57 件。残りの帯（各サブ分類の予約番号）は リリース時点 / リリース時点 / 採用後の運用拡大時 で追加採番する。
