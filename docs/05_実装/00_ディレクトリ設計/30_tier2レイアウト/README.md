...
本章は tier2 層（`src/tier2/`）の物理配置を確定する。tier2 はドメイン共通業務ロジック層で、C# .NET と Go の両言語でサービスを実装する。Phase 1a 時点では雛形テンプレートのみ、Phase 1b 以降で JTC 業務の実稼働サービスを追加する。
...
- .NET 資産の活用（JTC の既存 .NET 資産を段階的に移行する受け皿）
