...
| [ADR-TEST-008](../../../02_構想設計/adr/ADR-TEST-008-e2e-owner-user-bisection.md)（e2e owner / user 二分構造） | E2E-001（責務分界）, E2E-002/003（環境契約）, E2E-004/005（ディレクトリ）, E2E-006/007（Makefile target）, E2E-009/010（CI 統合） |
| [ADR-TEST-009](../../../02_構想設計/adr/ADR-TEST-009-observability-e2e-five-checks-owner-only.md)（観測性 E2E 5 検証 owner only） | E2E-008（5 検証実装契約 + 5 helper client + baseline 管理） |
| [ADR-TEST-010](../../../02_構想設計/adr/ADR-TEST-010-test-fixtures-sdk-bundled.md)（test-fixtures 4 言語 SDK 同梱） | E2E-011（4 言語対称 API）, E2E-012（versioning）, E2E-013（mock builder 段階展開） |
| [ADR-TEST-011](../../../02_構想設計/adr/ADR-TEST-011-release-tag-gate-as-owner-e2e-alternative.md)（release tag ゲート代替保証） | E2E-014（cut.sh 拡張）, E2E-015（artifact 保管）, E2E-016（owner-e2e-results template） |

`IMP-CI-E2E-*` 系列の網羅は [`../35_e2e_test_design/90_対応IMP-CI-E2E索引/01_対応IMP索引.md`](../35_e2e_test_design/90_対応IMP-CI-E2E索引/01_対応IMP索引.md) で管理する。
