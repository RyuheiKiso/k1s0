...
├── 20_path_filter選択ビルド/
├── 30_quality_gate/        # fmt / lint / unit / coverage / L4 と L5 責務分界
├── 35_e2e_test_design/     # e2e owner / user 二分構造の実装設計（ADR-TEST-008/009/010/011）
├── 40_Harbor_Trivy_push/   # 構想設計 7 段ステージの物理化
├── 50_branch_protection/
├── 60_ローカル開発ツール/    # ローカル verify と PR title autofix
└── 90_対応IMP-CI索引/
```

## IMP ID 予約

本章で採番する実装 ID は `IMP-CI-*`（予約範囲: IMP-CI-001 〜 IMP-CI-099）。e2e 系列は `IMP-CI-E2E-*`（予約範囲: IMP-CI-E2E-001 〜 IMP-CI-E2E-099）として `35_e2e_test_design/90_対応IMP-CI-E2E索引/` で正典化する。
