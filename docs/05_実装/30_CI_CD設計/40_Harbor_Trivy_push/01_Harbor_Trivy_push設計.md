...
- 公開レジストリは リリース時点 では立てず、外部配布は 運用蓄積後で再設計
- ストレージバックエンド: Longhorn（3 replica）、容量は リリース時点で 500GB / 2TB に拡張
- 認証: Keycloak OIDC（人間は SSO）+ robot アカウント（CI のみ）
- ログ: Harbor → OTel Collector（60 章）→ Loki、監査ログは append-only ストレージに 7 年保管

エンドポイントを単一 FQDN に固定する理由は、Kyverno の `image-source` ポリシー（構想設計 採用初期 採用）が `harbor.k1s0.internal/*` 以外のイメージを admission で拒否するためである。別 FQDN を同居させると、その一貫性が崩れて下流の admission 検証が機能しなくなる。
...
自動隔離の実装は Harbor Webhook → Argo Events（採用後の運用拡大時 で正式化、リリース時点 は KEDA ScaledJob で代替）→ `harbor-quarantine-job` で `harbor/robot$quarantine-ci` が `POST /artifacts` で隔離先にコピー後、元を削除する。quarantine に隔離されたイメージは Kyverno 側で pull 不可とし、Security チームの作業場としてのみ利用する。

Trivy DB の更新は Harbor pod 内で日次 cron（2:00 JST）。エアギャップ環境では リリース時点 から Trivy DB オフラインミラー（`infra/data/trivy-mirror/`）を立て、Harbor が外部に出ない運用に移行する（IMP-CI-HAR-045）。
...
Rekor public instance への依存はエアギャップ運用で問題になるため、リリース時点 で Rekor private instance（`infra/security/rekor/`）の立ち上げを計画に入れる。採用初期 は public に記録しつつ、記録 URL を社内エビデンスとして `ops/runbooks/daily/rekor-mirror/` で日次スナップショット取得する。

## 段階展開と DR 対応

Harbor は k1s0 のサプライチェーン単一点障害になるため、DR / HA 対応を 段階別に段階導入する（IMP-CI-HAR-048）。

- リリース時点: 単一クラスタ、Longhorn 3 replica、DB は CloudNativePG 3 replica（HA）
- リリース時点: Harbor replication を DR サイトの Harbor へ自動（`harbor.k1s0-dr.internal`）、RPO 目標 1 時間
- リリース時点: in-line scanner を sigstore/policy-controller に差し替え検討、SBOM 参照の統一化
- 採用後の運用拡大時: 公開レジストリ（`harbor.k1s0.com`）への公開範囲検討、外部配布の is-it-safe 判定を自動化
...
Harbor / Trivy が停止すると CI の push 段が詰まり、結果として リリース時点 の deploy が止まる。障害時は `ops/runbooks/incidents/tier1/harbor-down.md` を Sev1 として発動し、Harbor 暫定 bypass（署名済イメージのみ GitOps 更新を許可する縮退運用）を判断する。この縮退運用の承認は Security（D）+ SRE（B）の共同で、1 営業日以内のタイムボックスを必須化する。
...
この規律が崩れると、「誰かが手で push した経路不明のイメージ」が Harbor に滞留し、サプライチェーン追跡が不能になる。リリース時点 から厳守し、例外を作らない。

## Retention とガベコレ

Harbor に push され続けるイメージは、適切な retention を設けないと指数的に PV 容量を消費する。リリース時点 の 500GB が 6 ヶ月で満杯になる試算があり、次の retention policy を リリース時点 から有効化する（IMP-CI-HAR-051）。
