...
`verify-quick` は `git diff --name-only origin/main...HEAD` で変更ファイルを取り、拡張子と path から「rust / go / dotnet / node / proto / docs / codegen」7 スコープのいずれが影響するかを判定する。`src/contracts/` 変更時は path-filter ([`tools/ci/path-filter/filters.yaml`](../../../../tools/ci/path-filter/filters.yaml)) と同じく**全 SDK と tier1 を波及対象**にする規約を共有する。
