...
本ファイルは ADR-TEST-008 で確定した `make e2e-user-{smoke,full}` の実装契約を実装段階の正典として固定する。各 target の起動経路（`tools/local-stack/up.sh --role user-e2e` 連携）、kind cluster ライフサイクル、go test 引数、artifact 出力先を ID として採番する。
...
```text
1. 起動: tools/local-stack/up.sh --role user-e2e
...
3. 終了: tools/local-stack/down.sh --role user-e2e
   → kind delete cluster
```
...
```makefile
e2e-user-smoke:
	@./tools/local-stack/up.sh --role user-e2e
	@trap './tools/local-stack/down.sh --role user-e2e' EXIT; \
