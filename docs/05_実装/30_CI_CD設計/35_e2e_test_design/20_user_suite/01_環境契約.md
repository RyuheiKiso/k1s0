...
| 起動経路 | `tools/local-stack/up.sh --role user-e2e` | ADR-POL-002 の SoT |
...
minimum 以外の stack は、test ごとに必要に応じて opt-in で追加起動する。task ごとに `tools/local-stack/up.sh --role user-e2e --add <component>` で起動する。
...
- ADR-POL-002（local-stack を構成 SoT に統一）— `--role user-e2e` の SoT 拡張
- ADR-TEST-010（test-fixtures SDK 同梱）— 利用者が本環境を import 1 行で起動する経路
- `tools/local-stack/up.sh --role user-e2e` — 実装本体
