...
![10 役別 Dev Container 全体像（L0 共通ベース→L1 features→L2 最終イメージ→L3 sparse cone の 4 層継承を全 10 役で示す設計図。L0 の `debian:slim base` と L2 の GHCR digest 配布は Phase 3 以降の到達点で、Phase 2 現状では L0 を Microsoft 公式 image、L2 をローカルビルドで代替している）](img/10_DevContainer10役継承.svg)

## なぜ 10 役別プロファイルが必要なのか

tier1 Rust 開発者に .NET SDK、pnpm、MAUI ワークロード、Longhorn CLI、OpenBao コマンドを全部入れた 12GB image を配ると、ビルドも rust-analyzer も遅くなり、VS Code の IntelliSense 応答が 10 秒を超える。逆に共通最小 image に寄せると、今度は「この tier の設定が入っていない」という Platform への問い合わせが毎回発生する。どちらも 採用側の小規模運用では耐えられるが、採用側の運用拡大期に入った瞬間に生産性が崩壊する。

ADR-DIR-003 の 10 役 cone 定義は、スパースチェックアウトだけでなく Dev Container もこの分割で提供することを前提に設計されている。本ファイルはその Dev Container 側の実装配置を固定し、「cone を切替えると同時に Dev Container も切替わる」という一貫した役割切替体験を リリース時点 から確定させる。
...
サイズ目標はリリース時点での目安であり、それ以降は Dev Container Features の bakery 化（Platform/Build チーム管理の社内レジストリ）で digest を固定しつつ、軽量化を継続する。`full` のみ 8GB を許容する理由は、CI での「全役割 matrix テスト」に使うためで、個人の日常開発では `full` を選ばせない運用とする。
...
本ファイルで設計する Dev Container は、time-to-first-commit SLI（IMP-DEV-POL-004、採用側の小規模運用 4h / 採用側の運用拡大期 2h）の主要短縮要素である。`postCreateCommand` の実行時間が SLI に直接効くため、次の計測点を GitHub Actions と Backstage Scorecards に露出する。
...
- **Docker runtime** : Windows + WSL2 では [ADR-DEV-002](../../../02_構想設計/adr/ADR-DEV-002-windows-wsl2-docker-runtime.md) により WSL ネイティブ docker-ce で固定し、Docker Desktop / Rancher Desktop は採用しない（物理配置は [`../05_ローカル環境基盤/01_WindowsWSL2環境構成.md`](../05_ローカル環境基盤/01_WindowsWSL2環境構成.md) を参照）。macOS / 純 Linux ホストの選定は別 ADR で扱う。Dev Container 起動スクリプトは `DOCKER_HOST` を自動検出し、Podman 経路は Paved Road 外として扱う（`CONTAINER_ENGINE=podman` 設定は exit plan ADR が必要）
...
## Phase 2 移行の現状（10 役全配置 + ローカル本番再現スタック稼働）

リリース時点の単一開発者フェーズで bootstrap として `tier1-rust-dev` 1 役のみが稼働していた状態から、Phase 2 へ移行が完了している。Phase 2 の到達定義は次の 3 点が同時に成立した状態とし、本ファイルと [`tools/devcontainer/README.md`](../../../../tools/devcontainer/README.md) の「設計書と現状の差分」表が合致した時点を指す。具体的には、(1) 10 役プロファイル全実体化（`tools/devcontainer/profiles/<role>/` の `Dockerfile` / `devcontainer.json` / `extensions.json` 三点揃い）、(2) `tools/sparse/checkout-role.sh` による cone と symlink の同時切替、(3) `tools/local-stack/up.sh` による kind ベース本番再現スタックの一括起動である。Paved Road（ADR-DEV-001）の「正しい道一本化」が各役割の独立した正しい道として並走可能になり、複数役を同一ホスト上で素早く切替える運用が成立する。

Phase 2 移行のトリガーは「単一端末ですべて開発する」運用方針の確立であった。リリース時点 〜運用拡大期 をまたいで「役の切替えが頻発するが端末は 1 台」という制約が支配的になるため、bootstrap で許容していた縮約（tier1 中心の唯一の道）を解いて全役を物理化した。

### Phase 2 で稼働する範囲

10 役のプロファイルが [`tools/devcontainer/profiles/<role>/`](../../../../tools/devcontainer/profiles/) に全配置されており、各ディレクトリには `Dockerfile`・`devcontainer.json`・`extensions.json` が揃う。VS Code 設定共有は [`/.devcontainer/settings/`](../../../../.devcontainer/settings/) に共通レイヤと役割別上書きを配置し（IMP-DEV-DC-013）、ルート [`/.devcontainer/devcontainer.json`](../../../../.devcontainer/devcontainer.json) は最軽量の `docs-writer` プロファイルへの相対 symlink になっている。役割切替は [`tools/sparse/checkout-role.sh <role>`](../../../../tools/sparse/checkout-role.sh) が cone 設定と symlink 張替えを同時に実行する（IMP-DIR-SPARSE-127 / IMP-DEV-DC-010）。

ローカル本番再現スタック（IMP-DEV-DC-014）は [`tools/local-stack/`](../../../../tools/local-stack/) に配置され、`up.sh --role <role>` を実行すると kind cluster（control-plane 1 + worker 3、Calico CNI、cloud-provider-kind 互換）と、その上に Argo CD・cert-manager・MetalLB・Istio Ambient・Kyverno・SPIRE・Dapr operator・flagd・CNPG・Strimzi Kafka・MinIO・Valkey・OpenBao・Backstage・Grafana/Loki/Tempo・Keycloak が役割別に段階配備される。Dapr Components（state / pub-sub / bindings / secret / config）は [`tools/local-stack/dapr/components/`](../../../../tools/local-stack/dapr/components/) に置かれ、`up.sh` の `apply_dapr` ステップで cluster に投入される。

### Phase 2 でも残る差分（解消は次フェーズ）

`base/Dockerfile`（debian slim 起点）からの派生は依然として未整備で、各プロファイルは Microsoft 公式 image（`mcr.microsoft.com/devcontainers/rust:1-bookworm` 等）を直接 `FROM` する。Dev Container image の GHCR digest 固定（IMP-DEV-DC-016）と Renovate 連動も未実装で、現状はローカルビルドのみ。これらは Platform/Build 体制が確立した時点で着手する。

`postCreate.sh` は toolchain 検証・`checkout-role.sh --verify`・`up.sh --role <role>` の連鎖を実行するが、kind スタック起動は分単位で時間を要するため、image build 直後の重いブートを避けたい場面のために `K1S0_SKIP_LOCAL_STACK=1` でスキップ可能にしてある。

bootstrap 段階の実装ステータスと Phase 2 移行後の差分は [`tools/devcontainer/README.md`](../../../../tools/devcontainer/README.md) に詳述する。
