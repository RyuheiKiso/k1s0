...
## Phase 1 bootstrap の現状（リリース時点 単一開発者フェーズ）

設計の正典は 10 役並走を前提とするが、リリース時点 の単一開発者フェーズでは段階的稼働として bootstrap を [`tools/devcontainer/`](../../../../tools/devcontainer/README.md) に配置している。bootstrap 段階の差分は次のとおりで、team 拡大段階で正典に揃える。

- **稼働中プロファイル**: `tier1-rust-dev` のみ（残り 9 役は team 拡大段階で順次配置）
- **base image 起点**: `mcr.microsoft.com/devcontainers/rust:1-bookworm`（Microsoft 公式）。設計の正典では `tools/devcontainer/base/Dockerfile`（debian slim 由来）からの派生だが、Platform/Build 体制が確立する Phase 2 で base/ を整備した時点で `FROM` を切替える
- **配布**: ローカルビルドのみ（GHCR digest 固定 / Renovate 連動は未実装、IMP-DEV-DC-016 の物理化は Phase 2）
- **役割切替**: `tools/sparse/checkout-role.sh` は未整備（1 役のみのため切替対象なし）
- **postCreate**: `tools/devcontainer/postCreate.sh` で Dapr CLI 導入と toolchain 検証のみを実施。`tools/local-stack/up.sh` の呼び出しは local-stack 整備時に追加する

bootstrap の実装ステータスと既知の差分は [`tools/devcontainer/README.md`](../../../../tools/devcontainer/README.md) に詳述する。Paved Road（ADR-DEV-001）の「正しい道一本化」は、現フェーズでは「tier1 中心の唯一の道」として一時的に縮約されており、team 拡大段階で各役割の独立した正しい道に分かれる。
