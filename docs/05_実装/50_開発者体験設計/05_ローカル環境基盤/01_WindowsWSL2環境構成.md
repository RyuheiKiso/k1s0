...
- group: 開発者ユーザーを `docker` グループに追加し、`sudo` なし運用を可能にする。**`usermod -aG docker $USER` の反映は新セッション以降に限定**される（既存の VS Code Server / Claude Code / シェルプロセスは古い group 集合のまま稼働）。Docker CE 導入を既に走っている VS Code セッションを跨いで実行した場合は、`wsl --shutdown` で WSL distribution を一旦終了して再接続するのが正規。一時しのぎとして `sudo setfacl -m u:$USER:rw /var/run/docker.sock` で当該ソケット inode に ACL を直接付与する手段もあるが、docker daemon 再起動で失われるため再接続できる時点で `wsl --shutdown` 経路に揃える
