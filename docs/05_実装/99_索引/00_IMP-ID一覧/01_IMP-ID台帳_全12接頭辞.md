...
|---|---|---|---|---|---|---|
| IMP-BUILD | 10 ビルド | 8 | 48 | 56 | 43 | CW（Rust workspace）/ GM（Go module）/ TP（pnpm workspace）/ DS（dotnet sln）/ PF（path-filter）/ CS（キャッシュ） |
| IMP-CODEGEN | 20 コード生成 | 8 | 32 | 40 | 59 | BUF（buf 生成）/ OAS（OpenAPI）/ SCF（Scaffold CLI）/ GLD（Golden snapshot） |
| IMP-CI | 30 CI/CD | 8 | 52 | 60 | 39 | RWF（reusable workflow）/ PF（path-filter）/ HAR（Harbor/Trivy）/ QG（quality gate）/ BP（branch protection）/ LCDT（ローカル開発ツール） |
...
リリース時点 全体で採番済 ID は計 625 件（POL 84 件 + 実装 541 件）、予約残は 563 件。実装 541 件は リリース時点 段階の「核心節 20 ファイル + 依存管理 3 節 + ガバナンス 4 節 + コード生成 OAS / GLD 2 節 + CI/CD PF / QG / BP / LCDT 4 節 + ビルド TP / DS / PF / CS 4 節 + 開発者体験 SO / BSN / ONB 3 節 + 観測性 LGTM / PYR / EB / RB 4 節 + リリース FFD / AT / RB 3 節 + サプライチェーン SBM / SLSA / FLG 3 節 + Identity OBO / CRT 2 節 + DX SPC / SCAF / TFC / EMR 4 節 + 索引 整合性 CI / catalog-info 検証 2 節」から抽出した純粋な採番で、POL を除く（核心節 20 = 19 + 開発者体験 ENV 1 節 = 05_ローカル環境基盤/01_WindowsWSL2環境構成.md）。
...
| IMP-BUILD-TP-030〜037 | tier3 web pnpm workspace 2 分割 / glob 分離 / workspace:* 強制 / lint 除外 / 3 層キャッシュ ほか | `10_ビルド設計/30_TypeScript_pnpm_workspace/01_TypeScript_pnpm_workspace.md` | 採用初期 |
| IMP-BUILD-DS-040〜047 | dotnet 3 sln 分割 / ProjectReference 強制 / Directory.Build.props / CPM / packages.lock.json / NuGet 2 層 ほか | `10_ビルド設計/40_dotnet_sln境界/01_dotnet_sln境界.md` | 採用初期 |
| IMP-BUILD-PF-050〜057 | 4 段 path-filter（tier / 言語 / workspace / 契約変更横断）/ reusable workflow 化 / フェイルセーフ ほか | `10_ビルド設計/50_選択ビルド判定/01_選択ビルド判定.md` | 0 |
| IMP-BUILD-CS-060〜067 | 3 層キャッシュ全体設計（ローカル / CI / リモート）/ キーハッシュ設計 / SLI 計測 / OIDC アクセス制御 ほか | `10_ビルド設計/60_キャッシュ戦略/01_キャッシュ戦略.md` | 採用初期 |
| IMP-BUILD-POL-008 | （次番予約 — 次期原則追記用） | `10_ビルド設計/00_方針/01_ビルド設計原則.md` | 0 |

予約: IMP-BUILD-CW-018〜019 / IMP-BUILD-GM-028〜029 / IMP-BUILD-TP-038〜039 / IMP-BUILD-DS-048〜049 / IMP-BUILD-PF-058〜059 / IMP-BUILD-CS-068〜069 / IMP-BUILD-POL-009（次期拡張用）。
...
| IMP-CODEGEN-POL-008 | （次番予約 — 次期コード生成原則追記用） | `20_コード生成設計/00_方針/01_コード生成原則.md` | 0 |

予約: IMP-CODEGEN-BUF-018〜019 / IMP-CODEGEN-OAS-028〜029 / IMP-CODEGEN-SCF-038〜039 / IMP-CODEGEN-GLD-048〜049 / IMP-CODEGEN-POL-009 / IMP-CODEGEN-050〜099（リリース時点 の Backstage 統合 / 5 言語目追加用）。
...
| IMP-CI-LCDT-080 | ローカル検査 orchestrator（`make verify` / `verify-quick` / `tools/ci/verify-local.sh`） | `30_CI_CD設計/60_ローカル開発ツール/01_ローカル検査と自動修正設計.md` | 0 |
| IMP-CI-LCDT-081 | PR title 自動補正 workflow（`.github/workflows/pr-title-autofix.yml`） | 同上 | 0 |
| IMP-CI-LCDT-082 | branch 命名規約（`<type>/<scope>/<subject>` 形式を入力源にする） | 同上 | 0 |
| IMP-CI-LCDT-083 | （次番予約 — LCDT 節追記用） | 同上 | 0 |
| IMP-CI-POL-008 | （次番予約 — 次期 CI 原則追記用） | `30_CI_CD設計/00_方針/01_CI_CD原則.md` | 0 |

予約: IMP-CI-RWF-022〜029 / IMP-CI-PF-038〜039 / IMP-CI-HAR-052〜059 / IMP-CI-QG-068〜069 / IMP-CI-BP-078〜079 / IMP-CI-LCDT-084〜089 / IMP-CI-POL-009（次期拡張用、merge queue / ポリシー段拡張）。
...
| IMP-DEP-REN-010〜019 | Renovate 中央 preset / preset バージョン固定 / cron 実行時刻統制 / Dependency Dashboard 有効化 / automerge 条件 / 15 package manager 横断 ほか | `40_依存管理設計/10_Renovate中央運用/01_Renovate中央運用設計.md` | リリース時点〜採用初期 |
| IMP-DEP-SBM-020〜027 | CycloneDX 形式統一 / syft による SBOM 生成 / diff スクリプト / CI ゲート / Grype 連動 / 脆弱性通知ワークフロー ほか | `40_依存管理設計/20_SBOM差分監視/01_SBOM差分監視設計.md` | リリース時点〜採用初期 |
| IMP-DEP-LIC-030〜037 | SPDX ライセンス分類 / BUSL/SSPL 即時拒否 / AGPL 6 件の分離検証 / GitHub Action gate / third_party UPSTREAM.md 強制 ほか | `40_依存管理設計/30_ライセンス判定/01_ライセンス判定設計.md` | リリース時点〜採用初期 |

予約: IMP-DEP-038〜099（運用蓄積後で CDN / CI キャッシュ整合 / 言語別 lockfile 検証など追加予定）。

## IMP-DEV（50 章 開発者体験設計）

Paved Road 思想・ローカル環境基盤（Windows + WSL2）・10 役 Dev Container・Golden Path examples・Scaffold CLI 運用・Backstage 連携・Onboarding 動線の 7 本柱を採番する。方針は `50_開発者体験設計/00_方針/01_開発者体験原則.md`、実装は ENV（環境基盤）/ DC（Dev Container）/ GP（Golden Path）/ SO（Scaffold）/ BSN（Backstage）/ ONB（Onboarding）の 6 節。
...
| IMP-DEV-ENV-060 | Windows 11 + WSL2 (Ubuntu 24.04 LTS) + systemd を標準ホスト環境とする | `50_開発者体験設計/05_ローカル環境基盤/01_WindowsWSL2環境構成.md` | 0 |
| IMP-DEV-ENV-061 | Docker ランタイムは WSL ネイティブ docker-ce で固定（Docker Desktop 不採用） | 同上 | 0 |
| IMP-DEV-ENV-062 | `.wslconfig` による resource allocation 一元管理（memory / processors / swap / nestedVirt 等） | 同上 | 0 |
| IMP-DEV-ENV-063 | Dev Container / kind / Dapr Local sidecars を同一 WSL2 distribution 内で sibling 起動 | 同上 | 0 |
| IMP-DEV-ENV-064 | Windows host 側残置責務の固定（VS Code host UI / Browser / GCM Core / SSH agent / `.wslconfig`） | 同上 | 0 |
| IMP-DEV-ENV-065 | リポジトリは WSL2 ext4 上に配置、`/mnt/c` 配下を禁止（9p ペナルティ回避） | 同上 | 0 |
...
| IMP-POL-KYV-010〜022 | validate/mutate/generate Policy 分類 / exception 30 日時限 / audit → enforce 段階移行 / Policy Report 集約 ほか | `90_ガバナンス設計/10_Kyverno_Policy/01_Kyverno_Policy二分所有設計.md` | リリース時点〜採用初期 |
...
| IMP-POL-STR-040〜047 | STRIDE 脅威モデル / 11 API マトリクス / 緩和策 ADR 化 / 再評価 trigger ほか | `90_ガバナンス設計/40_脅威モデル_STRIDE/01_STRIDE脅威モデル設計.md` | リリース時点〜採用初期 |
