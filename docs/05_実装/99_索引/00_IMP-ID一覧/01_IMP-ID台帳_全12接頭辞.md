...
リリース時点 全体で採番済 ID は計 398 件（POL 84 件 + 実装 314 件）、予約残は 790 件。実装 314 件は リリース時点 段階の「核心節 19 ファイル + 依存管理 3 節 + ガバナンス 4 節 + コード生成 OAS / GLD 2 節 + CI/CD PF / QG / BP 3 節 + 開発者体験 SO / BSN / ONB 3 節」から抽出した純粋な採番で、POL を除く。
