...
| IMP-DEP | 40 依存管理 | 7 | 0 | 7 | 92 | 未採番（Phase 1a 以降で POL→実装展開） |
...
| IMP-POL | 90 ガバナンス | 7 | 0 | 7 | 92 | 未採番（Phase 1a で Kyverno ポリシー展開） |
| IMP-DX | 95 DX メトリクス | 7 | 11 | 18 | 81 | DORA（DORA Four Keys） |
| IMP-TRACE | 99 索引 | 7 | 0 | 7 | 92 | 未採番（POL のみ。索引は運用フェーズ 1a〜） |

Phase 0 全体で採番済 ID は計 268 件（POL 84 件 + 実装 184 件）、予約残は 920 件。実装 184 件は Phase 0 段階の「核心節 19 ファイル」から抽出した純粋な採番で、POL を除く。
...
Renovate 中央集約・lockfile commit 必須・SBOM 添付の 7 原則のみ採番済。実装 ID は Phase 1a 以降で Renovate preset / lockfile drift 検出 / SBOM 差分監視の各節採番で展開する。
...
予約: IMP-DEP-010〜099（Phase 1a 以降の実装節で REN / LOC / SBO 等のサブ接頭辞で採番）。
...
Kyverno による dual ownership（Platform + Security）ポリシーを 7 原則として採番済。実装 ID は Phase 1a 以降の Policy Exception Runbook / NetworkPolicy / OPA 連携で採番される予定。
...
予約: IMP-POL-010〜099（Phase 1a: validate/mutate/generate ポリシーの節採番、Phase 1b: OPA 連携）。
