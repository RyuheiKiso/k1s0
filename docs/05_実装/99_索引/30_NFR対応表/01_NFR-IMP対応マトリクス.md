...
本ファイルは `03_要件定義/30_非機能要件/` 配下の NFR-\* ID（非機能要件 ID）と、本実装フェーズで採番された IMP-\* ID の対応を「NFR から IMP を逆引き」する方向で表現する。監査対応や受け入れ基準レビュー時に「この NFR を満たす実装 ID は何か」を 1 ファイルで特定できる状態を IMP-TRACE-POL-005（双方向リンク）の系として提供する。
...
各節の冒頭で、該当カテゴリが本実装フェーズで何を要求し、実装面ではどの章・どの IMP 群で受けるかを散文で示す。続いて NFR ID ごとに「直接対応 IMP」と「間接対応 IMP」の 2 列を表で列挙する。NFR 側の「受け入れ基準」は `03_要件定義/30_非機能要件/` の各カテゴリファイルで確定しているため、本ファイルは ID 間の対応のみを示す。
...
| NFR-A-DR-\* | Phase 1b 以降で対応（本章未結合） | `IMP-REL-ARG-010〜017`（ArgoCD による GitOps DR 基盤） |
| NFR-A-FT-002〜004（多重化 / 障害注入 / 縮退） | Phase 1a で対応（本章未結合） | `IMP-REL-PD-026〜028`（Canary / Blue-Green の多重化経路） |
...
| NFR-B-WL-\*（ワークロード規模） | Phase 1b で対応（本章未結合） | `IMP-OBS-OTEL-010〜019`（規模計測基盤） |
| NFR-B-RES-\*（水平・垂直拡張） | Phase 1a で対応（本章未結合） | `IMP-CI-RWF-013`（runner 自動スケール） |
| NFR-B-QA-\*（性能試験） | Phase 1b で対応（本章未結合） | `IMP-CI-RWF-018`（coverage 閾値段階導入の先に性能試験） |

## C 運用保守性（Operability / Maintainability）

2 名運用前提の NFR が 12 章全体に伸びる最重要カテゴリ。本章では 10 章ビルド・30 章 CI/CD・40 章依存管理・50 章 DX・90 章ガバナンス全てに結合する。

| NFR | 直接対応 IMP | 間接対応 IMP |
|---|---|---|
| NFR-C-NOP-001（2 名運用） | `IMP-DEV-POL-003`（10 役 Dev Container） / `IMP-DEV-POL-004`（time-to-first-commit SLI） | `IMP-BUILD-POL-006`（ビルド時間 SLI） / `IMP-DX-POL-001〜007`（DORA 4 keys） |
...
| NFR-D-TIM-\*（移行時期） | Phase 1a〜1b で対応（本章未結合） | `IMP-DEV-GP-025`（legacy-wrap example） |
| NFR-D-OBJ-\*（移行対象） | Phase 1a で対応（本章未結合） | `IMP-REL-ARG-010〜017`（移行対象の ArgoCD App 管理） |
| NFR-D-PLN-\*（移行計画） | Phase 1a で対応（本章未結合） | `IMP-DX-DORA-010〜020`（Deployment Frequency で移行進捗可視化） |
...
| NFR | 直接対応 IMP | 間接対応 IMP |
|---|---|---|
| NFR-F-SYS-\*（システム環境） | Phase 1a で対応（本章未結合） | `IMP-DEV-DC-010〜017`（Dev Container による環境均一化） |
| NFR-F-CHR-\*（時系列・ログ保全） | Phase 1a で対応（本章未結合） | `IMP-SEC-KC-021`（Keycloak event 7 年保存） |
...
| NFR | 直接対応 IMP | 間接対応 IMP |
|---|---|---|
| NFR-G-CLS-\*（データ分類） | Phase 1a で対応（本章未結合） | `IMP-OBS-OTEL-010〜019`（Gateway の PII transform） |
...
| NFR-I-SLO-101〜107（機能系 SLO） | `IMP-OBS-SLO-040〜047`（Phase 0 範囲） / Phase 1a で残分採番 | `IMP-DX-DORA-010〜020`（DORA 4 keys） |
...
Phase 0 時点で `03_要件定義/30_非機能要件/` に定義された NFR は 9 カテゴリ計 **148 件**（A: 13、B: 16、C: 20、D: 11、E: 27、F: 12、G: 18、H: 12、I: 19）。本章の IMP-\* が直接または間接で結合するのは **約 100 件（カバレッジ 約 68%）**。

未カバーの約 48 件は以下に分類される。(a) Phase 1a 以降で対応予定: NFR-A-DR-\*, NFR-A-FT-002〜004, NFR-B-WL-\*, NFR-B-RES-\*, NFR-B-QA-\*, NFR-D-TIM/OBJ/PLN, NFR-F-SYS/CHR の一部, NFR-G-CLS/DES/INT/LIF/PRV/RES で計 **約 35 件**。(b) 00 章 `IMP-DIR-*` で受ける: NFR-C-NOP-002 可視性の物理配置面など約 **5 件**。(c) 監査フェーズで確認のみ（IMP 対応不要）: NFR-F-STD / NFR-H-AUD-\* の純粋監査要件約 **8 件**。

IMP-TRACE-POL-005（双方向リンク）の系として、CI の孤立リンク検出（`tools/ci/trace-check/`）で Phase 1a 以降に NFR 全件の結合可否を自動判定する運用へ移行する。
