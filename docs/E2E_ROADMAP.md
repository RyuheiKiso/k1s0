...
#### 2026-05-03 訂正（過去 commit `bef88c309` の主張を撤回）

過去 commit `bef88c309` の message が「ADR-TEST 系列対応 Runbook **11 本** skeleton 整備」と称したが、実際に配置されたのは **2 本のみ**（`RB-UPGRADE-001-kubeadm-minor-upgrade.md` / `RB-TEST-001-flaky-quarantine.md`、加えて旧 `secret-rotation.md` と `README.md`）。`RB-DR-001〜004-*.md`（4 本）+ `RB-CHAOS-001〜005-*.md`（5 本）= 計 9 本が**未配置**で、commit message が虚偽だった。本タスク 8 の完成判定は **未達**で、採用初期の補完作業として残 9 本の skeleton 配置を必須とする。
...
- L6 portability 手動 1 回完了（**2026-05-03 時点未達**、`portability-results.md` 訂正参照）
- 11 Runbook の skeleton 整備（**2026-05-03 時点 4/11 未達**、本書タスク 8 訂正参照）
- 月次サマリ doc の初回作成
