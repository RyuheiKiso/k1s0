...
ADR-TEST-001 Phase 0 例外として「L6 portability は Phase 0 で手動 1 回、Phase 3 で自動化」と決定済。Phase 1 の最初の 1 ヶ月以内に EKS / GKE / AKS のいずれか 1 つで `make verify-e2e` 相当を手動実行し、結果（OK / FAIL / 制約） を `docs/40_運用ライフサイクル/portability-results.md`（新設）に記載する。これは「採用検討者が k1s0 はマネージド K8s で動くか」を判定する第一証跡となる。

依存: 1〜9 のうち最低 1（tools/local-stack 拡張）と 7（Makefile target）が完了していればよい。
工数: 1〜2 人日（cluster 立ち上げ + 1 回実走 + report 作成）。
完成判定: いずれか 1 つのマネージド K8s で `make verify-e2e` が完了し、結果が `portability-results.md` に記載されている。
...
- 24h soak test 月次運用化
- L6 portability 自動化（EKS / GKE / AKS の 3 並列、月次）
