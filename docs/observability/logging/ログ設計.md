...
> **dev 環境のログフォーマット**: dev 環境では開発者の可読性を優先し `text` フォーマットを許容する（[helm設計.md](../../infrastructure/kubernetes/helm設計.md) の values-dev.yaml 参照）。staging / prod では JSON 構造化ログを必須とする。
