...
- [system-vault-server設計](../../system-servers/vault/server設計.md) — シークレット管理サーバー設計
- [system-library-encryption設計](encryption設計.md) — 暗号化ライブラリ（シークレットの暗号化保存）
- [system-library-cache設計](../data/cache設計.md) — k1s0-cache ライブラリ（キャッシュ基盤）
- [system-library-kafka設計](../messaging/kafka設計.md) — Kafka コンシューマー（`k1s0.system.vault.secret_rotated.v1`）
