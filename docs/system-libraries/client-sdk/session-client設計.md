...
- [system-library-authlib設計](../auth-security/authlib設計.md) — JWT 認証ライブラリ（JWT 検証と組み合わせて利用）
- [system-library-cache設計](../data/cache設計.md) — Redis キャッシュライブラリ（セッションストアの内部実装）
- [認証設計.md](../../auth/design/認証設計.md) — 認証設計
- [JWT設計.md](../../auth/mechanisms/JWT設計.md) — JWT 設計
