...
```
tracing/
├── tracing.go         # TraceContext, Baggage, InjectContext, ExtractContext
├── tracing_test.go
└── go.mod
```
...
│   ├── types.ts         # TraceContext, toTraceparent, fromTraceparent
│   ├── baggage.ts       # Baggage クラス
│   └── propagation.ts   # injectContext, extractContext
├── __tests__/
│   └── tracing.test.ts
├── package.json
└── tsconfig.json
...
│   └── src/
│       ├── trace_context.dart   # TraceContext
│       ├── baggage.dart         # Baggage
│       └── propagation.dart     # injectContext, extractContext
├── test/
│   └── tracing_test.dart
└── pubspec.yaml
```
...
- [system-library-correlation設計](correlation設計.md) — k1s0-correlation（相関ID・トレースID管理）
- [可観測性設計.md](../../observability/overview/可観測性設計.md) — 可観測性設計ガイドライン
