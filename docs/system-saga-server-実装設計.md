...
system-saga-server（Sagaオーケストレーションサーバー）の Rust 実装詳細を定義する。概要・API 定義・アーキテクチャは [system-saga-server設計.md](system-saga-server設計.md) を参照。
...
Saga の状態遷移時に以下のイベントを `k1s0.system.saga.events.v1` トピックに発行する:
...
- [system-saga-server設計.md](system-saga-server設計.md) -- 概要・API 定義・アーキテクチャ
- [system-server-実装設計.md](system-server-実装設計.md) -- auth-server 実装設計（同一パターン）
- [system-saga-database設計.md](system-saga-database設計.md) -- Saga データベーススキーマ
- [メッセージング設計.md](メッセージング設計.md) -- Saga パターンの基本方針
