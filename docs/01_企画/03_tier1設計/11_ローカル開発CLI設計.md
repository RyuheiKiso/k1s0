...
| フェーズ | 施策 | 備考 |
|---|---|---|
| Phase 2 前半 | `k1s0 dev start` / `stop` / `status` / `logs` の実装 | tier2 開発開始の前提条件 |
| Phase 2 前半 | エラー自動翻訳の初期ルール (4.1 節) の実装 | 主要エラー 10 件をカバー |
| Phase 2 前半 | 自動復旧 (Dapr / Valkey / Kafka) の実装 | 最小構成で提供開始 |
| Phase 2 後半 | `k1s0 dev config` / `secrets` の実装 | `k1s0.config.yaml` の導入 |
| Phase 2 後半 | `k1s0 dev health` / `dashboard` / `trace` の実装 | 診断機能との統合 |
| Phase 3 | `k1s0 dev rebuild` の実装 | ビルドキャッシュ問題の対処 |
...
- [`09_デバッグ支援ツール.md`](./09_デバッグ支援ツール.md) — Tilt 診断ダッシュボードとランブック
- [`08_診断機能設計.md`](./08_診断機能設計.md) — 診断コンテキストと Diag API
...
- [`10_Saga補償パターン支援.md`](./10_Saga補償パターン支援.md) — Saga テストランナー CLI
- [`../05_CICDと配信/05_共有開発インフラサーバー.md`](../05_CICDと配信/05_共有開発インフラサーバー.md) — 共有サーバー導入によるローカル k8s 排除と CLI 内部実装の変更
- [`13_パフォーマンス帰属設計.md`](./13_パフォーマンス帰属設計.md) — `k1s0 dev trace` の拡張 (ウォーターフォール出力・遅延リクエスト一覧)
