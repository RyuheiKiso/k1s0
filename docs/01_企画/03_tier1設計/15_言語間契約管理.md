...
[Buf](https://buf.build/) は Protobuf スキーマの lint / 後方互換性検証の事実上の標準ツール ([`../04_技術選定/10_追加採用OSS_2.md`](../04_技術選定/10_追加採用OSS_2.md) で採用済み)。本資料では `buf breaking` を CI に組み込んで破壊的変更を機械的に検出する。
...
Testcontainers を用いて以下を実行する ([`../04_技術選定/07_追加採用OSS.md#P`](../04_技術選定/07_追加採用OSS.md))。
...
Phase 2 以降、Litmus ([`../04_技術選定/07_追加採用OSS.md`](../04_技術選定/07_追加採用OSS.md)) で以下を検証する。
...
- [`../04_技術選定/10_追加採用OSS_2.md`](../04_技術選定/10_追加採用OSS_2.md) — Buf の採用根拠
