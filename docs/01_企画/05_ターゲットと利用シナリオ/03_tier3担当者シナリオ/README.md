...
- ジュニア担当者が誤って業務ロジックを UI 層に書いてしまい、テスト困難なモノリスが生まれる
- 業務エラー (並行編集 / ERP 応答遅延) の UX 設計がファイルごとに異なり、一貫性がない
- オフライン対応 (IndexedDB / Service Worker) の実装を各担当者が独自にやると、データ消失事故が起きる
- a11y / i18n 対応を後付けで行うと、既存コンポーネントの大規模改修が必要になる
- WebAuthn / break-glass / step_up の認証ロジックを UI 側に持ち込むと、セキュリティ抜けが発生する

## k1s0 でこう変わる

- 13 層強制機構が業務ロジック混入を物理拒否し、ジュニア担当者は業務 UI 表現に集中できる
- BusinessConflict 4 subtype の標準 UX コンポーネントで、エラー UX が全画面で統一される
- 4 layer client state (ST / OL / PQ / DR) と Idempotency-Key が透過的に機能し、オフライン業務が自然に成立する
- Backstage Software Template で a11y / i18n が day-1 から自動組込される
- 認証コンテキスト伝播を tier1 が集中所有し、UI 側には認証ロジックが持ち込まれない構造にする
...
| 12 | [検索一覧UX実装](12_検索一覧UX実装.md) | 業務担当者要求または新業務画面に検索 / 一覧 UX が必要になった時 | 月次〜週次 | クライアント状態適合仕様 | [計画] |
| 13 | [通知UI_4種実装](13_通知UI_4種実装.md) | 業務通知 UI (toast / badge / banner / inbox) の追加要求が来た時 | 月次 | クライアント状態適合仕様 / Bidi 適合仕様 | [計画] |
| 14 | [状態遷移UI_FSM実装](14_状態遷移UI_FSM実装.md) | FSM 状態に連動する UI 遷移の実装が必要になった時 | 月次〜四半期 | クライアント状態適合仕様 | [計画] |
