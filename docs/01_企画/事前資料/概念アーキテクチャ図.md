...
graph TB
    ...
    subgraph Tier1["tier1: 共通基盤サービス層"]
        subgraph T1API["tier1 公開 API (多言語クライアントライブラリ)"]
            CL[ログ / OTel / state / pub-sub / secrets<br/>workflow / 認証 / 監査ログ 等の統一 API<br/>C# / Go / Java / TS / Python 各言語向け]
        end
        subgraph T1Impl["tier1 内部実装 (tier2 / tier3 からは不可視)"]
            D[Dapr Control Plane + サイドカー<br/>各言語安定 SDK をライブラリ内でのみ利用]
            R[Rust 実装サービス<br/>JTC 固有機能 / Dapr Component 管理 / 雛形生成 CLI]
        end
        CL --> D
        CL --> R
    end
...
- tier1 は以下の 3 要素で構成される。
  ...
  - **tier1 内部実装 (不可視)** — 「Dapr」と「Rust 自前実装サービス」の並立で構成される。tier2 / tier3 からは隠蔽される。
...
- **利用判断**: tier1 公開 API の設計時に「機能が Dapr で賄えるなら内部で Dapr SDK を呼ぶ、不足するなら Rust 実装サービスを呼ぶ」という判断を tier1 側で行う。tier2 / tier3 開発者はこの判断に関与しない。
- 担当: システム基盤チーム
- 管轄ディレクトリ: `src/tier1/`
- 実装言語:
  - 公開 API: 各対象言語 (C# / Go / Java / TS / Python 等)。薄いラッパー。
  - 内部 Rust 実装: Rust 固定。Dapr Rust SDK は Alpha のため直接呼ばない。
    ...
    subgraph OnPrem["オンプレミス OR クラウド VM"]
        subgraph K8s["Kubernetes クラスタ"]
            ...
            subgraph NS2["namespace: tier1"]
                ...
                C5b[tier1 Rust サービス<br/>JTC 固有機能 / 認証プロキシ 等]
            end
...
flowchart LR
    subgraph MVP["MVP 対象 (暫定)"]
        ...
        M5[tier1 雛形生成 CLI<br/>Dapr annotation + tier1 lib 同梱]
    end
