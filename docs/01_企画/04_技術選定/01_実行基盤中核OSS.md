...
**採用方針**: OTel を計装層として統一し、バックエンドは差し替え可能な構成にする。tier1 公開 API (`k1s0.Telemetry.*`) が各バックエンドを抽象化する。
...
tier1 の内部サービスの実装言語選定。公開クライアントライブラリは別系統で各対象言語 (C# / Go / Java / TS / Python) に提供する。
