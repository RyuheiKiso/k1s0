...
[`09_ネットワークとテレメトリ基盤.md`](./09_ネットワークとテレメトリ基盤.md) (Q〜S) に続き、追加で採用が決定した OSS の選定根拠を整理する。対象カテゴリは以下のとおり。
...
2. **プラットフォーム自己監視の前提条件** — [`../02_アーキテクチャ/06_プラットフォーム自己監視.md`](../02_アーキテクチャ/06_プラットフォーム自己監視.md) で定義された infra 層・tier1 層の監視項目の多くが kube-state-metrics のメトリクスに依存する。これがなければアラートルールが機能しない
...
- [`07_追加採用OSS.md`](./07_追加採用OSS.md) — 追加 OSS の選定 (L〜S)
...
- [`../02_アーキテクチャ/06_プラットフォーム自己監視.md`](../02_アーキテクチャ/06_プラットフォーム自己監視.md) — kube-state-metrics が提供するメトリクスの利用先
