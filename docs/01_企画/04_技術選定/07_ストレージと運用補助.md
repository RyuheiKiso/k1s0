...
k1s0 のデータ保全・接続プーリング・運用自動化・イベント連携・統合テストに関わる追加採用 OSS を整理する。中核 ([`01_実行基盤中核OSS.md`](./01_実行基盤中核OSS.md)) および周辺 A〜K ([`02_周辺OSS.md`](./02_周辺OSS.md)) で埋めきれない領域を補完する。対象カテゴリは以下のとおり。
