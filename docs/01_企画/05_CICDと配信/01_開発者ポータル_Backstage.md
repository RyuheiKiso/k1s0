...
k1s0 には独自の **雛形生成 CLI** がある ([`../03_tier1設計/03_API設計原則.md`](../03_tier1設計/03_API設計原則.md) 参照)。Backstage の Software Templates 機能はこの CLI と連動させる。

| ステップ | 担当 |
|---|---|
| テンプレート選択 UI (C# / Go / Java 等) | Backstage |
...
| データベース | PostgreSQL (Backstage 標準)。`infra` 層の共通 PostgreSQL を利用 |
| バックアップ / アップグレード | 運用チーム責任 |
| 参照先 | k8s API / Jaeger / Grafana / Loki / Prometheus / GitHub / tier1 監査ログ API |
...
| **Phase 2** | Software Templates (雛形生成 CLI 統合) / Kubernetes / Jaeger / Grafana / GitHub Actions / Argo CD プラグイン |
...
- [`../02_アーキテクチャ/03_配置形態.md`](../02_アーキテクチャ/03_配置形態.md) — `operation` namespace の配置方針
- [`../03_tier1設計/03_API設計原則.md`](../03_tier1設計/03_API設計原則.md) — 雛形生成 CLI の位置付け
- [`../04_技術選定/02_周辺OSS.md`](../04_技術選定/02_周辺OSS.md) — Keycloak / Argo CD の選定根拠
