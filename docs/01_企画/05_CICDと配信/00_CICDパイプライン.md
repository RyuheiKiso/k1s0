...
| チェック点 | ツール | 検出時の動作 |
|---|---|---|
| ビルド前 (FS スキャン) | Trivy FS (GHA step) | `HIGH` 以上で警告、`CRITICAL` でパイプライン失敗 |
| push 後 (イメージスキャン) | Harbor 内蔵 Trivy | `CRITICAL` を含むイメージは `pull` を拒否 (Harbor プロジェクトポリシー) |
| 開発ブランチ例外 | — | `dev` ブランチは警告のみ、`main` のみ厳格適用 |
| Phase 2 以降 | Cosign + Kyverno | 未署名イメージの deploy を k8s Admission Webhook で拒否 |
