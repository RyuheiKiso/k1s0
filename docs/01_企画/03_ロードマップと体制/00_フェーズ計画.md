...
OSS の導入時期は [`../../02_構想設計/03_技術選定/01_俯瞰/19_フェーズ別採用タイムライン.md`](../../02_構想設計/03_技術選定/01_俯瞰/19_フェーズ別採用タイムライン.md)、体制の詳細は [`02_体制と役割.md`](./02_体制と役割.md) を参照。本章では「いつ何をするか」の時系列、「04 章」では「どの OSS が Intro/Prod になるか」、「02 章」では「誰が何を担当するか」を扱う。
...
|---|---|---|
| Phase 1a 終了 | デモが決裁者に受容されず、協力者のアサインが得られない | 15 万円 (4 人日 × 3.8 万円/日。単独 VM 1 台の環境破棄 + ドキュメント整理のみ) |
| Phase 1b 終了 | バス係数 2 の実証失敗 (協力者が独立して環境再構築できない) / パイロット業務のユーザー受容失敗 | 120 万円 (25 人日 × 4.8 万円/日。kubeadm HA 3 ノード破棄 + パイロット業務の旧環境への差戻し + Argo CD / OpenTofu state の削除) |
| Phase 1c 終了 | 運用引継ぎ失敗 (起案者不在で障害対応不可) / Runbook が実用レベルに到達しない | 230 万円 (43 人日 × 5.3 万円/日。Harbor / OpenBao / バックアップ基盤の撤収 + 稼働中パイロット業務の代替基盤への移設 + 教育費の一部未回収分) |
| Phase 2 終了 | tier2 サービスの採用が進まない (稼働業務が 1 本も追加されない) / 開発者ポータル利用率が目標の 20% 未満 | 520 万円 (90 人日 × 5.8 万円/日。Istio / Kafka / Temporal 等の新規 OSS を含む運用基盤の撤収 + tier2 サンプルの破棄 + Backstage Software Catalog からの全サービス登録解除) |
| Phase 3 以降 | 経営判断 (会社統合・予算削減・方針転換) | 業務数に比例 (1,200〜2,240 万円。稼働業務 1 本あたり 400 万円の Buy 移行費 × 3〜5.6 本を想定。[`../01_背景と目的/04_撤退戦略.md`](../01_背景と目的/04_撤退戦略.md) 第 5 章の算出根拠に基づく) |
...
- [`../../02_構想設計/01_アーキテクチャ/02_可用性と信頼性/04_マルチクラスタ戦略.md`](../../02_構想設計/01_アーキテクチャ/02_可用性と信頼性/04_マルチクラスタ戦略.md) — Phase 3 以降のマルチクラスタ設計
- [`../../02_構想設計/01_アーキテクチャ/02_可用性と信頼性/05_SLOとエラーバジェット.md`](../../02_構想設計/01_アーキテクチャ/02_可用性と信頼性/05_SLOとエラーバジェット.md) — SLO 定義とエラーバジェット運用
- [`../../02_構想設計/02_tier1設計/04_診断と補償/05_ワークフロー振り分け基準.md`](../../02_構想設計/02_tier1設計/04_診断と補償/05_ワークフロー振り分け基準.md) — Temporal / Dapr Workflow の振り分け基準
- [`../../02_構想設計/03_技術選定/03_周辺OSS/06_イベントスキーマレジストリ.md`](../../02_構想設計/03_技術選定/03_周辺OSS/06_イベントスキーマレジストリ.md) — Apicurio Registry の段階導入
- [`../../02_構想設計/04_CICDと配信/04_ローカル開発環境.md`](../../02_構想設計/04_CICDと配信/04_ローカル開発環境.md) — Tilt の段階導入
- [`../../02_構想設計/04_CICDと配信/00_CICDパイプライン.md`](../../02_構想設計/04_CICDと配信/00_CICDパイプライン.md) — CI/CD のフェーズ分離
- [`../../02_構想設計/04_CICDと配信/02_アプリ配信ポータル.md`](../../02_構想設計/04_CICDと配信/02_アプリ配信ポータル.md) — 配信ポータルのフェーズ分離
- [`../../02_構想設計/03_技術選定/02_中核OSS/03_ルールエンジン.md`](../../02_構想設計/03_技術選定/02_中核OSS/03_ルールエンジン.md) — ZEN Engine の段階導入
- [`../../02_構想設計/03_技術選定/03_周辺OSS/12_マニフェストとワークフロー.md`](../../02_構想設計/03_技術選定/03_周辺OSS/12_マニフェストとワークフロー.md) — Kustomize + Helm / Temporal の段階導入
- [`../../02_構想設計/03_技術選定/03_周辺OSS/14_オートスケーラー.md`](../../02_構想設計/03_技術選定/03_周辺OSS/14_オートスケーラー.md) — KEDA の段階導入
- [`../../02_構想設計/03_技術選定/03_周辺OSS/16_プロファイリング.md`](../../02_構想設計/03_技術選定/03_周辺OSS/16_プロファイリング.md) — Grafana Pyroscope の段階導入
- [`../04_定量試算/02_開発工数試算.md`](../04_定量試算/02_開発工数試算.md) — 本フェーズ計画の人月・期間の数的裏付け
- [`../04_定量試算/03_運用工数試算.md`](../04_定量試算/03_運用工数試算.md) — Phase 2 以降の運用工数詳細
- [`../04_定量試算/01_TCO5年試算.md`](../04_定量試算/01_TCO5年試算.md) — フェーズ計画を積算した 5 年 TCO
