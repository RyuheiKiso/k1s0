...
本資料は、k1s0 プロジェクトの投資判断・進行判断・撤退判断に使う「観測可能な基準値」を一枚に集約する。[`00_フェーズ計画.md`](./00_フェーズ計画.md) が「次に何をやるか」を定義し、[`../01_背景と目的/04_撤退戦略.md`](../01_背景と目的/04_撤退戦略.md) が「どう畳むか」を扱うのに対し、本資料は「続けるか畳むかをいつ・誰が・何を見て決めるか」を扱う。進行判断の責任は本章に集約される。

金額面の基準値（初期構築費・年間運用 FTE・商用案との TCO 差分）は [`../04_定量試算/01_TCO5年試算.md`](../04_定量試算/01_TCO5年試算.md) を一次ソースとして参照し、本資料はその閾値運用に特化する。数字で撤退条件を宣言することで、Phase 進行中に「惰性で続いてしまう」リスクを構造的に抑える。
...
|---|---|---|---|---|
| **初期構築費 (Phase 0〜1c 累計)** | 1,850 万円以内 | 1,850〜2,400 万円 (+30%) | 2,400 万円超 | [`../04_定量試算/02_開発工数試算.md`](../04_定量試算/02_開発工数試算.md) 第 6.3 節 |
| **Phase 0〜1c 期間** | 12 か月以内 | 12〜18 か月 (+50%) | 18 か月超 | [`./01_MVPスコープ.md`](./01_MVPスコープ.md) |
| **年間運用 FTE (Phase 2 以降)** | 小 3 / 中 5 / 大 10（規模別計画値）以内 | 計画値 +30% 以内 | 計画値 +50% 超 | [`../04_定量試算/01_TCO5年試算.md`](../04_定量試算/01_TCO5年試算.md) 第 2.2 節 |
| **tier1 API SLO（可用性）** | 99.9% 以上 | 99.5〜99.9% | 99.5% 未満 | [`../../02_構想設計/01_アーキテクチャ/02_可用性と信頼性/05_SLOとエラーバジェット.md`](../../02_構想設計/01_アーキテクチャ/02_可用性と信頼性/05_SLOとエラーバジェット.md) |
| **バス係数** | 2 名以上 | 2 名（1 名が休職中など不安定） | 1 名（起案者単独） | [`./02_体制と役割.md`](./02_体制と役割.md) |
| **パイロット業務稼働数** | 計画値通り | 計画値 -25% 以内 | 計画値 -50% 超 | [`./01_MVPスコープ.md`](./01_MVPスコープ.md) |
| **採用 OSS の継続性** | CNCF Graduated / 財団ガバナンス維持 | Incubating / 主要メンテナ離脱の噂 | Sandbox 降格 / ライセンス変更アナウンス | [`../../02_構想設計/03_技術選定/01_俯瞰/17_OSS長期戦略.md`](../../02_構想設計/03_技術選定/01_俯瞰/17_OSS長期戦略.md) |
| **商用案との TCO 差分** | 次点商用案より 15% 以上優位 | 5〜15% 優位 | 5% 未満または商用が優位 | [`../04_定量試算/01_TCO5年試算.md`](../04_定量試算/01_TCO5年試算.md) 第 1 節 |
...
中核 OSS（[`../../02_構想設計/03_技術選定/02_中核OSS/01_実行基盤中核OSS.md`](../../02_構想設計/03_技術選定/02_中核OSS/01_実行基盤中核OSS.md) の 7 種: k8s / Istio / Envoy Gateway / Kafka / Dapr / Go / Rust）のうち、2 種以上が 6 か月以内に非 OSI 承認ライセンスへ移行した場合。個別 1 種の有償化は [`../../02_構想設計/03_技術選定/01_俯瞰/17_OSS長期戦略.md`](../../02_構想設計/03_技術選定/01_俯瞰/17_OSS長期戦略.md) の差し替え戦略で吸収するが、複数同時は吸収コストが TCO 優位を崩す可能性が高い。
...
Phase 遷移判定会議は Phase 終了予定の 2 週間前に招集し、KPI 実績値・ゲート条件充足状況・撤退トリガー発動有無の 3 点を確認する。臨時判定会議（KPI 赤 2 系統 / 撤退トリガー発動）は発生から 2 週間以内に開催する。いずれも議事録は本ディレクトリ配下に蓄積し、次 Phase の判断材料にする。
...
再設定にあたっては、[`../04_定量試算/00_試算前提.md`](../04_定量試算/00_試算前提.md) の人月単価・インフラ単価の出典を最新版に更新した上で、本資料の閾値を機械的に再計算する。閾値の手動調整は原則行わず、前提の更新が閾値の更新に直結する構造を維持することで、「都合のよい閾値」への恣意的な変更を防ぐ。
