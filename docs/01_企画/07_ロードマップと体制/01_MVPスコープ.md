...
- **MVP-1a**: パイロット業務に必要な最小構成のみで開始する (コンポーネント数 12)
...
| ノード数 | 3 |
| vCPU / ノード | 4 |
| メモリ / ノード | 8 GB (合計 24 GB、余裕 約 11 GB) |
| ディスク / ノード | 200 GB SSD |
| ネットワーク | 1 Gbps |
...
項目 6 が MVP 全体の最重要完了条件 (バス係数 2 の実証)。
...
| Sealed Secrets | Secret の暗号化 Git 管理 | Vault (Phase 3) までの橋渡し |
...
ハードウェアは MVP-1a と同一 (3 ノード x 8 GB = 24 GB)。余裕 約 7 GB。
...
| 5 | 全 Secret が Sealed Secrets で暗号化 Git 管理されている | リポジトリ上に平文 Secret がないことを確認 |
...
| Cosign / Kyverno | Phase 2 で Harbor + サプライチェーン強化と併せて | Phase 2 |
