...
| **Phase 1b (MVP-1a)** | パイロット運用開始、2 名体制、VM 3 台 | k3s HA + Argo CD + GHA + Prometheus / Grafana + OpenTofu |
| **Phase 1c (MVP-1b)** | 運用品質の確保、2 名体制、VM 3 台 (同一) | Harbor / Trivy + Loki + Sealed Secrets + バックアップ訓練 + Runbook |
| **Phase 2** | 基盤拡張、2〜3 名体制 | Istio + Kafka + Jaeger / OTel + Backstage + tier1 Rust / ZEN Engine + tier2 サンプル |
