...
| **Phase 1b (MVP-1a)** | パイロット運用開始、2 名体制、VM 3 台 | k3s HA + Argo CD + GHA + Prometheus / Grafana + OpenTofu + Tilt (ローカル開発環境) |
...
**ハードウェア**: VM 3 台 (4 vCPU / 8 GB / 200 GB SSD)。メモリ概算 約 13 GB (合計 24 GB に対して余裕 約 11 GB)。
...
**ハードウェア**: MVP-1a と同一。追加コンポーネントのメモリ増加分 約 4 GB (合計 約 17 GB)。
