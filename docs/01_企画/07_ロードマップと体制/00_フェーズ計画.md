...
| **Phase 2** | 基盤拡張、2〜3 名体制 | Istio + Kafka + Apicurio Registry + Grafana Tempo / OTel Gateway + Backstage + tier1 Rust / ZEN Engine + OpenFeature / flagd (Feature Flag) + Litmus (Chaos Engineering) + **Temporal** (ワークフローエンジン) + **KEDA** (イベント駆動オートスケーラー) + tier2 サンプル |
...
- [`../02_アーキテクチャ/11_マルチクラスタ戦略.md`](../02_アーキテクチャ/11_マルチクラスタ戦略.md) — Phase 3 以降のマルチクラスタ設計
- [`../02_アーキテクチャ/12_SLOとエラーバジェット.md`](../02_アーキテクチャ/12_SLOとエラーバジェット.md) — SLO 定義とエラーバジェット運用
...
- [`../04_技術選定/12_追加採用OSS_4.md`](../04_技術選定/12_追加採用OSS_4.md) — Kustomize + Helm / Temporal の��階導入
- [`../04_技術選定/14_追加採用OSS_6.md`](../04_技術選定/14_追加採用OSS_6.md) — KEDA の段階導入
