...
| **Phase 1c (MVP-1b)** | 運用品質の確保、2 名体制、VM 3 台 (同一) | Harbor / Trivy + Loki + Sealed Secrets + Kyverno + MinIO (バックアップ / Harbor バックエンド / OpenTofu State) + バックアップ訓練 + Runbook |
...
| **Phase 3** | エンドユーザー体験の拡充 | ネイティブ配信 (MSIX) + 端末設定コピー + マルチクラスタ + Vault |
...
- External Secrets Operator + Vault (Sealed Secrets から移行)
