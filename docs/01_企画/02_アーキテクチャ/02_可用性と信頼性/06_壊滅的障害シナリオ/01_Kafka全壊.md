...
雛形 CLI が tier2 / tier3 のサービススケルトンを生成する際に、Outbox テーブル定義 (PostgreSQL の `outbox(id, event_type, payload, created_at, sent_at)` ) と、定期的に未送信レコードを Kafka に再送するワーカーを自動で含める。Phase 2 で [Debezium](../../../04_技術選定/03_周辺OSS/02_周辺OSS.md) の Outbox Event Router パターンの導入を検討するが、MVP では「アプリ内ワーカーで定期的に sent_at NULL のレコードを再送」する単純実装で十分である。
...
このパターンを「リファレンス実装」として `03_tier1設計/03_開発者体験/14_tier3開発者体験設計.md` に明記し、tier2/tier3 開発者が PR 提出時に CI でチェックされる。冪等性を実装しなければ CI を通過できない仕組みである。
...
- [`../../../04_技術選定/02_中核OSS/01_実行基盤中核OSS.md`](../../../04_技術選定/02_中核OSS/01_実行基盤中核OSS.md) — Strimzi / KRaft 採用根拠
- [`../../../04_技術選定/03_周辺OSS/06_イベントスキーマレジストリ.md`](../../../04_技術選定/03_周辺OSS/06_イベントスキーマレジストリ.md) — イベントスキーマ管理
- [`../../../03_tier1設計/04_診断と補償/10_Saga補償パターン支援.md`](../../../03_tier1設計/04_診断と補償/10_Saga補償パターン支援.md) — Kafka 障害時の Saga 補償動作
