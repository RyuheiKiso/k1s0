...
MQTT ↔ Kafka のブリッジは tier1 境界内に閉じ込める。**tier2 / tier3 からは MQTT も Kafka も一切見えず、`k1s0.PubSub` API のみが接点**になる (案 X、詳細は [`../../../img/全体構成図.svg`](../../../img/全体構成図.svg))。ブリッジは SRP で上り/下りに分離する。
