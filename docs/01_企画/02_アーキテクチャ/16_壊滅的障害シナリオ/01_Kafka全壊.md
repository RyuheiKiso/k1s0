...
3 broker KRaft 構成の Kafka クラスタが、ノード障害・ストレージ破損・Operator のバグ等により短時間で全停止する。Strimzi Operator の自動再起動でも復旧不能で、broker メタデータが整合性を失った状態を本章の対象とする。1 broker 障害は Strimzi の `replication.factor=3` で自動的に縮退運転に入るため、本章の対象外である ([`../05_障害復旧とバックアップ.md`](../05_障害復旧とバックアップ.md) 4.1 参照)。

この事象が稟議で繰り返し問われる理由は、k1s0 が **イベントソーシング前提のアーキテクチャ** を採用しているからである。Saga パターンによる長期トランザクションも、tier2 サービス間の非同期連携も、`k1s0.PubSub` API の裏側で全て Kafka に依存する。Kafka が完全に倒れた瞬間に「全イベントが失われ、Saga が中断し、復旧後に手動の整合性回復作業が必要」となるなら、稟議は通らない。本章はそうではないことを示す。
...
雛形 CLI が tier2 / tier3 のサービススケルトンを生成する際に、Outbox テーブル定義 (PostgreSQL の `outbox(id, event_type, payload, created_at, sent_at)` ) と、定期的に未送信レコードを Kafka に再送するワーカーを自動で含める。Phase 2 で [Debezium](../../04_技術選定/02_周辺OSS.md) の Outbox Event Router パターンの導入を検討するが、MVP では「アプリ内ワーカーで定期的に sent_at NULL のレコードを再送」する単純実装で十分である。

これにより、**Kafka 全停止中も Producer 側のイベント欠損はゼロ** になる。滞留容量は `outbox` テーブルのディスク容量に依存し、CloudNativePG の PV (Longhorn 3 ノードレプリケーション) が許す限り溜め続けられる。MVP 構成 (PV 100GB / サービス) では、平均 1KB のイベントを 1 億件保持できる計算になり、60 分の RTO に対して 1000 倍以上の余裕がある。
...
このパターンを「リファレンス実装」として `03_tier1設計/14_tier3開発者体験設計.md` に明記し、tier2/tier3 開発者が PR 提出時に CI でチェックされる。冪等性を実装しなければ CI を通過できない仕組みである。
...
「自動復旧の限界をどこに引くか」は Operator の `spec.kafka.config.controller.quorum.election.timeout.ms` 等で調整可能だが、MVP ではデフォルト設定を維持し、Phase 2 以降に Litmus 試験結果に基づきチューニングする。
...
各ステップの正確なコマンドと kubectl 出力例は Phase 1 完了時に TechDocs として整備し、起案者以外でも実行できるようにする。

## RTO / RPO の根拠

| 指標 | 値 | 根拠 |
|---|---|---|
| **RTO** | 60 分 | Strimzi の自動復旧失敗判定 (5 分) + 手動介入による CR 再適用 (15 分) + Topic 再作成 (10 分) + Consumer リセット (20 分) + 動作確認 (10 分) の積算。Phase 3 で MirrorMaker 2 による別クラスタフェイルオーバー導入で 15 分に短縮予定 |
...
Phase 2 以降、以下の Litmus ChaosExperiment を週次の CronChaosEngine で自動実行する。
...
| `kafka-cluster-pv-corrupt` (Phase 3) | Kafka の PV を破壊し、Operator の自動復旧失敗を再現 | 本章 Runbook の手動介入手順が機能することを検証。RTO 60 分以内 |

最後の `pv-corrupt` 試験は本章の Runbook そのものを検証するため、Phase 3 で本番に近い検証環境で実施する。

### 復旧訓練

Phase 2 から四半期に 1 回、検証環境で本章 Runbook の全手順を実施し、所要時間と詰まりポイントを TechDocs に記録する。訓練結果に基づき Runbook を更新する。

## 関連ドキュメント

- [`00_概要.md`](./00_概要.md) — 壊滅的障害シナリオ全体の俯瞰
- [`../05_障害復旧とバックアップ.md`](../05_障害復旧とバックアップ.md) — Kafka のバックアップ戦略 (2.3 節)
- [`../08_グレースフルデグラデーション.md`](../08_グレースフルデグラデーション.md) — `k1s0.PubSub.*` の縮退動作 (2.3 節)
- [`../09_データアーキテクチャ.md`](../09_データアーキテクチャ.md) — Outbox Pattern とイベントソーシングの位置づけ
- [`../../04_技術選定/01_実行基盤中核OSS.md`](../../04_技術選定/01_実行基盤中核OSS.md) — Strimzi / KRaft 採用根拠
- [`../../04_技術選定/06_イベントスキーマレジストリ.md`](../../04_技術選定/06_イベントスキーマレジストリ.md) — イベントスキーマ管理
- [`../../03_tier1設計/10_Saga補償パターン支援.md`](../../03_tier1設計/10_Saga補償パターン支援.md) — Kafka 障害時の Saga 補償動作
