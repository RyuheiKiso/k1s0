...
- ラズパイ↔k1s0 クラスタ間は、ユーザー拠点の既存閉域ネットワーク経由で到達可能とする。インターネット経由 SaaS は使用しない ([`../03_配置形態.md`](../03_配置形態.md) のネットワーク方針に準拠)。
- tier1 側の入口は Envoy Gateway および Dapr pub/sub building block とし、tier2 / tier3 からエッジ固有の通信プロトコルは隠蔽する ([`../../03_tier1設計/`](../../03_tier1設計/) の隠蔽方針に準拠)。
...
- [`../03_配置形態.md`](../03_配置形態.md) — クラスタ側の namespace 配置とネットワーク方針
- [`../02_依存ルールと通信経路.md`](../02_依存ルールと通信経路.md) — tier 間の通信経路
