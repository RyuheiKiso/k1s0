...
エッジエージェントは Rust または Go で書く。C++ や Python は本番で選ぶべきではない (Python は GIL とメモリ管理、C++ はメモリ安全性のコスト)。k1s0 本体の tier1 も Go+Rust ハイブリッドのため ([`../../03_tier1設計/`](../../03_tier1設計/))、チームの言語スキルを流用できる。以下は選定の観点。
