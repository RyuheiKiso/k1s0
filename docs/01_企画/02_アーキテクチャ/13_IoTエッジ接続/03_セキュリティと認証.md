...
```hocon
# emqx-acl.conf : CN ベースの bidirectional ACL (上り publish + 下り subscribe + ack publish)
# 上り telemetry : Pi から tier1 方向
...
# 下り command : tier1 の Edge.Dispatcher が publish、Pi が subscribe
{ allow, { client, { cn_match, "edge-pi-.*" } },            subscribe, ["k1s0/+/+/${clientid}/+/cmd"] }
{ allow, { client, { cn_match, "tier1-edge-dispatcher" } }, publish,   ["k1s0/+/+/+/+/cmd"] }
# 下り command の ack : Pi が publish、tier1 の Edge.Dispatcher が subscribe
{ allow, { client, { cn_match, "edge-pi-.*" } },            publish,   ["k1s0/+/+/${clientid}/+/cmd/ack"] }
{ allow, { client, { cn_match, "tier1-edge-dispatcher" } }, subscribe, ["k1s0/+/+/+/+/cmd/ack"] }
{ deny,  all }
```

Pi は自機 clientid 配下の `/data` `/state` `/$LWT` `/cmd/ack` にしか publish できず、他拠点機器への成りすまし publish は broker 側で拒否される。下り `/cmd` は tier1 の Edge.Dispatcher (別 CN を持つ) だけが publish 可能にし、Pi 側は subscribe のみに制限する。これにより **broker が侵害された場合でも、横方向に勝手な command を発行できない** 構造になる。Dispatcher は単一 Deployment として OpenBao から別系統の mTLS クライアント証明書を取得し、その CN を ACL で別扱いする。
...
#### 4.3.1 command 改竄防止と実行監査

下り command は broker の TLS / ACL に加え、**tier2 発行時の ed25519 署名** + **Pi 側 OpenBao 公開鍵での検証** + **tier1 監査ログ** の 3 層で守る。broker 侵害と中間者改竄のどちらに対しても致命傷を避ける構造。

- **署名検証 (改竄防止)**: tier2 操業制御 Service が cmd envelope ([`02_エッジソフトウェアと通信設計.md §6.1.2`](./02_エッジソフトウェアと通信設計.md#612-command-envelope-cmdv1)) を発行する際、サービス専用 ed25519 鍵で署名する。Pi 側 `edge-agent` は対応する公開鍵 (OpenBao から OTA 時に配布) で検証し、**検証失敗時は Modbus WRITE を実行せず ack に `signature_mismatch` を返す**。broker が侵害されて偽 command が流入しても Pi 側で拒否される。
- **idempotency キャッシュ (二重実行防止)**: Pi 側に 1 時間保持の `idempotency_key` キャッシュを持ち、同一キーが来た場合は **既存 ack を再送するのみ**で Modbus WRITE は再実行しない。broker ↔ Pi 間の再送で機器状態が破壊されるのを防ぐ。
- **command 実行監査**: Edge.Dispatcher は MQTT publish の前後で監査ログを `auditd + Fluent Bit → tier1 Loki` に送る。ack も同様にログ化し、`command_id` を pivot に「誰がいつ何を指示して、どの Pi がいつどう実行したか」を突き合わせ可能にする (共通スキーマは 6.1 節の `agent` 系イベントに `command.dispatched` / `command.acked` / `command.rejected` を追加)。
