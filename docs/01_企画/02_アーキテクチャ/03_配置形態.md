...
| namespace | 配置するもの |
|---|---|
| `infra` | Envoy Gateway / Istio Control Plane / Prometheus / Loki / Jaeger / Grafana / Kafka (Strimzi) / Harbor / Valkey / CloudNativePG + PostgreSQL |
...
| Pod 権限 | PodSecurityAdmission + Kyverno ポリシー (Phase 2 以降) |
| イメージ署名 | Cosign (Phase 2 以降) + Kyverno 未署名拒否 |

MVP ではこの境界の最小構成 (Istio mTLS + 基本的な Secrets) のみを構築する。Kyverno / Cosign は Phase 2 以降で段階的に追加する。
