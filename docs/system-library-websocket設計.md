...
public struct WsConfig: Sendable {
    ...
    public init(endpoint: URL, authToken: String? = nil, reconnectInterval: Duration = .seconds(5), maxReconnectAttempts: Int = 10, heartbeatInterval: Duration = .seconds(30))
}
...
public protocol WsClient: Sendable {
    ...
    func messages() -> AsyncStream<WsMessage>
    var state: ConnectionState { get async }
    func close() async
}

public actor URLSessionWsClient: WsClient {
    public init(config: WsConfig)
    public func connect() async throws
    public func send(_ message: WsMessage) async throws
    public func messages() -> AsyncStream<WsMessage>
    public var state: ConnectionState { get async }
