...
ポリシーは **2階層構造** で管理する。共通シークレット用の限定ポリシー（`k1s0-system`）と、サービス固有のポリシー（最小権限）を組み合わせる。Kubernetes Auth ロールではサービス固有ポリシーのみを `token_policies` に列挙する（例: `["auth-server"]`）。

#### 共通シークレット用ポリシー（`infra/vault/policies/k1s0-system.hcl`）

共通シークレット（`common/` 配下）への読み取りアクセスのみを提供する限定ポリシー。

```hcl
# k1s0-system.hcl --- system tier 共通シークレット用の限定ポリシー
...
| --------------- | --------------- | ------------ | --------------------------------- |
| `auth-server`   | `auth-server`   | `k1s0-system` | `["auth-server"]`                 |
| `config-server` | `config-server` | `k1s0-system` | `["config-server"]`               |
| `dlq-manager`   | `dlq-manager`   | `k1s0-system` | `["dlq-manager"]`                 |
| `saga-server`   | `saga-server`   | `k1s0-system` | `["saga-server"]`                 |
...
## ローカル開発から本番への移行（L-03 監査対応）

### Vault dev モードの制限事項

`docker-compose.yaml` の Vault サービスは `VAULT_DEV_ROOT_TOKEN_ID` が設定されており、**dev モード**で動作する。
dev モードは以下の制限があるため、**本番環境では絶対に使用しないこと**。

| 制限事項 | 影響 |
|---------|------|
| インメモリストレージ | コンテナ再起動でシークレットが全消失 |
| Seal 機能なし | 常に unseal 状態（tamper 検知不可） |
| ルートトークン固定 | トークンローテーション不可 |
| TLS 無効 | 通信が平文（本番では必須） |

### 本番環境への移行手順

本番環境では `infra/terraform/modules/vault/` の Terraform モジュールを使用して Vault Operator をデプロイする。

**Step 1: Vault クラスタの初期化**

```bash
# Vault が未初期化の場合、初期化して Unseal Key と Root Token を取得する
vault operator init \
  -key-shares=5 \
  -key-threshold=3 \
  -format=json > vault-init.json

# ⚠️ vault-init.json を安全な場所に保管すること（KMS や HSM への格納を推奨）
# ⚠️ リポジトリへのコミットは絶対に禁止

# Unseal Key で Vault を開封する（3 つ以上の Key が必要）
...
**Step 2: Auto Unseal の設定（推奨）**

クラスタ再起動時の手動 Unseal 操作をなくすため、クラウドプロバイダーの KMS を使用した Auto Unseal を設定すること。

```hcl
# Vault 設定（infra/terraform/modules/vault/config.hcl）
seal "awskms" {
...
ローカル開発用 `init-vault.sh` で定義したシークレットパスを Terraform またはプロビジョニングスクリプトで本番 Vault に登録する。

```bash
# 本番 Vault への接続設定
export VAULT_ADDR="https://vault.k1s0-system.svc.cluster.local:8200"
export VAULT_TOKEN="<root-token-or-app-role-token>"

# KV v2 シークレットエンジンの有効化（初回のみ）
vault secrets enable -path=secret kv-v2

# シークレットの登録例（CI/CD で自動化すること）
vault kv put secret/k1s0/system/auth/database password="<strong-password>"
vault kv put secret/k1s0/system/auth/keycloak client_secret="<strong-secret>"
```

**Step 4: Kubernetes Auth の有効化**

```bash
# Kubernetes Auth メソッドの有効化
...
# サービスアカウントとロールのバインド（auth-server の例）
vault write auth/kubernetes/role/auth-server \
  bound_service_account_names=auth-server-sa \
  bound_service_account_namespaces=k1s0-system \
...
- [ ] `vault-init.json` を KMS / HSM に保管した
- [ ] Auto Unseal を設定した（または手動 Unseal 手順を整備した）
- [ ] 全シークレットを本番 Vault に登録した
- [ ] Kubernetes Auth ロールを全サービス分設定した
- [ ] AppRole を CI/CD パイプライン向けに設定した
- [ ] 監査ログ（`vault audit enable file path=/vault/logs/audit.log`）を有効化した
- [ ] TLS 証明書を設定した（`vault listener "tcp" { tls_cert_file / tls_key_file }`）
