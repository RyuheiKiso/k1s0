...
> **注記**: Database シークレットエンジン（`vault_mount.database`）は `vault-database` モジュールに一元化した。本モジュールでは定義しない。

```hcl
# modules/vault/main.tf
# NOTE: vault_mount.database は vault-database モジュールに移管済み。
# 本モジュールでは KV v2 / PKI / 監査ログ / Kubernetes Auth のみを管理する。
...
# vault_mount.database は modules/vault-database/main.tf に一元化（二重定義防止）
...
> **注記: vault_mount.database の二重定義の解消**
>
> 以前は `modules/vault/main.tf` と `modules/vault-database/main.tf` の両方に `vault_mount "database"` リソースが定義されていた。この二重定義は `terraform apply` 時にリソース競合エラーを引き起こす。解消方針として、`vault_mount.database` は `vault-database` モジュール側にのみ定義し、`vault` モジュールからは削除する。`vault` モジュールは KV v2 / PKI / 監査ログ / Kubernetes Auth に責務を限定する。

主要リソース:
- `vault_mount.database`: Database エンジンのマウント（パス: `database`）— **本モジュールのみで定義**（vault モジュール側の定義は削除済み）
...
# NOTE: vault_mount.database は本モジュールでのみ定義する。
# modules/vault/main.tf には定義しないこと（二重定義防止）。
...
# NOTE: PostgreSQL ホスト名の統一
# var.postgres_host は全環境で Kubernetes 内部 DNS 名 "postgresql.database.svc.cluster.local" を使用する。
# サーバー config.yaml 側の database.host もこれに合わせること。
# 以前は "postgres.k1s0-system.svc.cluster.local" と "postgresql.database.svc.cluster.local" が混在していた。
# 正規のホスト名は database namespace に配置された PostgreSQL Helm リリースの Service 名に基づく。
...
`environments/prod/variables.tf` および `modules/vault/variables.tf` の本番環境用変数には、`example.com` を含むプレースホルダーの使用を禁止する。

各変数には `validation` ブロックを設け、Terraform が `plan` / `apply` 時にプレースホルダーを自動検出して実行を中断する。

| 変数 | バリデーション内容 |
| --- | --- |
| `harbor_domain` | `example.com` を含まないこと |
| `ceph_s3_endpoint` | `example.com` を含まないこと |
| `ldap_url` | `example.com` を含まないこと |
| `ldap_user_dn` | `example.com` を含まないこと |
| `ldap_group_dn` | `example.com` を含まないこと |
| `ldap_bind_dn` | `example.com` を含まないこと |
| `ldap_bind_password` | 空文字でないこと |
| `react_spa_redirect_uris` | いずれの要素も `example.com` を含まないこと |
| `react_spa_web_origins` | いずれの要素も `example.com` を含まないこと |
| `bff_redirect_uris` | いずれの要素も `example.com` を含まないこと |

実際の値は `terraform.tfvars` または CI/CD の環境変数（Vault シークレット）で渡すこと。
