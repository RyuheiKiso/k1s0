...
| ----------- | ---------------------------- | ----------------------------------------- |
| Rust        | `rust:1.93-bookworm`         | `debian:bookworm-slim`                    |
...
| Flutter Desktop | `ghcr.io/cirruslabs/flutter:3.24.0` (ビルド) | ネイティブバイナリ（exe / AppImage / dmg）— Ceph RGW で配布（[アプリ配布基盤設計](../distribution/アプリ配布基盤設計.md) 参照） |
