...
- 既存インフラ（GitHub Actions + Harbor + PV）を最大限活用する
- 新規サービスは App Registry Server と App Catalog UI の 2 つに限定する
- App Registry Server がローカルFS（PV）からファイルを直接配信する。外部オブジェクトストレージ（S3/Ceph RGW）不要
...
- Rust (axum)
- PostgreSQL（メタデータ DB）
- ローカルファイルシステム（PV マウント）によるファイル直接配信
...
GET    /api/v1/apps/{id}/versions/{ver}/download   バイナリ直接配信
...
**配置:** `regions/system/library/{go,rust,typescript,dart}/app-updater/`（Dart は `app_updater`）
...
アプリバイナリの実体を PersistentVolume（`/data/apps`）に保存する。S3/Ceph RGW 不要。
...
- PV は App Registry Server Pod のみマウント
- ダウンロードは App Registry Server の認証済み API 経由のみ
- CI/CD はバージョン登録 API を呼び出すが、ファイルのアップロード方式は別途定義
...
jobs:
  build:
    ...
    steps:
      ...
      # メタデータ登録（storage_key: サーバー上のファイル保存パス）
      - run: |
...
6. App Registry Server がローカルFS（PV）からファイルをバイナリストリーミング配信
7. ダウンロード後、チェックサムと署名を検証
8. インストール完了後、自動更新クライアントが有効になる

App Registry Server が直接ファイルを配信するため、外部オブジェクトストレージ（S3/Ceph RGW）は不要。
...
4. App Registry Server の download API からバックグラウンドダウンロード（直接配信）
...
- **認証済み API 経由配信** — Bearer トークン検証後にファイルを直接配信
...
- ファイル配信は App Registry Server を経由するため外部ストレージへの直接アクセス経路が不要
...
| 自動更新 SDK | `regions/system/library/{go,rust,typescript,dart}/app-updater/` |
| Helm Chart | `infra/helm/services/system/app-registry/` |
| DB マイグレーション | `regions/system/database/app-registry-db/` |

## 環境別設定

| 設定項目 | dev | staging | prod |
|---|---|---|---|
| ストレージパス（PV） | `/data/apps` | `/data/apps` | `/data/apps` |
