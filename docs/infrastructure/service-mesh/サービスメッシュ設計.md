...
すべての DestinationRule は `infra/istio/destinationrules/default.yaml` に統合定義されており、合計2件存在する。order-server の canary subsets と Circuit Breaker 設定も同ファイルに統合されている（`destinationrules/canary.yaml` は存在しない）。

| 名前          | Namespace   | 役割                                            |
| ------------- | ----------- | ----------------------------------------------- |
| `auth-server` | k1s0-system | system Tier Circuit Breaker（maxConnections=200）|
| `order-server`| k1s0-service| canary subsets 定義 + service Tier Circuit Breaker |
