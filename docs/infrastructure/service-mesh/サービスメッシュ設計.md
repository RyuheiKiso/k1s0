...
> **注記:** 以下の task-server の例では timeout=10s, retry attempts=3 を設定しているが、これは service Tier のデフォルト値（timeout=15s, retry=2、「Tier 別デフォルト値」表を参照）をサービス固有の要件に基づいて意図的に上書きしたカスタマイズ例である。task-server はタスク処理の特性上、より短いタイムアウトと多いリトライ回数が適切と判断している。各サービスは Tier デフォルト値を基本としつつ、業務要件に応じて個別に調整できる。
...
#### カナリアリリース（ヘッダーベースルーティング統合）

技術監査対応として、カナリアリリース用 VirtualService を修正し、**ヘッダーベースルーティングとウェイトベースのトラフィック分割を単一の VirtualService に統合** した。従来はカナリアリリースとヘッダーベースルーティングが別々の VirtualService として定義されていたが、統合により以下の利点がある:

- `x-canary: "true"` ヘッダー付きリクエストは常にカナリアへ直接ルーティング（テスト用）
- それ以外のリクエストはウェイトベースで段階的にトラフィックを分割
- VirtualService の競合（同一ホストに複数定義）を防止

```yaml
# infra/istio/virtualservices/canary.yaml
apiVersion: networking.istio.io/v1
kind: VirtualService
metadata:
  name: task-server
...
すべての DestinationRule は `infra/istio/destinationrules/default.yaml` に統合定義する。

運用上の目的は「定義の分散を防ぎ、差分レビューを容易にする」ことであり、DestinationRule は原則このファイルに追記して管理する（`destinationrules/canary.yaml` のような用途別ファイル分割は行わない）。

#### 追加・更新の運用指針

- サービス追加時は、対象サービスの Namespace に 1 つ DestinationRule を追加する
- `spec.host` は Kubernetes Service 名（例: `task-server`）に合わせる
- mTLS は原則 `ISTIO_MUTUAL`（Istio 管理証明書）を指定する
- Canary を行う場合のみ `subsets`（例: `stable`/`canary`）を定義する

以下は定義例（詳細と最新状態は `infra/istio/destinationrules/default.yaml` を正とする）。
...
![サービスメッシュ トラフィックフロー](images/service-mesh-traffic-flow.svg)
...
![カナリアリリース段階](images/service-mesh-canary-stages.svg)
...
#### Tier 別デフォルト VirtualService 一覧

3つの Tier デフォルト VirtualService はすべて `infra/istio/virtual-service.yaml` に統合定義されている。

| 名前             | Namespace      | Timeout | Retry 回数 | perTryTimeout | retryOn                                    |
| ---------------- | -------------- | ------- | ---------- | ------------- | ------------------------------------------ |
| `default-system`   | k1s0-system    | 5s      | 3          | 2s            | `5xx,reset,connect-failure`                |
| `default-business` | k1s0-business  | 10s     | 3          | 3s            | `5xx,reset,connect-failure`                |
| `default-service`  | k1s0-service   | 15s     | 2          | 5s            | `5xx,reset,connect-failure,retriable-4xx`  |

> **business と service の差異:** `default-business` は `retriable-4xx` を retryOn に含まない（業務ロジック層では 4xx を原則リトライしない）。`default-service` は `retriable-4xx` を含み、409 Conflict 等の一時的な競合状態に対してもリトライを行う。
...
**防御多層化:** メッシュワイドデフォルトに加え、各 Tier の Namespace にも個別の PeerAuthentication を定義している（[認証認可設計.md](../../architecture/auth/認証認可設計.md) の「サービス間認証 mTLS」セクション参照）。メッシュワイド設定が誤って変更された場合でも、Namespace レベルの設定が防御層として機能する。
...
---
# observability Namespace は STRICT mTLS を強制しつつ、Prometheus scrape ポートのみ例外許可（H-01 対応）
# Namespace 全体を PERMISSIVE にすると平文通信が許可されメトリクスの盗聴リスクがある。
# portLevelMtls で 15020（Istio envoy sidecar の Prometheus 集約ポート）のみ PERMISSIVE とし、
# それ以外のポートは STRICT を維持してセキュリティを確保する。
apiVersion: security.istio.io/v1
kind: PeerAuthentication
metadata:
...
spec:
  mtls:
    mode: STRICT
  portLevelMtls:
    # Prometheus が Istio envoy sidecar からメトリクスを scrape する標準ポート（15020）のみ例外許可
    15020:
      mode: PERMISSIVE

---
# k1s0-system Namespace: STRICT mTLS（防御多層化）
apiVersion: security.istio.io/v1
kind: PeerAuthentication
metadata:
  name: default
...
---
# k1s0-business Namespace: STRICT mTLS（防御多層化）
apiVersion: security.istio.io/v1
kind: PeerAuthentication
metadata:
  name: default
...
---
# k1s0-service Namespace: STRICT mTLS（防御多層化）
apiVersion: security.istio.io/v1
kind: PeerAuthentication
metadata:
  name: default
...
**Namespace レベルポリシーの一覧（`infra/istio/peerauthentication.yaml` 定義済み）:**

| リソース名     | Namespace     | mode       | portLevelMtls | 用途                                      |
| -------------- | ------------- | ---------- | ------------- | ----------------------------------------- |
| `default`      | service-mesh  | STRICT     | -             | メッシュワイドデフォルト（istiod の Namespace）|
| `observability-mtls` | observability | STRICT | 15020: PERMISSIVE | Namespace は STRICT、Prometheus scrape ポート（15020）のみ例外許可（H-01 対応） |
| `default`      | k1s0-system   | STRICT     | -             | system Tier 防御多層化                    |
| `default`      | k1s0-business | STRICT     | -             | business Tier 防御多層化                  |
| `default`      | k1s0-service  | STRICT     | -             | service Tier 防御多層化                   |

> **H-01 対応について:** observability Namespace を旧来の `PERMISSIVE`（全ポート平文許可）から `STRICT` + `portLevelMtls` に変更した。これによりメトリクスの盗聴リスクを排除しつつ、Prometheus が Istio envoy sidecar のメトリクス集約ポート（15020）を scrape できる構成を維持している。

#### PeerAuthentication が未定義の Namespace の扱い

PeerAuthentication は **Istio メッシュ参加（サイドカー注入）しているワークロード**に対してのみ意味を持つ。
`service-mesh` のメッシュワイドデフォルト（STRICT）があるため、メッシュ参加しているワークロードは原則 mTLS が強制される。

一方で、PeerAuthentication を Namespace に明示していない場合でも、以下の方針で「読者が迷わない」ように扱いを固定する。

| Namespace | 位置づけ | mTLS の扱い |
| --- | --- | --- |
| `ingress` | 外部入口（Nginx Ingress Controller） | 外部→入口は通常 Istio mTLS の範囲外。メッシュ内（入口→Kong→各サービス）は STRICT を前提とする。入口 Namespace からサービスへ直接到達させる設計は避ける |
| `messaging` | 基盤（Kafka 等） | 原則 Istio mTLS の対象外（コンポーネント自身の TLS/SASL 等で保護する）。メッシュ参加させる場合は mesh-wide STRICT が適用されるため、要件に応じて設計する |

### AuthorizationPolicy

Tier 間のアクセス制御を Istio AuthorizationPolicy で強制する。AuthorizationPolicy の詳細定義（`allow-from-lower-tiers`, `allow-from-service-tier`, `allow-from-ingress`, `deny-bff-to-bff` 等）は [認証認可設計](../../architecture/auth/認証認可設計.md) を参照。
...
spec:
  ...
  http:
    - route:
      ...
      retries:
        attempts: 3
        perTryTimeout: 3s    # 基本ルーティングの task-server 設定と統一
        retryOn: "5xx,reset,connect-failure,retriable-4xx"
...
CronJob の定義ファイルは `infra/istio/fault-injection/cronjob.yaml` に配置されている。毎週月曜日 10:00 JST（UTC 01:00）に自動実行し、`fault-injection-config` ConfigMap に格納した `fault-delay.yaml` と `fault-abort.yaml` を適用して 30 分間テストを実施後、自動削除する。
...
- [認証認可設計.md](../../architecture/auth/認証認可設計.md) — 認証・認可・AuthorizationPolicy
