...
| Integration Test  | `integration-test.yaml` | PR 時 (`regions/system/server/rust/**`) | postgres:17 + kafka:3.8.0 起動、26サーバー自動検出・並列統合テスト |
...
- **対象サービス**: `scripts/ci-list-integration-servers.sh` で 26 サーバーを自動検出し、`matrix` で並列実行（`fail-fast: false`）
- **テスト実行**: `cargo test --all`（`--ignored` フラグは廃止。通常テストと統合テストを一括実行）
  ...
  integration-test:
    ...
    steps:
      ...
      - name: Run integration tests (${{ matrix.server }})
        run: cd ${{ matrix.server }} && cargo test --all
```
