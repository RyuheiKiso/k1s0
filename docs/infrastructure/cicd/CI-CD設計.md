...
- **サービスコンテナ**: `postgres:17.4`（ヘルスチェック付き）+ `apache/kafka:3.8.0`（KRaft モード）。バージョンは `docker-compose.yaml` と同期すること（HIGH-02 対応）
...
|------|------|------|
| `service-path` | 必須 | サービスのディレクトリパス |
| `go-version` | 任意 | Go バージョン（デフォルト: 1.26.2） |
| `golangci-lint-version` | 任意 | golangci-lint バージョン（デフォルト: v1.64.8） |

**カバレッジ基準（QUALITY-001）:** `test` ジョブの「カバレッジ基準チェック（70%）」ステップで `go tool cover -func` の合計行を解析し、カバレッジが 70% を下回る場合は `exit 1` で CI をブロックする。
...
### AVAIL-002: migrate-all の DB オーナー不整合修正

`just migrate-all` を `dev` ユーザーで実行するとテーブルオーナーが `dev` になり、サービス起動時に `k1s0` ユーザーが `ALTER TABLE ENABLE ROW LEVEL SECURITY` 等を実行する際に `must be owner of table` エラーが発生していた。

**修正内容（justfile の `migrate-all` レシピ）:**

| tier | 実行ユーザー | 理由 |
| --- | --- | --- |
| system | `dev`（`PG_USER` デフォルト） | 専用 `_rw` ロール（`k1s0_auth_rw` 等）には DDL 権限がないため `dev` で DDL を実行し、`99-finalize-grants.sh` で DML 権限を付与する設計 |
| business | `k1s0`（`K1S0_USER` デフォルト） | サービスが `k1s0` ユーザーで接続するため `k1s0` がテーブルオーナーである必要がある |
| service | `k1s0`（`K1S0_USER` デフォルト） | 同上。`K1S0_DB_PASSWORD` 環境変数でパスワードを設定（デフォルト: `dev-k1s0-local`） |

**対象マイグレーションディレクトリ:**
- system tier: `regions/system/database/*/migrations/`
- business tier: `regions/business/*/database/*/migrations/`（`project-master-db` → `k1s0_business`）
- service tier: `regions/service/*/database/*/migrations/`（`postgres` → `k1s0_service`）

**local-up-dev への影響:** Phase 1.5 で自動実行される `just migrate-all` が business/service tier のマイグレーションも対象となり、`k1s0` ユーザーでテーブルを作成するため、サービス起動後の RLS 設定エラーが解消される。
