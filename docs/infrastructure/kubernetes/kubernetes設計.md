...
**NetworkPolicy ファイル構成 (CRIT-001/HIGH-006 監査対応):**
`infra/kubernetes/network-policies/kustomization.yaml` に以下のファイルがすべて登録されている:
- `system-per-service.yaml` / `system-per-service-2.yaml` 〜 `system-per-service-5.yaml`: system 名前空間の各サービス個別ポリシー（全25+サービス対応）
- `business-per-service.yaml`: business 名前空間のサービス個別ポリシー
- `service-per-service.yaml`: service 名前空間のサービス個別ポリシー
- `infra-{cert-manager,harbor,ingress,service-mesh}.yaml`: インフラ名前空間の個別ポリシー
- `system.yaml` / `business.yaml` / `service.yaml` / `messaging.yaml` / `observability.yaml`: Tier 間通信ポリシー
  ...
  metrics:
    ...
    - type: Resource
      resource:
        ...
        target:
          type: Utilization
          # MED-001 監査対応: メモリ閾値を 60% に統一（INFRA-HIGH-002 監査対応で引き下げ済み）
          # 80%では GC プレッシャーや突発的なメモリスパイクに対応できないため 60% を標準とする
...
MED-001 監査対応: 全サービスで統一する閾値ルールを以下に定義する。

| メトリクス | 標準閾値 | 理由 |
|-----------|---------|------|
| CPU | **70%** | CPU 使用率が 70% を超えると応答遅延が顕著になるため |
| Memory | **60%** | GC プレッシャーや突発スパイクに 40% のバッファを確保するため。80% では OOM Kill のリスクが高まる |

**例外**: Kong のようなプロキシ系サービスはメモリ特性が異なるため 70% を許容する。例外設定時は `values.yaml` にコメントで理由を記載すること。

`k1s0-common` の `values.yaml` デフォルト（CPU: 70% / Memory: 60%）が基準値。個別サービスで上書きする場合は必ずコメントを添える。
