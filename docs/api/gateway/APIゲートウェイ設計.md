...
SPA（React）からのアクセスは BFF Proxy を経由し、HttpOnly Cookie とBearer Token の変換を行う（詳細は [認証認可設計](../../auth/design/認証認可設計.md) の「SPA トークン保存方式」参照）。
...
| post-function        | グローバル | ユーザー情報（claims）をバックエンドへのリクエストヘッダーに転送（[認証認可設計](../../auth/design/認証認可設計.md) の「ヘッダー転送」参照） |
