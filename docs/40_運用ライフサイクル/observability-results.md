...
- **PASS（2 / 4 検証）**:
  - TestPrometheusCardinality（`/api/v1/labels` で 102 label name 取得 / `up` metric cardinality=10）
  - TestSLOAlertManagerEndpoint（`/api/v2/status` 4 field / `/api/v2/alerts` 0 件、Alertmanager 0.x.x install 後）
- **FAIL（採用初期で対応）**:
...
- **Alertmanager 起動完了**: `helm install alertmanager prometheus-community/alertmanager` で起動、TestSLOAlertManagerEndpoint が PASS（前は Skip）
- **環境構築経路**:
