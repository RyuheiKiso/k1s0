...
## Swift ライブラリテンプレート E2E テスト

### E2E テスト — Swift ライブラリテンプレート存在・内容検証

テストファイル: `e2e/tests/docs_compliance/test_template_content_library_swift.py`

```python
ROOT = Path(__file__).resolve().parents[3]
LIBRARY_SWIFT = ROOT / "CLI" / "templates" / "library" / "swift"


class TestLibrarySwiftTemplates:
    """Swift ライブラリテンプレートの存在検証。"""

    @pytest.mark.parametrize(
        "template",
        [
            "Package.swift.tera",
            "Sources/{{ library_name_snake }}/Client.swift.tera",
            "Sources/{{ library_name_snake }}/LibError.swift.tera",
            "Sources/{{ library_name_snake }}/Config.swift.tera",
            "Tests/{{ library_name_snake }}_tests/ClientTests.swift.tera",
            "README.md.tera",
        ],
    )
    def test_library_swift_template_exists(self, template: str) -> None:
        path = LIBRARY_SWIFT / template
        assert path.exists(), f"library/swift/{template} が存在しません"

    def test_library_swift_package_swift_product_name(self) -> None:
        content = (LIBRARY_SWIFT / "Package.swift.tera").read_text(encoding="utf-8")
        assert "{{ library_name_kebab }}" in content

    def test_library_swift_package_swift_target_name(self) -> None:
        content = (LIBRARY_SWIFT / "Package.swift.tera").read_text(encoding="utf-8")
        assert "{{ library_name_snake }}" in content

    def test_library_swift_client_struct_name(self) -> None:
        content = (LIBRARY_SWIFT / "Sources/{{ library_name_snake }}/Client.swift.tera").read_text(encoding="utf-8")
        assert "{{ library_name_pascal }}" in content

    def test_library_swift_lib_error_enum_name(self) -> None:
        content = (LIBRARY_SWIFT / "Sources/{{ library_name_snake }}/LibError.swift.tera").read_text(encoding="utf-8")
        assert "{{ library_name_pascal }}" in content

    def test_library_swift_config_struct_name(self) -> None:
        content = (LIBRARY_SWIFT / "Sources/{{ library_name_snake }}/Config.swift.tera").read_text(encoding="utf-8")
        assert "{{ library_name_pascal }}" in content

    def test_library_swift_client_tests_class_name(self) -> None:
        content = (LIBRARY_SWIFT / "Tests/{{ library_name_snake }}_tests/ClientTests.swift.tera").read_text(encoding="utf-8")
        assert "{{ library_name_pascal }}" in content

    def test_library_swift_readme_title(self) -> None:
        content = (LIBRARY_SWIFT / "README.md.tera").read_text(encoding="utf-8")
        assert "{{ library_name_kebab }}" in content
```

#### テンプレート変数置換検証

`library_name_kebab = "k1s0-shared-utils"` でレンダリングした場合の期待値:

| 変数 | 期待値 |
|------|--------|
| `library_name_kebab` | `k1s0-shared-utils` |
| `library_name_pascal` | `K1s0SharedUtils` |
| `library_name_snake` | `k1s0_shared_utils` |

#### テストメソッド一覧

| テストメソッド | 検証内容 |
|--------------|---------|
| `test_library_swift_template_exists[Package.swift.tera]` | Package.swift テンプレートの存在確認 |
| `test_library_swift_template_exists[Sources/.../Client.swift.tera]` | Client.swift テンプレートの存在確認 |
| `test_library_swift_template_exists[Sources/.../LibError.swift.tera]` | LibError.swift テンプレートの存在確認 |
| `test_library_swift_template_exists[Sources/.../Config.swift.tera]` | Config.swift テンプレートの存在確認 |
| `test_library_swift_template_exists[Tests/.../ClientTests.swift.tera]` | ClientTests.swift テンプレートの存在確認 |
| `test_library_swift_template_exists[README.md.tera]` | README.md テンプレートの存在確認 |
| `test_library_swift_package_swift_product_name` | Package.swift の product 名変数検証 |
| `test_library_swift_package_swift_target_name` | Package.swift の target 名変数検証 |
