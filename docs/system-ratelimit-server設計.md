...
| 言語 | パス |
| --- | --- |
| Rust | `regions/system/server/rust/ratelimit/` |

---

## 設計方針

[APIゲートウェイ設計.md](APIゲートウェイ設計.md) のレート制限方針に基づき、以下の方針で実装する。
...
全エンドポイントは [API設計.md](API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_RATE_` とする。
...
[helm設計.md](helm設計.md) のサーバー用 Helm Chart を使用する。ratelimit 固有の values は以下の通り。
...
- [system-ratelimit-server-実装設計.md](system-ratelimit-server-実装設計.md) -- 実装設計の詳細
- [system-ratelimit-server-デプロイ設計.md](system-ratelimit-server-デプロイ設計.md) -- デプロイ設計の詳細

---

## 関連ドキュメント

- [APIゲートウェイ設計.md](APIゲートウェイ設計.md) -- Kong API ゲートウェイ設計
