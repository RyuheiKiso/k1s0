...
| service-region  | 部門固有領域選択 → Client | `system-region/` + `business-region/{選択した部門固有領域}/` + `service-region/client/` |
...
flowchart TD
    ...
    SRCS --> SRC["Client"]
    SRCS --> SRS["Server"]
    SRC --> SC3C["sparse-checkout set system-region/ business-region/{選択した部門固有領域}/ service-region/client/"]
