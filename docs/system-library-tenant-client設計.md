...
**配置先**: `regions/system/library/csharp/tenant-client/`

```
tenant-client/
├── src/
│   ├── TenantClient.csproj
│   ├── ITenantClient.cs            # テナント操作インターフェース
│   ├── GrpcTenantClient.cs         # gRPC 実装（IMemoryCache 内蔵）
│   ├── Tenant.cs                   # Tenant・TenantStatus・TenantFilter・TenantSettings
│   ├── TenantClientConfig.cs       # サーバー URL・キャッシュ TTL 設定
│   └── TenantException.cs          # 公開例外型
├── tests/
│   ├── TenantClient.Tests.csproj
│   ├── Unit/
│   │   ├── TenantTests.cs
│   │   └── TenantSettingsTests.cs
│   └── Integration/
│       └── GrpcTenantClientTests.cs
├── .editorconfig
└── README.md
```
...
**名前空間**: `K1s0.System.TenantClient`

**主要クラス・インターフェース**:

| 型 | 種別 | 説明 |
|---|------|------|
| `ITenantClient` | interface | テナント操作の抽象インターフェース |
| `GrpcTenantClient` | class | gRPC 経由の tenant-server 接続実装（キャッシュ内蔵）|
| `Tenant` | record | テナント情報（ID・名称・ステータス・プラン・設定・作成日時）|
| `TenantStatus` | enum | Active / Suspended / Deleted |
| `TenantFilter` | record | テナント一覧取得フィルター |
| `TenantSettings` | class | テナント設定値ラッパー |
| `TenantClientConfig` | record | サーバー URL・キャッシュ TTL・最大キャッシュサイズ |
| `TenantException` | class | テナントエラーの公開例外型 |
...
public interface ITenantClient : IAsyncDisposable
{
...
public sealed class GrpcTenantClient : ITenantClient
{
    public GrpcTenantClient(TenantClientConfig config);
    ...
    public Task<bool> IsActiveAsync(string tenantId, CancellationToken ct = default);
    public Task<TenantSettings> GetSettingsAsync(string tenantId, CancellationToken ct = default);
...
public protocol TenantClient: Sendable {
    ...
    func getSettings(tenantId: String) async throws -> [String: String]
}
...
**配置先**: `regions/system/library/python/tenant_client/`
...
│       ├── client.py             # TenantClient ABC・GrpcTenantClient（TTL キャッシュ内蔵）
│       ├── tenant.py             # Tenant dataclass・TenantStatus enum・TenantFilter・TenantSettings
│       ├── config.py             # TenantClientConfig
│       ├── exceptions.py         # TenantError
│       └── py.typed
└── tests/
...
| 型 | 種別 | 説明 |
|---|------|------|
| `TenantClient` | ABC | テナント操作抽象基底クラス |
| `GrpcTenantClient` | class | gRPC 経由の tenant-server 接続実装（TTL キャッシュ内蔵）|
| `Tenant` | dataclass | テナント情報（ID・名称・ステータス・プラン・設定・作成日時）|
| `TenantStatus` | Enum | ACTIVE / SUSPENDED / DELETED |
| `TenantFilter` | dataclass | テナント一覧取得フィルター |
| `TenantSettings` | class | テナント設定値ラッパー（`get` メソッド付き）|
| `TenantClientConfig` | dataclass | サーバー URL・キャッシュ TTL・最大キャッシュサイズ |
| `TenantError` | Exception | テナントエラー基底クラス |
...
config = TenantClientConfig(
    server_url="http://tenant-server:8080",
    cache_ttl=300.0,  # 5分
    cache_max_capacity=1000,
)
client = GrpcTenantClient(config)

# X-Tenant-ID ヘッダーのテナント検証
tenant_id = request.headers.get("X-Tenant-ID")
is_active = await client.is_active(tenant_id)
if not is_active:
    raise TenantError(f"Tenant {tenant_id} is not active")

# テナント設定値の取得
settings = await client.get_settings(tenant_id)
max_users = settings.get("max_users") or "100"

# アクティブテナント一覧
filter_ = TenantFilter(status=TenantStatus.ACTIVE)
tenants = await client.list_tenants(filter_)
print(f"アクティブテナント数: {len(tenants)}")
```
