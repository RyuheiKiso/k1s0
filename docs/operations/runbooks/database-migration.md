...
> **注意:** 本ドキュメントの「マイグレーション」は**データベーススキーマの変更**を指す。
> k1s0 CLI の `k1s0 migrate` コマンド（`analyze`, `plan`, `apply`, `status`）は**既存プロジェクトを k1s0 管理下に取り込む**ためのコマンドであり、データベースマイグレーションとは別物である。
> 既存プロジェクト取り込みについては CLI ドキュメントを参照のこと。
