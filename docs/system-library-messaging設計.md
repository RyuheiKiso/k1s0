...
public protocol EventProducer: Sendable {
    func publish(envelope: EventEnvelope) async throws
}

// イベント購読プロトコル
public protocol EventConsumer: Sendable {
    func consume() async throws -> ConsumedMessage
    func commit(message: ConsumedMessage) async throws
}
...
public struct NoOpEventProducer: EventProducer {
    public init()
    public func publish(envelope: EventEnvelope) async throws {}
}
```
...
public enum MessagingError: Error, Sendable {
    case publishFailed(topic: String, underlying: Error)
    case consumeFailed(underlying: Error)
    case commitFailed(offset: Int64, underlying: Error)
