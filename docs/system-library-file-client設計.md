...
**配置先**: `regions/system/library/csharp/file-client/`

```
file-client/
├── src/
│   ├── FileClient.csproj
│   ├── IFileClient.cs          # ファイルストレージ操作インターフェース
│   ├── ServerFileClient.cs     # file-server 経由実装
│   ├── S3FileClient.cs         # 直接 S3 API 実装
│   ├── FileClientConfig.cs     # バックエンド・認証設定
│   ├── FileMetadata.cs         # ファイルメタデータモデル
│   ├── PresignedUrl.cs         # プリサインドURLモデル
│   ├── MultipartUpload.cs      # マルチパートアップロード管理
│   └── FileClientException.cs  # 公開例外型
├── tests/
│   ├── FileClient.Tests.csproj
│   ├── Unit/
│   │   ├── ServerFileClientTests.cs
│   │   └── FileMetadataTests.cs
│   └── Integration/
│       └── S3FileClientTests.cs
├── .editorconfig
└── README.md
```
...
| パッケージ | 用途 |
|-----------|------|
| AWSSDK.S3 3.7 | S3 互換 API クライアント |

**名前空間**: `K1s0.System.FileClient`

**主要クラス・インターフェース**:

| 型 | 種別 | 説明 |
|---|------|------|
| `IFileClient` | interface | ファイルストレージ操作の抽象インターフェース |
| `ServerFileClient` | class | file-server 経由実装（HttpClient 使用） |
| `S3FileClient` | class | 直接 S3 API 実装（AWSSDK.S3 使用） |
| `FileClientConfig` | record | バックエンド設定・エンドポイント・認証情報 |
| `FileMetadata` | record | パス・サイズ・コンテンツタイプ・ETag・更新日時・タグ |
| `PresignedUrl` | record | URL・メソッド・有効期限・追加ヘッダー |
| `FileClientException` | class | file-client ライブラリの公開例外型 |
...
public interface IFileClient
{
    Task<PresignedUrl> GenerateUploadUrlAsync(
        string path, string contentType, TimeSpan expiresIn,
        CancellationToken ct = default);

    Task<PresignedUrl> GenerateDownloadUrlAsync(
        string path, TimeSpan expiresIn,
        CancellationToken ct = default);
...
public record FileClientConfig(
...
// クライアント設定
public struct FileClientConfig: Sendable {
    public let serverUrl: URL?
    public let s3Endpoint: URL?
...
// file-server 経由実装
public actor ServerFileClient: FileClient {
    public init(config: FileClientConfig)
    public func generateUploadUrl(path: String, contentType: String, expiresIn: Duration) async throws -> PresignedUrl
