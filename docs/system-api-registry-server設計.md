...
全エンドポイントは [API設計.md](API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_APIREG_` とする。
...
[helm設計.md](helm設計.md) のサーバー用 Helm Chart を使用する。api-registry 固有の values は以下の通り。
...
- [system-library-schemaregistry設計.md](system-library-schemaregistry設計.md) -- Kafka Avro スキーマレジストリライブラリ（Kafka 向け、当サーバーは REST/gRPC 向け）
- [proto設計.md](proto設計.md) -- Protobuf スキーマ設計ガイドライン
- [gRPC設計.md](gRPC設計.md) -- gRPC 設計ガイドライン
