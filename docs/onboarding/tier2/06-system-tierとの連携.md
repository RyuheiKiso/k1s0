...
<!-- business サーバーから5つの system サーバーへの放射状 gRPC 接続と各サービスの用途を示す図 -->
![system tier gRPC呼び出しパターン](../img/system-grpc-calls.svg)
...
<!-- 8段階の初期化フローと各ステップで使用する system tier ライブラリを示す図 -->
![サーバー起動時初期化フロー](../img/server-startup-sequence.svg)
