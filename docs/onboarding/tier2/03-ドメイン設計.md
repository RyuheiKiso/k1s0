...
たとえば経理の業務なら「仕訳」「勘定科目」「元帳」といった業務用語がそのままコード上のクラス名や関数名になります。業務担当者とエンジニアが同じ言葉で会話できるので、認識のズレが起きにくくなります。
...
```
┌─────────────────────────────────┐
│  adapter（外の世界との接点）      │ ← HTTPリクエスト、gRPC呼び出しを受ける
├─────────────────────────────────┤
│  usecase（何をするか）           │ ← 「注文を作成する」「カテゴリを更新する」
├─────────────────────────────────┤
│  domain（業務のルール）          │ ← 「金額は0以上」「ステータスはこの順で遷移」
├─────────────────────────────────┤
│  infrastructure（技術的な実装）  │ ← DB保存、Kafka送信、外部API呼び出し
└─────────────────────────────────┘
```
...
| 関係パターン | 説明 | 例 |
| --- | --- | --- |
| Published Language | 共有するイベントスキーマ | 経理 → FA への資産取得イベント |
| Anti-Corruption Layer | 外部コンテキストのモデルを自コンテキストのモデルに変換 | FA が経理の仕訳データを受け取る際の変換層 |
...
```rust
// Rust の例（accounting/server/rust/domain-master より）
pub struct MasterItem {
    pub id: MasterItemId,           // 識別子
...
```rust
// MasterCategory が集約ルート
// MasterItem は MasterCategory を category_id で参照する（ID参照）
...
```rust
// domain/repository/category_repository.rs（インターフェース）
#[async_trait]
pub trait CategoryRepository: Send + Sync {
    async fn find_by_id(&self, id: &CategoryId) -> Result<Option<MasterCategory>>;
    async fn find_all(&self) -> Result<Vec<MasterCategory>>;
    async fn save(&self, category: &MasterCategory) -> Result<()>;
    async fn delete(&self, id: &CategoryId) -> Result<()>;
}
```

```rust
// infrastructure/persistence/category_repo_impl.rs（実装）
...
impl CategoryRepository for CategoryRepositoryImpl {
    async fn find_by_id(&self, id: &CategoryId) -> Result<Option<MasterCategory>> {
        sqlx::query_as("SELECT * FROM master_categories WHERE id = $1")
            .bind(id.as_ref())
...
```rust
// ドメインイベントの定義
pub struct MasterItemCreated {
    pub item_id: MasterItemId,
    pub category_id: CategoryId,
    pub code: String,
    pub name: String,
...
impl ManageItemsUseCase {
    pub async fn create_item(&self, input: CreateItemInput) -> Result<MasterItem> {
        ...
        // ドメインイベントを発行
        let event = MasterItemCreated::from(&item);
        self.event_publisher.publish("accounting.master-item.created", &event).await?;
...
```protobuf
// proto/k1s0/business/accounting/events/v1/master_item.proto
message MasterItemCreated {
  string item_id = 1;
...
```
accounting/server/rust/domain-master  ←→  accounting/server/rust/journal-processor
        gRPC で直接通信（同一コンテキスト内）
```
...
異なる領域間（例: `accounting/` と `fa/`）では Kafka を使った非同期メッセージングを使用する。

```
accounting/server  --[Kafka]-→  fa/server
   MasterItemCreated イベント発行       イベント受信・処理
```

- トピック命名規則: `{領域名}.{集約名}.{イベント名}`（例: `accounting.master-item.created`）
