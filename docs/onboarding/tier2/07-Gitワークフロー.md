...
main（本番用ブランチ。常に動く状態を保つ）
 ├── feat/add-project-type-description   ← 機能追加ブランチ
 ├── fix/project-type-validation-error   ← バグ修正ブランチ
...
| 接頭辞 | 用途 | 例 |
| --- | --- | --- |
| `feat/` | 新しい機能の追加 | `feat/add-project-type-description-api` |
| `fix/` | バグの修正 | `fix/project-type-not-found-error` |
| `chore/` | 依存ライブラリの更新、CI設定変更など | `chore/update-sqlx-version` |
| `docs/` | ドキュメントの追加・修正 | `docs/add-api-usage-guide` |
| `refactor/` | 動作を変えないコード改善 | `refactor/simplify-project-type-usecase` |
| `test/` | テストの追加・修正 | `test/add-project-type-handler-tests` |
...
| 種類 | 意味 | 例 |
| --- | --- | --- |
| `feat:` | 新機能の追加 | `feat: プロジェクトタイプ説明文の更新APIを追加` |
| `fix:` | バグ修正 | `fix: プロジェクトタイプコードが空の場合のバリデーション追加` |
| `chore:` | 雑務（依存更新、設定変更等） | `chore: sqlx を 0.8.1 に更新` |
| `docs:` | ドキュメントの変更 | `docs: APIエンドポイントの説明を追加` |
| `refactor:` | 動作を変えないコード改善 | `refactor: プロジェクトタイプリポジトリのクエリを整理` |
| `test:` | テストの追加・修正 | `test: ManageProjectTypesUseCaseのテストを追加` |

### 良い例・悪い例

```
# 良い例 --- 何を・なぜ変更したかがわかる
feat: プロジェクトタイプの説明文を更新するAPIを追加
fix: 存在しないプロジェクトタイプを更新しようとした時にpanicする問題を修正
...
git checkout -b feat/add-project-type-description-api
```

ここで `feat/add-project-type-description-api` の部分は、自分のタスクに合った名前にしてください。
...
# 特定のファイルをステージングに追加
git add src/domain/entity/master_project_type.rs
git add src/usecase/manage_project_types.rs

# コミット
git commit -m "feat: プロジェクトタイプの説明文を更新するAPIを追加"
```
...
git push -u origin feat/add-project-type-description-api
...
```markdown
## 概要
プロジェクトタイプの説明文を更新する gRPC API を追加した。

## 変更内容
- proto 定義に UpdateProjectTypeDescription RPC を追加
- usecase 層に update_project_type_description メソッドを追加
...
```bash
gh pr create \
  --title "feat: プロジェクトタイプ説明文の更新APIを追加" \
  --body "## 概要
プロジェクトタイプの説明文を更新する gRPC API を追加した。
