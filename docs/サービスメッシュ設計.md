...
**防御多層化:** メッシュワイドデフォルトに加え、各 Tier の Namespace にも個別の PeerAuthentication を定義している（[認証認可設計.md](認証認可設計.md) の「サービス間認証 mTLS」セクション参照）。メッシュワイド設定が誤って変更された場合でも、Namespace レベルの設定が防御層として機能する。
...
```yaml
# system Tier: business/service および同一 Tier からのアクセスを許可
apiVersion: security.istio.io/v1
...
---
# business Tier: service および同一 Tier からのアクセスを許可
apiVersion: security.istio.io/v1
kind: AuthorizationPolicy
metadata:
...
spec:
  ...
  rules:
    - from:
        - source:
            namespaces: ["ingress"]               # Kong API Gateway 経由のトラフィック
    - from:
        - source:
            namespaces: ["k1s0-system"]           # system 層からのアクセス
    - from:
        - source:
