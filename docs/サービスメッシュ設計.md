...
Tier 間のアクセス制御を Istio AuthorizationPolicy で強制する。AuthorizationPolicy の詳細定義（`allow-from-lower-tiers`, `allow-from-service-tier`, `allow-from-ingress`, `deny-bff-to-bff` 等）は [認証認可設計](認証認可設計.md) を参照。
...
- [認証認可設計.md](認証認可設計.md) — 認証・認可・AuthorizationPolicy
