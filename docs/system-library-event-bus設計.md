...
public actor EventBus {
    public init(config: EventBusConfig = .default)

    public func publish<T: DomainEvent>(_ event: T) async throws

    public func subscribe<H: EventHandler>(
        _ handler: H
...
public struct EventBusConfig: Sendable {
    public let bufferSize: Int
    public let handlerTimeout: Duration
    public static let `default` = EventBusConfig(bufferSize: 1024, handlerTimeout: .seconds(5))
}
```
...
```swift
public enum EventBusError: Error, Sendable {
    case publishFailed(underlying: Error)
    case handlerFailed(eventType: String, underlying: Error)
    case channelClosed
}
```
