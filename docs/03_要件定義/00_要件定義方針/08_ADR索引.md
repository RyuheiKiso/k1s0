...
- **ADR-TEST-008: e2e テストを owner / user の 2 系統に物理分離し、環境契約で各系統の責務を固定する**
  - 判断: `tests/e2e/owner/`（48GB host 専用、multipass + kubeadm + Cilium + Longhorn + MetalLB + フルスタック、CI 不可）と `tests/e2e/user/`（16GB host OK、kind + minimum stack、PR + nightly CI 可）に物理分離。起動経路は `tools/e2e/owner/up.sh` / `tools/e2e/user/up.sh` の専用スクリプトに完全分離（`tools/local-stack/up.sh --role` の cone profile 引数空間と物理分離）。owner full は不定期実走、user full は nightly cron。tier3-web のみ Go(chromedp、owner) + TS(Playwright、user) の二重提供。前 ADR-TEST-002 の後継分割の 1 本目（残り 3 本: ADR-TEST-009 / 010 / 011）
