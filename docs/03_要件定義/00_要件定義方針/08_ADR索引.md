...
- **ADR-TEST-002（撤回 / 再策定予定）: L4 E2E 自動化経路 ADR**
  - 状態: e2e テスト基盤刷新に伴い本 ADR は撤回。テスト基盤再構築後に新 ADR で再策定する
  - 影響: 本索引と関連 ADR（ADR-TEST-001 / 003 / 005 / 007、ADR-NET-001、ADR-POL-002 等）の cite はすべて「テスト基盤刷新後の新 ADR で再策定」に置換済
...
- **ADR-TEST-006（撤回 / 再策定予定）: 観測性 E2E ADR**
  - 状態: e2e テスト基盤刷新に伴い本 ADR は撤回。テスト基盤再構築後に新 ADR で再策定する
  - 影響: ADR-OBS-001 / 002 / 003 / OPS-001 等の cite は「観測性 E2E に関する関連 ADR はテスト基盤再構築後に追記」に置換済
