...
7 カテゴリ構造で要件を分類する。番号帯は共通フレーム 2013 の要件定義プロセスと対応付け、読み順は番号順で完結するよう設計した。

```
03_要件定義/
├── README.md                           # 本書
├── 00_要件定義方針/                    # 要件定義プロセスの定義
├── 10_業務要件/                        # k1s0 が支える業務の定義
├── 20_機能要件/                        # 機能要件（tier1 11 API を含む）
├── 30_非機能要件/                      # IPA 非機能要求グレード 6 大項目
...
通読する場合は 00 → 10 → 20 → 30 → 80 → 90 の順に読むことで、プロセス定義 → 業務 → 機能 → 非機能 → トレーサビリティ → エビデンスと、IPA 共通フレーム 2013 の要件定義活動をなぞる構造になっている。部分参照する場合は、以下を目安に該当カテゴリへ飛ぶ。
