...
内部実装は Dapr State Building Block の Go SDK を利用する。バックエンドは Valkey で、採用後の運用拡大時で切り替え余地を残す（Component YAML を書き換えるだけで影響を tier1 内に閉じる）。

## 機能要件

### FR-T1-STATE-001: 基本 Get/Set/Delete

**業務根拠**: BR-PLATUSE-001（tier2 開発者が接続管理を個別実装しない）、BR-PLATGOV-001（テナント越境事故の構造的防止）。

**現状**: tier2 が Redis を直接使うと、接続文字列・認証・プール設定を各アプリで個別管理することになる。Dapr State を使えば抽象化されるが、Dapr のキー命名規約（アプリ名プレフィックス）と tenant 分離の強制が個別実装となる。採用側組織の想定 tier2 チーム数 20 チームで、1 チームあたり接続管理の設計 4 時間・実装 6 時間・テスト 4 時間 = 14 人時を個別に要すると、累計 280 人時（約 35 人日）の重複工数が発生する。

**要件達成後**: `k1s0.State.Get(key)`、`Set(key, value, options)`、`Delete(key)` で基本操作を提供する。キーの内部プレフィックスには `tenant_id` が自動付与され、テナント越境が構造的に不可能になる。値のシリアライズは JSON（デフォルト）と Protobuf（オプション）を選択可能。重複工数 280 人時が tier1 の 1 回実装（40 人時見込み）に集約され、年間約 240 人時の開発工数削減効果となる。

**崩れた時**: tier2 開発者が tenant_id を手でキーに含める必要が生じ、書き忘れが起きる。社内既存ライブラリの運用実績では、tenant_id 手動付与のチームあたり書き忘れ発生率は約 5%（1 リリースに 1 件程度）。20 チーム全体では月 1〜2 件のテナント越境アクセスが発生する想定となり、これは個人情報保護法上の「漏えい等」として報告義務対象（24 時間以内に個人情報保護委員会へ第一報）となる可能性がある。複数 tier2 アプリがキー名空間を衝突させ、意図しない読み書きが発生する二次被害も発生する。
...
- **リリース時点**: FR-T1-STATE-001、002（Go SDK）
- **リリース時点**: FR-T1-STATE-003、004 を追加、C# SDK
- **リリース時点**: FR-T1-STATE-005（トランザクション）実装判定
- **採用後の運用拡大時**: Python / Rust SDK、代替バックエンド（etcd 等）検討

## 関連非機能要件

- **NFR-B-PERF-003**: State Get p99 < 10ms（キャッシュレイテンシ目標）
- **NFR-A-CONT-001**: Valkey 単一ノード障害時のフェイルオーバー（採用側のマルチクラスタ移行時、リリース時点は単一ノード）
- **NFR-E-AC-003**: tenant_id クレーム検証とキー越境防止
- **NFR-B-RES-001**: Valkey メモリ水平拡張（採用側のマルチクラスタ移行時）
