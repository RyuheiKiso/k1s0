...
リリース時点で Go / C# / Rust / Python の SDK を提供し、採用後の運用拡大時で追加言語を検討する。tier3 の UI 層（MAUI / React）向けは HTTP/1.1 プロキシ経由で利用する。提供言語・版の詳細は各 API ファイルの「段階対応」節を参照。
...
- クライアント SDK は minor version を越えた後方互換破壊を リリース時点〜採用後の運用拡大時 では発生させない
