...
| Workflow | Workflow（Dapr Workflow or Temporal）| **未決（U-005）** | Dapr Workflow と Temporal の採否は Phase 1b で確定。どちらを採用するかで Dapr 依存レベルが変わる | Temporal（候補）/ Dapr Workflow（候補） | ADR-RULE-002 で Phase 1b 中に確定 |
...
| Workflow (FR-T1-WORKFLOW-001〜005) | COMP-T1-WORKFLOW | Go / Deployment 固定 3 replica / sticky | Temporal PostgreSQL or Valkey（U-005 で Phase 1b 中に確定） |
