...
| Workflow | Workflow（Dapr Workflow or Temporal）| **未決（U-WORKFLOW-001）** | Dapr Workflow と Temporal の採否は Phase 1b で確定。どちらを採用するかで Dapr 依存レベルが変わる | Temporal（候補）/ Dapr Workflow（候補） | ADR-RULE-002 で Phase 1b 中に確定 |
...
|---|---|---|---|
| Service Invoke (FR-T1-INVOKE-\*) | COMP-T1-STATE | Go / Deployment+HPA 3–10 / Stateless | Istio Ambient mTLS 経由の tier2 間 gRPC |
| State (FR-T1-STATE-\*) | COMP-T1-STATE | 同上 | Valkey Cluster (Dapr state.valkey) |
| PubSub (FR-T1-PUBSUB-\*) | COMP-T1-STATE | 同上 | Apache Kafka (Dapr pubsub.kafka) |
| Secrets (FR-T1-SECRETS-\*) | COMP-T1-SECRET | Go / Leader-Elected (active 1 / standby 2) | OpenBao (動的 Secret / PKI / Transit) |
| Binding (FR-T1-BINDING-\*) | COMP-T1-STATE | Go / Deployment+HPA 3–10 | 外部 HTTP / SMTP / MinIO |
| Workflow (FR-T1-WORKFLOW-\*) | COMP-T1-WORKFLOW | Go / Deployment 固定 3 replica / sticky | Temporal PostgreSQL or Valkey（U-WORKFLOW-001 で Phase 1b 中に確定） |
| Log (FR-T1-LOG-\*) | Pod 化しない（SDK + OTel Collector） | — | OpenTelemetry Collector → Loki |
| Telemetry (FR-T1-TELEMETRY-\*) | Pod 化しない（SDK + OTel Collector） | — | OpenTelemetry Collector → Tempo / Mimir |
| Decision (FR-T1-DECISION-\*) | COMP-T1-DECISION | Rust / Deployment+HPA 2–8 / in-mem | ZEN Engine in-process（外部ストア無し） |
| Audit (FR-T1-AUDIT-\*) | COMP-T1-AUDIT | Rust / StatefulSet / PVC per replica | PostgreSQL WORM append-only + MinIO Object Lock |
| Pii (FR-T1-PII-\*) | COMP-T1-PII | Rust / Deployment+HPA 2–6 / sidecar-free | なし（純関数、設定は Decision 経由） |
| Feature (FR-T1-FEATURE-\*) | COMP-T1-STATE（現行） | Go / Deployment+HPA 3–10 | flagd |
