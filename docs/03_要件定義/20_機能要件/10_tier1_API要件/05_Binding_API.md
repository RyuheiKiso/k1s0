...
**現状**: tier2 が外部 HTTP API を呼ぶには、各言語の HTTP クライアント、リトライ、タイムアウト、認証ヘッダ管理を個別実装する。外部 URL の allowlist 管理が tier2 ごとにバラつく。

**要件達成後**: `k1s0.Binding.Send("http", { method, url, headers, body })` で外部 HTTP API を呼び出す。URL の allowlist は Component YAML で tenant 単位に管理し、allowlist 外の URL への送信は `K1s0Error.Forbidden` を返す。

**崩れた時**: tier2 が許可外の外部 URL にアクセスし、データ流出や SSRF 攻撃の踏み台となるリスクが発生する。
