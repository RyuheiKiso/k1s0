...
**業務根拠**: BR-PLATGOV-003（漏えい時影響範囲の時間限定による監査要件充足）。

**現状**: PostgreSQL 接続情報を長期不変のパスワードで管理すると、漏えい時の影響範囲が大きい。定期ローテーションは運用負荷が高く、実質回らない。社内既存システムでは DB パスワードの平均滞留期間が 18 か月、最長で 4 年超のものも存在する（2025 年度セキュリティ監査指摘）。年次監査指摘の 60% は「DB 認証情報の長期使い回し」起因で、是正対応に 1 指摘あたり平均 40 人時を要している。

**要件達成後**: `k1s0.Secrets.GetDynamic("postgres", "role_name")` で TTL 1 時間の動的 Secret を発行する。OpenBao の Database Engine が PostgreSQL ユーザを都度生成し、TTL 経過で自動失効。tier2 は取得直後にその認証情報で接続し、長期保持しない。監査指摘 60% の構造的解消により、是正工数 20 指摘 × 40 人時 = 800 人時/年の削減効果を見込む。漏えい時の影響範囲は最大 1 時間に限定され、従来「最長 4 年分のアクセスログ精査」が「直近 1 時間分のみ」に短縮される。

**崩れた時**: 長期パスワードの使い回しで漏えい検出が遅れ、影響範囲の特定に時間がかかる。ローテーションが人手に依存し、1 年以上放置される Secret が発生する。漏えいインシデント時の調査工数が数百人時規模に膨らみ、対応開始まで 72 時間超の遅延が発生するおそれ。
...
本 API の機械可読な契約骨格（Protobuf IDL）は [40_tier1_API契約IDL.md の 04. Secrets API セクション](../40_tier1_API契約IDL.md#04-secrets-api) に定義されている。SDK 生成・契約テストは IDL 側を正とする。以下は SDK 利用者向けの疑似インタフェースであり、IDL の `SecretsService` RPC と意味論的に対応する。
