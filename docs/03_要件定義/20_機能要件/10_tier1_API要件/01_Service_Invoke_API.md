...
**現状**: 素の Dapr でタイムアウトを設定するには gRPC context と Component YAML の双方を意識する必要がある。リトライ可否も呼び出し元実装に委ねると、同じエラーでもアプリごとに再試行有無や待機時間がバラつく。

**要件達成後**: SDK メソッドのオプション引数でタイムアウト秒数を指定する。既定値は 5 秒とし、リトライは [00_tier1_API共通規約.md](00_tier1_API共通規約.md) の既定ポリシーに従う。`Unavailable` / `ResourceExhausted` / 副作用なしの `DeadlineExceeded` のみを自動再試行対象とし、サーバが `retry_after_ms` を返した場合はその値を優先する。
...
- 自動リトライの対象エラー、回数上限、バックオフは `00_tier1_API共通規約.md` の規定と一致し、SDK 独自の per-call 再試行回数指定を公開契約に持ち込まない
- `retry_after_ms` が返された場合は SDK がその値を優先して待機する
...
本 API の機械可読な契約骨格（Protobuf IDL）は [40_tier1_API契約IDL/01_Service_Invoke_API.md](../40_tier1_API契約IDL/01_Service_Invoke_API.md) に定義されている。SDK 生成・tier2/tier3 連携実装・契約テストはすべて IDL 側を正とし、本書は要件記述と疑似インタフェースで補足する。
...
k1s0.Invoke(
    ...
    options?: {
        ...
        // リトライは 00_tier1_API共通規約 の既定ポリシーに従う
        propagate_auth?: bool   // default true、IDL の TenantContext 自動設定
    }
) -> (response: Proto message, error: K1s0Error?)
```

SDK の option フィールド名と IDL の message フィールド名は必ずしも一致しない（言語慣習に合わせる）。対応関係はコメントで明示する。IDL 変更時は本表も同時更新する（[40_tier1_API契約IDL/README.md](../40_tier1_API契約IDL/README.md) の責任分界表の「API 要件ファイルとの対応」節参照）。
...
- **リリース時点**: FR-T1-INVOKE-001、003、005（Go SDK、gRPC のみ）
- **リリース時点**: FR-T1-INVOKE-002、004 を追加（HTTP/1.1 プロキシ、Circuit Breaker、C# SDK）
- **リリース時点**: Python SDK 追加
- **採用後の運用拡大時**: Rust SDK（自作）追加、Feature Flag 連動のサーキットブレーカ
