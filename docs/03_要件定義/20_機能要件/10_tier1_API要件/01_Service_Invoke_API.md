...
- 対象サービスが稼働中であれば、中規模 3,000 名想定のピーク RPS（150 RPS）で p99 < 500ms（NFR-B-PERF-002 参照）
- 対象サービスが存在しない場合、`K1s0Error.ServiceNotFound` を返す（Dapr 生エラーを露出しない）
- Dapr SDK 固有の import が tier2 コードに出現しないこと（`grep -r "dapr.io" <tier2-code>` がゼロ）
...
疑似インタフェースを以下に示す。実 SDK の signature は言語別に調整する。

```
k1s0.Invoke(
    target: string,           // 対象サービス名（k8s Service 相当）
    method: string,           // 呼び出しメソッド名
    request: Proto message,   // gRPC リクエスト
    options?: {
        timeout_seconds?: int,
        retry_count?: int,
        propagate_auth?: bool  // default true
    }
) -> (response: Proto message, error: K1s0Error?)
```

エラー型 `K1s0Error` は以下のバリアントを持つ。

- `ServiceNotFound`: 対象サービスが解決できない
- `Unauthorized`: JWT 検証失敗
- `Forbidden`: tenant_id 越境等の認可失敗
- `Timeout`: タイムアウト
- `Unavailable`: 対象サービスが 503 / gRPC UNAVAILABLE
- `Internal`: その他のサーバエラー
