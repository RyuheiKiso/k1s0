...
RPN 100 超はオンコール現場での再現性と影響範囲の両面で看過できない水準であり、Phase 1b までに対策計画を確定して Phase 1c までに運用へ組み込む。本節は個別故障モードを単なる対策行で終わらせず、「なぜこの RPN になったか」「どの業務が崩れるか」「対策がどう効くか」を 2〜3 文で示す。表や箇条書きのラベル羅列では、対策担当者が着手時に意図を読み取れないためである。

- **Temporal Determinism 違反（RPN 135）**: Workflow リプレイ時に非決定処理（時刻取得、ランダム、外部呼出）が混入していると再実行結果が原本と食い違い、業務データの状態が発散する。対策は CI に replay test を必須化し、Determinism 違反検知ツール（`go test` + Temporal SDK の replayer）を PR パイプラインで走らせること。Phase 1b で仕組みを導入し、Phase 1c で違反ゼロを合格ライン化する
- **ZEN Engine ルール評価バグ（RPN 126）**: 決定表の評価結果が誤っても呼出側は誤り気づけないため、業務判定が静かに壊れる。ゴールデンケース回帰テスト（ルール × 入力の期待出力マトリクス）を Phase 1a から積み上げ、段階リリース（canary 1% → 10% → 100%）で本番差分を検出する。評価 trace は常時記録し、事後の原因特定に用いる
