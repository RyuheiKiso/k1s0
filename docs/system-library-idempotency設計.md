...
**配置先**: `regions/system/library/csharp/idempotency/`

```
idempotency/
├── src/
│   ├── Idempotency.csproj
│   ├── IIdempotencyStore.cs            # キー管理インターフェース
│   ├── RedisIdempotencyStore.cs        # Redis バックエンド実装
│   ├── PostgresIdempotencyStore.cs     # PostgreSQL バックエンド実装
│   ├── IdempotencyRecord.cs            # レコード型
│   ├── IdempotencyStatus.cs            # ステータス列挙型
│   ├── IdempotencyMiddleware.cs        # ASP.NET Core ミドルウェア
│   ├── IdempotencyConfig.cs            # 設定
│   └── IdempotencyException.cs         # 公開例外型
├── tests/
│   ├── Idempotency.Tests.csproj
│   ├── Unit/
│   │   ├── IdempotencyRecordTests.cs
│   │   └── IdempotencyMiddlewareTests.cs
│   └── Integration/
│       └── RedisIdempotencyStoreTests.cs
├── .editorconfig
└── README.md
```
...
**名前空間**: `K1s0.System.Idempotency`

**主要クラス・インターフェース**:

| 型 | 種別 | 説明 |
|---|------|------|
| `IIdempotencyStore` | interface | キー管理の抽象インターフェース |
| `RedisIdempotencyStore` | class | Redis バックエンド実装（TTL 付き） |
| `PostgresIdempotencyStore` | class | PostgreSQL バックエンド実装 |
| `IdempotencyRecord` | record | キー・ステータス・レスポンス・TTL |
| `IdempotencyStatus` | enum | Pending / Completed / Failed |
| `IdempotencyMiddleware` | class | ASP.NET Core ミドルウェア（Idempotency-Key ヘッダー自動処理） |
| `IdempotencyConfig` | record | TTL・ヘッダー名・バックエンド設定 |
| `IdempotencyException` | class | idempotency ライブラリの公開例外型 |
...
public interface IIdempotencyStore
{
    ...
    Task MarkCompletedAsync(string key, string responseBody, int statusCode,
        CancellationToken ct = default);

    Task MarkFailedAsync(string key, string error,
        CancellationToken ct = default);
}
...
public protocol IdempotencyStore: Sendable {
    func get(key: String) async throws -> IdempotencyRecord?
    func set(key: String, record: IdempotencyRecord) async throws
    func markCompleted(key: String, response: Data, statusCode: Int) async throws
    func markFailed(key: String, error: String) async throws
}
...
public struct IdempotencyConfig: Sendable {
    ...
    public init(ttl: Duration, headerName: String = "Idempotency-Key")
}
```
...
public enum IdempotencyError: Error, Sendable {
    case duplicateRequest(key: String)
    case storeError(underlying: Error)
    case recordExpired(key: String)
