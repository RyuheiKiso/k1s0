...
k1s0 の各レイヤが k8s クラスタ上でどの namespace にどのように配置されるかを整理する。各レイヤの責務は [`01_レイヤ構成と責務.md`](../01_基礎/01_レイヤ構成と責務.md) を参照。
...
| namespace | 配置するもの |
|---|---|
| `infra` | Envoy Gateway / Istio Control Plane / Prometheus / Loki / Grafana Tempo / Grafana / OTel Collector (DaemonSet) / Kafka (Strimzi) / Harbor / Valkey / CloudNativePG + PostgreSQL / PgBouncer (CloudNativePG Pooler) / Kyverno / cert-manager / MinIO / Longhorn / OpenBao / MetalLB |
...
| `operation` | Backstage / Argo CD / Keycloak / GitHub Actions self-hosted runner / Renovate (GHA runner 内で実行) / Argo Events (採用後の運用拡大時) |
...
| 項目 | 方針 |
|---|---|
| k8s 実装 | セルフマネージド (`kubeadm` がメインプラン、`k3s` がサブプラン) |
...
| ストレージ | Longhorn (CNCF Incubating)。3 ノードレプリケーションでデータ保護。CloudNativePG / MinIO の PV を管理。kubeadm / k3s いずれの構成でも動作 |

クラウドマネージドを採用しない理由は「申請のハードルを下げる」「ベンダーロックイン回避」という k1s0 のコア原則に由来する。詳細は [`../../../01_企画/01_背景と目的/02_解決する価値.md`](../../../01_企画/01_背景と目的/02_解決する価値.md) を参照。
...
リリース時点はサイドカー方式で開始する。リソース消費が問題となった時点で Dapr Shared (DaemonSet / Deployment モード) への切替を検討する。この切替は tier1 チーム内の判断で完結し、tier2 / tier3 への影響はない。
...
- OTel Collector が Prometheus / Loki / Grafana Tempo にテレメトリを転送
- バックエンド追加・変更時にサービス Pod の再デプロイは不要

### 4.4 monitoring サブネット

- Prometheus / Loki / Grafana Tempo / Grafana / OTel Collector は `infra` namespace に配置
...
| イメージ署名 | Cosign (採用後の運用拡大時) + Kyverno 未署名拒否 |

採用初期 で cert-manager / MetalLB / kube-vip / OTel Collector を導入し、TLS 証明書の自動管理・ベアメタル LB・Control Plane HA VIP・テレメトリパイプラインを確立する。採用初期 で Kyverno を導入し Pod セキュリティとイメージ制御を Admission レベルで強制する。採用後の運用拡大時 で Cosign 署名検証ポリシーを追加する。
...
- [`02_依存ルールと通信経路.md`](../01_基礎/02_依存ルールと通信経路.md) — レイヤ間の制約と通信フロー
- [`13_IoTエッジ接続/`](../05_接続と共存/01_IoTエッジ接続/) — RS-232C 機器を Raspberry Pi で収容する 3 案の比較と物理層・ソフトウェア・セキュリティ・運用の深掘り
- [`../../03_技術選定/02_中核OSS/01_実行基盤中核OSS.md`](../../03_技術選定/02_中核OSS/01_実行基盤中核OSS.md) — 採用 OSS の選定根拠
- [`../../03_技術選定/03_周辺OSS/09_ネットワークとテレメトリ基盤.md`](../../03_技術選定/03_周辺OSS/09_ネットワークとテレメトリ基盤.md) — MetalLB / kube-vip / OTel Collector の選定根拠
