...
雛形 CLI が tier2 / tier3 のサービススケルトンを生成する際に、Outbox テーブル定義 (PostgreSQL の `outbox(id, event_type, payload, created_at, sent_at)` ) と、定期的に未送信レコードを Kafka に再送するワーカーを自動で含める。Phase 2 で [Debezium](../../../03_技術選定/03_周辺OSS/02_周辺OSS.md) の Outbox Event Router パターンの導入を検討するが、MVP では「アプリ内ワーカーで定期的に sent_at NULL のレコードを再送」する単純実装で十分である。
