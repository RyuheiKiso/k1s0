...
この事象が採用検討で問われる理由は、Istio が **「設定が複雑で運用負荷が高い」** という業界的な評判を持つからである。「Istio を導入した瞬間にメッシュ自体が単一障害点になり、業務が常に止まるリスクを抱える」という懸念に対し、本章は **「データプレーン (Envoy) は Control Plane 障害でも動き続ける xDS アーキテクチャである」** ことを根拠に反論する。
...
istiod 障害時に新規 VirtualService / DestinationRule の反映が止まることは、**設計上許容する** 判断を取る。「常に最新設定が即座に反映されないと業務が回らない」というユースケースは、k1s0 のターゲット (採用側組織の情シス部門) では稀であり、設定変更のロールアウトは Argo CD 経由で計画的に行うことを前提としている。
...
これらの試験を 採用後の運用拡大時、週次の CronChaosEngine で自動実行する。

### 復旧訓練

採用後の運用拡大時 から半期に 1 回、検証環境で本章の各 Runbook を実施し、所要時間と詰まりポイントを TechDocs に記録する。

## 関連ドキュメント

- [`00_概要.md`](./00_概要.md) — 壊滅的障害シナリオ全体の俯瞰
- [`../../03_セキュリティ/01_セキュリティモデル.md`](../../03_セキュリティ/01_セキュリティモデル.md) — Istio mTLS による Zero Trust 設計
- [`../../02_可用性と信頼性/03_グレースフルデグラデーション.md`](../../02_可用性と信頼性/03_グレースフルデグラデーション.md) — Istio Control Plane 障害時の縮退動作 (4.1 節)
- [`../../03_セキュリティ/02_脅威モデル詳細.md`](../../03_セキュリティ/02_脅威モデル詳細.md) — 信頼境界と脅威分析
- [`../../../03_技術選定/02_中核OSS/01_実行基盤中核OSS.md`](../../../03_技術選定/02_中核OSS/01_実行基盤中核OSS.md) — Istio / Envoy Gateway 採用根拠
- [`../../../03_技術選定/03_周辺OSS/02_周辺OSS.md`](../../../03_技術選定/03_周辺OSS/02_周辺OSS.md) — cert-manager / Litmus
- [`../../../03_技術選定/03_周辺OSS/08_シークレット管理.md`](../../../03_技術選定/03_周辺OSS/08_シークレット管理.md) — OpenBao によるルート CA 管理
