...
tier1 内部が Go と Rust のハイブリッド構成 ([`02_内部言語ハイブリッド.md`](../01_設計の核/02_内部言語ハイブリッド.md) である以上、Go サービスと Rust サービス間の **gRPC 契約の整合性** を構造的に担保する仕組みが必須となる。本資料は Protobuf スキーマの集中管理・後方互換性検証・CI ガード・テスト戦略を定義し、「型不整合バグを構造的に防止する」([`02_内部言語ハイブリッド.md`](../01_設計の核/02_内部言語ハイブリッド.md) 第 6 章) を具体化する。
...
これらは [`02_内部言語ハイブリッド.md`](../01_設計の核/02_内部言語ハイブリッド.md) 第 3 章の「言語境界の規約」を技術的に実装する。
...
`common/v1/` は Go / Rust 双方がインポートする共通型。エラーコードの enum は tier1 内部で統一され、[`02_内部言語ハイブリッド.md`](../01_設計の核/02_内部言語ハイブリッド.md) 第 3 章の「エラーコード体系を言語横断で同一にする」を実現する。
...
[Buf](https://buf.build/) は Protobuf スキーマの lint / 後方互換性検証の事実上の標準ツール ([`../../03_技術選定/03_周辺OSS/10_Protobufとロールアウト.md`](../../03_技術選定/03_周辺OSS/10_Protobufとロールアウト.md) で採用済み)。本資料では `buf breaking` を CI に組み込んで破壊的変更を機械的に検出する。
...
Testcontainers を用いて以下を実行する ([`../../03_技術選定/03_周辺OSS/07_ストレージと運用補助.md#P`](../../03_技術選定/03_周辺OSS/07_ストレージと運用補助.md))。
...
採用後の運用拡大時、Litmus ([`../../03_技術選定/03_周辺OSS/07_ストレージと運用補助.md`](../../03_技術選定/03_周辺OSS/07_ストレージと運用補助.md)) で以下を検証する。
...
本資料の全施策を リリース時点 から完全導入するのは現実的でない。以下の段階で導入する。

| 適用段階 | 導入施策 |
|---|---|
| リリース時点 (採用初期) | Protobuf 集中管理 (`src/tier1/contracts/`) + buf lint のみ |
| リリース時点 (採用初期) | buf breaking + 生成コードの diff 検証 + CODEOWNERS |
| リリース時点 (採用初期) | シリアライズ互換性テスト + gRPC E2E テスト |
| 採用後の運用拡大時 | protoc-gen-validate 導入 + カオステスト |
| 採用側のマルチクラスタ移行時 | Rust / Go 双方の analyzer 強化 ([`03_API設計原則.md`](03_API設計原則.md) 第 7 項) |

リリース時点 時点で CI の後方互換性検証が機能し、「知らぬ間に破壊的変更が混入」する事態を防止する。
...
| エラーメッセージ | 日本語 (採用側組織の運用者向け) + 英語 (開発者向け) の 2 言語併記 |
...
- [`02_内部言語ハイブリッド.md`](../01_設計の核/02_内部言語ハイブリッド.md) — ハイブリッド方針の設計根拠
...
- [`../../03_技術選定/03_周辺OSS/10_Protobufとロールアウト.md`](../../03_技術選定/03_周辺OSS/10_Protobufとロールアウト.md) — Buf の採用根拠
- [`../../03_技術選定/01_俯瞰/18_言語選定の定量分析.md`](../../03_技術選定/01_俯瞰/18_言語選定の定量分析.md) — Go / Rust 選定の定量根拠
- [`../../04_CICDと配信/00_CICDパイプライン.md`](../../04_CICDと配信/00_CICDパイプライン.md) — CI での契約検証の統合
