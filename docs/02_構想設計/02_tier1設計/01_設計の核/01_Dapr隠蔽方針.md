...
3. 採用側組織の固有要件 (ログ標準化 / 監査改ざん防止 / 個人情報マスキング) に対応する
...
| ログ標準化 (構造化ログ形式統一、採用側組織の監査要件) | **◎** (アプリ内部ログを強制統一) | △ (アプリ内部ログは対象外) | ◎ | **tier1 が必須** |
...
| 採用側組織の固有要件 | 自由にカスタマイズ可能 | カスタムコンポーネントとして拡張可能だが学習コストあり |

→ **Dapr が機能面で優位**。tier1 自前実装の合理性は 採用側組織の固有要件への最適化が必要な範囲に限定。
...
| 採用側組織の監査要件 | 自由にカスタマイズ可能 (個人情報マスキング / 監査フォーマット / 改ざん防止) | カスタマイズ余地は限定的 |
...
→ **棲み分け可能だが両方必要**。両方の計装結果を同じ OTel Collector → Grafana Tempo に流すことでエンドツーエンドのトレースが一本化される。
...
| Rust 統一性 | 高 | 中 |
| 業界標準への適合 | 弱 | **強** |
| リリース時点 の現実性 | やや困難 | **現実的** |

tier1 の実装規模が劇的に縮小し、Kafka / state store / secret store クライアント等の自前実装が不要になる。OSS リリース時点での採用範囲が現実的な範囲に収まり、k1s0 の「車輪の再発明を避ける」原則と整合する。
...
| Secrets | `k1s0.Secrets.*` | Go ファサード → Dapr Go SDK → OpenBao (採用初期 以降) |
...
| 採用側組織の固有機能 (監査改ざん防止 / PII マスキング) | `k1s0.Audit.*` / `k1s0.Pii.*` | tier1 Rust サービス |
...
これらの違反は CI ガード (Roslyn analyzer / `golangci-lint` / YAML lint) で検出する。詳細は [`03_API設計原則.md`](../02_API契約/03_API設計原則.md) を参照。
...
この階層付けにより「infra 層への直接依存は tier1 のみ」という依存ルール ([`../../01_アーキテクチャ/01_基礎/02_依存ルールと通信経路.md`](../../01_アーキテクチャ/01_基礎/02_依存ルールと通信経路.md)) を維持できる。
...
| Dapr Control Plane の運用コスト | tier1 チームが Dapr 運用を担当。リリース時点で運用手順書を整備 |
...
3. 採用側組織の固有要件が Dapr Component で完全に吸収できるようになった → 自前実装部分の縮小

見直しは各段階 終了時のレトロスペクティブで定量的にチェックする。
...
- [`03_API設計原則.md`](../02_API契約/03_API設計原則.md) — Dapr 隠蔽を強制する CI ガード
- [`05_ワークフロー振り分け基準.md`](../04_診断と補償/05_ワークフロー振り分け基準.md) — Temporal / Dapr Workflow の定量的振り分け基準
- [`../../01_アーキテクチャ/01_基礎/02_依存ルールと通信経路.md`](../../01_アーキテクチャ/01_基礎/02_依存ルールと通信経路.md) — 依存ルールと通信
- [`../../03_技術選定/02_中核OSS/01_実行基盤中核OSS.md`](../../03_技術選定/02_中核OSS/01_実行基盤中核OSS.md) — Dapr の選定根拠
- [`../../../01_企画/02_競合と差別化/01_主要製品別評価.md`](../../../01_企画/02_競合と差別化/01_主要製品別評価.md) — Dapr との重複と棲み分け
