...
- [k1s0/tier1/audit/v1/audit_service.proto](#k1s0_tier1_audit_v1_audit_service-proto)
    ...
    - [ExportAuditChunk](#k1s0-tier1-audit-v1-ExportAuditChunk)
    - [ExportAuditRequest](#k1s0-tier1-audit-v1-ExportAuditRequest)
...
- [k1s0/tier1/secrets/v1/secrets_service.proto](#k1s0_tier1_secrets_v1_secrets_service-proto)
    ...
    - [GetDynamicSecretRequest](#k1s0-tier1-secrets-v1-GetDynamicSecretRequest)
    - [GetDynamicSecretResponse](#k1s0-tier1-secrets-v1-GetDynamicSecretResponse)
    - [GetDynamicSecretResponse.ValuesEntry](#k1s0-tier1-secrets-v1-GetDynamicSecretResponse-ValuesEntry)
...
<a name="k1s0-tier1-audit-v1-ExportAuditChunk"></a>
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| data | [bytes](#bytes) |  | フォーマット済みデータの 1 部分（バイナリ透過、UTF-8 を想定）。 |
| sequence | [int64](#int64) |  | 0 起点のチャンク連番（再構成 / 監査時の参照用）。 |
| event_count | [int64](#int64) |  | この chunk に含まれる event 数（chunk_bytes ベースの場合は variable）。 |
| is_last | [bool](#bool) |  | ストリーム末尾の chunk なら true。最後の &#34;]&#34; や EOF newline を含む。 |
...
<a name="k1s0-tier1-audit-v1-ExportAuditRequest"></a>
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| from | [google.protobuf.Timestamp](#google-protobuf-Timestamp) |  | 範囲開始（任意）。未指定（zero）は全履歴の先頭。 |
| to | [google.protobuf.Timestamp](#google-protobuf-Timestamp) |  | 範囲終了（任意）。未指定（zero）は最新まで。 |
| format | [ExportFormat](#k1s0-tier1-audit-v1-ExportFormat) |  | 出力フォーマット。EXPORT_FORMAT_UNSPECIFIED は NDJSON 扱い。 |
| chunk_bytes | [int32](#int32) |  | 1 chunk あたりの最大バイト数（既定 65536、上限 1048576）。 |
| context | [k1s0.tier1.common.v1.TenantContext](#k1s0-tier1-common-v1-TenantContext) |  | 呼出元コンテキスト（テナント境界の検証に必須）。 |
...
| context | [k1s0.tier1.common.v1.TenantContext](#k1s0-tier1-common-v1-TenantContext) |  | 呼出元コンテキスト |
| idempotency_key | [string](#string) |  | 冪等性キー（共通規約 §「冪等性と再試行」: 24h TTL の dedup） 重複 audit event 書込を防ぐ（hash chain 整合性が乱れないよう）。 同一キーでの再試行は副作用を重複させず初回 audit_id を返す。 |
...
<a name="k1s0-tier1-audit-v1-VerifyChainRequest"></a>
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| from | [google.protobuf.Timestamp](#google-protobuf-Timestamp) |  | 範囲開始（任意）。未指定（zero）はテナント配下の全履歴を対象。 |
| to | [google.protobuf.Timestamp](#google-protobuf-Timestamp) |  | 範囲終了（任意）。未指定（zero）は最新まで。 |
| context | [k1s0.tier1.common.v1.TenantContext](#k1s0-tier1-common-v1-TenantContext) |  | 呼出元コンテキスト（テナント境界の検証に必須） |
...
<a name="k1s0-tier1-audit-v1-VerifyChainResponse"></a>
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| valid | [bool](#bool) |  | チェーン全体の整合性が取れていれば true。 |
| checked_count | [int64](#int64) |  | 検証対象だったイベント件数（valid に関わらず set される）。 |
| first_bad_sequence | [int64](#int64) |  | valid=false 時のみ意味あり。最初に不整合を検出した sequence_number。 valid=true 時は 0。 |
| reason | [string](#string) |  | 不整合の理由（&#34;prev_hash mismatch&#34; / &#34;event_hash mismatch&#34; / &#34;tenant boundary&#34; 等）。 valid=true 時は空文字。 |
...
| Name | Number | Description |
| ---- | ------ | ----------- |
| EXPORT_FORMAT_UNSPECIFIED | 0 | 既定（指定なし）。サーバが NDJSON にフォールバックする。 |
| EXPORT_FORMAT_CSV | 1 | CSV（RFC 4180、ヘッダ行を最初の chunk に出力）。 |
| EXPORT_FORMAT_NDJSON | 2 | 改行区切り JSON（1 行 = 1 event）。Splunk / fluentd 取り込み向け。 |
...
| Export | [ExportAuditRequest](#k1s0-tier1-audit-v1-ExportAuditRequest) | [ExportAuditChunk](#k1s0-tier1-audit-v1-ExportAuditChunk) stream | 監査ログのテナント単位エクスポート（FR-T1-AUDIT-002 疑似 IF &#34;Audit.Export&#34;）。 server-streaming で範囲内の events を batch（chunk）に分けて配信し、 大量レコードでもメモリを圧迫しない。format で CSV / JSON / NDJSON を選択する。 |
...
| context | [k1s0.tier1.common.v1.TenantContext](#k1s0-tier1-common-v1-TenantContext) |  | 呼出元コンテキスト |
| idempotency_key | [string](#string) |  | 冪等性キー（共通規約 §「冪等性と再試行」: 24h TTL の dedup） 外部送信（SMTP / S3 等）の重複防止に必須。同一キーでの再試行は初回 response を返す。 |
...
<a name="k1s0-tier1-secrets-v1-GetDynamicSecretRequest"></a>

### GetDynamicSecretRequest
...
| ----- | ---- | ----- | ----------- |
| context | [k1s0.tier1.common.v1.TenantContext](#k1s0-tier1-common-v1-TenantContext) |  | 呼出元コンテキスト（テナント境界の検証に必須） |
| engine | [string](#string) |  | 発行エンジン名（&#34;postgres&#34; / &#34;mysql&#34; / &#34;kafka&#34; 等、OpenBao の database engine 種別） |
| role | [string](#string) |  | OpenBao 側で予め定義されたロール名（tenant_id でスコープされた role） |
| ttl_sec | [int32](#int32) |  | TTL 秒数（0 = 既定 3600 秒 = 1 時間、最大 86400 秒 = 24 時間） |
...
<a name="k1s0-tier1-secrets-v1-GetDynamicSecretResponse"></a>

### GetDynamicSecretResponse
GetDynamic 応答


| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| values | [GetDynamicSecretResponse.ValuesEntry](#k1s0-tier1-secrets-v1-GetDynamicSecretResponse-ValuesEntry) | repeated | 発行された credential 一式（&#34;username&#34; / &#34;password&#34; 等の key=value） |
| lease_id | [string](#string) |  | OpenBao 側 lease ID（renewal / revoke 用、削除時に呼び返す） |
| ttl_sec | [int32](#int32) |  | 実際に付与された TTL 秒数（要求値が ceiling を超えたら短縮される） |
| issued_at_ms | [int64](#int64) |  | 発効時刻（Unix epoch ミリ秒） |
...
<a name="k1s0-tier1-secrets-v1-GetDynamicSecretResponse-ValuesEntry"></a>

### GetDynamicSecretResponse.ValuesEntry
...
| Method Name | Request Type | Response Type | Description |
| ----------- | ------------ | ------------- | ------------|
| Get | [GetSecretRequest](#k1s0-tier1-secrets-v1-GetSecretRequest) | [GetSecretResponse](#k1s0-tier1-secrets-v1-GetSecretResponse) | 単一シークレット取得（テナント越境参照は即 PermissionDenied、FR-T1-SECRETS-001） |
...
| context | [k1s0.tier1.common.v1.TenantContext](#k1s0-tier1-common-v1-TenantContext) |  | 呼出元コンテキスト |
| idempotency_key | [string](#string) |  | 冪等性キー（共通規約 §「冪等性と再試行」: 24h TTL の dedup） 同一キーでの再試行は副作用を重複させず初回 SetResponse を返す |
...
| context | [k1s0.tier1.common.v1.TenantContext](#k1s0-tier1-common-v1-TenantContext) |  | 呼出元コンテキスト |
| backend | [WorkflowBackend](#k1s0-tier1-workflow-v1-WorkflowBackend) |  | バックエンド hint（BACKEND_AUTO で tier1 が自動選択）。 |
| idempotency_key | [string](#string) |  | 冪等性キー（共通規約 §「冪等性と再試行」: 24h TTL の dedup） 同一キーでの再試行は副作用を重複させず初回 StartResponse を返す |
...
| backend | [WorkflowBackend](#k1s0-tier1-workflow-v1-WorkflowBackend) |  | 実際に選択された backend（AUTO で起動された場合の解決結果が入る）。 |
...
| Name | Number | Description |
| ---- | ------ | ----------- |
| BACKEND_AUTO | 0 | 既定値: tier1 が workflow_type / 期待実行時間に基づいて自動選択する。 |
| BACKEND_DAPR | 1 | 短期ワークフロー向け（Dapr Workflow building block、上限 7 日）。 |
| BACKEND_TEMPORAL | 2 | 長期ワークフロー向け（Temporal、上限なし）。 |
