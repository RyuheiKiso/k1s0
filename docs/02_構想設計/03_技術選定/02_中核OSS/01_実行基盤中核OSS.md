...
- **初期開発コスト**: 認証・レート制限・変換を tier1 で実装する必要がある → リリース時点では認証を最優先、他は後続段階に送る
...
**採用理由**: トラフィック制御・mTLS・観測性を一括で提供。CNCF Graduated で信頼性が高い。採用側組織の長期要件に対応しやすい機能面で Istio を選ぶ。
...
| | Jaeger | 不採用 | CNCF Graduated で実績豊富だが、バックエンドに Elasticsearch / Cassandra が必要であり 採用側の小規模運用での運用が困難。Red Hat が 2025 年 11 月に Jaeger サポートを終了し Tempo 移行を公式推奨 |
...
| 自前実装 (Rust のみ) | 併用 (補完) | Dapr で対応できない領域のみ tier1 で自前実装 (ログ標準化 / OTel アプリ内部計装 / 採用側組織の固有機能 / 決定エンジン / 雛形 CLI) |
...
- Rust は Dapr 非依存の自作領域 (ZEN Engine 統合 / 採用側組織の固有機能 / 雛形 CLI / Component バリデータ) に限定
...
| **Rust (自作領域)** | 採用 | メモリ安全・高性能・長期保守性。ZEN Engine の in-process 統合 / 雛形生成 CLI / 採用側組織の固有機能で Rust の優位性が出る |
