...
k1s0 のデータ保全・接続プーリング・運用自動化・イベント連携・統合テストに関わる追加採用 OSS を整理する。中核 ([`01_実行基盤中核OSS.md`](../02_中核OSS/01_実行基盤中核OSS.md)) および周辺 A〜K ([`02_周辺OSS.md`](02_周辺OSS.md)) で埋めきれない領域を補完する。対象カテゴリは以下のとおり。
...
| Rook-Ceph | 却下 | 運用負荷が 採用側の小規模運用に対して過大 |
| ローカル PV | 却下 | ノード障害時にデータ消失 |

### 採用理由 (Longhorn)

1. **SUSE 開発で実績豊富** — CNCF Incubating プロジェクトとしてコミュニティも活発。kubeadm / k3s いずれの環境でも動作
2. **3 ノードレプリケーション / スナップショット / バックアップを UI 管理可能** — 採用側の小規模運用でも運用可能な低い学習曲線
...
3. **PostgreSQL デフォルト max_connections=100 に対し、採用初期 時点で約 85 接続が見込まれ余裕がない** — 接続プーリングなしでは接続枯渇リスクが高い
...
| 手動更新 | 却下 | 30+ OSS の CVE 追跡を 採用側の小規模運用で実施不能 |

### 採用理由 (Renovate)

1. **Go modules / Cargo.toml / NuGet / npm / Dockerfile / Helm chart / k8s manifest のイメージタグを一元管理** — 全技術スタックの依存を単一ツールでカバー
2. **CVE 検知 → 修正 PR 自動作成 → CI 自動実行でマージボタンを押すだけ** — 採用側の小規模運用での運用負荷を最小化
3. **非機能要件「CVE Critical: 48 時間以内」を 採用側の小規模運用で達成する唯一の手段** — 手動では 30+ OSS の追跡が物理的に不可能
...
- リリース時点では不要 (イベントソースとなる Kafka がまだない)
- 採用後の運用拡大時 の Kafka / Istio 導入と同時に構築
...
採用初期 の runner イメージ (Kaniko / Trivy / crane 同梱) に DinD sidecar を追加する。
...
- 採用後の運用拡大時 から導入 (tier1 Rust サービスの本格実装と同時)
...
| Qdrant | 却下 | 同上。Rust 製で高性能だが 採用側の小規模運用で別 DB を運用する余裕がない |
| Milvus | 却下 | 大規模向け。採用側組織の利用規模にはオーバースペックでリソース消費が大きい |
| ベクトル検索なし | 却下 | 採用側の全社展開期 以降の AI/ML 連携 (RAG / セマンティック検索) で必要になる |
...
| インデックス | 特徴 | 推奨用途 |
|---|---|---|
| IVFFlat | 構築が高速。精度はやや劣る | データ量が少ない初期段階 (採用後の運用拡大時-3) |
| HNSW | 検索精度が高い。構築にメモリを消費する | データ量が増えた 採用側の全社展開期 以降 |

### OSS リリース時点での採用範囲

- 採用後の運用拡大時 で PostgreSQL 拡張として有効化 (CloudNativePG の `postgresql.conf` に追加)
- 採用後の運用拡大時 時点では拡張の有効化のみ。実際のベクトルデータ格納は 採用側の全社展開期 以降
- tier1 Rust サービスでの利用を想定 (sqlx は pgvector 型をサポート)

### トレードオフ

- 大量のベクトルデータ (100 万件以上) を扱う場合は HNSW インデックスのメモリ消費が大きくなる → 採用側の全社展開期 以降の実データ量に応じてインデックス戦略を再評価
...
- [`../../01_アーキテクチャ/04_非機能とデータ/03_レート制限.md`](../../01_アーキテクチャ/04_非機能とデータ/03_レート制限.md) — Rate Limiting
- [`../../01_アーキテクチャ/02_可用性と信頼性/01_障害復旧とバックアップ.md`](../../01_アーキテクチャ/02_可用性と信頼性/01_障害復旧とバックアップ.md) — Longhorn によるデータ保護
- [`../../01_アーキテクチャ/04_非機能とデータ/02_データアーキテクチャ.md`](../../01_アーキテクチャ/04_非機能とデータ/02_データアーキテクチャ.md) — PgBouncer による接続管理 / スキーマ進化戦略
- [`../../04_CICDと配信/00_CICDパイプライン.md`](../../04_CICDと配信/00_CICDパイプライン.md) — Renovate / Argo Events のパイプライン統合
- [`../../04_CICDと配信/03_テスト戦略.md`](../../04_CICDと配信/03_テスト戦略.md) — Testcontainers のテスト戦略への統合
