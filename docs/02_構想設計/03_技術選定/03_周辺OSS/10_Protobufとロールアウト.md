...
- `buf generate` は 採用初期 では C# / Go の 2 言語。TypeScript は 採用後の運用拡大時
...
2. **Istio のトラフィック分割と連携したカナリアリリース** — Istio (採用後の運用拡大時 で導入) の VirtualService を自動操作し、新バージョンへのトラフィック割合を段階的に増加させる。5% → 25% → 50% → 100% のような段階制御が宣言的に定義できる
...
4. **採用側組織の環境での安全なデプロイを実現** — 採用側組織では「新バージョンで障害が出たら即座に旧バージョンに戻せること」が信頼獲得の前提条件。Argo Rollouts の自動ロールバックはこの要件に直接対応する
...
- リリース時点 では Istio が未導入のため、Argo Rollouts も導入しない。リリース時点では標準の kubectl rollout で運用する
- カナリア分析の閾値設定は運用実績に基づいて調整が必要。採用後の運用拡大時 初期は手動承認ステップを残し、メトリクス閾値の妥当性を確認してから全自動に移行する
...
5. **軽量で運用負荷が極めて低い** — Deployment 1 つ (1〜2 レプリカ) をデプロイするだけで動作する。設定はほぼデフォルトで十分であり、採用側の小規模運用でも追加の運用負荷はほぼゼロ
...
3 ノード / Pod 数 50 未満の リリース時点 規模ではメモリ消費は極めて少ない。採用後の運用拡大時でオブジェクト数が増加した場合も 0.2〜0.3 GB で収まる。

### トレードオフ

- kube-state-metrics は k8s API Server を watch するため、API Server への負荷が微増する。ただし read-only の watch であり、リリース時点規模では影響は無視できる
- メトリクスのカーディナリティが高い (namespace × resource × name の組合せ) ため、Prometheus のストレージ消費がやや増加する。採用後の運用拡大時で `metricLabelsAllowlist` / `metricAnnotationsAllowList` の設定で不要なメトリクスを除外することを検討する
