...
ルールエンジン (BRE) は [`03_ルールエンジン.md`](../02_中核OSS/03_ルールエンジン.md)、イベントスキーマレジストリは [`06_イベントスキーマレジストリ.md`](06_イベントスキーマレジストリ.md)、ローカル開発ツールは [`../../04_CICDと配信/04_ローカル開発環境.md`](../../04_CICDと配信/04_ローカル開発環境.md) でそれぞれ別扱いとする。
...
| Dex | 却下 | OIDC のフロントに特化。ユーザー DB を持たないため リリース時点で単独採用は不可 |
| ORY Hydra + Kratos | 却下 | 柔軟性は高いが複数コンポーネントの組み合わせが必要で 採用側組織の運用に不向き |

### 採用理由 (Keycloak)

1. **ユーザー DB を自前で持てる** — リリース時点では AD 連携なしで Keycloak ローカル DB を認証源とする
2. **将来の AD / LDAP フェデレーションを追加設定のみで実現可能** — 採用後の運用拡大時で 採用側組織の既存 AD を取り込む退路を確保
...
- ローカル DB 運用の煩雑さ → 採用後の運用拡大時で AD 連携に切替える前提で、リリース時点ではユーザー数を絞る
...
| Argo CD + Argo Rollouts | 併用候補 | カナリア / Blue-Green リリース。採用後の運用拡大時で追加検討 |
| Spinnaker | 却下 | 機能豊富だが重量級 |
| Jenkins X | 却下 | Jenkins 依存。k1s0 の軽量指向と整合しない |

### 採用理由 (Argo CD)

1. **Web UI によるデプロイ可視化** — 採用側組織の情シスの GitOps 学習曲線を緩和
...
4. **Tekton は同等パイプラインを k8s ネイティブで提供する退路** — リリース時点ではインストールしない
5. **どちらも OSS** — runner は Apache 2.0、Tekton は CNCF Graduated

詳細なパイプライン設計は [`../../04_CICDと配信/00_CICDパイプライン.md`](../../04_CICDと配信/00_CICDパイプライン.md) を参照。
...
- イメージ署名は 採用後の運用拡大時 (Cosign 導入時に追加)
...
3. **tier2 業務サービスの RDBMS ニーズ** — 業務ドメインロジックが RDBMS を要求するケースは 採用側組織ので極めて多い。tier1 が共有クラスタを提供し、tier2 / tier3 がスキーマを分離して利用する
4. **CloudNativePG は CNCF Sandbox (Apache 2.0)** — k1s0 の OSS ライセンス方針に完全適合
5. **宣言的 HA** — プライマリ障害時に自動フェイルオーバー。採用側組織の情シスの夜間対応負荷を軽減
...
- tier2 / tier3 サービス向けのデータベースプロビジョニングは 採用後の運用拡大時

### トレードオフ

- 共有 PostgreSQL クラスタの障害は複数コンポーネントに波及する → レプリカ + 自動フェイルオーバーで緩和。採用側のマルチクラスタ移行時 でクリティカル度に応じたクラスタ分離を検討
...
| OPA / Gatekeeper | 次点 | CNCF Graduated。Rego 言語によるポリシー記述。表現力は高いが Rego の習得コストが 採用側組織向きではない |
| Kubewarden | 却下 | CNCF Sandbox。WASM ベースで柔軟だがエコシステムが発展途上 |
| Polaris | 却下 | ベストプラクティス検証に特化。Admission 制御の汎用性が不足 |

### 採用理由 (Kyverno)

1. **ポリシーが k8s ネイティブ YAML** — Rego のような独自言語を覚える必要がない。採用側組織の情シスの学習曲線を最小化
...
4. **cosign 署名検証を Admission で実行** — 採用後の運用拡大時 の署名済みイメージのみデプロイ許可を実現
...
| サプライチェーン (採用後の運用拡大時) | cosign 未署名イメージの deploy 拒否 |

### OSS リリース時点での採用範囲

- 採用初期 で Kyverno を `infra` namespace にデプロイ (HA 3 replicas)
- PSS Restricted 相当の ClusterPolicy を適用し、PodSecurityAdmission を無効化
- Harbor レジストリ制限ポリシーと必須ラベルポリシーを適用
- 採用後の運用拡大時 で cosign 署名検証ポリシーを追加
...
- 採用初期 で cert-manager を `infra` namespace にデプロイ
...
- 自己署名 CA の信頼配布 — クライアント端末に CA 証明書を信頼させる必要がある → 採用側組織では Active Directory グループポリシーでの配布が一般的
...
| tier1 Audit (採用側のマルチクラスタ移行時) | 監査ログのアーカイブ先 |
| Loki (採用側のマルチクラスタ移行時) | ログの長期保存先 |

### OSS リリース時点での採用範囲

- 採用初期 で MinIO を `infra` namespace にデプロイ (単一インスタンス、レプリカなし)
- Harbor のバックエンドとして構成
- CloudNativePG のバックアップ先として構成
- OpenTofu State の S3 バックエンドとして構成
- 採用後の運用拡大時で HA 構成 (Erasure Coding) に拡張

### トレードオフ

- AGPL-3.0 ライセンス → 社内利用のため改変配布義務は発生しない。Grafana / Loki と同じ扱い
- 単一インスタンスの リリース時点構成ではストレージ冗長性がない → ノードをまたぐ PV (Longhorn 等) との組み合わせ、または 採用後の運用拡大時 での HA 化で対応
...
3. **flagd は単一バイナリ・軽量** — メモリ 64〜128 MB で動作。採用側組織の限られた VM リソースに適合
...
- 採用後の運用拡大時 で flagd を `tier1` namespace にデプロイ
- tier1 公開 API に `k1s0.Feature` を追加
- フラグ定義は Git リポジトリの `flags/` ディレクトリに JSON で配置し、Argo CD で同期
- 採用側のマルチクラスタ移行時でユーザーセグメント / 段階的ロールアウト (パーセンテージベース) を本格活用
...
3. **Web UI (Litmus Portal)** — 実験の実行状況・結果を可視化。採用側組織の運用チームの学習曲線を緩和
4. **定期実行 (CronChaosEngine)** — 週次 / 日次で自動的に障害注入テストを実行し、縮退動作を継続的に検証
5. **Grafana 連携** — 実験結果をダッシュボードで表示。既存の Grafana スタックに統合

### k1s0 における役割

グレースフルデグラデーション設計 ([`../../01_アーキテクチャ/02_可用性と信頼性/03_グレースフルデグラデーション.md`](../../01_アーキテクチャ/02_可用性と信頼性/03_グレースフルデグラデーション.md)) で定義した縮退動作が実装と一致することを自動検証する。
...
- 採用後の運用拡大時 で Litmus を `operation` namespace にデプロイ
- 初期実験セット: PostgreSQL フェイルオーバー / Valkey 障害 / Dapr Sidecar 障害の 3 パターン
- CronChaosEngine で週次自動実行
- Grafana ダッシュボードで結果を可視化
- 採用側のマルチクラスタ移行時でネットワーク障害注入・カスケード障害テストに拡張

### トレードオフ

- 本番環境での障害注入リスク → リリース時点 / 採用後の運用拡大時 では専用の test namespace で実施。本番 namespace への適用は 採用側のマルチクラスタ移行時 のマルチクラスタ (staging / prod 分離) 後
...
- [`../../04_CICDと配信/00_CICDパイプライン.md`](../../04_CICDと配信/00_CICDパイプライン.md) — GHA / Argo CD / Harbor の統合フロー
- [`../../04_CICDと配信/04_ローカル開発環境.md`](../../04_CICDと配信/04_ローカル開発環境.md) — Tilt によるローカル開発環境
- [`../../01_アーキテクチャ/02_可用性と信頼性/03_グレースフルデグラデーション.md`](../../01_アーキテクチャ/02_可用性と信頼性/03_グレースフルデグラデーション.md) — Litmus で検証する縮退動作の定義
- [`../../01_アーキテクチャ/02_可用性と信頼性/01_障害復旧とバックアップ.md`](../../01_アーキテクチャ/02_可用性と信頼性/01_障害復旧とバックアップ.md) — MinIO をバックアップ先として利用
