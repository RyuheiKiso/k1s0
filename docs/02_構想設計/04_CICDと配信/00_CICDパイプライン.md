...
7. **依存パッケージの更新は Renovate で自動化** — 30+ OSS の脆弱性追跡を 2 名体制で実現するため、Renovate が更新 PR を自動生成する
...
## 5.5 Argo Events によるイベント駆動自動化 (Phase 2)
...
MVP では Argo Events はインストールしない。Phase 2 で Kafka と同時に導入する。
...
非機能要件「CVE Critical: 48 時間以内にパッチ適用」を 2 名体制で達成する。30+ の OSS 依存 (Go modules / Cargo.toml / NuGet / npm / Dockerfile / Helm chart / k8s manifest のイメージタグ) の脆弱性追跡を手動で行うことは不可能である。
...
Phase 1 (MVP-1a) から導入。tier1 Go / Rust 依存 + Dockerfile + Helm chart を対象に開始する。
...
- ~~Secret 管理方式~~ → **決定済み**: OpenBao (MPL 2.0) を MVP-1b で導入。SealedSecrets は GitOps 内 Secret の暗号化用途で併用。詳細は [`../03_技術選定/03_周辺OSS/08_シークレット管理.md`](../03_技術選定/03_周辺OSS/08_シークレット管理.md)
