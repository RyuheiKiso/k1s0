...
- 既存 `tests/e2e/scenarios/tenant_onboarding_test.go` の `t.Skip("PHASE: release-initial")` を採用初期で実装に置き換える際、本 ADR の Test Pyramid 比率と CI 時間予算に整合させる
...
- 関連 ADR（採用検討中、本 ADR から派生）: ADR-TEST-002（E2E 自動化経路）/ ADR-TEST-003（CNCF Conformance / Sonobuoy）/ ADR-TEST-004（Chaos Engineering ツール選定）/ ADR-TEST-005（Upgrade / DR drill）/ ADR-TEST-006（観測性 E2E）/ ADR-TEST-007（テスト属性タグ + 実行フェーズ分離）
