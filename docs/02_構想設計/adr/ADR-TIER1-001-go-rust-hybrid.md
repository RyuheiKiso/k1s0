...
- JTC 固有機能（監査ログ完整性、PII 自動検出、国内法令対応）は性能/堅牢性要件が厳しく、GC 言語よりシステム言語が向く場面がある
- 運用チームは 2 名体制のため、言語スタックを 2 つまでに絞ることが現実的（3 言語は運用負荷破綻）
...
- メリット:
  ...
  - 採用市場での Go と Rust の両方を人材プールに持てる（JTC 採用強化策）
