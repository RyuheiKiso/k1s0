...
- ADR-TEST-001（Test Pyramid + testcontainers）— 4 言語（Rust / Go / .NET / TypeScript）でテスト戦略が標準化される
- ADR-TEST-007（テスト属性タグ + CI 実行フェーズ分離）— 言語追加時は 4 タグ（@slow / @flaky / @security / @nightly）の implementation 追加が必要
