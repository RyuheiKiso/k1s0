...
**e2e テストを `tests/e2e/owner/`（オーナー専用、本番再現フルスタック）と `tests/e2e/user/`（利用者向け、kind + minimum stack）に物理分離し、各系統の環境契約を本 ADR で正典化する。** 両系統は別 Go module（`go.mod` 分離）として管理し、起動経路（`tools/local-stack/up.sh --role` 引数）と CI 経路（owner = CI 不可、user = PR / nightly で機械検証）を構造的に分ける。
...
`tools/local-stack/up.sh --role owner-e2e`（新設）で本番再現スタックを起動する。各要素は本番設計の ADR を直接投影する。
...
`tools/local-stack/up.sh --role user-e2e`（新設）で k1s0 install の最小成立形を起動する。kind 既存設計（ADR-NET-001 の「kind multi-node = Calico」）と整合させる。
...
owner 系 target は `tools/local-stack/up.sh --role owner-e2e` を呼び、user 系 target は `--role user-e2e` を呼ぶ。同一 shell script の `--role` 引数切替で両系統を吸収することで、owner と user で起動経路が割れない（ADR-POL-002 の SoT 単一化が e2e 領域でも成立）。

`tools/qualify/portability/run.sh` の multipass ロジックは owner-e2e role に統合して二重管理を避ける。portability 検証は owner-e2e の subset として再定義し、`docs/40_運用ライフサイクル/portability-results.md` に本 ADR との relate-back を追記する。
...
- `tools/qualify/portability/run.sh` が owner-e2e role の subset として再定義されることで、portability 検証と owner full の二重管理が解消される
...
- `tools/local-stack/up.sh` に `--role owner-e2e` / `--role user-e2e` を追加し、それぞれ multipass + kubeadm + Cilium + Longhorn + MetalLB + フルスタック起動 / kind + minimum stack 起動を実装
...
- `tools/qualify/portability/run.sh` の multipass ロジックを `tools/local-stack/up.sh --role owner-e2e` に統合、portability は owner-e2e の subset として再定義し、`docs/40_運用ライフサイクル/portability-results.md` に relate-back を追記
...
- `docs/05_実装/30_CI_CD設計/90_対応IMP-CI索引/01_対応IMP-CI索引.md` に本 ADR の対応 IMP-CI を追記（owner-e2e role / user-e2e role / make target / reusable workflow）
...
- ADR-POL-002（local-stack を構成 SoT に統一）— `--role owner-e2e` / `--role user-e2e` の SoT 拡張
