...
Google SRE Book・Atlassian DevOps 成熟度モデル・DORA State of DevOps Report はいずれも DX メトリクスと稼働 SLO を別管理する運用を前提としている。k1s0 は Phase 0 稟議承認段階で観測基盤の設計が確定しつつある今、メトリクスの所有境界を明文化しなければ、Grafana 上の「とりあえず全部置くダッシュボード」が 10 年分の技術負債として残る。

2 名運用フェーズでは Platform/SRE と EM が兼任に近い状態で回るため、同一ダッシュボードでも運用上は回る錯覚が生じる。しかし Phase 2 以降の 10 名フェーズでロール分離が進んだ瞬間に、ダッシュボードの読者層不一致が顕在化する。10 年保守では分離が不可避であり、Phase 0 時点でメトリクスの所有境界を決めることは、将来のダッシュボード再設計コスト（数百工数規模）を事前に排除する意味を持つ。
...
本 ADR は「メトリクスの所有境界と表示経路」の意思決定である。個別メトリクスの算出ロジック・収集頻度・ダッシュボードレイアウトは実装フェーズ（`docs/05_実装/50_開発者体験設計/` 等）で扱う。DORA 4 keys の具体的な計測方法は別 ADR で扱う可能性がある。
...
- DX メトリクス計測のための Backstage plugin 開発工数が Phase 1b に追加される
...
- DX メトリクスの計測基盤を Backstage Scorecards で実装（Phase 1b、ADR-BS-001 連動）
...
- Phase 1c 終了時点で DX メトリクスと稼働 SLI の相関（DX 悪化が稼働悪化の先行指標か）を分析し、レビューサイクルの適正性を検証
- Paved Road（ADR-DEV-001）の time-to-first-commit / Scaffold 利用率と本 ADR の DX メトリクスの整合性を Phase 1b 終了時点で確認し、定義の二重化・不整合を排除
