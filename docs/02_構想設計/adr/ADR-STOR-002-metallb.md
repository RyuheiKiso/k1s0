...
- Phase 1 初期は L2 モード（ARP）でも運用可能、Phase 2 で BGP に切替
...
**→ Phase 1 初期のみ L2 モード、Phase 2 で BGP に移行する段階的戦略を採用**
...
- ハードウェア LB コストを回避、Phase 1 予算内で実現
...
- Phase 1 L2 モード → Phase 2 BGP モード移行時の切替ダウンタイム計画
...
- Phase 1 L2 モード設定、境界ルータと共存
- Phase 2 BGP モード移行計画: BGP AS 番号設計、ピア設定、検証環境での事前試験
