...
k1s0 は tier1 の building blocks 実装として Dapr を採用済みであり、各アプリ Pod に Dapr サイドカー (`daprd`) が注入される構成が確定している。tier 間の mTLS・L7 認可・トラフィック分割を担うサービスメッシュとして Istio の導入を予定している。
...
- **採用側の小規模運用で回せる規模**を維持すること (採用側組織の情シスの定員制約)
...
本 ADR はリリース時点で採択し、サービスメッシュ構築計画・運用負荷見積もり・Runbook 整備の起点とする。
...
- リリース直後に 1 週間の POC を実施し、Dapr-mTLS と ztunnel HBONE の共存・レイテンシ予算・分散トレースの継承を実測で検証する
...
- デメリット:
  ...
  - 採用側のマルチクラスタ構成では east-west gateway の扱いが Sidecar モードと異なるため再検証が必要
...
- メリット:
  - 運用 OSS 数を 1 つ削減でき、採用側の小規模運用の負荷を直接的に下げられる
...
- デメリット:
  ...
  - 「業界標準のサービスメッシュを導入していない」ことへの採用判断者の心理的抵抗
...
2. **採用側の小規模運用との適合性**: 案 D が最も運用負荷が低いが、将来サービスメッシュが必要になった際の後付け移行コストが大きい。案 A は Ambient の運用ノウハウが少ない点で案 D より重いが、waypoint proxy を段階配置できるため初期負荷は抑えられる
...
4. **技術的美学と業界の方向性**: 案 A は Google / IBM / Microsoft / Alibaba が推進する次世代標準であり、「Pod 内にネットワーク関心事を入れない」という Unix 哲学に合致する。案 D は YAGNI 原則として美しいが、採用側の全社ロールアウト後のサービス数を考えると「結局必要になる」可能性が高い

**最終的な判断**: 案 A を採用する。案 D は企画段階の議論では十分魅力的だが、**採用側のサービス数が 30+ を超えた時点で結局サービスメッシュが必要になる**という中長期予測を重視した。その時点で新たに Ambient を入れるより、リリース時点から Ambient で始めてしまう方が総運用負荷が少ない。

案 B・C は Pod 内 3 コンテナ同居が解消されない時点で、本決定の本質的動機 (サイドカー二重注入の回避) を満たさないため採用しない。

**フォールバック経路**: 案 D を明示的に撤退経路として保持する。リリース直後の POC で Ambient Mesh の成熟度に問題が見つかった場合、案 D に切り替える。この切替判断は POC 開始から 1 ヶ月以内に行う。
...
- 起動順序・ポートキャプチャ・トレース断絶の 3 問題が構造的に消滅するため、サービスメッシュ稼働後の障害切り分け Runbook が簡素化される
...
- waypoint proxy の配置戦略 (namespace 単位か service 単位か) が新しい設計判断として追加で必要になり、構想設計の負荷が増加する見込み
- Dapr と Ambient Mesh の相互作用に関する公式ドキュメントは 2026 年時点でも断片的であり、POC での実測が必須となる
- 採用側のマルチクラスタ構成では east-west gateway の扱いが Sidecar モードと異なるため、マルチクラスタ移行前に再検証が必要

### 移行・対応事項

- リリース直後に 1 週間の Ambient Mesh POC を実施する (tier1 Go サービス × 2 本 + Dapr pub-sub の最小構成で検証)
- POC の検証項目: ztunnel HBONE と Dapr-mTLS の共存 / tier1 API p99 レイテンシへの影響 / W3C Trace Context の継承 / AuthorizationPolicy の動作
- POC 不成立時の切替判断 (案 D へのフォールバック) は POC 開始から 1 ヶ月以内に行う
...
- `docs/01_企画/img/全体構成図.drawio` の Istio サイドカー表現を Ambient 構成 (ztunnel DaemonSet + waypoint proxy) に揃える
