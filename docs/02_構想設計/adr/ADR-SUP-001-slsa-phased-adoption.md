# ADR-SUP-001: SLSA v1.1 を Phase 0 で Level 2・Phase 1b で Level 3 に段階到達
...
k1s0 は JTC 向け商用プラットフォームであり、顧客側の監査部門・情報システム部門から「ビルド由来の改ざん耐性をどう証明するか」を Phase 0 稟議の中で問われる。SolarWinds SUNBURST（2020）、Codecov bash uploader（2021）、3CX DesktopApp（2023）、xz-utils backdoor（2024）と続くビルドサプライチェーン事件の系譜を受け、JTC の情報セキュリティ指針は「ビルド成果物が改ざんされていないことを第三者検証可能にする」ことを CVE 対応と同等に重視する方向に傾いている。
...
k1s0 の Phase 0 稟議時点では人員 2 名・時間制約が厳しく、Level 3 のハーメティックビルド要求（ビルド実行時に外部ネットワークアクセスを遮断し、全依存を事前解決）を満たすための GitHub Actions カスタムランナー整備は間に合わない。一方、Level 1 のみでは JTC の監査要件を満たさず、稟議通過が困難になる。

同時に、「どの SLSA Level にいつ到達するか」を明示しないまま稟議を通すと、体制拡大後に「どのレベルにいるのか分からない」状態になり、監査対応で不利になる。本 ADR は Phase 0 で Level 2、Phase 1b で Level 3 に到達する段階計画を確定し、署名基盤（sigstore/cosign keyless）・SBOM（CycloneDX）・Forensics Runbook（image hash から tier1 公開 11 API への影響経路逆引き）までを固定する。

## 決定

**SLSA v1.1 Build Track の Level 2 を Phase 0 稟議承認時点で到達、Level 3 を Phase 1b（運用品質確保）時点で到達する。**
...
- Phase 1a 中に両経路の PoC を並行実施し、Phase 1b 開始時点で採用経路を別 ADR（ADR-SUP-002）で確定
...
Phase 1a 期間中に以下の準備を並行で進め、Phase 1b 開始時点で移行判定を行う。

- hermetic ビルド経路 α / β の PoC を `99_壁打ち/` 配下で評価、ビルド時間・再現性・運用負荷の 3 軸で定量比較
- Phase 0 時点の Level 2 構成で蓄積した provenance attestation の再現性（同一 git ref / 同一依存で再ビルドした際の digest 一致率）を週次サンプリングで計測
- 不一致が発生した場合の原因（非決定的タイムスタンプ・ビルド環境差・依存バージョン漂流）を分類し、Level 3 移行前に Runbook `RB-SUP-001: ビルド再現性逸脱対応` を整備

Phase 1b 開始時点で Level 3 到達判定会議を Product Council 経由で実施し、PoC 結果に基づく採用経路の最終確定を別 ADR（ADR-SUP-002）で記録する。判定が遅延した場合でも Level 2 構成は継続して成立し、監査証跡が途切れないことを保証する。
...
- BCP 対応として、Phase 1b 時点で sigstore の OSS コンポーネント（fulcio / rekor / trillian）の self-host PoC を実施し、Phase 2 で自社運用に切り替える経路を別 ADR で管理する
- 公共 sigstore 運用中も署名対象のメタデータ（image digest / signer identity / timestamp）は自社側にミラー保管し、Rekor がダウンしても検証経路を喪失しない構成を取る

## 検討した選択肢

### 選択肢 A: Phase 0 で Level 2、Phase 1b で Level 3 に段階到達（採用）

- 概要: 稟議通過時点で Level 2、運用品質確保段階で Level 3
- メリット:
  - Phase 0 稟議の「改ざん耐性を示せるか」に Level 2 到達で即答可能
  - ハーメティックビルドの整備を Phase 1a 中に PoC で詰め、運用負荷が増える時期（Phase 1b 以降）に Level 3 に昇格するため無理がない
...
- デメリット:
  ...
  - 公共 sigstore への依存が Phase 1b まで残る（BCP 計画を別途整備）

### 選択肢 B: Phase 0 から Level 3 を一気通貫で到達

- 概要: 稟議時点でハーメティックビルド・再現可能ビルド・強い非否認性を完備
- メリット: 段階移行のオーバヘッドがない、監査対応が最強
- デメリット:
  - 2 名運用のフェーズで hermetic runner 整備と tier 別ビルド再現性確保は工数過大
  - 不確実性の高い取り組みを Phase 0 稟議のクリティカルパスに置くリスクが大きい
  - 稟議承認を遅延させる原因になりうる

### 選択肢 C: Level 1 のみ到達、将来検討

- 概要: ビルド自動化 + プロビナンス文書化のみで稟議通過を狙う
- メリット: 即時対応が可能、追加投資ほぼ不要
- デメリット:
  - JTC の情報セキュリティ監査（J-SOX IT 統制含む）で通らない可能性が高い
  - 顧客側監査部門から「Level 1 では SolarWinds 級事件を防げない」と指摘された際に反論できない
...
- Phase 0 稟議に対し「SLSA Level 2 到達済み、Level 3 到達計画を保有」という明示的な回答が可能
- cosign keyless + CycloneDX + Kyverno ImageVerify の 3 点セットにより、本番 namespace での改ざん検知が構造的に担保される
- Rekor 登録で第三者検証可能性が自動的に得られ、顧客側監査・FedRAMP 類似審査への耐性が向上
- image hash 逆引き Forensics により、インシデント時の「影響範囲特定」が分単位で行える
- 10 年保守の途中でサプライチェーン事件が発生しても、事前整備により RTO を最小化できる

### ネガティブな帰結

- Phase 1b での Level 3 昇格時に hermetic runner の運用負荷（ビルド時間増・依存事前解決の整備）が発生
- cosign keyless は公共 sigstore 依存であり、ダウン時の BCP を別途整備する必要がある（Phase 2 で self-host 移行を計画）
- SBOM 差分レビューが PR レビュー工程に追加され、レビュー時間が 5〜10 分程度増える
- sigstore ecosystem 自体の脆弱性が発見された場合の影響評価が必要（依存対象として Renovate 監視に載せる）

### 移行・対応事項

- `tools/ci/slsa-provenance.yml` ワークフローを Phase 0 承認直後に配置、`slsa-github-generator` の呼び出しを tier 別に定義
...
- Phase 1a 中に hermetic runner の 2 経路 PoC（経路 α / 経路 β）を `99_壁打ち/` で評価、採用経路を ADR-SUP-002 で確定
