...
k1s0 は 採用側組織の社内の多数のシステムから利用されるプラットフォームであり、認証認可基盤の選定は以下の要件を同時に満たす必要がある。

- **SSO**: 採用側組織の既存の Active Directory/LDAP 連携で業務システムと共通の ID で SSO（NFR-E-AC、NFR-D-OBJ-003）
...
- SSO は OpenID Connect、Kerberos/LDAP 連携で 採用側組織の既存 AD と統合
...
- デメリット:
  ...
  - 採用側組織の既存 AD 連携の実績が薄い（Keycloak の LDAP/Kerberos 対応が強力）
...
- 採用側組織の既存 AD との SSO 実現、従業員は単一 ID で業務システム + k1s0 プラットフォーム利用
- テナント分離が Realm で実現、BC-ONB-003 の自動プロビジョニングと統合可能
- OIDC/SAML で将来の外部 IdP 連携が容易
- SPI 拡張で 採用側組織の固有要件（例: マイナンバー連携）に対応可能
