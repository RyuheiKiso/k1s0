...
JTC の現行基幹系は、.NET Framework 4.x（Windows Server 2012R2 / 2016）で運用されている資産が多数存在する。これらは以下の理由で k1s0 への一括書換が現実的でない。

- **資産規模**: 数百万行級コード、ビジネスロジックが複雑で書換工数が膨大
...
- デメリット:
  - 書換工数膨大（数百人月）、Phase 1 〜 2 では現実的でない
