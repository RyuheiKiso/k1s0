...
実行結果は MTTR / Availability / Error Rate の 3 指標で評価し、SLO 破綻が発生したシナリオは設計見直しの起点とする。Grafana LGTM スタック（ADR-OBS-002）との統合により、Litmus 実行中の SLI 推移を可視化する（観測性 E2E 用 ADR はテスト基盤刷新後に再策定し、その時点で詳細を再確定する）。
...
- ADR-OBS-002（Grafana LGTM）— Litmus 実行中の SLI 可視化（観測性 E2E 用 ADR はテスト基盤刷新後に再策定）
...
- 関連 ADR（採用検討中）: ADR-TEST-005（Upgrade / DR drill）/ ADR-TEST-007（テスト属性タグ + 実行フェーズ分離）。観測性 E2E はテスト基盤刷新後に新 ADR で再策定
