...
| `HttpRateLimitClient` | 構造体 | HTTP REST 経由の ratelimit-server 接続実装（ADR-0041 により `GrpcRateLimitClient` からリネーム）|
| `GrpcRateLimitClient` | 型エイリアス | `HttpRateLimitClient` への deprecated エイリアス（後方互換性用）|
