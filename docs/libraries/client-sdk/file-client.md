...
| `S3FileClient` | 構造体/クラス | AWS S3 / GCS / Ceph 直接実装。**Go/TypeScript/Dart に実装あり**。Rust は `direct-mode` feature flag で実装あり。Dart は crypto パッケージによる AWS Sig V4 署名で実装 |
...
| `MultipartUploadSession` | 構造体 | マルチパートアップロードセッション管理（Rust の `direct-mode` feature 限定） |
| `MultipartPart` | 構造体 | マルチパートアップロードの個別パート（Rust の `direct-mode` feature 限定） |
...
│   ├── multipart.rs    # MultipartUploadSession・MultipartPart（direct-mode feature 限定）
...
pub struct FileClientConfig {
    ...
    // access_key_id / secret_access_key は Rust では未実装（Go/TypeScript に実装あり、Dart は未実装）
...
impl FileClientConfig {
    ...
    /// S3 直接モード用コンフィグを生成する（direct-mode feature 限定）
    pub fn direct_mode(s3_endpoint: impl Into<String>, bucket: impl Into<String>, region: impl Into<String>) -> Self;
...
> **コンストラクタ言語別対応**:
> - Rust: `FileClientConfig::server_mode(url)` → `ServerFileClient::new(config)`, `InMemoryFileClient::new(config)`; `FileClientConfig::direct_mode(s3_endpoint, bucket, region)` → `S3FileClient::new(config)`（direct-mode feature 限定）
...
export interface FileClientConfig {
  ...
  bucket?: string;
  region?: string;
  accessKeyId?: string;     // S3 アクセスキー（Go/TypeScript に実装あり、Rust・Dart は未実装）
...
class FileClientConfig {
  ...
  final String? bucket;
  final String? region;
  final String? accessKeyId;     // S3 アクセスキー（定義のみ。S3FileClient は Dart 未実装）
  final String? secretAccessKey; // S3 シークレットキー（同上）
...
**注記**: Dart の S3FileClient は crypto パッケージによる AWS Sig V4 署名で実装済み。プリサインドURL生成（アップロード・ダウンロード）が動作する。
