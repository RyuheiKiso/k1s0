...
| `GraphQlHttpClient` | 構造体 | 本番用 HTTP クライアント（endpoint・headers 設定、HTTP POST でクエリ送信） |
...
> **注記 — export 状況**: TypeScript の `index.ts` から `GraphQlHttpClient` / `InMemoryGraphQlClient` は export 済み。`ErrorLocation` は独立型としては export されず、`GraphQlError.locations` のインライン型（`{ line: number; column: number }`）として表現される。
...
class ClientError implements Exception {
  ...
  ClientError.deserialization(this.message) : kind = ClientErrorKind.deserialization;
...
// 本番用 HTTP クライアント（package:http 使用）
class GraphQlHttpClient implements GraphQlClient {
  GraphQlHttpClient({required String endpoint, Map<String, String>? headers});
  // ...（GraphQlClient のメソッドを実装）
}
```
