...
| `GrpcSearchClient` | クラス/構造体 | HTTP REST 経由の search-server 接続実装（現状 HTTP REST、将来 gRPC 予定）。**Go・TypeScript・Dart に実装あり。Rust は未実装。** |
...
// Rust には GrpcSearchClient の本実装なし（Go/TypeScript/Dart は実装あり）。
...
// Go・TypeScript・Dart に実装あり。Rust には存在しない。
