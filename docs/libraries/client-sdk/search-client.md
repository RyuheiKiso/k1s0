...
| `GrpcSearchClient` | クラス/構造体 | HTTP REST 経由の search-server 接続実装（現状 HTTP REST、将来 gRPC 予定）。**全言語で実装あり。Rust は `feature = "grpc"` 有効時に HTTP/REST 実装を提供。** |
| `InMemorySearchClient` | クラス/構造体 | テスト・開発環境用インメモリ実装。全言語で実装あり（Rust は `feature = "test-utils"` 有効時に公開）。 |
...
// GrpcSearchClient（feature = "grpc" 有効時、HTTP/REST 実装）
// 名称は gRPC だが現状は HTTP/JSON API 経由で search-server と通信する。
pub struct GrpcSearchClient { /* ... */ }
impl GrpcSearchClient {
    pub fn new(addr: &str) -> Result<Self, SearchError>;
    pub fn with_http_client(addr: &str, http_client: reqwest::Client) -> Result<Self, SearchError>;
}

// InMemorySearchClient（test-utils 向け）
...
// 全言語で実装あり（Rust は feature = "grpc" 有効時）。
...
| 言語 | コンストラクタ | 備考 |
|------|----------------|------|
| Rust | `InMemorySearchClient::new()` | `feature = "test-utils"` 有効時に公開 |
...
テスト・デバッグ用にインデックス内のドキュメント件数を返すメソッド。全言語で実装済み（Rust は `feature = "test-utils"` 有効時）。
