...
**依存追加**: `k1s0-audit-client = { path = "../../system/library/rust/audit-client" }`（[追加方法参照](../_common/共通実装パターン.md#cargo依存追加)）
...
**配置先**: `regions/system/library/go/audit-client/`（[定型構成参照](../_common/共通実装パターン.md#定型ディレクトリ構成)）
...
**配置先**: `regions/system/library/typescript/audit-client/`（[定型構成参照](../_common/共通実装パターン.md#定型ディレクトリ構成)）
...
- [system-library-serviceauth設計](../auth-security/serviceauth.md) — サービス間認証ライブラリ
- [system-library-authlib設計](../auth-security/authlib.md) — 認証ライブラリ
- [system-library-correlation設計](correlation.md) — トレース ID 伝播（trace_id 連携）
- [認証設計](../../architecture/auth/認証設計.md) — 認証・認可全体設計
- [可観測性設計](../../architecture/observability/可観測性設計.md) — ログ・メトリクス設計
