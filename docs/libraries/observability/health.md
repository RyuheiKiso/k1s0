...
`/readyz` エンドポイントからビジネスロジック（ユースケース・リポジトリ等）を呼び出すことを**禁止**する。

**理由**:
1. RLS（Row Level Security）やテナント分離の影響を受け、誤って 503 を返す可能性がある
2. エラーが `is_ok()` で吸収され、ログに詳細が残らず診断不能になる
3. readyz が K8s `readinessProbe` に失敗すると、Pod がトラフィックから除外され全リクエストが 503 になる

**正しい実装**: DB 疎通確認には `SELECT 1` のような単純なクエリを使用すること（`Rust共通実装.md` の `readyz ハンドラの実装パターン` を参照）。
