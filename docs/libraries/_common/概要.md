...
| ライブラリ | 用途 | 利用者 | 詳細設計 |
|-----------|------|--------|---------|
| k1s0-server-common | axum 向けエラー型・構造化エラーコードの共通インフラ（HTTP エラーレスポンス型・共通エラーコード定数） | Rust サーバー（axum / utoipa feature 有効時） | [system-library-server-common設計](./server-common.md) |
| k1s0-bb-ai-client | AI Gateway への HTTP クライアント（AiClient trait・HttpAiClient・InMemoryAiClient・mockall 対応） | ai-agent-server・LLM 連携が必要な Rust サーバー | [system-library-bb-ai-client設計](../client-sdk/bb-ai-client.md) |
