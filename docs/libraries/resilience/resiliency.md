...
k1s0-resiliency は以下のスタンドアロンライブラリを内部で組み合わせるオーケストレーターである。

| 層 | ライブラリ | 役割 |
|---|---|---|
| スタンドアロン | k1s0-retry | 指数バックオフリトライ |
| スタンドアロン | k1s0-circuit-breaker | サーキットブレーカー |
| スタンドアロン | k1s0-bulkhead | バルクヘッド（同時実行制御） |
| オーケストレーター | k1s0-resiliency | 上記3つ + タイムアウトの統合ポリシー |

各スタンドアロンライブラリは単独で使用可能。k1s0-resiliency は複数パターンを組み合わせたい場合に使用する。
...
- [system-library-bulkhead設計](bulkhead.md) — k1s0-bulkhead ライブラリ（内部依存）
