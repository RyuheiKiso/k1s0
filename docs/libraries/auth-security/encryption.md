...
Vault から取得したキーを使った AES-256-GCM 対称暗号化と RSA-OAEP 非対称暗号化、および Argon2id によるパスワードハッシュ化を提供する。PII データの保護や設定値の暗号化保管に利用する。PBKDF2 は現行実装のスコープ外とする。
...
│   ├── rsa.rs          # generate_rsa_key_pair・rsa_encrypt・rsa_decrypt
...
- RSA の正常系/異常系（異鍵復号失敗・不正 PEM）の検証
