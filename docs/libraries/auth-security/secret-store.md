...
**配置先**: `regions/system/library/rust/secretstore/`
...
| `EnvSecretStore` | 構造体 | 環境変数実装 |
| `FileSecretStore` | 構造体 | ファイル実装（Kubernetes Secrets マウント対応） |
...
**依存追加**: `k1s0-secretstore = { path = "../../system/library/rust/secretstore" }`
...
│   ├── traits.rs       # SecretStore トレイト定義
│   ├── secret.rs       # Secret 構造体
│   ├── error.rs        # SecretStoreError
│   ├── vault.rs        # VaultSecretStore（feature = "vault"）
│   ├── env.rs          # EnvSecretStore
│   ├── file.rs         # FileSecretStore
│   └── in_memory.rs    # InMemorySecretStore
└── Cargo.toml
```
...
**配置先**: `regions/system/library/go/secretstore/`（[定型構成参照](../_common/共通実装パターン.md#定型ディレクトリ構成)）
...
type VaultSecretStore struct { /* k1s0-vault-client ラッパー */ }
func NewVaultSecretStore() *VaultSecretStore

type EnvSecretStore struct { /* 環境変数 */ }
func NewEnvSecretStore() *EnvSecretStore

type FileSecretStore struct { /* ファイル（K8s Secrets マウント） */ }
func NewFileSecretStore(basePath string) *FileSecretStore

type InMemorySecretStore struct { /* テスト用 */ }
func NewInMemorySecretStore() *InMemorySecretStore
```

## TypeScript 実装

**配置先**: `regions/system/library/typescript/secretstore/`（[定型構成参照](../_common/共通実装パターン.md#定型ディレクトリ構成)）
...
**配置先**: `regions/system/library/dart/secretstore/`（[定型構成参照](../_common/共通実装パターン.md#定型ディレクトリ構成)）
...
class VaultSecretStore implements SecretStore { /* k1s0-vault-client ラッパー */ }
class EnvSecretStore implements SecretStore { /* 環境変数 */ }
class FileSecretStore implements SecretStore { /* ファイル */ }
class InMemorySecretStore implements SecretStore { /* テスト用 */ }
```
