...
| `VaultError` | enum/struct | Rust: enum（`NotFound`・`PermissionDenied`・`ServerError`・`Timeout`・`LeaseExpired`） / Go: `VaultError{Code, Message}` + helper 関数 |
...
**配置先**: `regions/system/library/typescript/vault-client/`（[定型構成参照](../_common/共通実装パターン.md#定型ディレクトリ構成)）
...
**配置先**: `regions/system/library/dart/vault_client/`（[定型構成参照](../_common/共通実装パターン.md#定型ディレクトリ構成)）
...
- [system-vault-server設計](../../servers/vault/server.md) — シークレット管理サーバー設計
- [system-library-encryption設計](encryption.md) — 暗号化ライブラリ（シークレットの暗号化保存）
- [system-library-cache設計](../data/cache.md) — k1s0-cache ライブラリ（キャッシュ基盤）
- [system-library-kafka設計](../messaging/kafka.md) — Kafka コンシューマー（`k1s0.system.vault.secret_rotated.v1`）
