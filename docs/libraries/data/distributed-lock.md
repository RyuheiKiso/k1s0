...
| PostgreSQL | `PostgresDistributedLock` | `PostgresLock` | `PostgresDistributedLock` | `PostgresDistributedLock` |
...
| Go | なし | あり（concrete method、インターフェースには含まれない） | なし（advisory lock は TTL なし） |
| TypeScript | なし | あり | なし |
| Dart | なし | なし | なし |
...
| `PostgresLock` | 構造体 | PostgreSQL advisory lock 実装 |
| `PostgresLockOption` | 関数型 | `PostgresLock` の設定オプション |
| `WithPostgresLockPrefix` | オプション関数 | advisory lock キーのプレフィックス設定 |
...
impl RedisDistributedLock {
    /// 既存の MultiplexedConnection からインスタンスを生成する。
    pub fn from_connection(conn: MultiplexedConnection) -> Self

    /// Redis キーのプレフィックスを設定する。
    pub fn with_prefix(self, prefix: &str) -> Self
}

// PostgreSQL バックエンド
impl PostgresDistributedLock {
    /// PgPool からインスタンスを生成する。
    pub fn new(pool: PgPool) -> Self

    /// advisory lock キーのプレフィックスを設定する。
    pub fn with_prefix(self, prefix: &str) -> Self
}
```
...
**依存関係**: `github.com/redis/go-redis/v9`, `database/sql`（標準ライブラリ）
...
**RedisLock.Extend**（`DistributedLock` インターフェースには含まれない concrete method）:

```go
// RedisLock 固有メソッド（DistributedLock インターフェースには含まれない）
