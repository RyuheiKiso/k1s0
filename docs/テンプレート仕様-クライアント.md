...
Flutter クライアントは React と同じ nginx.conf を使用する。SPA ルーティング・gzip 圧縮・セキュリティヘッダーの設定は共通であるため、[3.14 nginx.conf](#314-nginxconf) をそのまま適用する。
...
### 4.10 lib/providers/service_provider.dart
...
| `README.md`（React / Flutter）               | —                                                  | プロジェクト概要・セットアップ手順           |
