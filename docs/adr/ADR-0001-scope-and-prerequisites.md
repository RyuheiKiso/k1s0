...
- `k1s0 new-screen`: フロントエンド画面の雛形生成
- `k1s0 lint`: 規約違反の検査（`--fix` による自動修正対応）
- `k1s0 upgrade --check`: 差分提示と衝突検知
- `k1s0 upgrade`: テンプレート更新の適用
- `k1s0 doctor`: 開発環境の健全性チェック
- `k1s0 completions`: シェル補完スクリプト生成
- `k1s0 domain-*`: domain 管理コマンド群（list, version, dependents, impact, catalog, graph）
- `k1s0 feature-update-domain`: feature の domain 依存更新
- `k1s0 registry`: テンプレートレジストリ操作
- `k1s0 docker`: Docker ビルド・compose 操作
- `k1s0 playground`: サンドボックス環境の管理
- `k1s0 migrate`: 既存プロジェクトの k1s0 移行支援

#### 2. templates

- backend-rust: Rust バックエンドサービスの雛形
- backend-go: Go バックエンドサービスの雛形
- backend-csharp: C# バックエンドサービスの雛形
- backend-python: Python バックエンドサービスの雛形
- backend-kotlin: Kotlin バックエンドサービスの雛形
- frontend-react: React フロントエンドの雛形
- frontend-flutter: Flutter フロントエンドの雛形
- frontend-android: Android フロントエンドの雛形
- framework 共通サービス: auth-service / config-service / endpoint-service の雛形

#### 3. framework

共通パッケージ（Rust / Go / C# / Python / Kotlin の各言語で 12 パッケージ）：
...
- `k1s0-grpc-server`: gRPC サーバー基盤
- `k1s0-grpc-client`: gRPC クライアント共通（deadline 必須、retry 原則禁止）
- `k1s0-health`: ヘルスチェック
- `k1s0-db`: データベース接続・トランザクション
- `k1s0-cache`: Redis キャッシュ
- `k1s0-resilience`: リトライ・サーキットブレーカー
- `k1s0-domain-event`: ドメインイベント pub/sub・outbox

**Tier 3（Tier 1/2 依存可）:**
- `k1s0-auth`: 認証・認可

フロントエンドパッケージ（React: 10、Flutter: 8、Android: 8）
...
- **K001-K003**: manifest の存在・必須キー・値の整合性
- **K010-K011**: 必須ディレクトリ・ファイルの存在検査（自動修正対応）
- **K020-K029**: コード品質（環境変数禁止、機密直書き禁止、Clean Architecture 依存違反、設定ファイル命名、Domain 層プロトコル型使用禁止、未使用 domain 依存、panic/unwrap/expect 検出）
- **K030-K032**: gRPC リトライ設定の検出・ADR 参照・完全性
- **K040-K047**: 層間依存ルール（依存方向違反、domain 未検出、バージョン制約、循環依存、非推奨 domain、framework バージョン、破壊的変更影響、domain バージョン必須）
- **K050, K053**: セキュリティ（SQL インジェクション、機密データログ出力）
- **K060**: インフラ（Dockerfile ベースイメージ未固定）
