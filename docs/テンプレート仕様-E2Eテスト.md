...
テンプレートファイル（`.tera`）の存在と仕様書コードブロックとの一致を Python + pytest で検証する E2E テストの仕様。
...
## Library Chart E2E テスト — ヘルパーテンプレート存在検証
...
- [テンプレート仕様 — レンダリングテスト](テンプレート仕様-レンダリングテスト.md) — テスト2層構成の概要・テスト対象マトリクス・CI統合
- [テンプレート仕様 — Rust 統合テスト](テンプレート仕様-Rust統合テスト.md) — Rust 統合テスト（cargo test）の仕様
