...
orphan は **2 系統**で別々に集計する。両者は発生源も影響範囲も異なるため、合算しない:

| 系統 | 定義 | 影響 | 検出 |
|---|---|---|---|
| **code-orphan** | `src/infra/deploy/tools/examples` で cite ∩ ADR ファイル無し | コードが無効な ADR ID を参照、実装と設計の乖離 | `tools/audit/lib/coverage.sh` で IMPL_PATHS 走査 |
| **docs-orphan** | `docs/` 全体で cite ∩ ADR ファイル無し | 採用検討者が docs を読んで存在しない ADR を探す事故 / 統合 / 改名 / 削除の追跡漏れ | `tools/audit/lib/coverage.sh` で docs/ 全体走査 |

いずれも 0 件であることが望ましい。code-orphan の最初の機械化動機は SHIP_STATUS F10 の手作業（「ADR-CNCF-004 / ADR-DAPR-001 / ADR-DEVEX-002 等 8 件がコードから引用されているが ADR 未起票」型 drift）。docs-orphan は 2026-05-02 #6 の audit ツール盲点修正で新設（39 件発見、過去のリファクタで ID を docs に残したまま ADR を統合 / 削除 / 改名した形跡）。
