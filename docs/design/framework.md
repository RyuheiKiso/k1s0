...
├── k1s0_config/         # YAML設定管理（未実装）
├── k1s0_http/           # API通信クライアント（未実装）
├── k1s0_auth/           # 認証クライアント（未実装）
├── k1s0_observability/  # OTel/ログ（未実装）
├── k1s0_ui/             # Design System（未実装）
└── k1s0_state/          # 状態管理（未実装）
```
...
| パッケージ | 状態 | 説明 |
|-----------|:----:|------|
| k1s0_config | ⬜ | YAML設定管理（後続フェーズ） |
| k1s0_http | ⬜ | Dioベース通信クライアント（後続フェーズ） |
| k1s0_auth | ⬜ | 認証クライアント（後続フェーズ） |
| k1s0_observability | ⬜ | OTel/ログ（後続フェーズ） |
| k1s0_ui | ⬜ | Design System（後続フェーズ） |
| k1s0_state | ⬜ | Riverpod状態管理（後続フェーズ） |
