...
└── src/
    ...
    ├── references.rs     # 参照検索
    ├── symbols.rs        # シンボル機能
    ├── code_action.rs    # コードアクション（QuickFix）
    └── document_link.rs  # ドキュメントリンク
```
...
診断情報（lint violations）に対する QuickFix アクションを提供します。エディタ上で診断の電球アイコンからワンクリックで修正を適用できます。

### 対応するルールとアクション

| ルール | アクション | 修正内容 |
|--------|----------|---------|
| K001 | manifest.json 作成 | `.k1s0/manifest.json` を最小構成で作成 |
| K010 | 必須ディレクトリ作成 | 不足ディレクトリを `.gitkeep` 付きで作成 |
| K011 | 必須ファイル作成 | 不足ファイルを空ファイルとして作成 |
| K020 | TODO コメント挿入 | 環境変数使用箇所に k1s0-config への移行コメントを挿入 |
| K021 | 機密キー変換 | `password` → `password_file` のようにファイル参照形式に変換 |
| K022 | TODO コメント挿入 | Clean Architecture 違反箇所に修正を促すコメントを挿入 |
...
pub fn get_code_actions(
    ...
    // 1. 診断情報からルール ID を抽出
    // 2. ルールに応じた修正アクション（WorkspaceEdit）を生成
    // 3. CodeAction（kind: QuickFix）として返す
}
```

## ドキュメントリンク機能

manifest.json 内のファイルパスやディレクトリパスをクリック可能なリンクに変換します。Ctrl+クリック（Cmd+クリック）でリンク先を開けます。
...
|---------|----------|
| `template.path` | テンプレートディレクトリ |
| `managed_paths` | 管理対象のディレクトリ/ファイル |
| `protected_paths` | 保護対象のディレクトリ/ファイル |
| `checksums` のキー | チェックサム対象ファイル |
| `template_snapshot` のキー | スナップショット対象ファイル |
...
pub fn get_document_links(
    ...
    // 1. manifest.json をパース
    // 2. パス値を含むキーを走査
    // 3. 各パスの Range と target URI を算出
    // 4. DocumentLink のリストを返す
}
```
...
7. コードアクション（QuickFix による lint violation 自動修正）
8. ドキュメントリンク（manifest.json 内パスの Ctrl+クリック）
...
| code_action.rs | - | ルール別 QuickFix 生成、WorkspaceEdit 構築 |
| document_link.rs | - | パス値抽出、リンク Range 算出、URI 解決 |
