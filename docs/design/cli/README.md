...
| [migrate コマンド](./commands-migrate.md) | 既存プロジェクトの k1s0 移行支援 |
...
| `playground start` | サンプル付き playground 環境を起動 | `--type`, `--name`, `--mode`, `--with-grpc`, `--with-rest`, `--with-db`, `--with-cache`, `--port-offset`, `-y` |
...
| `migrate analyze` | 既存プロジェクトの準拠状況を分析 | `--path`, `--type`, `--json`, `--verbose` |
| `migrate plan` | 移行計画を生成 | `--path`, `--name`, `--type`, `--output`, `--dry-run` |
| `migrate apply` | 移行計画を適用 | `--path`, `--plan`, `--phase`, `--dry-run`, `-y/--yes`, `--skip-backup` |
| `migrate status` | 移行の進捗状況を表示 | `--path`, `--plan`, `--json` |
