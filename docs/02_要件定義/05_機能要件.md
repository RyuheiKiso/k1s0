...
- **Phase 1b 暫定運用 (BR-032 対応)**: 退職・異動時の Keycloak ユーザ無効化を手動で 24 時間以内に完了する手順書が TechDocs に整備されていること。手順書には (1) 人事からの連絡受領、(2) Keycloak 管理コンソールでのユーザ無効化操作、(3) 全アプリからの 403 応答確認、(4) 監査ログへの操作記録確認 の 4 ステップを含む。この暫定手順は FR-013 (権限変更即時反映、Phase 2) の実装により自動化に置き換わる
...
**優先度 SHOULD / 関連 BR-031 / 提供層 tier1 (バックエンド: PostgreSQL)・tier3 / Phase 3**
