...
[可観測性設計.md](../../observability/overview/可観測性設計.md) D-107（監視・アラート設計）および D-110（分散トレーシング設計）に基づき、サーバーテンプレート生成時に可観測性のスケルトンコードを自動生成する。メトリクスは Prometheus 形式（RED メソッド）、トレースは OpenTelemetry OTLP gRPC Exporter を使用する。

### src/infra/observability/tracer.rs

`src/infra/observability/tracer.rs.tera` — tracing-opentelemetry 初期化。[可観測性設計.md](../../observability/overview/可観測性設計.md) D-110 の Rust 実装例に準拠する。
