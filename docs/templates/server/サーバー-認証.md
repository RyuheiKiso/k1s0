...
本ドキュメントは、[テンプレート仕様-サーバー](サーバー.md) から分割された認証認可 Middleware テンプレートの詳細仕様である。

---

## 認証認可 Middleware テンプレート

[認証認可設計.md](../../architecture/auth/認証認可設計.md) D-004（Kong API Gateway 認証フロー）および D-005（アプリケーションレベル RBAC）に基づき、Kong から転送されるヘッダーを使用した認証認可ミドルウェアのスケルトンコードを生成する。
