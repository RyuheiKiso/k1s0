...
**service tier（イベント proto を持つサービス）の場合:** `k1s0-proto-build` 共通クレートを使用する。サービス proto とイベント proto を単一呼び出しでコンパイルし、共通型の上書き消失を防止する。

```rust
{% if tier == "service" and api_styles is containing("grpc") %}
// service tier は共通クレートを使用（サービス proto + イベント proto を単一コンパイル）
...
> **補足**: service tier の `[build-dependencies]` には `k1s0-proto-build` を使用する。system tier では `tonic-build = "0.12"` を直接使用する。詳細は [Rust共通実装 build.rs](../../servers/_common/Rust共通実装.md#共通buildrs) を参照。
