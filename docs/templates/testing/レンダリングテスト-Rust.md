...
テスト配置: `CLI/crates/k1s0-cli/tests/` 配下。
...
`CARGO_MANIFEST_DIR` を用いて `CLI/crates/k1s0-cli/templates/` への絶対パスを取得する。テスト実行時のカレントディレクトリに依存しない。
...
テスト配置: `CLI/crates/k1s0-cli/tests/snapshot_tests.rs`
...
テスト配置: `CLI/crates/k1s0-cli/tests/snapshot_tests.rs`（スナップショットテストと同ファイル、または `CLI/crates/k1s0-cli/tests/helm_library_chart_tests.rs` として分離）
