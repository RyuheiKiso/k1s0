...
本ドキュメントは、[テンプレート仕様-クライアント](クライアント.md) から分割された React テンプレートの詳細仕様である。
...
- **app-catalog はテンプレート非適用** — app-catalog は独自の構成を持つため、本テンプレートによるコード生成の対象外である
...
| `eslint.config.mjs` / `.prettierrc`         | [コーディング規約](../../architecture/conventions/コーディング規約.md)            | TypeScript ツール・設定                      |
| `tsconfig.node.json`                         | ---                                                | Vite 設定ファイル用 TypeScript 設定          |
| `package.json`（変数展開）                  | [テンプレートエンジン仕様](../engine/テンプレートエンジン仕様.md) | テンプレート変数一覧・フィルタ               |
| `tests/testutil/setup.ts`                    | [コーディング規約](../../architecture/conventions/コーディング規約.md)            | Vitest セットアップ                          |
| `tests/App.test.tsx`                         | [コーディング規約](../../architecture/conventions/コーディング規約.md)            | コンポーネントテスト（Testing Library）      |
...
`CLI/templates/client/react/tsconfig.node.json.tera`

Vite 設定ファイル（`vite.config.ts`）用の TypeScript 設定。`tsconfig.json` の `references` から参照される。`composite: true` により project references として機能し、`allowSyntheticDefaultImports` で Vite プラグインの default import を許容する。
