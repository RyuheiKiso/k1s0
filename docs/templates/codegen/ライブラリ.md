...
ライブラリは **system** と **business** の2階層にのみ配置する。**service tier にはライブラリを置かない**。
...
`{lang}` は言語識別子（`go` / `rust` / `typescript` / `dart` ）、`{domain}` は業務領域名（例: `taskmanagement`, `fa`）を表す。
...
- [tier-architecture](../../architecture/overview/tier-architecture.md) --- Tier 依存ルール
