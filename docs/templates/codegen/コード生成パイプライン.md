...
```
┌──────────────────┐
│ テンプレート生成   │  Tera エンジンで CLI/crates/k1s0-cli/templates/ からひな形を出力
...
パイプラインの実装は `CLI/crates/k1s0-core/src/commands/generate/` 配下の以下のモジュールに対応する。

| モジュール | 関数 | 役割 |
|---|---|---|
| `execute.rs` | `execute_generate_with_config()` | 生成全体のオーケストレーション |
| `template.rs` | `render_scaffold_preview()` | テンプレートエンジンによる生成 |
| `post_process.rs` | `run_post_processing()` | 後処理コマンドの逐次実行 |
| `post_process.rs` | `determine_post_commands()` | kind × language × api_styles から実行コマンドリストを決定 |
| `context.rs` | `build_template_context()` | GenerateConfig → TemplateContext 変換 |
| `paths.rs` | `build_output_path()` | 出力先パス構築 |
| `conflict.rs` | `ensure_generate_targets_available()` | 競合検出・バリデーション |
| `infra_gen.rs` | `generate_helm_chart()` / `generate_cicd_workflows()` | Helm Chart・CI/CD 生成 |
...
`gqlgen.yml.tera` から生成された設定ファイルと `api/graphql/schema.graphql.tera` から生成された GraphQL スキーマを入力として、リゾルバーのスケルトンコードを生成する。`post_process.rs` の `determine_post_commands()` に実装済み。
