...
生成されるファイル:
- `infra/service-mesh/task-server/virtual-service.yaml` -- timeout=15s, retry=2, HTTP ルートのみ
- `infra/service-mesh/task-server/destination-rule.yaml` -- maxConnections=100, consecutive5xxErrors=5, baseEjectionTime=60s
- `infra/service-mesh/task-server/peer-authentication.yaml` -- STRICT
- `infra/service-mesh/task-server/authorization-policy.yaml` -- ingress / service からのアクセス許可
...
生成されるファイル:
- `infra/service-mesh/task-bff/virtual-service.yaml` -- timeout=15s, retry=2, HTTP + gRPC ルート
- `infra/service-mesh/task-bff/destination-rule.yaml` -- maxConnections=100, h2UpgradePolicy=UPGRADE
- `infra/service-mesh/task-bff/peer-authentication.yaml` -- STRICT + gRPC ポート portLevelMtls
- `infra/service-mesh/task-bff/authorization-policy.yaml` -- ingress / service からのアクセス許可 + BFF 間通信禁止 DENY ポリシー
...
```json
{
  "service_name": "taskmanagement-api",
  "tier": "business",
...
生成されるファイル:
- `infra/service-mesh/taskmanagement-api/virtual-service.yaml` -- timeout=10s, retry=3
- `infra/service-mesh/taskmanagement-api/destination-rule.yaml` -- maxConnections=100, consecutive5xxErrors=5, baseEjectionTime=30s
- `infra/service-mesh/taskmanagement-api/peer-authentication.yaml` -- STRICT
- `infra/service-mesh/taskmanagement-api/authorization-policy.yaml` -- service / business からのアクセス許可
