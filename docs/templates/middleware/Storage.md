...
k1s0 CLI ひな形生成のStorageテンプレート仕様。データベースを使用するサービスに対して、`tier` に応じたストレージサイズの PVC と、Ceph RBD ベースの StorageClass を自動生成する。

ストレージ設計の全体像は [インフラ設計](../../infrastructure/overview/インフラ設計.md) を、Terraform による StorageClass 管理は [terraform設計](../../infrastructure/terraform/terraform設計.md) を参照。
...
Storage テンプレートで使用する変数を以下に示す。変数の定義と導出ルールの詳細は [テンプレートエンジン仕様](../engine/テンプレートエンジン仕様.md) を参照。
...
生成されるファイル:
- `infra/storage/taskmanagement-api/storage-class.yaml` -- rbd.csi.ceph.com、reclaimPolicy=Retain
- `infra/storage/taskmanagement-api/pvc.yaml` -- 20Gi、storageClassName=ceph-block、database-type=mysql
...
- [インフラ設計](../../infrastructure/overview/インフラ設計.md) -- オンプレミスインフラ構成・Ceph ストレージ設計
- [terraform設計](../../infrastructure/terraform/terraform設計.md) -- Terraform kubernetes-storage モジュール・StorageClass 管理
- [テンプレート仕様-Helm](../infrastructure/Helm.md) -- Helm テンプレート仕様（PVC マウント設定連携）
- [テンプレート仕様-Config](../data/Config.md) -- Config テンプレート仕様（データベース接続設定連携）
- [テンプレート仕様-Observability](../observability/Observability.md) -- Observability テンプレート仕様
- [kubernetes設計](../../infrastructure/kubernetes/kubernetes設計.md) -- Namespace・StorageClass 設計
