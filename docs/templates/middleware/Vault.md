...
k1s0 CLI ひな形生成のVaultテンプレート仕様。SecretProviderClass（CSI Secrets Store Driver によるシークレット注入）、Vault Policy（サービス別最小権限ポリシー）、Vault Kubernetes Auth Config（認証ロール設定）を、サービスの `tier` と有効な機能フラグに応じて自動生成する。
