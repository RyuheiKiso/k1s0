...
生成されるファイル:
- `infra/vault/task-server/vault-agent-annotations.yaml` -- API キー + DB パスワード（2種のシークレット）
- `infra/vault/task-server/vault-policy.hcl` -- API キー + DB（静的・動的）の read 権限のみ
- `infra/vault/task-server/vault-auth.yaml` -- ServiceAccount `task-server`、Namespace `k1s0-service`
