...
Kafka テンプレートで使用する変数を以下に示す。変数の定義と導出ルールの詳細は [テンプレートエンジン仕様](../engine/テンプレートエンジン仕様.md) を参照。
...
- **トピック命名**: `k1s0.{tier}.{domain}.events.v1` の規則に従う（[メッセージング設計](../../architecture/messaging/メッセージング設計.md) の D-119 準拠）
...
- **Subject 命名**: `{topic-name}-value` の規則に従う（[メッセージング設計](../../architecture/messaging/メッセージング設計.md) の Schema Registry セクション準拠）
...
生成されるファイル:
- `infra/kafka/task-server/kafka-topic.yaml` -- トピック `k1s0.service.task.events.v1`、パーティション3、レプリカ1、min.insync.replicas=1
- `infra/kafka/task-server/schema-registry-subject.yaml` -- Subject `k1s0.service.task.events.v1-value`、互換性 BACKWARD
