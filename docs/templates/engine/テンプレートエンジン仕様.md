...
| 変数名 | 型 | 説明 | 例 |
|---|---|---|---|
| `service_name` | String | サービス名（kebab-case） | `task-api` |
| `service_name_snake` | String | サービス名（snake_case） | `task_api` |
| `service_name_pascal` | String | サービス名（PascalCase） | `TaskApi` |
| `service_name_camel` | String | サービス名（camelCase） | `taskApi` |
| `tier` | String | 配置階層 | `system` / `business` / `service` |
| `domain` | String | 業務領域名（business Tier 時） | `taskmanagement` / `fa` / `hr` |
| `module_path` | String | regions/ からの相対パス | `regions/service/task/server/rust` |
| `language` | String | 言語識別子 | `rust` / `typescript` / `dart` |
...
| `go_module` | String | Go モジュール名（Go 時） | `github.com/org/k1s0/regions/service/task/server/go` |
| `rust_crate` | String | Rust クレート名（Rust 時） | `task-api` |
...
| `helm_path` | String | Helm Chart の Tier 別相対パス | `service/task` / `business/taskmanagement/ledger` |
| `namespace` | String | Kubernetes Namespace（`k1s0-{tier}` 形式） | `k1s0-system` / `k1s0-business` / `k1s0-service` |
| `environment` | String | デプロイ先環境 | `dev` / `staging` / `prod` |
| `server_port` | Number | サーバーのリッスンポート番号（server/bff 時） | `80` / `3000` / `8080` |
...
|---|---|---|---|
| `task-api` | `task_api` | `TaskApi` | `taskApi` |
...
| tier | domain | 説明 |
|---|---|---|
| `system` | （空文字列） | system tier にドメイン概念はない |
| `business` | `taskmanagement` / `fa` / `hr` 等 | 業務領域ごとにディレクトリを分離 |
| `service` | （空文字列） | service tier はサービス名で直接分離 |

> **重要:** business Tier では `domain` が**必須**である。`TemplateContextBuilder` の `validate()` および `try_build()` は business tier で `domain` が空文字列の場合にエラーを返す。`GenerateConfig` から `TemplateContext` を構築する際は、`config.placement` の値を `builder.domain()` に設定すること。

### module_path の構成ルール

`module_path` は **Tier ごとに異なるルール** で構成される。各 Tier の配置パスは [テンプレート仕様-サーバー](../server/サーバー.md)・[テンプレート仕様-クライアント](../client/クライアント.md)・[テンプレート仕様-ライブラリ](../codegen/ライブラリ.md) の Tier 別配置パスに準拠する。
...
|---|---|---|---|---|---|
| `service` | — | `task` | `server` | `go` | `regions/service/task/server/go` |
| `service` | — | `task` | `server` | `rust` | `regions/service/task/server/rust` |
| `system` | — | `auth` | `library` | `rust` | `regions/system/library/rust/auth` |
| `business` | `taskmanagement` | `ledger-api` | `server` | `go` | `regions/business/taskmanagement/server/go/ledger-api` |
| `business` | `taskmanagement` | `ledger-api` | `server` | `rust` | `regions/business/taskmanagement/server/rust/ledger-api` |
...
`namespace` は `tier` の値を用いて以下のように構成される。Kubernetes の Namespace 名として使用する。
...
`environment` はデプロイ先環境を表す変数であり、CLI の対話フローまたは CI/CD パイプラインのコンテキストから設定される。インフラリソーステンプレート（Kafka、Consul 等）で環境別の設定値を切り替える際に使用する。
...
`server_port` は **server** / **bff** kind のみで使用される変数であり、サーバーのリッスンポート番号を表す。CLI の対話フローで入力されるか、デフォルト値 `80` が適用される。

| kind | server_port | 説明 |
|---|---|---|
| `server` | `80`（デフォルト） | HTTP サーバーポート |
| `bff` | `3000`（デフォルト） | BFF サーバーポート |
| `client` | （未使用） | — |
| `library` | （未使用） | — |
| `database` | （未使用） | — |
...
| `business` | `taskmanagement` | `ledger` | `business/taskmanagement/ledger` |

### go_module の構成ルール

`go_module` は **Go** 言語の場合のみ使用される変数であり、`go.mod` に記載する Go モジュール名を表す。`module_path` を用いて以下のように構成される。
...
|---|---|
| `regions/service/task/server/go` | `github.com/org/k1s0/regions/service/task/server/go` |
| `regions/system/server/rust/auth` | `github.com/org/k1s0/regions/system/server/rust/auth` |
| `regions/business/taskmanagement/server/go/ledger-api` | `github.com/org/k1s0/regions/business/taskmanagement/server/go/ledger-api` |
...
> **Note**: `api_style`（単数形）は後方互換のため `api_styles` の先頭要素が設定されるが、新規テンプレートでは `api_styles is containing()` を使用すること。
...
{% if api_styles is containing("rest") and language == "go" %}
// gin を用いた REST ハンドラ
{% elif api_styles is containing("rest") and language == "rust" %}
// axum を用いた REST ハンドラ
{% endif %}
```
...
| フィルタ | 説明 | 入力例 | 出力例 |
|---|---|---|---|
| `snake_case` | snake_case に変換 | `task-api` | `task_api` |
| `pascal_case` | PascalCase に変換 | `task-api` | `TaskApi` |
| `camel_case` | camelCase に変換 | `task-api` | `taskApi` |
| `kebab_case` | kebab-case に変換 | `task_api` | `task-api` |
| `upper_case` | 全て大文字に変換 | `task-api` | `TASK-API` |
| `lower_case` | 全て小文字に変換 | `TaskApi` | `taskapi` |
...
ネットワーク依存のコマンド（`go mod tidy`、`npm install` 等）は、一時的なエラー時に最大3回の指数バックオフリトライを行う。リトライの詳細仕様は [テンプレート仕様-コード生成パイプライン](../codegen/コード生成パイプライン.md) の「リトライ機構」セクションを参照。

### 後処理の失敗時

後処理コマンドが失敗した場合（リトライ後も含む）、CLI は以下の動作を行う。
...
- テンプレートファイルは `.tera` 拡張子を使用する（例: `Cargo.toml.tera`, `main.rs.tera`）
...
1. テストコンテキスト（JSON）を作成する
    ...
    cat > test-context.json << 'EOF'
    {
      ...
      "service_name_snake": "task_api",
      "service_name_pascal": "TaskApi",
      "service_name_camel": "taskApi",
      "tier": "service",
      "domain": "",
      "module_path": "regions/service/task-api/server/rust",
      "language": "rust",
      "kind": "server",
      "api_style": "rest",
      "api_styles": ["rest"],
...
全テンプレート種別で共通して「生成しない」対象:

| 種別 | 生成対象外 | 理由 |
| --- | --- | --- |
| client | BFF・Flutter・React テンプレートでは server/library/database を生成しない | クライアント専用のため |
| library | server/client/database を生成しない | ライブラリ専用のため |
| database | server/client/library を生成しない | データベース専用のため |

各テンプレート仕様書の生成対象表では上記の「生成しない」行を省略し、**生成される対象のみ**を記載する。

### 共通変数 {#共通変数}

全テンプレートで利用可能な共通変数。各テンプレート仕様書では以下を「エンジン仕様参照」として省略できる。

| 変数 | 説明 |
| --- | --- |
| `service_name` | サービス名（kebab-case） |
| `service_name_snake` / `service_name_pascal` / `service_name_camel` | ケース変換済みサービス名 |
| `tier` | 配置階層（system / business / service） |
| `namespace` | Kubernetes Namespace（`k1s0-{tier}` 形式） |
| `domain` | 業務領域名（business Tier 時） |
| `module_path` | regions/ からの相対パス |
| `language` | 言語識別子（rust / go / typescript / dart） |
| `kind` | 種別（server / client / library / database） |
| `environment` | デプロイ先環境（dev / staging / prod） |

詳細な変数一覧とケース変換ルールは [テンプレート変数一覧](#テンプレート変数一覧) を参照。
...
- [ディレクトリ構成図](../../architecture/overview/ディレクトリ構成図.md) --- 生成先のディレクトリ構成
- [テンプレート仕様-サーバー](../server/サーバー.md) --- サーバーテンプレートの詳細
- [テンプレート仕様-クライアント](../client/クライアント.md) --- クライアントテンプレートの詳細
- [テンプレート仕様-ライブラリ](../codegen/ライブラリ.md) --- ライブラリテンプレートの詳細
- [テンプレート仕様-データベース](../data/データベース.md) --- データベーステンプレートの詳細
- [テンプレート仕様-コード生成パイプライン](../codegen/コード生成パイプライン.md) --- 後処理パイプラインの詳細
- [テンプレート仕様-レンダリングテスト](../testing/レンダリングテスト.md) --- テンプレートレンダリングテストの仕様
- [テンプレート仕様-Helm](../infrastructure/Helm.md) --- Helm Chart テンプレートの詳細
- [テンプレート仕様-CICD](../infrastructure/CICD.md) --- CI/CD ワークフローテンプレートの詳細
- [テンプレート仕様-Observability](../observability/Observability.md) --- Observability テンプレートの詳細
- [テンプレート仕様-Grafana](../observability/Grafana.md) --- Grafana ダッシュボードテンプレートの詳細
- [テンプレート仕様-OpenTelemetry](../observability/OpenTelemetry.md) --- OpenTelemetry Collector テンプレートの詳細
- [テンプレート仕様-Loki](../observability/Loki.md) --- Loki/Promtail テンプレートの詳細
- [テンプレート仕様-Alertmanager](../observability/Alertmanager.md) --- Alertmanager テンプレートの詳細
- [テンプレート仕様-Kafka](../middleware/Kafka.md) --- Kafka メッセージングテンプレートの詳細
- [テンプレート仕様-Vault](../middleware/Vault.md) --- Vault シークレット管理テンプレートの詳細
- [テンプレート仕様-Flagger](../middleware/Flagger.md) --- Flagger カナリアリリーステンプレートの詳細
- [テンプレート仕様-Consul](../middleware/Consul.md) --- Consul テンプレートの詳細
- [テンプレート仕様-Storage](../middleware/Storage.md) --- Storage テンプレートの詳細
- [テンプレート仕様-ServiceMesh](../middleware/ServiceMesh.md) --- ServiceMesh テンプレートの詳細
- [コンセプト](../../architecture/overview/コンセプト.md) --- プロジェクトの目的・技術スタック
