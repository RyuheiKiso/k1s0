...
> **NOTE**: 既存のサービス CI/Deploy ワークフローは reusable workflow（`_rust-service-ci.yaml`, `_go-service-ci.yaml`, `_service-deploy.yaml`）を呼び出す薄いラッパーに統合されている。CLI テンプレートで生成される新規ワークフローも同じ reusable workflow パターンに従うこと。
...
CI/CD テンプレートで使用する変数を以下に示す。変数の定義と導出ルールの詳細は [テンプレートエンジン仕様](../engine/テンプレートエンジン仕様.md) を参照。
...
タグ規則の詳細は [Dockerイメージ戦略](../../infrastructure/docker/Dockerイメージ戦略.md) を参照。
...
Helm Chart の詳細は [helm設計](../../infrastructure/kubernetes/helm設計.md) を参照。

---

## 言語バージョン

CI/CD ワークフローで使用する言語・ツールのバージョンを以下に示す。バージョンは [CI-CD設計](../../infrastructure/cicd/CI-CD設計.md) および [devcontainer設計](../../infrastructure/devenv/devcontainer設計.md) と同期する。

| 言語/ツール | バージョン | セットアップ Action              |
| ----------- | ---------- | -------------------------------- |
| Go          | 1.24       | `actions/setup-go@v5`            |
| Rust        | 1.93       | `dtolnay/rust-toolchain@1.93`    |
...
```json
{
  ...
  "module_path": "regions/service/task/server/go",
  "language": "go",
  "kind": "server",
  "tier": "service",
  "api_styles": ["rest"],
...
生成されるファイル:
- `.github/workflows/task-api-ci.yaml` — lint (golangci-lint) → migration-test → test → build → security-scan
- `.github/workflows/task-api-deploy.yaml` — build-and-push → deploy-dev → deploy-staging → deploy-prod
...
```json
{
  "service_name": "task-bff",
  "module_path": "regions/service/task/bff/go",
  "language": "go",
  "kind": "bff",
  "tier": "service",
  "api_styles": ["graphql"],
...
生成されるファイル:
- `.github/workflows/task-bff-ci.yaml` — lint (golangci-lint) → test → build → security-scan
- `.github/workflows/task-bff-deploy.yaml` — build-and-push (Trivy image scan + SBOM 生成含む) → deploy-dev → deploy-staging → deploy-prod
...
```json
{
  ...
  "module_path": "regions/business/taskmanagement/client/react/ledger-app",
  "language": "typescript",
  "kind": "client",
  "framework": "react"
}
...
- [テンプレート仕様-データベース](../data/データベース.md) — データベーステンプレート仕様
