...
k1s0 CLI ひな形生成のHelm Chartテンプレート仕様。Helm Chart は Kubernetes 上へのデプロイに必要なマニフェスト一式（Deployment, Service, ConfigMap, HPA, PDB）と環境別 values ファイルを自動生成する。
...
- [認証認可設計](../../architecture/auth/認証認可設計.md) --- Vault Agent Injector・シークレット管理
- [CI-CD設計](../../infrastructure/cicd/CI-CD設計.md) --- デプロイパイプラインとの連携

## Istio サービスメッシュ統合

`k1s0-common` Library Chart は Istio サービスメッシュとの統合をサポートする。
消費側チャートは `istio.enabled: true` を設定するだけで、サイドカーインジェクションと Istio リソースを有効化できる。

### values.yaml 設定

```yaml
istio:
  enabled: true                        # Istio 統合を有効化
  sidecar:
    inject: true                       # sidecar.istio.io/inject アノテーション
    proxyConfig:
      holdApplicationUntilProxyStarts: true
  timeout: 30s                         # デフォルトタイムアウト
  virtualService:
    enabled: true                      # VirtualService を有効化
  ...
  destinationRule:
    enabled: true                      # DestinationRule を有効化
    circuitBreaker:
...
### Library Chart が提供するヘルパー

| ヘルパー名 | 説明 | 呼び出し方 |
|-----------|------|----------|
| `k1s0-common.istioAnnotations` | Pod に Istio アノテーションを注入（`_deployment.tpl` が自動 include） | 通常は直接呼ばない |
| `k1s0-common.virtualService` | VirtualService リソースを生成 | `istioResources` 経由 |
| `k1s0-common.destinationRule` | DestinationRule リソースを生成 | `istioResources` 経由 |
| `k1s0-common.istioResources` | VirtualService + DestinationRule をまとめて生成 | 消費側チャートで使用 |

> `k1s0-common.istioAnnotations` は `_deployment.tpl` の `template.metadata.annotations` に自動的に include されるため、`istio.enabled: true` にするだけで Pod へのアノテーション注入が有効化される。

### 消費側チャートでの VirtualService / DestinationRule 有効化

消費側チャートに `templates/istio.yaml` を追加し、`k1s0-common.istioResources` を呼び出す:
...
生成される manifest の例（両方 enabled の場合）:
...
`istio.enabled: true` にすることで `_deployment.tpl` の Pod template に以下が追加される:

1. **annotations**: `sidecar.istio.io/inject: "true"` と `proxy.istio.io/config`（タイムアウト設定）
2. **labels**: `version: <image.tag>` — Istio のトラフィック分割（カナリアリリース）で使用

### CLI テンプレート変数

| テンプレート変数 | 条件 | 影響範囲 |
|---------------|------|---------|
| `has_service_mesh` | `true` | `istio.enabled: true` を values.yaml に設定し、`templates/istio.yaml` を生成 |
