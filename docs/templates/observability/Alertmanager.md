...
生成されるファイル:
- `infra/alertmanager/board-server/alertmanager-config.yaml` -- group_wait=30s、critical repeat=1h、warning repeat=4h、dev/staging 抑制ルール
...
生成されるファイル:
- `infra/alertmanager/task-bff/alertmanager-config.yaml` -- group_wait=30s、critical repeat=1h、warning repeat=4h、dev/staging 抑制ルール
