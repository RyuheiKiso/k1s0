...
```json
{
  ...
  "service_name_snake": "task_server",
  "tier": "service",
...
生成されるファイル:
- `infra/observability/task-server/servicemonitor.yaml` -- interval=30s、scrapeTimeout=15s
- `infra/observability/task-server/alerts.yaml` -- エラー率5%/warning、レイテンシ2s/info、Pod再起動5回/warning
