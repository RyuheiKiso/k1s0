...
生成されるファイル:
- `infra/loki/board-server/promtail-config.yaml` -- debug ドロップ（info 以上を収集）、namespace/app/tier/pod ラベル付与
...
生成されるファイル:
- `infra/loki/task-bff/promtail-config.yaml` -- debug ドロップ（info 以上を収集）、namespace/app/tier/pod ラベル付与
