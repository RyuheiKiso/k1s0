---
id: env.ops.platform_operator_docs_lint
axis: ops
phase: env_setup
kind: enforcement
status: draft
depends_on:
  - env.ops.platform_operator_lint_format
covered_by:
...
- プラットフォーム運営者は docs/ 配下のドキュメント（runbook を含む）に変更を加えた際、bash 版（run_lint.sh）と Python 版（run_lint.py）の両方を実行し、全 check が green になることを push 前の必須条件とする。
...
| 8/8 | repository layout 検査（root allowlist） |

## ops 軸固有の FAIL パターン

| FAIL メッセージ | 原因 | 対処 |
|---|---|---|
| `id prefix 'ops' != expected 'env'` | id が `ops.` で始まっている | `env.ops.<slug>` に修正する |
| `depends_on 'env.ops.platform_operator_xxx' 解決不能` | 依存先ファイルが未作成 | 依存先ファイルを先に作成する |
