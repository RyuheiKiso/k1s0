---
id: env.tier2.tier2_docs_lint
axis: tier2
phase: env_setup
kind: enforcement
status: draft
depends_on:
  - env.tier2.tier2_lint_format
covered_by:
...
| `id prefix 'xxx' != expected 'env'` | `05_環境構築/` 配下で id が `env.` で始まっていない | id を `env.tier2.<slug>` に修正 |
| `depends_on 'xxx' 解決不能` | 依存先 id が存在しない | 依存先ファイルを先に作成するか depends_on から削除 |
| `dangling link: xxx.md` | body 中のリンクが存在しないファイルを指している | リンク先を修正 |
