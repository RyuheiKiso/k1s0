---
id: env.data.data_docs_lint
axis: data
phase: env_setup
kind: enforcement
status: draft
depends_on:
  - env.data.data_lint_format
covered_by:
...
- bash 版（run_lint.sh）と Python 版（run_lint.py）の 2 スクリプトを両方実行し、それぞれ全 check が green になることを data 軸ドキュメントの push 前条件とする。
...
data 軸のドキュメントは `docs/05_環境構築/05_data/` 配下に配置する。id は `env.data.<slug>` パターンで統一する。SQL ファイル（`.sql`）/ YAML ファイル（`.yaml`）へのリンクは lint の body link check 対象外のため安全。

```bash
# data 軸のみの FAIL を確認
python3 tools/docs_lint/run_lint.py 2>&1 | grep -E "FAIL.*(05_data|env\.data)"
```
...
| `id prefix 'xxx' != expected 'env'` | 05_環境構築/ 配下で id が `env.` で始まっていない | id を `env.data.<slug>` に修正 |
