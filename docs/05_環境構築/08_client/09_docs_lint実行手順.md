---
id: env.client.client_docs_lint
axis: client
phase: env_setup
kind: enforcement
status: draft
depends_on:
  - env.client.client_lint_format
covered_by:
...
## client 軸固有の FAIL と対処

| FAIL メッセージ | 原因 | 対処 |
|---|---|---|
| `id prefix 'client' != expected 'env'` | 08_client/ 配下で id が `env.client.` で始まっていない | id を `env.client.client_<slug>` に修正 |
