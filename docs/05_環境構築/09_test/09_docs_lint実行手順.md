---
id: env.test.test_docs_lint
axis: test
phase: env_setup
kind: enforcement
status: draft
depends_on:
  - env.test.test_lint_format
covered_by:
...
| FAIL メッセージ | 原因 | 対処 |
|---|---|---|
| `id prefix 'test' != expected 'env'` | 09_test/ 配下で id が `env.test.` で始まっていない | id を `env.test.test_<slug>` に修正 |
| `dangling link: 07_テスト検証環境.md` | リンク先ファイルが存在しない | ファイルを作成するかリンクを修正 |
| `depends_on 'env.test.test_xxx' 解決不能` | 依存先 id が存在しない | 依存先ファイルを先に作成する |
