...
本ファイルは構想設計書 [../../02_構想設計/adr/](../../02_構想設計/adr/) で採択された 47 個の ADR（Architecture Decision Record）を、概要設計書から参照する際の一覧索引である。各 ADR の決定内容要約・却下された代替案・影響章・改訂履歴を付加情報として記録し、概要設計書からの逆引きと、構想設計改訂時の波及把握の双方に耐える索引として機能させる。
...
ADR を技術領域で 15 グループ（基本構造 / ディレクトリ構造 / データ層 / セキュリティ / ルールエンジン / CI/CD・リリース・ポリシー / 観測性 / Feature Flag / 開発者体験 / 依存管理・サプライチェーン / ストレージ・移行・ポータル / Kubernetes 基盤・ネットワーク / 分散ランタイム・スケール / tier3 アーキテクチャ / 運用ライフサイクル）に分類する。同じ領域の ADR は互いに関連性が強く、相互参照の頻度が高い。k1s0 リリース前までの集約時点で 47 件に到達しており、内訳は基本構造系が骨格を成し、周辺領域がそれを支える形で配置される。
...
| ADR-POL-002 | ローカル kind cluster の構成 SoT を tools/local-stack/up.sh に統一 | up.sh apply_* と argocd Application 経由でのみ helm release を許可。Kyverno block-non-canonical-helm-releases (runtime) + CI drift-check (PR) + `--mode {dev,strict}` (運用境界) の三層防御で drift を構造的に阻止 | SoT なし継続、Kyverno のみ、CI のみ、完全 argocd 一本化、docs 表現のみ | 70_DEVX/01, 50_NFR/05, 55_OPS | v1.0 2026-04-30 |
...
### Kubernetes 基盤・ネットワーク ADR（3 件）

vanilla K8s 上のクラスタ ブートストラップ、互換性維持、データプレーン CNI を確定する領域。tier1〜tier3 すべての前提となる土台 ADR 群。

| ADR 番号 | タイトル | 決定内容要約 | 却下された代替案 | 影響章 | 改訂履歴 |
|---|---|---|---|---|---|
| ADR-INFRA-001 | Kubernetes クラスタを kubeadm + Cluster API で構築 | Cluster API + kubeadm（KubeadmControlPlane）を標準、HA 3 control-plane、kubeadm 直接実行を小規模オンプレ向けに併存維持 | kubeadm 直接単独、Rancher RKE2、Talos Linux、マネージド K8s（EKS/GKE/AKS） | 10_SYS/01, 50_NFR/01, 55_OPS | v1.0 2026-05-02 |
| ADR-CNCF-001 | vanilla Kubernetes（CNCF Conformance 互換）を維持 | sonobuoy で Conformance テスト pass 状態を維持、独自 admission / API server 改造 / upstream-incompatible distribution は不採用 | OpenShift / RKE2 派生、Talos / k0s 系特化、独自 K8s fork | 10_SYS/01, 70_DEVX/01 | v1.0 2026-05-02 |
| ADR-NET-001 | production CNI に Cilium、kind 検証用に Calico を使い分け | production = Cilium 1.16+（eBPF dataplane）、kind multi-node = Calico v3.29+、kind single-node = kindnet | 全環境 Cilium 統一、全環境 Calico 統一、Weave Net、Flannel | 10_SYS/03, 50_NFR/05 | v1.0 2026-05-02 |

### 分散ランタイム・スケール ADR（2 件）

tier1 ファサード層の分散ランタイムと event 駆動スケールを束ねる領域。アプリ層の建付けを規定する。

| ADR 番号 | タイトル | 決定内容要約 | 却下された代替案 | 影響章 | 改訂履歴 |
|---|---|---|---|---|---|
| ADR-DAPR-001 | 分散ランタイムに Dapr Operator を採用 | Dapr 1.17 LTS、control-plane HA 3 replica（operator/placement/sentry/sidecar-injector/scheduler）、mTLS 必須、Component CRD で backend 切替 | 自作 abstraction、Spring Cloud + Spring Cloud Stream、Knative Eventing + Serving、Service Mesh sidecar 直接 | 20_SW/01, 03, 50_NFR/05 | v1.0 2026-05-02 |
| ADR-SCALE-001 | Event-driven autoscaling に KEDA を採用 | KEDA 2.16 LTS、operator HA + metrics-apiserver HA + admission webhook、HPA を内部生成、scale-from-zero 対応 | HPA 単独、Knative Serving、自作 controller、クラウド事業者 autoscaler | 20_SW/04, 50_NFR/02, 55_OPS | v1.0 2026-05-02 |

### tier3 アーキテクチャ ADR（3 件）

Web SPA / Native アプリの集約戦略と実装方式を確定する領域。client 多様性と採用組織の人材プールを橋渡しする。

| ADR 番号 | タイトル | 決定内容要約 | 却下された代替案 | 影響章 | 改訂履歴 |
|---|---|---|---|---|---|
| ADR-TIER3-001 | tier3 client ごとに専用 BFF を配置 | Web SPA は Go BFF（GraphQL + REST）、Native は SDK + tier1 直接、Legacy wrap は .NET サイドカー BFF | 共通 API Gateway、GraphQL Federation、client から tier1 直接、Next.js API Routes | 20_SW/02, 30_CF/01, 50_NFR/05 | v1.0 2026-05-02 |
| ADR-TIER3-002 | tier3 Web を React + Vite SPA + Go BFF で構成 | React 18+ + TypeScript + Vite + pnpm workspace、SSR 不採用（docs-site のみ Astro pre-rendering を将来検討） | Next.js SSR、Remix SSR、Vue + Vite、Svelte/SvelteKit、Astro、MPA | 20_SW/02, 70_DEVX/04 | v1.0 2026-05-02 |
| ADR-TIER3-003 | tier3 Native アプリに .NET MAUI を採用 | .NET MAUI 8 LTS、Android / iOS / macOS Catalyst / Windows の 4 OS 対応、tier2 .NET SDK を ProjectReference 共有 | Flutter、React Native、Tauri Mobile、各 OS Native（Swift / Kotlin / WinUI）、Web View ラッパー、Electron | 20_SW/02, 70_DEVX/04 | v1.0 2026-05-02 |

### 運用ライフサイクル ADR（1 件）

夜間休日 SEV1 を起案者不在で対応するためのプロセス資産を確定する領域。Runbook の形式・粒度・検証を構造化することで属人性を排除し、バス係数 2 の構造的担保を実現する。

| ADR 番号 | タイトル | 決定内容要約 | 却下された代替案 | 影響章 | 改訂履歴 |
|---|---|---|---|---|---|
| ADR-OPS-001 | Runbook 標準化（8 セクション + YAML frontmatter + Chaos Drill） | Markdown + Backstage TechDocs + 8 セクション順序固定 + YAML frontmatter（runbook_id 等 10 項目）+ 3 分粒度 / 1 時間制約 + 四半期 Chaos Drill 検証、Alertmanager `runbook_url` 必須、品質指標 4 種を Grafana 月次計測 | Confluence/Notion SaaS Wiki、自由形式（起案者判断）、ITIL v4 準拠の重厚 Runbook | 55_OPS/08, 09, 50_NFR/04 | v1.0 2026-05-02 |
...
- ADR-OPS-001 Runbook 標準化（8 セクション + YAML frontmatter + Chaos Drill）
...
| 2026-04-30 | ADR-POL-002 | ローカル kind cluster の構成 SoT を tools/local-stack/up.sh に統一 |
| 2026-05-02 | ADR-INFRA-001 | Kubernetes クラスタを kubeadm + Cluster API で構築 |
| 2026-05-02 | ADR-CNCF-001 | vanilla Kubernetes（CNCF Conformance 互換）を維持 |
| 2026-05-02 | ADR-NET-001 | production CNI に Cilium、kind 検証用に Calico を使い分け |
| 2026-05-02 | ADR-DAPR-001 | 分散ランタイムに Dapr Operator を採用 |
| 2026-05-02 | ADR-SCALE-001 | Event-driven autoscaling に KEDA を採用 |
| 2026-05-02 | ADR-TIER3-001 | tier3 client ごとに専用 BFF を配置 |
| 2026-05-02 | ADR-TIER3-002 | tier3 Web を React + Vite SPA + Go BFF で構成 |
| 2026-05-02 | ADR-TIER3-003 | tier3 Native アプリに .NET MAUI を採用 |
| 2026-05-02 | ADR-OPS-001 | Runbook 標準化（8 セクション + YAML frontmatter + Chaos Drill） |
