...
ADR を技術領域で 11 グループ（基本構造 / ディレクトリ構造 / データ層 / セキュリティ / ルールエンジン / CI/CD・リリース・ポリシー / 観測性 / Feature Flag / 開発者体験 / 依存管理・サプライチェーン / ストレージ・移行・ポータル）に分類する。同じ領域の ADR は互いに関連性が強く、相互参照の頻度が高い。Phase 0 稟議承認前までの集約時点で 36 件に到達しており、内訳は基本構造系が骨格を成し、周辺領域がそれを支える形で配置される。
...
モノレポ直下の物理配置を確定する ADR 群。実装コードがまだ書かれていない Phase 0 段階で昇格・分離を決め、実装開始後の移動コストを排除することを狙う。契約・インフラ・運用の責務境界を `src/contracts/` / `infra/` / `deploy/` / `ops/` として階層化し、スパースチェックアウトで役割別ワーキングセットを最小化する。
...
| ADR-REL-001 | Progressive Delivery を Phase 0 から全リリース必須化 | Argo Rollouts による Progressive Delivery を Phase 0 から全リリースで必須、例外は SRE 承認制（emergency patch / 内部ツール / バッチのみ）。共通 AnalysisTemplate セット + flagd 定義ファイル cosign 署名強制 | tier1 公開 API のみ PD、任意採用（推奨レベル）、Blue-Green 固定 | 70_DEVX/01, 50_NFR/01, 30_CF/09 | v1.0 2026-04-24 |
...
開発者体験（DX）の思想と計測基盤を定義する ADR 群。Paved Road（舗装道路）の採用で Golden Path を一本化し、time-to-first-commit や Scaffold 利用率など DX メトリクスを稼働系 SLI とは別経路で管理する。Platform/SRE の支援コストを有限化しつつ、EM レイヤーが独立に DX 健全性を判断できる構造を与える。
...
依存更新とビルドサプライチェーンの信頼性を担保する ADR 群。Renovate を依存更新の中枢とし、SLSA v1.1 Build Track で Phase 0 に Level 2、Phase 1b に Level 3 へ段階到達する。cosign keyless・CycloneDX SBOM・Kyverno ImageVerify の 3 点で本番 namespace の改ざん耐性を構造的に担保する。
...
| ADR-SUP-001 | SLSA v1.1 を Phase 0 で Level 2、Phase 1b で Level 3 に段階到達 | SLSA v1.1 Build Track を Phase 0 で Level 2（cosign keyless + CycloneDX SBOM + Kyverno ImageVerify）、Phase 1b で Level 3（hermetic runner + Rekor 登録）に到達。`RB-SEC-005: Image Hash 逆引き Forensics` を整備 | Phase 0 で Level 3 一気到達、Level 1 のみ、SLSA 不採用（独自 provenance） | 30_CF/08, 50_NFR/05, 55_OPS/02 | v1.0 2026-04-24 |
...
- ADR-REL-001 Progressive Delivery を Phase 0 から全リリース必須化
...
| 2026-04-24 | ADR-REL-001 | Progressive Delivery を Phase 0 から全リリース必須化 |
| 2026-04-24 | ADR-SUP-001 | SLSA v1.1 を Phase 0 で Level 2、Phase 1b で Level 3 に段階到達 |
...
概要設計書を執筆する過程で、既存 36 ADR では決定理由を十分に固定化できない論点が 14 件浮上した。これらは構想設計段階で議論が行われた選定結果を概要設計で参照してしまっているか、概要設計で新たに確定させた方針がそのまま ADR 化に値するため、正式な ADR 起票前の「仮番」として本節に登録する。Phase 1a 開発着手前までに構想設計側で本体を起票し、本索引の領域別索引に移し替える運用とする。
