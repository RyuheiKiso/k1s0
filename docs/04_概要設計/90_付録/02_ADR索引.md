...
ADR を技術領域で 11 グループ（基本構造 / ディレクトリ構造 / データ層 / セキュリティ / ルールエンジン / CI/CD・リリース・ポリシー / 観測性 / Feature Flag / 開発者体験 / 依存管理・サプライチェーン / ストレージ・移行・ポータル）に分類する。同じ領域の ADR は互いに関連性が強く、相互参照の頻度が高い。k1s0 リリース前までの集約時点で 36 件に到達しており、内訳は基本構造系が骨格を成し、周辺領域がそれを支える形で配置される。
