...
**確定段階**: リリース時点。**対応要件**: ADR-TIER1-001、ADR-TIER1-002、NFR-E-NW-001〜004。
...
**確定段階**: リリース時点。**対応要件**: NFR-B-PERF-\*、NFR-D-TRACE-\*。
...
**確定段階**: リリース時点。**対応要件**: ADR-TIER1-002、NFR-E-ERR-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-H-INT-001、ADR-TIER1-002。
...
**確定段階**: リリース時点。**対応要件**: NFR-E-ERR-\*、DX-GP-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-E-ERR-\*、NFR-A-FT-001。
...
**確定段階**: リリース時点。**対応要件**: NFR-D-TRACE-\*、FR-T1-TELEMETRY-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-D-TRACE-\*、NFR-D-MON-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-D-TRACE-\*、DX-MET-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-A-FT-001、ADR-TIER1-001。
...
**確定段階**: リリース時点。**対応要件**: NFR-E-AC-001〜005、NFR-E-ENC-001、ADR-TIER1-001。**参照**: 構想設計 ADR-0001（Istio Ambient Mesh）。
...
**確定段階**: リリース時点。**対応要件**: NFR-A-CONT-003、NFR-A-FT-001、ADR-0001。
...
| 設計 ID | 内容 | 確定段階 |
|---|---|---|
| DS-SW-IIF-050 | Protobuf gRPC のみ許可（FFI/共有メモリ禁止） | リリース時点 |
| DS-SW-IIF-051 | gRPC-Web / gRPC-Gateway は未使用 | リリース時点 |
| DS-SW-IIF-052 | 基本型の言語マッピング | リリース時点 |
| DS-SW-IIF-053 | 時刻型の言語間変換 | リリース時点 |
| DS-SW-IIF-054 | Option / nil の言語間変換 | リリース時点 |
| DS-SW-IIF-055 | 列挙型の言語間対応 | リリース時点 |
| DS-SW-IIF-056 | map / repeated の順序保証 | リリース時点 |
| DS-SW-IIF-057 | エラーコードの Protobuf enum 統一 | リリース時点 |
| DS-SW-IIF-058 | エラー変換の wrap 規約 | リリース時点 |
| DS-SW-IIF-059 | panic / unwrap の禁止範囲 | リリース時点 |
| DS-SW-IIF-060 | W3C Trace Context propagation | リリース時点 |
| DS-SW-IIF-061 | baggage の伝搬 | リリース時点 |
| DS-SW-IIF-062 | span の命名規約 | リリース時点 |
| DS-SW-IIF-063 | Rust 側サーバ graceful shutdown | リリース時点 |
| DS-SW-IIF-064 | Go 側クライアント再接続戦略 | リリース時点 |
| DS-SW-IIF-065 | Rust 側 startup probe | リリース時点 |
| DS-SW-IIF-066 | Go 側 daprd 連携ライフサイクル | リリース時点 |
| DS-SW-IIF-067 | mTLS は Istio 側任せ | リリース時点 |
| DS-SW-IIF-068 | L7 authz は waypoint 委譲 | リリース時点 |
| DS-SW-IIF-069 | Circuit Breaker / Retry は Istio 側 | リリース時点 |
