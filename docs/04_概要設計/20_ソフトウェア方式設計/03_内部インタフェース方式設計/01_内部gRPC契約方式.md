...
内部 gRPC 契約の設計が緩いと、(a) Go ⇔ Rust の型不整合で runtime 失敗、(b) 破壊的変更が検知されずに deployment が壊れる、(c) deadline / trace が言語境界で欠落して SLO 違反の原因追跡が破綻、といった事象が起きる。これらは 運用蓄積後の運用を崩壊させるため、リリース時点 段階で契約方式を厳密に固定化することが必要である。
...
**確定段階**: リリース時点。**対応要件**: ADR-TIER1-002、NFR-B-PERF-004、NFR-B-PERF-006。
...
**確定段階**: リリース時点。**対応要件**: ADR-TIER1-002、NFR-C-NOP-002。

### DS-SW-IIF-003 buf CLI による破壊的変更検出

`buf breaking` を CI で必須実行し、既存フィールド削除・フィールド番号変更・enum 値削除・required → optional 変更などの破壊的変更を自動検出する。検出時は CI が fail し、PR マージ不可となる。`buf.yaml` では `breaking.use: [FILE]`（FILE レベル）で設定し、WIRE_JSON レベルでは追加検出（例: JSON tag 不整合）は 採用後の運用拡大時 で拡張する。`buf format` も CI で必須化し、フォーマット統一を強制する。

**確定段階**: リリース時点。**対応要件**: ADR-TIER1-002、DX-CICD-\*。
...
**確定段階**: リリース時点。**対応要件**: ADR-TIER1-002、DX-GP-\*。

### DS-SW-IIF-006 Unary / Streaming の使い分け

gRPC の 4 通信モードは以下の基準で使い分ける。Unary（単一 request / 単一 response）は 95% 以上の標準ケースで使用。Server Streaming（単一 request / 複数 response）は AUDIT の `VerifyChain`（長期間の検証 progress 返却）のように、結果が段階的に返る場合に使う。Client Streaming（複数 request / 単一 response）は AUDIT の監査イベント batch 受付のような高スループット書込で 採用後の運用拡大時に検討。Bidirectional（複数 request / 複数 response）は現在のユースケースでは使わず、将来的な Workflow のイベントプッシュで検討する。

**確定段階**: リリース時点（Unary / Server Streaming）、採用後の運用拡大時（他）。**対応要件**: ADR-TIER1-002、NFR-B-PERF-\*。
...
**確定段階**: リリース時点。**対応要件**: ADR-TIER1-002。
...
**確定段階**: リリース時点。**対応要件**: ADR-TIER1-002、NFR-B-PERF-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-E-ERR-\*、NFR-D-TRACE-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-E-ERR-\*、DX-RB-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-B-PERF-\*、NFR-A-CONT-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-D-TRACE-\*、NFR-E-AC-001〜005、FR-T1-LOG-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-D-TRACE-\*、NFR-B-PERF-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-E-AC-001〜005。
...
**確定段階**: リリース時点（ルール）、採用後の運用拡大時（v2 発生時）。**対応要件**: ADR-TIER1-002、NFR-C-NOP-002。
...
**確定段階**: リリース時点。**対応要件**: NFR-C-NOP-002、DX-CICD-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-D-MON-\*、NFR-D-TRACE-\*、NFR-E-AC-001〜005。
...
**確定段階**: リリース時点。**対応要件**: NFR-A-FT-001、NFR-E-NW-001〜004。
...
**確定段階**: リリース時点。**対応要件**: NFR-B-CAP-\*、NFR-B-PERF-\*。

### DS-SW-IIF-026 Concurrent stream 制限

gRPC サーバは HTTP/2 の concurrent stream 上限を 100 に設定する。これを超えるリクエストはクライアント側で queue される。Istio Ambient の waypoint では `max_concurrent_streams=500` を設定し、Pod 側（100）のほうが厳しく制限して Pod 保護を優先する。上限到達は Prometheus metric `k1s0_grpc_concurrent_streams` で監視し、HPA スケール trigger に使える（採用後の運用拡大時）。

**確定段階**: リリース時点。**対応要件**: NFR-B-WL-\*、NFR-B-CAP-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-A-CONT-003、NFR-B-PERF-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-A-FT-001、NFR-A-CONT-\*。
...
**確定段階**: 採用初期。**対応要件**: NFR-A-CONT-003、NFR-A-FT-001。
...
| 設計 ID | 内容 | 確定段階 |
|---|---|---|
| DS-SW-IIF-001 | HTTP/JSON 禁止・Protobuf gRPC 必須 | リリース時点 |
| DS-SW-IIF-002 | .proto ファイル一元管理 | リリース時点 |
| DS-SW-IIF-003 | buf CLI 破壊的変更検出 | リリース時点 |
| DS-SW-IIF-004 | サービス定義命名規約 | リリース時点 |
| DS-SW-IIF-005 | メソッド命名規約 | リリース時点 |
| DS-SW-IIF-006 | Unary / Streaming の使い分け | 採用初期/2 |
| DS-SW-IIF-007 | 基本型の利用規約 | リリース時点 |
| DS-SW-IIF-008 | フィールド番号の予約 | リリース時点 |
| DS-SW-IIF-009 | Enum 命名規約 | リリース時点 |
| DS-SW-IIF-010 | message のネスト深さ制限 | リリース時点 |
| DS-SW-IIF-011 | google.rpc.Status によるエラー返却 | リリース時点 |
| DS-SW-IIF-012 | エラー詳細の構造化（ErrorDetail） | リリース時点 |
| DS-SW-IIF-013 | エラーコードタクソノミ | リリース時点 |
| DS-SW-IIF-014 | gRPC deadline 必須設定 | リリース時点 |
| DS-SW-IIF-015 | deadline 伝搬方式 | リリース時点 |
| DS-SW-IIF-016 | deadline 残時間バッファ戦略 | リリース時点 |
| DS-SW-IIF-017 | 必須メタデータ項目 | リリース時点 |
| DS-SW-IIF-018 | metadata 言語境界伝搬 | リリース時点 |
| DS-SW-IIF-019 | metadata 未設定時の対応 | リリース時点 |
| DS-SW-IIF-020 | サービスバージョニング戦略 | リリース時点/2 |
| DS-SW-IIF-021 | 後方互換性維持基準 | リリース時点 |
| DS-SW-IIF-022 | 非推奨化の手順 | リリース時点 |
| DS-SW-IIF-023 | Interceptor の共通化 | リリース時点 |
| DS-SW-IIF-024 | Keepalive 設定 | リリース時点 |
| DS-SW-IIF-025 | Message size 制限 | リリース時点 |
| DS-SW-IIF-026 | Concurrent stream 制限 | リリース時点 |
| DS-SW-IIF-027 | Connection Pool とロードバランシング | リリース時点 |
| DS-SW-IIF-028 | リトライ戦略 | リリース時点 |
| DS-SW-IIF-029 | Circuit Breaker との連携 | 採用初期 |
