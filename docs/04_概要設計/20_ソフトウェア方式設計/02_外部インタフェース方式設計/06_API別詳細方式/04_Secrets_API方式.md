...
| NFR-B-PERF-004 | Secrets API p99 100ms | DS-SW-EIF-284, DS-NFR-PERF-004 | 完全 |
...
表に載せた要件数は FR-T1-SECRETS-* 4 件 + NFR 4 件 = 計 8 件。NFR-B-PERF-004 は Secrets API 固有の p99 性能要件で、企画書の [`03_要件定義/30_非機能要件/B_性能拡張性.md`](../../../../03_要件定義/30_非機能要件/B_性能拡張性.md) の改訂（Decision を -004 → -005 等にシフト）を反映する。
