...
本ファイルは OpenBao を単なる「外部 KV ストア」ではなく、静的シークレット・動的シークレット・PKI 発行の 3 つの責務を兼ねる統合基盤として位置付けて契約を定義する。単純な KV 参照だけであれば Kubernetes Secret で足りるが、DB パスワードの自動ローテーションや短命 TLS 証明書の on-demand 発行など、運用の人手を取り除く機能が DB 脱稟議の生産性試算の土台になっている。したがって本ファイルの設計項目は tier2 / tier3 開発者が「何を Secrets API に委ねられるか」を判断できる粒度で記述する。
...
Secrets API の全機能を Phase 1a から提供すると、動的シークレット検証のための DB / PKI 基盤が未整備の段階で tier2 / tier3 が動的シークレットを要求できてしまい、障害ポイントが拡散する。したがって機能は段階的に解放する。

**設計項目 DS-SW-EIF-291 Phase 別機能解放**

Phase 1a（MVP-0）は静的シークレット（KV v2）のみ提供する。`GetSecret` / `GetBulkSecret` は動作するが、動的シークレットパス（`database/*` / `pki/*` / `aws/*`）を指定すると `VALIDATION_FAILED` を返す。`RotateLease` は常に `UNIMPLEMENTED` を返す。Phase 1b（MVP-1a）は動的シークレット（Database Secrets Engine）を解放し、PostgreSQL role の on-demand 発行を開始、`RotateLease` を有効化する。Phase 1c（MVP-1b）は PKI Secrets Engine と AWS 互換（MinIO IAM）の 2 種を追加し、mTLS 用短命証明書の発行を可能にする。Phase 2 以降はエンタープライズ機能（Transit Engine による as-a-Service 暗号化、Namespace による tier0 マルチテナンシー）の採用可否を ADR 起票で検討する。
...
本ファイルは tier1 Secrets API の外部インタフェース設計であり、以下の要件 ID に対応する。

- FR-T1-SECRETS-001（静的シークレット取得）
- FR-T1-SECRETS-002（動的シークレット発行とリース管理）
- FR-T1-SECRETS-003（Transit Engine / as-a-Service 暗号化）
- FR-T1-SECRETS-004（Secret ローテーション自動化）
- NFR-E-ENC-004（シークレットの保存時暗号化）
- NFR-E-MON-005（シークレット利用監査）
- NFR-B-PERF-003（Secrets API p99 100ms）
- NFR-H-COMP-002（BSL 非該当 OSS 採用の強制）
