...
Input と Output で対応するプロトコルを整理する。根拠セルは単なる採用宣言ではなく「何故このプロトコルで、何故このフェーズか」を併記する。

| 方向 | プロトコル | 用途 | Phase | 根拠 |
|------|------------|------|-------|------|
| Output | MQTT (Mosquitto) | エッジデバイスへのコマンド送出 | 1a | JTC の現場センサ連携で実測採用、[04_エッジIoTインタフェース方式.md](../04_エッジIoTインタフェース方式.md) DS-SW-EIF-115〜117 と整合 |
| Output | HTTP/1.1 + HTTPS | 外部 SaaS / レガシー REST への POST | 1a | 既存 JTC の取引先連携の主力、ベストエフォートで p99 200ms |
| Output | Kafka | tier1 内 PubSub 以外の外部 Kafka クラスタ（顧客 Kafka 互換ミラー）へのブリッジ | 1b | tier1 内の Kafka は PubSub API に専念させ、外部 Kafka は Binding として責務分離 |
| Output | S3 互換（MinIO） | バッチ成果物 / エクスポートファイルのオブジェクトストア書き込み | 1b | 監査 WORM とは別経路、MinIO の AGPL は Pod 境界で隔離済み（[ADR-0003](../../../../02_構想設計/05_法務とコンプライアンス/)） |
| Output | SMTP | 通知メール送出 | 1c | 送信レート制御と DKIM 署名のため独立 binding 化、Phase 1c 以降 |
| Input | MQTT (Mosquitto) | エッジセンサからのイベント受信 | 1b | QoS 1 固定で at-least-once、デバイス側再送前提 |
| Input | HTTP Webhook | 外部 SaaS からのコールバック（Stripe / GitHub 等互換） | 1b | HMAC 署名検証とタイムスタンプ窓で改ざん防止 |
| Input | Cron | 定期ジョブトリガ | 1b | 業務バッチの起点として必須、UTC 基準で標準化 |
| Input | Kafka | 外部 Kafka クラスタからの購読ブリッジ | 1c | Phase 1b は PubSub で完結、Phase 1c で外部連携解放 |
...
**設計項目 DS-SW-EIF-310 Phase 別機能解放**

Phase 1a（MVP-0）は Output の MQTT / HTTP の 2 種のみ提供する。Input は Phase 1a では未提供で、アプリ側から外へ出る一方向に限定する。この割り切りは、Input 側の at-least-once 保証と DLQ 運用の検証期間を確保するためである。Phase 1b（MVP-1a）は Input の MQTT / HTTP Webhook / Cron の 3 種を解放し、Output に Kafka / S3 互換を追加、`Subscribe` メソッドを有効化する。Phase 1c（MVP-1b）は Output に SMTP、Input に Kafka を追加し 9 種全対応を完了する。Phase 2 以降は業界需要に応じて RabbitMQ / Azure Service Bus / AWS SNS 互換の追加を ADR 起票で検討する。
