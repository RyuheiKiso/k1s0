...
必須 Resource 属性は `service.name` / `service.version` / `deployment.environment` / `k1s0.tenant.id` の 4 つである。前 3 つは OpenTelemetry Semantic Conventions 1.28 準拠、最後の `k1s0.tenant.id` は tier1 拡張属性で `k1s0_tenant_id` の snake_case も互換として許容する。欠落時は `INVALID_RESOURCE` エラーを返し、OTel Collector の processor stage で reject する。リリース時点 は `service.name` のみ必須で残り 3 属性を warning / から 4 属性全必須に昇格させ、リリース時点の整備期間中に tier2 / tier3 側で計装を整備する時間を確保する。
...
プロファイル情報はトレース・メトリクスでは捕捉できない CPU / Heap / Goroutine の詳細を継続取得するため、Pyroscope と OTel Profiling Signal（2026 Q1 GA 予定）で構成する。リリース時点 時点ではまだ OTel Profiling が GA していないため Pyroscope 独自プロトコルを内部で採用する。

**設計項目 DS-SW-EIF-365 Profiles は pprof 互換と Pyroscope 連携**

`SubmitProfiles` は pprof 互換のバイナリ形式（`profile.proto`、gzip 圧縮）を受け付け、OTel Collector の Pyroscope exporter 経由で Pyroscope サーバへ保存する。プロファイル種類は `cpu` / `alloc_space` / `inuse_space` / `goroutine` / `mutex` / `block` の 6 種、tier1 クライアント SDK はデフォルトで cpu と alloc_space を 10 分間隔で取得する。OTel Profiling Signal が GA 後（採用後の運用拡大時）に互換レイヤを追加し、アプリ側コード変更なしで移行する方針は [ADR-OBS-002] に記録する。リリース時点 では提供せず / から限定公開する。
...
本番環境はクライアント SDK 側で head sampling 10%（`TraceIdRatioBased(0.1)`）、tier1 OTel Collector 側で tail sampling をエラースパン / 遅延スパン（> p95 × 2）/ 重要エンドポイントに対して 100% 維持する。head 10% の根拠は、Grafana Tempo の sizing guide で 1,000 spans/sec × 1KB × 30 日 = 2.6TB ストレージを起点に、採用側組織のでの許容予算 300GB/月 × 30% 余裕から逆算した値である。Pre / Dev は 100% サンプリングとし開発時の計装品質を担保する。メトリクスはサンプリングせず全量集計、プロファイルは 10 分間隔の時間サンプリングとシグナルごとに方針を分離する。
...
段階別に提供シグナルと機能を段階公開する。観測性基盤の運用成熟度（SRE チームの人員 / アラート設計 / ダッシュボード整備）に追従させるためである。

**設計項目 DS-SW-EIF-374 採用段階別提供範囲**

採用初期: `SubmitTraces` / `SubmitMetrics` のみ、サンプリング head 10% 固定、tail sampling 無効、テナント exemplar 無効。Tempo / Prometheus 構成。採用初期: `SubmitProfiles` 追加、tail sampling 有効化、exemplar 有効化、Pyroscope 追加。採用初期: `SubmitStream` 有効化、OTel Profiling Signal 互換層の試験運用。採用後の運用拡大時: OTel Profiling Signal の本番切り替え、マルチリージョン Tempo / Mimir への移行検討（ADR-OBS-003 として別途起票）。

## 対応要件一覧

本ファイルは Telemetry API 公開インタフェースの詳細方式設計であり、要件 ID → 設計 ID の 1:1 対応を以下の表で固定する。表形式併記は DR-COV-001 への緩和策として、CI スクリプトでの機械検証の一次入力となる。

| 要件 ID | 要件タイトル | 対応設計 ID | カバー状況 |
|---|---|---|---|
| FR-T1-TELEMETRY-001 | OTLP 互換 Metrics 計装 API | DS-SW-EIF-370, DS-CF-METRIC-001 | 完全 |
| FR-T1-TELEMETRY-002 | Traces 計装 API | DS-SW-EIF-371, DS-CF-TRACE-001 | 完全 |
| FR-T1-TELEMETRY-003 | OpenTelemetry Collector 経由配信 | DS-SW-EIF-372 | 完全 |
| FR-T1-TELEMETRY-004 | Profiles / Pyroscope 連携 | DS-SW-EIF-373 | 完全 |
| FR-EXT-MON-001 | 監視基盤連携（Grafana Tempo / Prometheus / Pyroscope） | DS-SW-EIF-374, DS-CF-METRIC-003 | 完全 |
| NFR-B-PERF-006 | Telemetry 計装オーバヘッド 10ms（内部目標 p99 5ms 非同期エンキュー） | DS-SW-EIF-371, DS-NFR-PERF-010 | 完全 |
| NFR-B-CAP-003 | テナント別キャパシティ公平性 | DS-SW-EIF-374, DS-NFR-CAP-003 | 完全 |
| NFR-C-OBS-001 | 分散トレース可観測性 | DS-SW-EIF-371, DS-NFR-OBS-001 | 完全 |
| NFR-C-OBS-002 | メトリクス可観測性 | DS-SW-EIF-370, DS-NFR-OBS-002 | 完全 |
| NFR-C-OBS-003 | プロファイル可観測性 | DS-SW-EIF-373, DS-NFR-OBS-003 | 完全 |
| NFR-F-COST-002 | テナント別コスト配賦 | DS-SW-EIF-374, DS-BUS-COST-002 | 完全 |

表に載せた要件数は FR-T1-TELEMETRY-* 4 件 + FR-EXT-* 1 件 + NFR 6 件 = 計 11 件。Telemetry の性能要件は [`03_要件定義/30_非機能要件/B_性能拡張性.md`](../../../../03_要件定義/30_非機能要件/B_性能拡張性.md) で **NFR-B-PERF-006（Log / Telemetry 計装オーバヘッド < 10ms）** として Log と一体採番されている（Feature Flag の NFR-B-PERF-007 とは別番号）。設計層ではインメモリ計装バッファ + 非同期エンキュー構成で内部目標 p99 5ms を掲げ、要件層の 10ms に対して 50% の余裕を持たせる。
