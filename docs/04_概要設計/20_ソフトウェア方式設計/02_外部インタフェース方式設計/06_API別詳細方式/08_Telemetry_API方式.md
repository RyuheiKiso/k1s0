...
| NFR-B-PERF-007 | Telemetry p99 5ms 非同期エンキュー | DS-SW-EIF-371, DS-NFR-PERF-007 | 完全 |
...
表に載せた要件数は FR-T1-TELEMETRY-* 4 件 + FR-EXT-* 1 件 + NFR 6 件 = 計 11 件。Telemetry 固有 p99 は [`03_要件定義/30_非機能要件/B_性能拡張性.md`](../../../../03_要件定義/30_非機能要件/B_性能拡張性.md) 改訂により NFR-B-PERF-007 で採番されており、旧記述の -001 流用ではなく正規番号に整合させた。
