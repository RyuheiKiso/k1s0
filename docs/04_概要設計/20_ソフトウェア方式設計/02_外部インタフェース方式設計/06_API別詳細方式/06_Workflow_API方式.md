...
本ファイルは tier1 Workflow API の外部インタフェース設計であり、以下の要件 ID に対応する。

- FR-T1-WORKFLOW-001（StartWorkflow とインスタンス管理）
- FR-T1-WORKFLOW-002（GetStatus と状態列挙）
- FR-T1-WORKFLOW-003（Terminate と Purge）
- FR-T1-WORKFLOW-004（SignalWorkflow と HMAC 検証）
- FR-T1-WORKFLOW-005（Saga 補償契約と Dapr/Temporal 二重化ルーティング）
- NFR-A-AVAIL-002（長期実行ワークフローの耐久性）
- NFR-B-PERF-005（Workflow Start p99 100ms）
- NFR-H-COMP-003（監査保持と WORM 退避）
