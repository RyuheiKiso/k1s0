...
.NET Framework 4.8 クライアント対応は 採用側組織の既存資産の大部分を占め、Service Invoke の HTTP/1.1 互換プロキシ機能は採用検討通過の条件の 1 つである。この対応が欠けると既存資産の漸進的移行が不能となり、短期 ROI が崩れる。本ファイルは gRPC 主・HTTP/1.1 補助の二層を方式として明確化する。
...
親ファイル DS-SW-EIF-012 で .NET Framework 4.8 は リリース時点 まで `Grpc.Core` を使用し、採用後の運用拡大時に HTTP/JSON 一本化と宣言した。Service Invoke は .NET Framework クライアントが tier2 / tier3 の既存 REST サーバを呼び出す正規経路でもあり、HTTP/1.1 互換プロキシを `InvokeHttp` RPC として提供する構成を取る。
...
親ファイル DS-SW-EIF-016 で Service Invoke は 採用初期から公開と宣言した。段階内での機能段階を明確化する。

**設計項目 DS-SW-EIF-229 Service Invoke の 段階別機能**

| 適用段階 | 公開機能 | 根拠 |
|-------|---------|------|
| 採用初期 | `Invoke` RPC のみ、gRPC only、再試行・サーキットブレーカ有効 | 最小構成で採用検討通過後の価値実証 |
| 採用初期 | `InvokeHttp` RPC 追加、HTTP/JSON 変換有効、.NET Framework `Grpc.Core` SDK 提供 | .NET Framework 資産の移行開始 |
| 採用初期 | ストリーミング RPC は非対応（Service Invoke は unary に限定） | 親ファイル DS-SW-EIF-008 と整合 |
| 採用後の運用拡大時 | `.NET Framework HTTP/JSON SDK` 一本化、`Grpc.Core` 廃止 | 公式非推奨ライブラリからの脱却 |

リリース時点 で HTTP/JSON を提供しない理由は、HTTP/JSON 変換の検証工数を 採用初期 のリソースでは確保できないためである。リリース時点 での追加により段階的に公開範囲を広げ、各段階 で契約の後方互換性を守る。

## 対応要件一覧

本ファイルは Service Invoke API の詳細方式設計であり、要件 ID → 設計 ID の 1:1 対応を以下の表で固定する。表形式併記は [`00_設計方針/06_設計リスクレジスタ.md`](../../../00_設計方針/06_設計リスクレジスタ.md) DR-COV-001（tier1 11 API 要件充足漏れ）への直接的な緩和策であり、CI スクリプトでの機械検証の一次入力となる。

| 要件 ID | 要件タイトル | 対応設計 ID | カバー状況 |
|---|---|---|---|
| FR-T1-INVOKE-001 | 同期 gRPC 呼び出し | DS-SW-EIF-220, DS-SW-EIF-221 | 完全 |
| FR-T1-INVOKE-002 | HTTP/1.1 互換プロキシ | DS-SW-EIF-222 | 完全 |
| FR-T1-INVOKE-003 | タイムアウト・リトライ制御 | DS-SW-EIF-223, DS-CTRL-RETRY-001 | 完全 |
| FR-T1-INVOKE-004 | サーキットブレーカ | DS-SW-EIF-224, DS-CTRL-IDEM-001 | 完全 |
| FR-T1-INVOKE-005 | 認証トークン自動伝搬 | DS-SW-EIF-225, DS-CF-AUTH-003 | 完全 |
| FR-EXT-DOTNET-001 | .NET Framework サイドカー連携 | DS-SW-EIF-226 | 完全 |
| NFR-B-PERF-001 | Service Invoke p99 300ms | DS-SW-EIF-227, DS-NFR-PERF-001 | 完全 |
| NFR-E-ENC-005 | 全区間暗号化（Istio Ambient mTLS） | DS-SW-EIF-228, DS-NFR-SEC-012 | 完全 |

表に載せた要件数は FR-T1-INVOKE-* 5 件 + FR-EXT-DOTNET-001 1 件 + NFR 2 件 = 計 8 件。tier1 11 API 方式全体の FR-T1-* 49 件のうち本ファイル担当は 5 件であり、[`80_トレーサビリティ/02_要件から設計へのマトリクス.md`](../../../80_トレーサビリティ/02_要件から設計へのマトリクス.md) の FR-T1-* 系サマリ行と一致する。
