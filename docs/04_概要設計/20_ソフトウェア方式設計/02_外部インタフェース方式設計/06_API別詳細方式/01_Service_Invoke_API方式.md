...
Service Invoke の通信は全区間で暗号化されている必要がある（NFR-E-SEC-005）。Istio Ambient Mesh を採用し、Pod 間通信（`facade-svcinv` → 相手 Pod）は透過的に mTLS 化する。
...
- NFR-E-SEC-005（全区間暗号化、Istio Ambient mTLS）
