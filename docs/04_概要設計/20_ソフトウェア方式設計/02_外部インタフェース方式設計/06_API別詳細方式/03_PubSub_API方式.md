...
message PublishRequest {
  // Dapr PubSub コンポーネント名（リリース時点 は "kafka-primary" 固定）
...
トピックの事前作成は `facade-pubsub` が Dapr PubSub コンポーネント経由で Kafka API を呼んで自動作成する。partition 数の初期値は 12（業界ベンチマーク 10〜20 の中央値）、replication factor は 3（リリース時点 で Strimzi 3 broker 前提）。partition 数は後から増やせるが減らせないため、初期値は慎重に設定する。
...
partition 数を 段階途中で増加させると hash modulo が変わり、同一 aggregate の event が異なる partition に分散する順序破壊が発生する。partition 数の変更は major version（`.v2` トピック並列運用）を伴う破壊的変更として扱い、移行は DS-SW-EIF-210 のセマンティック非後方互換ゲートで承認を要する。
...
リリース時点 では BSR 連携を optional とし、警告ログのみ出力する運用から開始する。運用蓄積後は mandatory に切り替え、BSR 未登録トピックへの Publish を拒否する構成に移行する。
...
Subscribe の 100ms 配信ウェイトは Kafka `fetch.max.wait.ms` の既定値（500ms）を 100ms に短縮してレイテンシ重視にする設定値を前提とする。fetch.max.wait.ms を短くすると broker への fetch 回数が増えるが、リリース時点 のスループット 5000 msg/sec では broker 負荷に余裕がある。

## 段階別展開

リリース時点 は単一 Kafka broker / で Strimzi 3 broker に拡張する。拡張時の互換性リスクを明示する。

**設計項目 DS-SW-EIF-271 段階別 Kafka 構成**

| 適用段階 | Kafka 構成 | 制約 |
|-------|-----------|------|
| 採用初期 | 単一 broker、KRaft 単一 controller、partition 3 / topic、replication 1 | 可用性低、データロスリスクあり、開発検証用途 |
| 採用初期 | Strimzi 3 broker、KRaft quorum 3 controller、partition 12 / topic、replication 3、ISR 2 min | 本番 SLO 達成構成、broker 1 台障害で継続可 |
| 運用蓄積後 | Strimzi 5 broker、partition 24 / topic、Rack-aware 配置、Tiered Storage（S3 長期保管） | 高スループット + 長期 retention、SaaS 外販向け |

リリース時点 → 1b の移行時、既存 partition の replication factor を 1 → 3 に変更する運用作業が発生する（`kafka-reassign-partitions`）。移行計画は `../../../40_運用ライフサイクル/03_環境構成管理.md` で別途定義する。クライアント SDK は 段階差を意識せず同一 API で呼び出せる抽象を維持する。
...
- ADR 参照: ADR-TIER1-001（Go+Rust）/ ADR-DATA-002（Kafka/Strimzi 採用、KRaft）/ ADR-PUBSUB-NNN（CloudEvents v1.0 準拠、未起票、リリース時点 前起票予定）
