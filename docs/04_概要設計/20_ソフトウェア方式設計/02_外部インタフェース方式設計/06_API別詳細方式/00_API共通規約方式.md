...
本ファイルは tier1 公開 API 共通規約の実装・検証設計であり、以下の要件 ID に対応する。

- FR-T1-001〜FR-T1-049（全 API 共通契約、横断要件）
- NFR-E-SEC-001〜003（JWT 必須、トークン検証、tenant 強制）
- NFR-E-SEC-010（Rate Limit 多軸）
- NFR-B-PERF-001（p99 レイテンシ目標、interceptor 加算 0.75ms 許容）
- NFR-DEVEX-001（API 学習コスト 1 時間以内）
- NFR-C-OPS-015（構造化エラー、問い合わせ対応 MTTR 短縮）
