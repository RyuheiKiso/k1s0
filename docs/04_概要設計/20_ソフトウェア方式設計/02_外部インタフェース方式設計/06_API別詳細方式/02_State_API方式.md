...
message GetRequest {
  // Dapr State Store 名（リリース時点 は "valkey-primary" 固定）
  string store_name = 1;
...
2. リリース時点 の Valkey は単一ノード構成（メモリ 16 GB）で、長期データが蓄積すると OOM リスクが高まる。30 日でサイクリングすることで実効メモリ使用量を予測可能に保つ。
3. 運用側のコスト予測（$200/月/クラスタ の リリース時点 想定）の前提値として 30 日上限を採用している。
...
Transaction は 1 リクエスト内に最大 10 操作を含められる。内部実装は Valkey の `MULTI` / `EXEC` で原子的に実行する。1 トランザクション内のキーがすべて同一 Valkey ノード上にあることを前提とし、リリース時点 の Cluster 構成では hash tag `{tenant_id}:{aid}` で slot 固定することで同一ノード配置を保証する。
...
`Consistency.EVENTUAL` は Replica から Get し、レプリカラグ（典型 1〜5ms）を許容する。`Consistency.STRONG` は Master から Get し、Set の場合は Valkey の `WAIT <numreplicas> <timeout_ms>` コマンドで指定レプリカ数（リリース時点 は 2）への同期完了を待つ。STRONG モードは p99 が EVENTUAL 比 2〜3 倍（Set で 20ms → 50ms）になるため、クライアントは必要時のみ指定する。
...
| `STATE_TX_CROSS_SLOT` | 10205 | `FAILED_PRECONDITION` | 424 | Transaction 内キーが同一 slot 外（リリース時点 Cluster） |
...
リリース時点 は単一 Valkey ノード / で Cluster 3 shard に拡張する。拡張時の互換性リスクを明示する。

**設計項目 DS-SW-EIF-251 段階別 Valkey 構成**

| 適用段階 | Valkey 構成 | 制約 |
|-------|-----------|------|
| 採用初期 | 単一ノード + 1 レプリカ、メモリ 16GB | Transaction は全キー同一ノード自動成立、Cluster 制約なし |
| 採用初期 | Cluster 3 shard、各 shard 1 Master + 1 Replica、メモリ合計 48GB | Transaction 内キーは hash tag `{tenant:aid}` で slot 固定必須、cross-slot は `STATE_TX_CROSS_SLOT` |
| 運用蓄積後 | Cluster 6 shard、Read Replica 追加 | Consistency.EVENTUAL は Read Replica から serve |

リリース時点 → 1b の移行時、既存キーのハッシュタグ再配置が必要。移行計画は `../../../40_運用ライフサイクル/03_環境構成管理.md` で別途定義する。クライアント SDK は 段階差を意識せず同一 API で呼び出せる抽象を維持する。

## 対応要件一覧

本ファイルは State API の詳細方式設計であり、要件 ID → 設計 ID の 1:1 対応を以下の表で固定する。表形式併記は [`00_設計方針/06_設計リスクレジスタ.md`](../../../00_設計方針/06_設計リスクレジスタ.md) DR-COV-001 への緩和策として、CI スクリプトでの機械検証の一次入力となる。

| 要件 ID | 要件タイトル | 対応設計 ID | カバー状況 |
|---|---|---|---|
| FR-T1-STATE-001 | Get/Set/Delete 基本操作 | DS-SW-EIF-240, DS-SW-EIF-241 | 完全 |
| FR-T1-STATE-002 | Bulk と Transaction | DS-SW-EIF-242, DS-CTRL-SAGA-001 | 完全 |
| FR-T1-STATE-003 | ETag 楽観ロック | DS-SW-EIF-243 | 完全 |
| FR-T1-STATE-004 | TTL とキースコープ強制（テナント分離） | DS-SW-EIF-244, DS-SW-EIF-245 | 完全 |
| FR-T1-STATE-005 | Consistency モード（Strong / Eventual） | DS-SW-EIF-246 | 完全 |
| NFR-B-PERF-003 | State API Get p99 10ms（Set p99 20ms は設計側内部目標） | DS-SW-EIF-247, DS-NFR-PERF-003 | 完全 |
| NFR-E-AC-006 | テナント分離（key prefix 強制） | DS-SW-EIF-245, DS-CF-AUTHZ-003 | 完全 |
| NFR-G-CLS-003 | PII 分離（State は扱わない、Audit-Pii 側で担当） | DS-SW-EIF-248 | 完全 |

表に載せた要件数は FR-T1-STATE-* 5 件 + NFR 3 件 = 計 8 件。
