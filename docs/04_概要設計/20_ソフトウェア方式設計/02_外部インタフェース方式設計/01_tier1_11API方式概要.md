...
| 7 | Log | 構造化ログ（JSON、ECS 準拠）、PII 自動マスキング | Rust（custom-log） | Grafana Loki（リリース時点）/ OTLP Collector | PII マスキングは Rust 側で確定的に実装し GDPR 対応 |
...
全 API で JWT Bearer Token を必須とする。gRPC metadata の `authorization` ヘッダ、HTTP/JSON の `Authorization: Bearer <jwt>` ヘッダで送付する。トークン未提示・失効・署名検証失敗はすべて gRPC `UNAUTHENTICATED` / HTTP 401 を返し、`WWW-Authenticate: Bearer error="invalid_token"` ヘッダを付与する。匿名アクセスは例外なく禁止し、リリース時点 段階でも Dev Realm の JWT 発行を前提に開発環境を構成する。リリース時点 末時点で `spiffe://k1s0.internal/*` の SVID JWT にも対応し、サービス間呼び出しの認証を人間ユーザと分離する。認証詳細は [03_認証認可インタフェース方式.md](03_認証認可インタフェース方式.md) で展開する。
...
.NET Framework 4.8（既存 採用側組織の資産の主力）は gRPC 公式サポートがないため、`Grpc.Core`（非推奨だが 4.8 稼働可）の C# レガシー SDK を リリース時点 まで提供する。採用後の運用拡大時に HTTP/JSON SDK（`k1s0-sdk-netfx-http`）に一本化し、段階的に gRPC 依存を廃止する。この方針は [05_レガシー連携インタフェース方式.md](05_レガシー連携インタフェース方式.md) と連動する。
...
リリース時点 の目標スループットは tier1 全体で 2,000 RPS（Service Invoke + State + PubSub の合算ピーク）とする。Valkey Cluster は 10,000 ops/sec、Kafka は 5,000 msg/sec、Postgres は 1,000 tx/sec を個別に担保する。負荷分配とスケール単位は [../../50_非機能方式設計/06_キャパシティ計画.md](../../50_非機能方式設計/06_キャパシティ計画.md) で展開する。

## 可観測性とリクエスト相関

全 API は 1 リクエスト単位で trace_id / tenant_id / user_id / api_name / method / status を必ずログ出力し、Log API と Audit-Pii API に自動ブリッジする。この相関は「採用側の小規模運用原則」の前提であり、障害時に tier2 開発者に質問することなく tier1 SRE が原因箇所を特定できることが合格基準となる。
...
**設計項目 DS-SW-EIF-016 API 公開の段階順**

採用初期は Service Invoke / State / PubSub / Feature / Log の 5 API を限定公開する。採用初期は Secrets / Binding / Workflow（Dapr Workflow のみ）/ Telemetry / Decision / Audit-Pii を追加し 11 API 完備状態に到達する。採用初期は Workflow の Temporal バックエンドを有効化し、長期実行対応を完了する。採用後の運用拡大時に既公開 API の機能拡張（Bulk 上限引き上げ、ストリーミング拡張）のみで、新規 API 追加は設計方針 [../../00_設計方針/02_設計原則と制約.md](../../00_設計方針/02_設計原則と制約.md) の原則 7 に従い ADR 起票を要する。
...
- NFR-B-PERF-001（tier1 API p99 500ms）、NFR-B-PERF-003（State Get 10ms）、NFR-B-PERF-004（Decision 1ms）、NFR-B-PERF-005（PubSub Publish 50ms）の p99 レイテンシ目標、および NFR-B-PERF-002（tier1 API スループット 150 RPS）
