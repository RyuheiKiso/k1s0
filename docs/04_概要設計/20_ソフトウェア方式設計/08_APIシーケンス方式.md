...
本章はこのギャップを埋め、**横断協調動作を単一の図で示す**。シーケンス図はコードの詳細化ではなく、責務境界・順序制約・タイムアウト / リトライ仕様・補償制御の連結表現である。採用検討時点では代表 5 シナリオを固定し、リリース時点 デモで実コード寸法に合わせた微修正、運用蓄積後でシナリオを追加する。
...
代表シナリオは以下 5 件とする。選定基準は「採用検討時点で リリース時点 までに実装する必要があり、複数 API の横断協調が顕在化するもの」である。
...
長時間処理の方式制約として、Activity 単体で 60 秒を超える処理は Activity 側で中間進捗を emit する設計とし、Dapr Workflow Engine の heartbeat 切れを防ぐ。Activity のタイムアウトは 5 分を上限とし、それを超える処理は Activity を分割する。Workflow 全体のタイムアウトは 24 時間を上限とし、それ以上は Temporal 採用時（採用後の運用拡大時）に拡張する。
...
Audit 書込の失敗処理は、Audit.Append が失敗しても本体 API は成功として応答する。ただし Audit 失敗はアラート対象とし、Loki に Error ログを出力する。Audit が恒久的に失敗する場合は k1s0-policy の仕組みで Write 系 API を一時停止する緊急モードを リリース時点 で実装する。
...
span の sampling は、tier1 公開 API のルート span は 100% sampling、内部呼出の子 span は親の sampling 判定を継承する。これは「失敗した呼出のトレースが抜ける」を防ぐためである。高 QPS 時のストレージ逼迫は リリース時点 で tail-based sampling（失敗のみ残し成功は 1% sampling）に切替検討する。
...
## 設計項目 DS-SW-SEQ-009 段階別シナリオ拡張

代表 5 シナリオは 段階進行に応じて以下の順で拡張する。

- **リリース時点（採用検討時点）**: 5 シナリオの骨子・図 3 枚を確定。実コード呼出は リリース時点 以降。
- **採用初期 (初版調整)**: 実コード呼出に合わせた微修正。PubSub Publish / Consume の追加図 1 枚。
- **採用初期 (機能図追加)**: Secrets Lease Rotation の図 1 枚、Feature Flag 評価の図 1 枚、Binding 外部連携の図 1 枚を追加。
- **採用初期 (障害図追加)**: 監査 hash_chain 整合性異常検知時の図 1 枚、災害復旧時のフェイルオーバー図 1 枚を追加。
- **採用後の運用拡大時**: Istio Ambient 採用時の mTLS 加入、Temporal 採用時の Workflow 表現差分を追加。

各段階 でシナリオ拡張時は、[../80_トレーサビリティ/02_要件から設計へのマトリクス.md](../80_トレーサビリティ/02_要件から設計へのマトリクス.md) を同時に更新し、要件 ID と設計 ID の対応を維持する。

## 設計項目 DS-SW-SEQ-010 個別 API 単体仕様との整合確認

本章の各シナリオと、個別 API 単体仕様（[02_外部インタフェース方式設計/06_API別詳細方式/](02_外部インタフェース方式設計/06_API別詳細方式/)）の整合を、リリース時点 着手前にチェックリストで確認する。
...
- `DS-SW-SEQ-009`（段階別シナリオ拡張）: 間接対応（段階進行管理）
