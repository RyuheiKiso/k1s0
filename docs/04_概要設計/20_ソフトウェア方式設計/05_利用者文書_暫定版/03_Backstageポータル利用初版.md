...
本ファイルは IPA 共通フレーム 2013 の **7.1.2.4 利用者文書（暫定版）の作成** に対応する利用者文書のうち、開発者・運用者・アーキテクトが k1s0 の開発者ポータル（Backstage）を利用する際の手引き初版である。Backstage は k1s0 の「開発者と運用者の共通玄関」として Phase 1c で導入し、Phase 2 で本格統合する。

## 本ファイルの位置付け

プラットフォーム製品において、サービス一覧・所有者情報・依存関係・技術文書・デプロイ状況・ダッシュボードが個別ツールにバラバラに存在すると、開発者は「どのサービスの何を調べたいか」の質問に対して毎回複数ツールを横断することになる。これは既存 JTC 情シス現場の「属人的ナレッジへの依存」を助長し、企画で約束した「バス係数 2」「2 名運用」を破綻させる典型経路である。
...
本章は Phase 1c 導入時点での「最初の 1 時間」で読者が Backstage を使いこなせるようになることを目的とする。画面操作の詳細は Phase 1c デモで動画化し、本章は文字情報として Phase 間を横断できる粒度で記述する。
...
**運用者ペルソナ**（JTC 情シス運用 2 名）: Catalog から障害サービスの Owner・SLA・Runbook リンクを引き出し、Argo CD プラグインでデプロイ状況を確認し、Grafana プラグインでダッシュボードに遷移する。最頻利用は Catalog 詳細と運用プラグイン群。
...
**プラグイン体系** は Argo CD / GitHub Actions / Harbor / Grafana / Kafka Topics / Keycloak / Flagd など外部ツールを Catalog の各コンポーネントページに埋め込む。プラグインは Backstage Marketplace から追加するか、JTC 固有プラグインを自作する。
...
Backstage への認証は Keycloak OIDC 経由の SSO で行う。JTC 社内 AD アカウントが Keycloak の Identity Provider として連携済みであり、開発者は AD アカウントで直接 Backstage にログインできる。
...
`catalog-info.yaml` の記述例（tier2 サンプルサービス）を以下に示す。Phase 0 時点では骨子のみで、Phase 1a デモで実サンプルに差し替える。
...
Software Templates は Backstage 上での「新規コンポーネント作成」ウィザードである。Phase 1c 時点で以下 4 テンプレートを提供する。
...
テンプレートのメンテナは k1s0 基盤チームである。破壊的変更を避けるため、既存テンプレートの必須項目削除は Phase 移行時のみ許可する。SDK バージョン更新は Renovate が自動 PR を作成し、基盤チームがレビューして main マージする。
...
Phase 1c 時点で以下 7 プラグインを有効化する。各プラグインは Catalog の各コンポーネントページに専用タブを追加する形で統合する。
...
Phase 2 以降、Temporal Web・Trivy Operator・Istio ダッシュボード の 3 プラグインを追加する。JTC 固有プラグインとして「稟議承認フロー連動」「既存 AD グループからの Ownership 自動算出」の 2 プラグインを自作する検討を進める。
...
## 設計項目 DS-SW-DOC-087 Phase 別導入ロードマップ

Backstage 導入は Phase 1c 着手で Phase 2 本格統合とする。Phase 0 稟議時点では導入は始まっていない。

- **Phase 0（稟議時点）**: 本章の骨子確定、Backstage の採用決定は既に構想設計 ADR 相当で確定済み。
- **Phase 1a（MVP-0）**: 未導入。Catalog 相当情報は GitHub README で代替。
- **Phase 1b（MVP-1a）**: 未導入。Catalog 相当情報を Confluence 代替で集約するかは検討中。
- **Phase 1c（MVP-1b）**: 導入開始。Catalog・Templates・TechDocs・Argo CD プラグインの 4 機能を Phase 1c 完了条件とする。
- **Phase 2 以降**: 全 7 プラグイン統合、JTC 固有プラグイン 2 種を自作、横断検索のチューニングを実施。

Phase 1c 導入後の運用負荷増は、運用 2 名体制の範囲内に収めるために Backstage 自体の運用を基盤チームが担当し、各 tier の開発者・運用者は Backstage を「使うだけ」の分担とする。

## 設計項目 DS-SW-DOC-088 導入後の典型的つまずきと対処

Phase 1c 導入時に他社事例で頻発するつまずきを事前に整理する。初期 3 か月で直面するであろう問題と対処を以下に示す。
...
- `DS-SW-DOC-080`（Backstage 4 機能）: `DX-BS-001` / `DX-BS-002` / `BR-PLATUSE-005`
- `DS-SW-DOC-081`（Keycloak SSO）: `FR-T1-AUTH-002` / `NFR-SEC-002`
- `DS-SW-DOC-082`（Software Catalog）: `DX-BS-003` / `DX-BS-004` / `BR-PLATUSE-006`
- `DS-SW-DOC-083`（Software Templates）: `DX-GP-003` / `DX-BS-005`
- `DS-SW-DOC-084`（TechDocs）: `DX-BS-006` / `DX-LD-002`
- `DS-SW-DOC-085`（プラグイン体系）: `DX-BS-007` / `NFR-OBS-003` / `OR-SUP-007`
- `DS-SW-DOC-086`（ペルソナ別画面遷移）: `DX-BS-008` / `BR-PLATOPS-006`
- `DS-SW-DOC-087`（Phase 別ロードマップ）: 間接対応（Phase 進行管理）
- `DS-SW-DOC-088`（つまずきと対処）: `DX-BS-009` / `OR-SUP-008`
