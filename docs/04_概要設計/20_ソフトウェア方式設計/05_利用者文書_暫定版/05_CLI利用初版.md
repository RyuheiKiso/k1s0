...
本章は Phase 0 時点で「サブコマンド体系・認証方式・出力形式・インストール経路」を確定し、Phase 1a で基本コマンド群を実装、Phase 2 で拡張コマンド群を追加する計画である。各 Phase での完成度を明示することで、未実装のコマンドを「使えるはず」と誤解されるリスクを抑える。

## 対象読者と前提スキル

対象読者は tier2 / tier3 開発者と、JTC 情シス運用者の両方である。前提スキルはターミナル（PowerShell / bash / zsh）の基本操作、Git の基礎、環境変数の概念である。CLI 内部の Rust 実装の知識は一切不要である。
...
代表的な 6 コマンドの使用例を以下に示す。Phase 0 時点では疑似出力、Phase 1a で実出力に差し替える。
...
CLI は以下 5 経路で配布する。JTC 社内の OS 比率（Windows 70% / macOS 20% / Linux 10%）を考慮し、各 OS の標準パッケージマネージャに対応する。

- **Windows**: `scoop install k1s0` または `winget install k1s0`
- **macOS**: `brew install k1s0/tap/k1s0`（JTC 専用 Homebrew tap）
- **Linux (Debian / Ubuntu)**: `apt install k1s0`（JTC 内部 APT リポジトリ経由）
- **Linux (RHEL / CentOS)**: `dnf install k1s0`（JTC 内部 DNF リポジトリ経由）
...
ローカル保存のトークン（`~/.k1s0/credentials.json`）の保護は、ファイル権限 0600 + JTC 社内端末の Windows Hello / macOS Touch ID によるデバイス認証を Phase 1c で追加する。共有端末からのトークン流出を防ぐため、`k1s0 logout` を自動化するスクリーンセーバ連動も検討する。
...
## 設計項目 DS-SW-DOC-146 Phase 別完成度

CLI は Phase 1a で基本コマンド群、Phase 2 で拡張を実装する段階計画である。

- **Phase 0（稟議時点）**: 本章骨子のみ。実装着手前。
- **Phase 1a（MVP-0）**: `new` / `dev` / `login` / `doctor` の 4 グループを実装。scoop / brew / 単体バイナリのみ配布。
- **Phase 1b（MVP-1a）**: `state` / `feature` / `secret get / put` を追加。apt / dnf 配布を開始。
- **Phase 1c（MVP-1b）**: `secret rotate` / `audit query` / `audit export` を追加。監査対応強化。
- **Phase 2 以降**: PubSub / Workflow / Decision / Telemetry 系の状態確認コマンドを追加。AI 補助（`k1s0 explain`）検討。

各 Phase 移行時に、CLI の後方互換性を CI で検証する。前 Phase のコマンドが Phase 移行後に動かなくなる破壊的変更は major バージョンアップ時のみに限定する。
...
この哲学は、シェルスクリプトや CI パイプラインへの組み込みを容易にする。GUI 操作だけでなくスクリプト自動化が可能であることは、運用自動化の前提であり、企画で約束した「2 名運用」の成立条件の一つである。
...
- `DS-SW-DOC-141`（OIDC Device Code Flow）: `FR-T1-AUTH-002` / `NFR-SEC-003`
- `DS-SW-DOC-142`（出力形式 JSON / YAML / Table）: `DX-LD-005`
- `DS-SW-DOC-143`（代表コマンド使用例）: `DX-LD-006` / `FR-T1-STATE-001` / `FR-T1-SEC-001` / `FR-T1-FEAT-001` / `FR-T1-AUD-003`
- `DS-SW-DOC-144`（インストール経路）: `DX-LD-007` / `BR-PLATUSE-013`
- `DS-SW-DOC-145`（セキュリティと権限境界）: `NFR-SEC-006` / `FR-T1-AUTHZ-001`
- `DS-SW-DOC-146`（Phase 別完成度）: 間接対応（Phase 進行管理）
- `DS-SW-DOC-147`（Unix 哲学準拠）: `DX-LD-008` / 企画「2 名運用」コミット
