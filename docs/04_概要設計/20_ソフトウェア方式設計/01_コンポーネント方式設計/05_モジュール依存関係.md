...
本ファイルは 2 つの依存軸を扱う。第 1 軸は runtime の内部 gRPC 呼び出し（コンポーネント間の runtime 依存）、第 2 軸は build-time のライブラリ import（crate / go.mod の静的依存）。この 2 軸が独立して健全に保たれていることが、採用初期 の 採用側の小規模運用と 採用後の運用拡大時 の拡張を両立させる前提である。
...
**確定段階**: リリース時点。**対応要件**: NFR-A-CONT-003、NFR-E-AC-001〜005。
...
**確定段階**: リリース時点。**対応要件**: NFR-A-CONT-003、NFR-C-NOP-001。
...
**確定段階**: リリース時点。**対応要件**: NFR-A-CONT-003、DX-CICD-\*。
...
**確定段階**: リリース時点。**対応要件**: NFR-A-CONT-003、NFR-H-INT-001。
...
**確定段階**: リリース時点（ルール / CI 組込）。**対応要件**: NFR-C-NOP-001、DX-CICD-\*。
...
**確定段階**: リリース時点（方式 / 実装）。**対応要件**: NFR-B-PERF-001、NFR-H-INT-001。
...
**確定段階**: リリース時点。**対応要件**: DX-CICD-\*、NFR-C-NOP-002。
...
**確定段階**: リリース時点。**対応要件**: ADR-TIER1-002（Protobuf）、NFR-C-NOP-002。
...
**確定段階**: リリース時点。**対応要件**: DX-GP-\*、NFR-C-NOP-002。
...
**確定段階**: リリース時点。**対応要件**: FR-T1-TELEMETRY-\*、NFR-D-MON-\*、NFR-D-TRACE-\*。

### DS-SW-COMP-110 k1s0-policy の役割

`k1s0-policy` は JWT 検証・Tenant 境界確認・OPA Rego 評価・Rate Limit・冪等性キー管理のユーティリティを集約する。facade 層の Policy Enforcer モジュールから import され、rust 層の各 Pod も tonic Interceptor で同じ policy を適用する。policy.rego の実体は ConfigMap で配布され、library は rego 評価エンジンを内包する（OPA SDK）。リリース時点で は JWT + Tenant のみ / RBAC / Rate Limit / 冪等性を追加する。

**確定段階**: 採用初期/1c。**対応要件**: NFR-E-AC-001〜005、DX-GP-\*。
...
**確定段階**: リリース時点。**対応要件**: DX-CICD-\*、NFR-C-NOP-001。
...
**確定段階**: リリース時点。**対応要件**: DX-CICD-\*、NFR-E-ENC-001、NFR-F-ENV-\*。
...
**確定段階**: リリース時点。**対応要件**: DX-BS-\*、NFR-C-NOP-002。

### DS-SW-COMP-117 新規コンポーネント追加時の依存審査

新規 rust / go コンポーネント追加は ADR 起票を要し、以下 4 点を審査する。(1) 依存方向が DS-SW-COMP-100 〜 103 に合致するか、(2) 共通ライブラリ（`k1s0-common` 等）のみ使用し他 Pod を runtime 依存としないか、(3) 既存コンポーネントの責務と重複しないか（[04_コンポーネント責務一覧.md](04_コンポーネント責務一覧.md) 参照）、(4) 採用初期 の 採用側の小規模運用を壊さないか。新 Pod 追加は 採用後の運用拡大時で原則解禁するが、採用初期 でも上記条件を満たせば例外承認できる。

**確定段階**: リリース時点（ルール）、各段階（新 Pod 審査）。**対応要件**: NFR-C-NOP-001、DS-SW-COMP-019（再評価条件）。
...
**確定段階**: リリース時点。**対応要件**: DX-BS-\*、DX-CICD-\*、NFR-C-NOP-002。

### DS-SW-COMP-119 外部 OSS バージョン更新の依存影響

Dapr Go SDK / tonic / prost / ZEN Engine / Kafka client / PG driver などの外部 OSS バージョン更新は、依存グラフを揺さぶる可能性がある。更新手順は Renovate（運用蓄積後）で自動 PR 化し、CI で全 Pod のビルド + unit test + integration test を実行して影響を可視化する。重大な破壊的変更（major version）は ADR 起票、マイナー/パッチは自動マージ（test pass 前提）とする。バージョン更新の頻度は週次（セキュリティパッチ）+ 月次（機能更新）を基準とする。

**確定段階**: 採用初期。**対応要件**: DX-CICD-\*、NFR-C-NOP-002、NFR-E-ENC-001。
...
| 設計 ID | 内容 | 確定段階 |
|---|---|---|
| DS-SW-COMP-100 | facade 層間の相互呼び出し禁止 | リリース時点 |
| DS-SW-COMP-101 | 自作層から facade 層への呼び出し禁止 | リリース時点 |
| DS-SW-COMP-102 | facade → rust の単方向依存 | リリース時点 |
| DS-SW-COMP-103 | rust → rust の依存は限定的に許可 | リリース時点 |
| DS-SW-COMP-104 | DAG 制約の CI 検査 | 採用初期 |
| DS-SW-COMP-105 | Audit イベントの非同期化 | 採用初期 |
| DS-SW-COMP-106 | 共通ライブラリは Pod を呼ばない | リリース時点 |
| DS-SW-COMP-107 | k1s0-proto の役割 | リリース時点 |
| DS-SW-COMP-108 | k1s0-common の役割 | リリース時点 |
| DS-SW-COMP-109 | k1s0-otel の役割 | リリース時点 |
| DS-SW-COMP-110 | k1s0-policy の役割 | 採用初期/1c |
| DS-SW-COMP-111 | Go パッケージの公開範囲 | リリース時点 |
| DS-SW-COMP-112 | Rust crate の公開範囲 | リリース時点 |
| DS-SW-COMP-113 | Go 依存検査方式 | リリース時点 |
| DS-SW-COMP-114 | Rust 依存検査方式 | リリース時点 |
| DS-SW-COMP-115 | Protobuf 破壊的変更検査 | リリース時点 |
| DS-SW-COMP-116 | 依存グラフの可視化と更新 | リリース時点 |
| DS-SW-COMP-117 | 新規コンポーネント追加時の依存審査 | リリース時点/各段階 |
| DS-SW-COMP-118 | deprecation 時の依存整理 | リリース時点 |
| DS-SW-COMP-119 | 外部 OSS バージョン更新の依存影響 | 採用初期 |

## 対応要件一覧

- NFR-A-CONT-003（バックエンド障害影響限定）、NFR-C-NOP-001（採用側の小規模運用）、NFR-C-NOP-002（可視性）
