...
**確定フェーズ**: Phase 1a（基盤）、Phase 1b（本格導入）。**対応要件**: NFR-B-PERF-004、NFR-E-ENC-001、DX-TEST-\*。**参照**: 構想設計 ADR-TIER1-001。
...
**確定フェーズ**: Phase 1b。**対応要件**: NFR-B-PERF-\*、NFR-A-FT-001。

### DS-SW-COMP-053 Dapr sidecar 不要の根拠

自作 Rust 3 Pod は Dapr サイドカー（daprd）を持たない。根拠は 3 つある。第一に、ZEN Engine / hash chain / PII parser のいずれも Dapr Building Block の抽象（state / pubsub / bindings / secrets / workflow / serviceinvocation）と 1 対 1 対応せず、Dapr 経由の呼び出しで性能・安全性が損なわれる。第二に、Pod 起動時間短縮（daprd 起動 500ms を削減）とメモリフットプリント削減（daprd 128MiB を削減）の効果が大きい。第三に、Pod 内プロセスを最小化することで failure mode を減らし Phase 1a〜1c の 2 名運用を容易にする。ネットワーク観測性は Istio Ambient の ztunnel + waypoint で代替する（[../../30_共通機能方式設計/](../../30_共通機能方式設計/) 参照）。

**確定フェーズ**: Phase 0。**対応要件**: NFR-B-PERF-006、NFR-F-ENV-\*、NFR-C-NOP-001。**参照**: 構想設計 [../../../02_構想設計/02_tier1設計/01_設計の核/01_Dapr隠蔽方針.md](../../../02_構想設計/02_tier1設計/01_設計の核/01_Dapr隠蔽方針.md)。
...
**確定フェーズ**: Phase 1b（kafka + hash + pg）、Phase 1c（dedup + minio）。**対応要件**: FR-T1-AUDIT-\*、NFR-H-INT-001、NFR-C-NOP-003。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-AUDIT-001、FR-INFO-AUDITMODEL-001、NFR-H-INT-001。**参照**: 構想設計 [../../../02_構想設計/02_tier1設計/](../../../02_構想設計/02_tier1設計/)。
...
**確定フェーズ**: Phase 1c。**対応要件**: FR-T1-AUDIT-003、NFR-C-NOP-003、NFR-H-INT-001。**参照**: 構想設計 ADR-DATA-003（MinIO Object Lock）。
...
**確定フェーズ**: Phase 1c。**対応要件**: FR-T1-AUDIT-001、NFR-H-INT-001、DX-RB-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-AUDIT-\*、NFR-A-CONT-003、NFR-H-INT-001。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-DECISION-001、FR-T1-DECISION-002、NFR-B-PERF-004。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-DECISION-003、DX-FM-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: NFR-B-PERF-004、NFR-D-MON-\*。

### DS-SW-COMP-065 Decision Pod の起動ウォームアップ

Pod 起動時は Control API から高頻度ルールセット（tenant top 10 × rule top 5 = 50 セット）をプリロードし、LRU cache に常駐化する。プリロード完了まで readiness probe を fail させることで、cold cache の Pod がトラフィックを受けないようにする。プリロード所要時間は 3〜5 秒を目標とする。プリロード対象は Backstage の Frequently Used Rules レポートから静的に決定する（Phase 2 で動的学習化）。

**確定フェーズ**: Phase 1c。**対応要件**: NFR-B-PERF-004、NFR-A-FT-001。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-PII-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-PII-001、NFR-G-ENC-\*、NFR-E-DOS-\*。
...
**確定フェーズ**: Phase 1b（マスク基本）、Phase 1c（AES-SIV）。**対応要件**: FR-T1-PII-002、NFR-G-ENC-\*、NFR-H-KEY-001。**参照**: 構想設計 ADR-SEC-002。
...
**確定フェーズ**: Phase 1c。**対応要件**: FR-T1-PII-\*、FR-T1-DECISION-\*、DX-FM-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: NFR-B-PERF-006、FR-T1-PII-\*。

## 自作層共通の横断事項

### DS-SW-COMP-071 tracing と OTel 統合

tracing は `tracing` crate + `tracing-subscriber` + `tracing-opentelemetry` で構築する。全 gRPC リクエストに root span を張り、`trace_id` / `span_id` を自動生成して Jaeger/Tempo 互換の OTLP で OpenTelemetry Collector に送出する。facade 層からの span 親子関係は `grpc-metadata` の `traceparent` ヘッダで伝搬し（[../03_内部インタフェース方式設計/03_Go_Rust間言語境界方式.md](../03_内部インタフェース方式設計/03_Go_Rust間言語境界方式.md) 参照）、Go ⇔ Rust の境界をまたいで単一 trace を維持する。sampling は Phase 1b で 10%、Phase 1c で 1% に削減する。

**確定フェーズ**: Phase 1a。**対応要件**: FR-T1-TELEMETRY-\*、NFR-D-TRACE-\*。
...
**確定フェーズ**: Phase 1a。**対応要件**: FR-T1-TELEMETRY-\*、NFR-D-MON-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: NFR-E-ERR-\*、FR-T1-LOG-\*。
...
Pod 起動後の定常 memory 目標は AUDIT 512 MiB、DECISION 1.5 GiB（ZEN cache 込み）、PII 256 MiB とする。目標超過時は Prometheus alert `k1s0_pod_memory_usage_bytes` で検知する。Rust の `jemalloc` は optional で有効化（`jemallocator` crate）し、Go ファサード層との align を取る。memory leak 検出は Phase 1c で `heaptrack` + `valgrind` の定期実行をテスト環境で行う。

**確定フェーズ**: Phase 1b（目標）、Phase 1c（leak 検出）。**対応要件**: NFR-B-CAP-\*、NFR-F-ENV-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: NFR-E-ENC-001、NFR-H-INT-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: NFR-B-PERF-\*、NFR-B-WL-\*。
...
**確定フェーズ**: Phase 1a。**対応要件**: DX-CICD-\*、DX-TEST-\*、NFR-F-ENV-\*。
...
| 設計 ID | 内容 | 確定フェーズ |
|---|---|---|
| DS-SW-COMP-050 | Rust 自作層の共通基盤 | Phase 1a/1b |
| DS-SW-COMP-051 | tonic gRPC サーバ実装方式 | Phase 1a |
| DS-SW-COMP-052 | tokio runtime チューニング | Phase 1b |
| DS-SW-COMP-053 | Dapr sidecar 不要の根拠 | Phase 0 |
| DS-SW-COMP-054 | AUDIT 5 モジュール構成 | Phase 1b/1c |
| DS-SW-COMP-055 | ハッシュチェーン生成方式（SHA-256 + CBOR） | Phase 1b |
| DS-SW-COMP-056 | WORM append 実装方式（PG トリガ） | Phase 1b |
| DS-SW-COMP-057 | replica パーティショニング方式 | Phase 1b/2 |
| DS-SW-COMP-058 | MinIO Object Lock 長期保管 | Phase 1c |
| DS-SW-COMP-059 | 監査改ざん検証 API | Phase 1c |
| DS-SW-COMP-060 | Kafka Consumer at-least-once 設定 | Phase 1b |
| DS-SW-COMP-061 | DECISION 4 モジュール構成 | Phase 1b |
| DS-SW-COMP-062 | ZEN Engine 統合方式 | Phase 1b |
| DS-SW-COMP-063 | テナント別ルールセット管理 | Phase 1b |
| DS-SW-COMP-064 | 評価結果 cache LRU 戦略（moka） | Phase 1b |
| DS-SW-COMP-065 | Decision 起動ウォームアップ | Phase 1c |
| DS-SW-COMP-066 | PII 4 モジュール構成 | Phase 1b |
| DS-SW-COMP-067 | PII 検出パーサ方式（regex + Aho-Corasick） | Phase 1b |
| DS-SW-COMP-068 | マスキング暗号方式（AES-SIV） | Phase 1b/1c |
| DS-SW-COMP-069 | テナント別 PII ルール取得 | Phase 1c |
| DS-SW-COMP-070 | PII 呼び出しの低レイテンシ設計 | Phase 1b |
| DS-SW-COMP-071 | tracing と OTel 統合 | Phase 1a |
| DS-SW-COMP-072 | メトリクス収集と公開 | Phase 1a |
| DS-SW-COMP-073 | エラーハンドリング方式（thiserror） | Phase 1b |
| DS-SW-COMP-074 | ヘルスチェックエンドポイント | Phase 1a |
| DS-SW-COMP-075 | メモリフットプリント目標 | Phase 1b/1c |
| DS-SW-COMP-076 | unsafe コードの扱い | Phase 1b |
| DS-SW-COMP-077 | CPU bound 処理の切り分け（spawn_blocking） | Phase 1b |
| DS-SW-COMP-078 | 自作層 CI ビルド方式 | Phase 1a |
| DS-SW-COMP-079 | graceful shutdown と状態保全 | Phase 1b |
