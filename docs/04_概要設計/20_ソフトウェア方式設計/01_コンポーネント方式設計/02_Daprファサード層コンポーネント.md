...
**確定フェーズ**: Phase 1a。**対応要件**: FR-T1-INVOKE-\*、NFR-E-AUTH-\*、NFR-D-MON-\*、NFR-D-TRACE-\*。
...
**確定フェーズ**: Phase 1a。**対応要件**: NFR-E-AUTH-\*、NFR-D-TRACE-\*、NFR-E-AUDIT-001。
...
**確定フェーズ**: Phase 1a（基本 publish）、Phase 1b（subscribe 本格化）、Phase 2（Schema Registry 連携）。**対応要件**: FR-T1-PUBSUB-\*、NFR-B-PERF-003。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-INVOKE-\*、NFR-E-AUTH-\*、NFR-E-CONN-\*。
...
**確定フェーズ**: Phase 1b/1c/2。**対応要件**: FR-T1-BINDING-\*、NFR-E-AUTH-\*。
...
**確定フェーズ**: Phase 1a（JWT/Tenant）、Phase 1b（RBAC/RateLimit）、Phase 1c（冪等性）。**対応要件**: NFR-E-AUTH-\*、NFR-E-AUTHZ-\*、DX-GP-\*。

### DS-SW-COMP-037 Log Adapter の構造化ログ方式

Log Adapter は `zerolog` を基盤とし、全ログエントリに `trace_id` / `span_id` / `tenant_id` / `component` / `request_id` / `level` / `msg` を必須フィールドとして含める。出力先は stdout（JSON Lines）のみで、OpenTelemetry Collector DaemonSet が拾い上げて Loki に転送する（Phase 1b では Loki 省略で stdout のみ）。機密情報（Secret 値・PII）は zerolog hook で自動マスキングし、COMP-T1-PII への gRPC 呼び出しで安全性を確保する（[03_自作Rust領域コンポーネント.md](03_自作Rust領域コンポーネント.md) 参照）。

**確定フェーズ**: Phase 1a。**対応要件**: FR-T1-LOG-\*、NFR-D-MON-\*、NFR-G-ENC-\*。
...
**確定フェーズ**: Phase 1c。**対応要件**: FR-T1-DECISION-\*、NFR-E-AUTHZ-\*、NFR-B-PERF-004。
...
- NFR-D-MON-\* / NFR-D-TRACE-\*
- NFR-E-AUTH-\* / NFR-E-AUTHZ-\* / NFR-E-ENC-001 / NFR-E-CONN-\* / NFR-E-AUDIT-001
