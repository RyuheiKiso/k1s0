...
[01_tier1全体コンポーネント俯瞰.md](01_tier1全体コンポーネント俯瞰.md) が 6 Pod の外形と配置を確定したのに対し、本ファイルはファサード層 3 Pod の内部を 1 階層掘り下げる。各 Pod の中で「API Router」「Dapr Adapter」「Log Adapter」「Metrics Emitter」「Policy Enforcer」の 5 モジュールがどの責務を担い、どの順序で呼び出されるかを固定することで、詳細設計フェーズでのモジュール境界の乱れを防ぐ。構想設計 [../../../02_構想設計/02_tier1設計/01_設計の核/01_Dapr隠蔽方針.md](../../../02_構想設計/02_tier1設計/01_設計の核/01_Dapr隠蔽方針.md) が「Dapr を tier1 内部に閉じ込める」方針を確立している前提で、本ファイルはその内部構造を仕様化する。

tier1 公開 API は Dapr SDK の引数をそのまま通すのではなく、5 モジュールの直列パイプラインを通過させることで横断的関心事（認証・テナント・監査・観測・エラー変換）を必ず強制する。このパイプライン設計が緩むと、例えば認証バイパスや OTel span 欠落が発生し、稟議で約束した SLO/SLA と監査要件が崩れる。本ファイルはパイプラインの構造と通過順を方式として確定させる。
...
**確定フェーズ**: Phase 1a。**対応要件**: FR-T1-INVOKE-\*、NFR-E-AC-\*、NFR-E-MON-\*、NFR-D-MTH-\*。
...
**確定フェーズ**: Phase 1a。**対応要件**: NFR-E-AC-\*、NFR-D-MTH-\*、NFR-E-MON-001。
...
**確定フェーズ**: Phase 1a。**対応要件**: DX-TEST-\*、DX-GP-\*。
...
API Router は 5 公開 API を別々の Proto service 定義として公開し、それぞれ `k1s0.v1.service_invoke.ServiceInvokeService` / `k1s0.v1.state.StateService` / `k1s0.v1.pubsub.PubSubService` / `k1s0.v1.binding.BindingService` / `k1s0.v1.feature.FeatureService` の 5 サービスを単一 gRPC サーバ上で登録する。単一ポート（50051）で全 5 サービスを serving し、gRPC Reflection を Phase 1b 以降の Stage 環境で有効化する（Production では無効）。サービス間の依存（例: PubSub 配信前に Feature 判定）は API Router レベルでは混ざらず、必要な場合は Policy Enforcer 経由で共通ロジックを呼ぶ。

**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-INVOKE-\*、FR-T1-STATE-\*、FR-T1-PUBSUB-\*、FR-T1-BINDING-\*、FR-T1-FEATURE-\*。

### DS-SW-COMP-024 Dapr Building Blocks の使い分け

COMP-T1-STATE は Dapr の 5 Building Block を使い分ける（Phase 2 での完成状態）。Service Invoke API は Dapr `serviceinvocation` BB を通じて tier2 マイクロサービス間の gRPC 相互呼び出しを仲介する。State API は Dapr `state` BB で Valkey Cluster に KV 操作する（TTL / ETag / Transaction 対応）。PubSub API は Dapr `pubsub` BB で Kafka topic に publish / subscribe する（CloudEvents 1.0 準拠）。Binding API は Dapr `bindings` BB で 外部 HTTP / SMTP / S3 等への input / output binding を行う。Feature API は Dapr ではなく `flagd` gRPC を直接呼ぶ（Feature Management 用 BB が Dapr 側で stable 化していないため）。各 BB のバージョン固定は `go.mod` で Dapr Go SDK のバージョンピンで管理する。

**確定フェーズ**: Phase 2（State / PubSub / Service Invoke / Binding / Feature の全 5 BB を一括で本格化）。Phase 1a〜1c は COMP-T1-STATE 自体が `k1s0.Log` のみの最小ファサードとして稼働し、State 以下 5 API はスタブで Dapr BB との結線を持たない（[01_MVPスコープ.md](../../../01_企画/03_ロードマップと体制/01_MVPスコープ.md) の MVP-0 / MVP-1a / MVP-1b のいずれにも `k1s0.State` 以下は含まれていないため、Dapr BB の stable 依存も Phase 2 に寄せる）。**対応要件**: FR-T1-STATE-\*、FR-T1-PUBSUB-\*、FR-T1-BINDING-\*、FR-T1-INVOKE-\*、FR-T1-FEATURE-\*。**参照**: 構想設計 ADR-DATA-002（Valkey Cluster）、ADR-MSG-001（Strimzi Kafka）。

### DS-SW-COMP-025 State API の内部モジュール

State API 内部では「etag 検証 → TTL 計算 → Consistency レベル決定 → Dapr state BB 呼び出し」の 4 ステップを経る。etag 検証は Optimistic Concurrency Control で書込衝突を検知し、衝突時は gRPC status `FailedPrecondition` を返す。TTL は tier2 からの指定値を tenant policy でクランプ（最小 1s / 最大 7 日）する。Consistency は Phase 1b では strong 固定、Phase 2 で eventual 切替を Feature Flag 経由で許容する。Dapr `state` BB 呼び出し時のタイムアウトは 200ms に設定し、p99 10ms 目標（NFR-B-PERF-002）の内訳に収まるよう監視する。

**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-STATE-001、FR-T1-STATE-002、FR-T1-STATE-003、NFR-B-PERF-002。

### DS-SW-COMP-026 PubSub API の内部モジュール

PubSub API 内部では「CloudEvents 変換 → Schema Registry 確認（Phase 2）→ Dapr pubsub BB publish」の 3 ステップを経る。CloudEvents 変換は tier2 から渡された JSON を CloudEvents 1.0 の必須ヘッダ（id / source / type / subject / time / datacontenttype / specversion）で包む。Phase 2 で Confluent Schema Registry と接続する場合、publish 前に schema 適合を検証する（Phase 1b では検証省略で tier2 側の責任）。Subscribe 側は Dapr pubsub BB の subscription を API Router で受け、ACK / NACK を Dapr SDK 経由で返す。p99 50ms 目標（NFR-B-PERF-003）に対して CloudEvents 変換 5ms / SR 検証 10ms / Kafka publish 30ms を内訳として配分する。

**確定フェーズ**: Phase 2（publish / subscribe 本格化 + Schema Registry 連携を一括で実施）。PubSub は [01_MVPスコープ.md](../../../01_企画/03_ロードマップと体制/01_MVPスコープ.md) で Phase 2 の「Kafka（Strimzi）」+「Apicurio Registry」導入と同期する。**対応要件**: FR-T1-PUBSUB-\*、NFR-B-PERF-005。

### DS-SW-COMP-027 Feature API の内部モジュール

Feature API は flagd gRPC クライアントを内部に抱え、評価コンテキスト（tenant_id / user_id / environment / segment）を構築して flagd に送る。flagd のレスポンスはローカル cache（5 秒 TTL）に保持し、p99 5ms を達成する。flagd が到達不能な場合は cache から stale 応答を返し、cache も miss の場合は変数ごとにコードに埋めた既定値を返す（graceful degradation）。tenant 別の flag 定義は flagd の sync provider（Phase 1b は file provider、Phase 2 で Kubernetes ConfigMap sync provider）で供給する。

**確定フェーズ**: Phase 1c。**対応要件**: FR-T1-FEATURE-\*、NFR-A-CONT-002、DX-FM-\*。**参照**: 構想設計 ADR-FM-001（flagd）。

### DS-SW-COMP-028 Service Invoke API の内部モジュール

Service Invoke API は Dapr `serviceinvocation` BB を通じて tier2 / tier3 サービス間の gRPC 呼び出しを仲介する。内部では「caller 認証 → callee 存在確認（Dapr app-id 登録確認）→ mTLS 確立（Istio Ambient）→ Dapr invoke」の順で実行する。Istio Ambient との役割分担は、mTLS は Istio 側、認可は Policy Enforcer 側とし、Dapr の mTLS 機能は Phase 1b では無効化して Istio に委譲する（[../../30_共通機能方式設計/](../../30_共通機能方式設計/) 参照）。

**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-INVOKE-\*、NFR-E-AC-001〜005、NFR-E-NW-001〜004。

### DS-SW-COMP-029 Binding API の内部モジュール

Binding API は external resource（外部 HTTP / SMTP / S3 / MQ）への output binding を担う。内部で「binding 定義読込 → Policy Enforcer の tenant 許可確認 → Dapr bindings BB invoke → 結果 wrap」の順で実行する。Phase 1b では HTTP binding のみ、Phase 1c で SMTP / S3 を追加、Phase 2 で MQTT を追加する。bindings 定義は Dapr Component YAML として ConfigMap で管理し、tenant 別の許可は Policy Enforcer 側の policy.rego で制御する（[../../30_共通機能方式設計/](../../30_共通機能方式設計/) 参照）。

**確定フェーズ**: Phase 1b/1c/2。**対応要件**: FR-T1-BINDING-\*、NFR-E-AC-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-SECRETS-\*、NFR-E-ENC-001、NFR-H-KEY-001。**参照**: 構想設計 ADR-SEC-002（OpenBao）。
...
**確定フェーズ**: Phase 1b。**対応要件**: NFR-A-CONT-002、NFR-A-FT-001。
...
**確定フェーズ**: Phase 1b 中に確定。**対応要件**: FR-T1-WORKFLOW-\*、U-WORKFLOW-001。**参照**: 構想設計 ADR-RULE-002。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-WORKFLOW-001、FR-T1-WORKFLOW-002、NFR-A-REC-001。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-WORKFLOW-003、FR-T1-WORKFLOW-004。
...
**確定フェーズ**: Phase 1a（JWT/Tenant）、Phase 1b（RBAC/RateLimit）、Phase 1c（冪等性）。**対応要件**: NFR-E-AC-001〜005、DX-GP-\*。

### DS-SW-COMP-037 Log Adapter の構造化ログ方式

Log Adapter は `zerolog` を基盤とし、全ログエントリに `trace_id` / `span_id` / `tenant_id` / `component` / `request_id` / `level` / `msg` を必須フィールドとして含める。出力先は stdout（JSON Lines）のみで、Phase 1b で導入する OTel Collector Agent DaemonSet が拾い上げて Prometheus（メトリック派生分）に転送し、Phase 1c で Loki が導入された後はログ本体を Loki に集約する（[01_MVPスコープ.md](../../../01_企画/03_ロードマップと体制/01_MVPスコープ.md) の MVP-1a で OTel Collector、MVP-1b で Loki 導入）。Phase 1a では stdout のみで kubectl logs による一次確認に留まる。機密情報（Secret 値・PII）は zerolog hook で自動マスキングし、COMP-T1-PII への gRPC 呼び出しで安全性を確保する（COMP-T1-PII 本体の本格実装は Phase 2 の tier1 自作 Rust 領域投入と同期、詳細は [03_自作Rust領域コンポーネント.md](03_自作Rust領域コンポーネント.md) 参照）。

**確定フェーズ**: Phase 1a（stdout JSON Lines 出力）、Phase 1b（OTel Collector 連携）、Phase 1c（Loki 集約）、Phase 2（PII マスキング同期呼び出し）。**対応要件**: FR-T1-LOG-\*、NFR-D-MON-\*、NFR-G-ENC-\*。
...
**確定フェーズ**: Phase 1a。**対応要件**: FR-T1-TELEMETRY-\*、NFR-D-MON-\*、NFR-B-PERF-\*。
...
**確定フェーズ**: Phase 1a。**対応要件**: NFR-B-CAP-\*、NFR-F-ENV-\*。
...
**確定フェーズ**: Phase 1a。**対応要件**: NFR-C-NOP-002、NFR-H-KEY-001。
...
各階層の所管が明確であることで、障害時の切り分けが「L2 以下の問題は Dapr ログ、L2 以上は k1s0 ログ」の 2 択に収束する。これが Phase 1a〜1c の 2 名運用を成立させる前提である。

**確定フェーズ**: Phase 0（責務境界）、Phase 1a（実装）。**対応要件**: NFR-C-NOP-001、DX-RB-\*。
...
**確定フェーズ**: Phase 1c。**対応要件**: NFR-C-NOP-\*、NFR-H-KEY-001。
...
**確定フェーズ**: Phase 1c。**対応要件**: DX-FM-\*、DX-GP-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-PII-\*、FR-T1-LOG-\*、NFR-G-ENC-\*。
...
**確定フェーズ**: Phase 1b。**対応要件**: FR-T1-AUDIT-\*、NFR-B-PERF-001、NFR-H-INT-001。
...
**確定フェーズ**: Phase 1c。**対応要件**: FR-T1-DECISION-\*、NFR-E-AC-001〜005、NFR-B-PERF-004。
...
| 設計 ID | 内容 | 確定フェーズ |
|---|---|---|
| DS-SW-COMP-020 | ファサード層 5 モジュール構成 | Phase 1a |
| DS-SW-COMP-021 | 呼び出しパイプライン固定順序 | Phase 1a |
| DS-SW-COMP-022 | モジュール間 interface と DI | Phase 1a |
| DS-SW-COMP-023 | STATE API Router の endpoint 分割 | Phase 1b |
| DS-SW-COMP-024 | Dapr Building Blocks の使い分け | Phase 1a/1b/1c |
| DS-SW-COMP-025 | STATE State API 内部モジュール | Phase 1b |
| DS-SW-COMP-026 | STATE PubSub API 内部モジュール | Phase 1a/1b/2 |
| DS-SW-COMP-027 | STATE Feature API 内部モジュール | Phase 1c |
| DS-SW-COMP-028 | STATE Service Invoke API 内部モジュール | Phase 1b |
| DS-SW-COMP-029 | STATE Binding API 内部モジュール | Phase 1b/1c/2 |
| DS-SW-COMP-030 | SECRET Leader Election 内部モジュール | Phase 1b |
| DS-SW-COMP-031 | SECRET Secrets API 内部モジュール | Phase 1b |
| DS-SW-COMP-032 | SECRET OpenBao 接続プールと health check | Phase 1b |
| DS-SW-COMP-033 | WORKFLOW Pluggable Backend | Phase 1b |
| DS-SW-COMP-034 | WORKFLOW task queue sticky 方式 | Phase 1b |
| DS-SW-COMP-035 | WORKFLOW Saga 補償トランザクション | Phase 1b |
| DS-SW-COMP-036 | Policy Enforcer の内部構造 | Phase 1a/1b/1c |
| DS-SW-COMP-037 | Log Adapter の構造化ログ方式 | Phase 1a |
| DS-SW-COMP-038 | Metrics Emitter の RED メトリック方式 | Phase 1a |
| DS-SW-COMP-039 | daprd サイドカー注入方式 | Phase 1a |
| DS-SW-COMP-040 | Dapr Component YAML 管理方式 | Phase 1a |
| DS-SW-COMP-041 | 4 階層コールスタックの責務境界 | Phase 0/1a |
| DS-SW-COMP-042 | Dapr バージョンアップ方式 | Phase 1c |
| DS-SW-COMP-043 | Feature Flag による段階的公開 | Phase 1c |
| DS-SW-COMP-044 | PII マスキングの呼び出し境界 | Phase 1b |
| DS-SW-COMP-045 | Audit への非同期書込 | Phase 1b |
| DS-SW-COMP-046 | Decision への同期呼び出し | Phase 1c |
...
| DS-SW-COMP-141 | Dapr 隠蔽 6 段階多層防御統括 | Phase 1a/1b/2 |
...
- NFR-B-PERF-001 / NFR-B-PERF-002 / NFR-B-PERF-003 / NFR-B-PERF-004 / NFR-B-WL-001 / NFR-B-CAP-\*
