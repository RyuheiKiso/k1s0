...
JTC 情シス向けオンプレ基盤である k1s0 は、tier2 / tier3 開発者に「認証の実装を自分で書かない」ことを約束している。企画書の核心思想「tier1 は tier2 を楽にするために存在する」を実現するには、認証の実装を tier1 側で吸収し、tier2 / tier3 は `tenant_id` / `user_id` を受け取る前提で業務ロジックのみを書けば良い状態を作る必要がある。本ファイルはそのための実装契約を定義する。
...
エンドユーザー認証は Keycloak 25.x（LTS）を採用し、OIDC 1.0 および SAML 2.0 を同時提供する。OIDC は tier3 SPA および tier3 モバイルアプリ向け、SAML 2.0 は JTC 既存基幹システムからの SP Initiated SSO 向けに提供する。Keycloak は Phase 1b 以降で HA 構成（Keycloak Cluster 3 ノード + PostgreSQL バックエンド）とし、Phase 1a は単一 Pod とする。

Keycloak の Realm は `k1s0-platform` を 1 つ設け、テナントは Realm Role で分離する（構想設計で Realm per Tenant ではなく Role per Tenant を選択済み、理由は Realm 数が増えるほど運用 2 名体制の限界を超えるため）。ユーザープロビジョニングは SCIM 2.0 で行い、JTC 既存 Active Directory / LDAP からの自動同期を Phase 2 で確定する（Phase 1a〜1c は手動プロビジョニング）。
...
Phase 1a は Keycloak 標準のパスワード認証のみ、Phase 1b で TOTP（RFC 6238）を管理者ロール向けに有効化、Phase 2 で一般ユーザーにも TOTP を展開し、WebAuthn（FIDO2）を管理者向けに追加する。多要素認証のポリシーは Keycloak の Authentication Flow で宣言的に定義し、Git 管理下の JSON で管理する。
...
ロックアウト閾値の 5 分 10 回は、自動ボットによる総当たり（typical 100 req/s）を実質的に遮断しつつ、正当ユーザーのタイポ連打（典型的に 3〜5 回以下）を誤検知しない範囲で選定した。Phase 2 で Keycloak Adaptive Authentication を有効化し、IP / デバイス / 地理情報による動的ロックアウト閾値調整を導入する計画である。

## 設計 ID 一覧

以降、本章で採番する設計 ID を列挙する。各 ID の詳細は上記各節の該当記述を参照する。

- **DS-CF-AUTH-001**: Keycloak 25.x LTS を OIDC/SAML プロバイダとして採用し、Realm per Platform × Role per Tenant 方式で運用する。確定フェーズ Phase 1a。
- **DS-CF-AUTH-002**: OIDC Authorization Code Flow with PKCE（S256）を tier3 クライアント認証の唯一のフローとして採用する。確定フェーズ Phase 1a。
- **DS-CF-AUTH-003**: Access Token 寿命 15 分、Refresh Token 寿命 24 時間、Authorization Code 寿命 60 秒、Clock Skew 5 秒を固定パラメータとする。確定フェーズ Phase 1a。
- **DS-CF-AUTH-004**: tier1 全 API の gRPC Interceptor で JWT 検証（署名 RS256 / iss / aud / exp / nbf / tenant_id / sub）を必須化する。確定フェーズ Phase 1a。
- **DS-CF-AUTH-005**: JWKS キャッシュ TTL 5 分、Keycloak 鍵ローテーション 180 日、grace period 30 日を固定する。確定フェーズ Phase 1b。
- **DS-CF-AUTH-006**: SPIFFE/SPIRE による X.509 SVID を tier1 内部通信の ID とし、Istio Ambient ztunnel で自動 mTLS 化する。確定フェーズ Phase 1b。
- **DS-CF-AUTH-007**: SVID 有効期限 1 時間、先行ローテーション 30 分周期を固定する。確定フェーズ Phase 1b。
- **DS-CF-AUTH-008**: Refresh Token のブラックリスト機構を Redis で実装し、ログアウト / パスワード変更 / 管理者失効で即時失効を実現する。確定フェーズ Phase 1b。
- **DS-CF-AUTH-009**: MFA を Phase 1b で管理者 TOTP、Phase 2 で一般ユーザー TOTP と管理者 WebAuthn に段階展開する。確定フェーズ Phase 1b / Phase 2。
- **DS-CF-AUTH-010**: 認証失敗 5 分類のマッピングと、一時ロックアウト閾値 5 分 10 回・ロックアウト期間 15 分を固定する。確定フェーズ Phase 1a。
- **DS-CF-AUTH-011**: JTC 既存 Active Directory / LDAP との SAML 2.0 Federation を Phase 2 で確定する。Phase 1a〜1c は手動プロビジョニング + SCIM 2.0 準備。確定フェーズ Phase 2。

## 対応要件一覧

本章は以下の要件 ID に直接対応する。各要件の詳細は [../../03_要件定義/30_非機能要件/E_セキュリティ.md](../../03_要件定義/30_非機能要件/) および機能要件側の認証関連エントリを参照する。

- **NFR-E-AUTH-001** 〜 **NFR-E-AUTH-008**: エンドユーザー認証方式、MFA、パスワードポリシー、セッション管理、フェデレーション。
- **NFR-E-AUTH-009** 〜 **NFR-E-AUTH-012**: サービス間認証、ワークロード ID、mTLS 強制。
- **NFR-E-AUTHZ-001**（部分）: JWT クレーム `tenant_id` / `sub` の検証要件は認可章と重複するが、検証ロジックは本章に帰属。
