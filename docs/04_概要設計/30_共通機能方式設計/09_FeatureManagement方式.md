...
本ファイルは k1s0 全体で利用する Feature Flag（機能フラグ）の評価エンジン、フラグ定義の管理経路、評価条件、パフォーマンス要件、フラグ寿命管理、Kill Switch 機構を確定させる。Feature Flag は機能の段階的リリース・運用時スイッチ・A/B テスト・テナント別先行公開を実現するための運用上の基本ツールであり、不在だと「機能を prod に出して不具合が出たら即ロールバック（Deploy 取り消し）」しか選択肢が無く、運用 採用側の小規模運用ではリスクが許容範囲を超える。

## 本ファイルの位置付け

採用側組織の情シスのリリース運用では、機能リリース = システム停止を伴うデプロイ、という硬直運用が典型的である。k1s0 は Feature Flag を基盤機能として全開発者に提供することで、「デプロイは頻繁、機能公開はフラグで段階的」という運用に移行する。これにより Mean Time To Recovery が短縮され、運用 採用側の小規模運用でも耐える設計に到達する。
...
flagd を採用する理由は、OpenFeature 標準に準拠しているためベンダロックインが無いこと、Single Binary で軽量稼働し運用 採用側の小規模運用に適合すること、CNCF Sandbox プロジェクトでエンタープライズ事例（Dynatrace 他）が蓄積されていること、の 3 点である。LaunchDarkly 等の SaaS は閉域運用前提の k1s0 方針（設計原則 10）と不適合のため採用しない。

### 段階別構成

リリース時点 は flagd スタンドアロン（単一 Pod、JSON ファイル直読み）で起動し、運用蓄積後で GitOps 化（Git → Argo CD → ConfigMap → flagd watch）、採用後の運用拡大時 で GUI エディタ（flagd-ui または自作 Backstage プラグイン）を導入する計画である。

リリース時点 は flagd プロセス 1 つで全フラグ評価を捌く。中規模 150 RPS × 1 リクエスト平均 5 フラグ評価 = 750 ops/sec は flagd 単一インスタンスで余裕に対応できる（flagd 公式ベンチ 10,000 ops/sec 以上）。運用蓄積後は HA 構成（flagd 2 Pod + Kubernetes Service）とする。
...
セキュリティインシデント発生時や壊滅的バグ検知時に、全 Feature Flag を即時 default variant に戻す「Kill Switch」機構を提供する。Kill Switch は Platform Admin ロールのみが起動可能で、起動には 複数名承認（Platform Admin（複数名）の同意）を必須とする。
...
**設計項目 DS-CF-FM-014 Production Ops Flag 複数名承認フロー**
...
Flag による機能無効化（Kill Switch 含む）は、通常の変更と同様に監査 API に `feature.rollback` イベントとして記録する。緊急オフの場合も、事後 24 時間以内に PR を起票し、Git log + Audit API の両方で痕跡を残すことを運用ルール化する。四半期ごとに Product Council で Flag 変更監査レビューを開催し、過去 3 か月の Production 変更件数・緊急オフ件数・未解決の期限切れ Release Toggle 件数をレビューする。監査レビューの指摘事項は Backstage の検索可能インデックスに追加し、次回レビューで追跡する。確定段階は リリース時点。

## 設計 ID 一覧

- **DS-CF-FM-001**: flagd（CNCF Sandbox、OpenFeature 準拠）を Feature Flag 評価エンジンとして採用、リリース時点 スタンドアロン → リリース時点 HA → 採用後の運用拡大時 GUI 段階導入。確定段階 リリース時点 / リリース時点 / 採用後の運用拡大時。
- **DS-CF-FM-002**: Feature Flag を 4 種類（Release / Ops / Experiment / Permission）に分類、各種別に寿命と責任主体を紐付けする。確定段階 リリース時点。
- **DS-CF-FM-003**: Release Toggle の寿命は 90 日以内、CI で `expires_at` 超過を検出して PR ブロックする。確定段階 リリース時点。
- **DS-CF-FM-004**: Flag 定義は Git 管理（`infra/feature-flags`）+ Argo CD + ConfigMap 反映、ConfigMap → flagd 反映遅延 p99 10 秒以内。確定段階 リリース時点。
- **DS-CF-FM-005**: 評価条件は JSONLogic、Evaluation Context 6 種（tenant_id / user_id / env / percentage_rollout / date_range / source_ip_class）に限定。確定段階 リリース時点。
- **DS-CF-FM-006**: Flag 評価 p99 1 ミリ秒以内、in-process provider でネットワーク不要評価を実現。確定段階 リリース時点。
- **DS-CF-FM-007**: Kill Switch は Platform Admin（複数名）承認必須、起動から反映 10 秒以内、監査記録 + Slack 通知必須。確定段階 リリース時点。
- **DS-CF-FM-008**: CI 静的チェック `k1s0-feature-flag-lint` で期限切れ / 孤立 / ハードコード / 所有者不在を検出、全 PR に適用。確定段階 リリース時点。
- **DS-CF-FM-009**: Flag 削除は 5 段階標準フロー（コード削除 → prod 反映 → 1 週間経過確認 → Flag 削除 → ConfigMap 更新）を遵守する。確定段階 リリース時点。
- **DS-CF-FM-010**: tier1 Feature API は OpenFeature SDK を gRPC ラップして提供、tier2 / tier3 は flagd を意識せず Feature Flag 評価可能。確定段階 リリース時点。
- **DS-CF-FM-011**: A/B テスト基盤（一貫ハッシュ割当 / OTel Metrics 統合 / 統計有意性判定 / Backstage 実験登録 / A/A 偏り検査）を整備、SHOULD 優先度で 採用後の運用拡大時に稼働。確定段階 採用後の運用拡大時。
- **DS-CF-FM-012**: 評価パフォーマンス目標（p99 10ms、キャッシュヒット率 90%、push 伝播 5 秒）と計測ダッシュボード `k1s0-feature-flag-perf`、ネットワーク障害時フェイルセーフ。確定段階 リリース時点。
- **DS-CF-FM-013**: Flag 変更の監査ログ 7 年保全、Git log + tier1 Audit API + PostgreSQL/MinIO の 3 層記録、整合ずれ検出ダッシュボード。確定段階 リリース時点。
- **DS-CF-FM-014**: Production Ops Flag 複数名承認フロー（SRE + Security Officer）、Release / Experiment / Permission Toggle の権限境界定義。確定段階 リリース時点。
- **DS-CF-FM-015**: Flag ロールバック記録と四半期監査レビューサイクル、Kill Switch 発動も同様の監査対象。確定段階 リリース時点。
