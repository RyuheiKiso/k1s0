...
採用側組織の情シスが従来抱えていた問題は「サービス A を呼び出したら遅かった、でも A 内部のどこで遅かったか分からない」というブラックボックス状態である。分散トレーシングはこの問題を、API ごとの Span ツリーを可視化することで解消する。企画書で約束した「障害原因特定時間の短縮」（MTTR 短縮）の主要実現手段である。
...
リリース時点 は AGPL OSS（Tempo）を採用しないため、Jaeger All-in-One（Apache 2.0）をスタンドアロン運用する。Jaeger All-in-One は単一プロセスで受信・格納・UI を提供し、単一 VM 構成の リリース時点 と相性が良い。ストレージは Jaeger 内蔵の badger DB、保管期間は 7 日とする。

### 運用蓄積後のバックエンド

リリース時点 で Grafana Tempo に移行する。Tempo は AGPL OSS のため ADR-0003 の 5 原則を満たす構造で運用する。
...
- **DS-CF-TRACE-001**: OpenTelemetry Protocol（OTLP）を tier1 / tier2 / tier3 の標準計装プロトコルとして採用する。独自プロトコル禁止。確定段階 リリース時点。
- **DS-CF-TRACE-002**: 言語別 SDK は公式 OpenTelemetry を採用し、`k1s0-otel-<lang>` ラッパで初期化を標準化する。Go v1.26+ / Rust v0.23+ / C# v1.8+ / TS v1.24+。確定段階 リリース時点。
- **DS-CF-TRACE-003**: W3C Trace Context（traceparent / tracestate）をサービス間必須ヘッダとし、Kafka メッセージヘッダにも含める。確定段階 リリース時点。
- **DS-CF-TRACE-004**: サンプリングは parent-based head 10% + tail-based（error / latency / feature-flag で 100% 補足）のハイブリッド、tail バッファ 30 秒。確定段階 リリース時点。
- **DS-CF-TRACE-005**: k1s0 標準属性 7 項目（tenant_id / tier / api / version / env / error.code / error.type）を全 Span で設定する。確定段階 リリース時点。
- **DS-CF-TRACE-006**: 高カーディナリティ値（resource_id 生値 / user_id / 動的値）は Span 属性に入れず、Span Event またはログ側で保持する。確定段階 リリース時点。
- **DS-CF-TRACE-007**: リリース時点 は Jaeger All-in-One（保管 7 日 / badger DB）、運用蓄積後は Grafana Tempo（保管 30 日 / Critical Path 90 日 / MinIO バックエンド）。確定段階 リリース時点 / リリース時点。
- **DS-CF-TRACE-008**: AGPL OSS（Tempo / MinIO）は ADR-0003 の 5 原則（プロセス分離 / 非リンク / 無改変 / 境界遮断 / 監査可検証性）を満たす構造で運用する。確定段階 リリース時点。
- **DS-CF-TRACE-009**: trace_id はログの必須フィールドと一致し、Grafana で Trace ↔ Log ↔ Metrics の pivot を実現する。Exemplar を有効化する。確定段階 リリース時点。
- **DS-CF-TRACE-010**: 想定 Span 量は中規模 150 RPS で日次 2,500 万 Span（圧縮後 20 GB / 日）、30 日保管で 600 GB をストレージ予算とする。確定段階 リリース時点。
