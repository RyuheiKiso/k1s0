...
本ファイルは要件定義書 [../../03_要件定義/30_非機能要件/A_可用性/](../../03_要件定義/30_非機能要件/A_可用性.md) の NFR-A 系 13 要件を受けて、k1s0 が稼働率 SLA 99%（月 7.2 時間許容）および内部 SLO 99.9%（月 43 分許容）を達成するための方式を確定する。Pod 単位・ノード単位・拠点単位・DC 全損の 4 段階の障害シナリオに対し、どのコンポーネントがどの冗長度でどの RPO / RTO を実装するかを設計する層である。
...
稼働率の計算式は ITU-T 標準 `availability = uptime / (uptime + downtime)` を採用し、「計画停止（事前周知のメンテナンス窓）」は uptime に含めず、「計画外停止（障害）」のみをダウンタイムとして集計する。計画停止を含めると SLA 値が機械的に低下し、採用検討で約束した 99% の意味が曖昧になる。計画停止は年 12 時間以内（月 1 時間以内）を別途 SLA として合意する。

**設計項目 DS-NFR-AVL-001 稼働率目標と許容ダウンタイム**

- 外部 SLA: 99.0%（月間ダウンタイム 7 時間 12 分以内、年間 87 時間 36 分以内。企画書と要件定義 NFR-A-CONT-001 で確定。）
...
- 確定段階: 採用初期でパイロット稼働時に計測開始、リリース時点 で SLO レビュー実施。
...
- 確定段階: リリース時点で tier1 全 Pod に実装 / データ層 Pod へ拡張。
...
- 確定段階: リリース時点 で全 tier1 Pod および全データ Pod に PDB 付与。
...
kubeadm の制御プレーンは 3 ノード構成で etcd クォーラムが 2 ノードで成立するため、1 ノード喪失では API Server 稼働継続となる。ただし 2 ノード喪失では etcd クォーラム喪失となり、全 API 不可の状態（実質クラスタ全損）に陥る。この構造的制約を受け入れ、2 ノード同時喪失は DC 全損扱いで 採用後の運用拡大時 のマルチ DC 設計で対処する。
...
- 確定段階: リリース時点 で tier1 / データ層すべてに TSC 付与。
...
- 確定段階: リリース時点 検証 / で演習実施。

## データ DR 方式と RPO

データ層の可用性は Pod 冗長度に加えてバックエンド固有のレプリケーションで確保する。各 OSS の RPO は以下で確定している（要件定義 NFR-A-FT-002 / NFR-A-CONT-003 / NFR-A-DR-002）。
...
- 確定段階: リリース時点で CloudNativePG デプロイ / DR 演習。
...
- MirrorMaker2（採用後の運用拡大時）: 2 拠点間で非同期レプリケーション、RPO 1 秒。
- 確定段階: リリース時点 で 3 ブローカ実装、採用後の運用拡大時 で MirrorMaker2 追加。
...
- Bucket Replication: 採用後の運用拡大時 で 2 拠点間双方向レプリケーション、RPO 1 時間（非同期スケジュール）。
- バージョニング: 有効化、誤削除から 30 日間復元可能。
- Object Lock: 監査 WORM バケットのみ有効化、保管期限 7 年。
- 確定段階: リリース時点 で単一拠点 EC、採用後の運用拡大時 で拠点間レプリケーション。
...
| 2 ノード同時死 | — | クラスタ全損扱い | 採用後の運用拡大時 マルチ DC 前提 | — | — |
| ゾーン死（同 DC 内） | 採用後の運用拡大時 導入 | Operator + 手動 | 採用後の運用拡大時 要件 | — | — |
...
DR は 採用初期は単一 DC のため、DC 全損想定では「別 DC の既存 VM 資産にデプロイして復旧」というシナリオになる。採用後の運用拡大時 で 2 DC マルチクラスタ化を導入し、DC 間 Active-Standby で RTO 4 時間を自動化する。

![3 AZ クラスタ冗長と DR 戦略トポロジー](img/ha_dr_topology.svg)

上図は Primary DC の 3 AZ 冗長配置と DR 拠点への非同期転送を同一ビューに重ねた全体図である。左側 Primary DC 内では AZ 1 / AZ 2 / AZ 3（Node A/B/C）が破線で分離され、tier1 API Pod が `topologySpreadConstraints` により 1:1:1 で分散配置される。データ層の CNPG Primary + 同期 Replica + 非同期 Replica、Strimzi Broker 3、Valkey Master + Replica 2 + Sentinel 3、OpenBao Raft 3、MinIO EC 3+2、etcd 3 は全て AZ 間で冗長化し、1 ノード喪失で縮退稼働可能な構成を守る。2 ノード同時喪失は etcd クォーラム喪失に相当するため、本図では対象外とし 採用後の運用拡大時 マルチ DC で対処する。

右側 DR 拠点は リリース時点 の冷スタンバイ VM × 3 と、採用後の運用拡大時 で追加される MinIO Bucket Replication（RPO 1h）・barman cloud WAL（RPO 数秒）・Kafka MirrorMaker 2（RPO 1s）・etcd 日次 snapshot（RPO 24h）の非同期転送経路を描く。紫の点線がバックエンド別の非同期レプリケーション、灰の点線が制御プレーン（etcd）のスナップショット転送である。Argo CD は GitOps desired state の単一ソースとして DR 拠点側からも Primary を再構成可能で、障害時は冷スタンバイ VM に kubeadm init 後すぐに全リソースを再適用する。

赤枠の RTO タイムボックスは DS-NFR-AVL-012 / 014 / 015 / 016 に対応し、5 ステップ合計 4 時間で復旧する手順を固定する。各ステップは目標時間の 1.5 倍を超えた時点で Incident Commander へ自動エスカレーションし、演習結果は Grafana `k1s0-dr-drill` に 6 ヶ月トレンドで蓄積する。バックエンド別 RPO 一覧は同図右下にあり、PostgreSQL は同期 0s / PITR 5s、Valkey は 1s、Kafka ISR は 0s、MinIO は リリース時点 で 24h / 採用後の運用拡大時 で 1h、etcd は 24h という階層的な目標を採る。これは「業務データは秒、クラスタ状態は時間単位」という設計思想を 1 枚で示しており、復旧作業のステップ順と RPO 制約の両方を同時に意識させる目的で集約した。

**設計項目 DS-NFR-AVL-012 リリース時点 DR 手順（単一 DC）**

- 前提: バックアップはすべて MinIO の別 DC レプリケーション済み Bucket に存在（採用後の運用拡大時）。リリース時点 は同 DC 内の冷スタンバイ VM 3 台を用意。
...
- RTO: 4 時間（SRE × 2 時間並行作業想定）。
- RPO: 最長 24 時間（MinIO 非レプリケーション時）、採用後の運用拡大時 レプリケーション後は 1 時間。
- 確定段階: リリース時点 で手順書化、採用後の運用拡大時 で半自動化（Argo CD ApplicationSet + 事前スクリプト）。

**設計項目 DS-NFR-AVL-013 DR 演習の定期実施**

- リリース時点: 初回 DR 演習を リリース時点 末に実施。SREが Runbook 単体で復旧できるか検証。
- 採用後の運用拡大時: 四半期ごとに演習、年 1 回は通しの DC 全損シナリオを実施。
- 演習結果は [../55_運用ライフサイクル方式設計/02_インシデント対応方式.md](../55_運用ライフサイクル方式設計/02_インシデント対応方式.md) の体制と合流し、Runbook を継続更新する。
- 確定段階: リリース時点 初回、採用後の運用拡大時定期化。
...
- 確定段階: リリース時点 で Runbook 化、採用後の運用拡大時 で半自動化スクリプト化。
...
- 演習: リリース時点 DR 演習で Primary 強制 kill → PITR で 5 秒前まで復元できることを確認。
- 確定段階: リリース時点で CloudNativePG 設定 / 演習。
...
- 確定段階: リリース時点 で CronJob 化、採用後の運用拡大時 で 2 拠点同期。

**設計項目 DS-NFR-AVL-017 マルチリージョン DR の 採用側の全社ロールアウト 方針**

要件定義 NFR-A-DR-004 は 採用側の全社ロールアウト の WON'T 要件だが、概要設計では「採用側の全社ロールアウト で採用する候補技術」を事前に識別しておかないと、採用後の運用拡大時〜4 の設計選択で後戻りできない経路に入るリスクがある。したがって本設計項目では方式候補のみ確定し、詳細数値は 採用側の全社ロールアウト 再検討とする。

- 候補構成: Argo CD マルチクラスタ管理 + PostgreSQL 論理レプリケーション + Kafka MirrorMaker 2 + MinIO Bucket Replication。
- 候補 RPO/RTO: RPO 1 時間 / RTO 1 時間（採用側の全社ロールアウト で再確定）。
- 前提制約: 企画書の試算範囲外のため、採用側の全社展開期 末に別途の予算申請で予算確保。
- 採用後の運用拡大時〜4 で採用する技術は全てマルチリージョン対応可能なもの（MirrorMaker 2 対応の Strimzi Operator など）に限定し、採用側の全社ロールアウト での不可逆な技術入替を回避。
- 確定段階: 採用側の全社ロールアウト 判断、本章は方針のみ。

**設計項目 DS-NFR-AVL-018 Workflow 実行の永続化と再開性**

要件定義 NFR-A-CONT-004 は「ワークフロー実行が途中で停止しても再開可能」を約束する。採用初期は Dapr Workflow が Actor + Reminder を使って State を Valkey / PG に保持し、Worker Pod 再起動後も途中ステップから再開する。採用後の運用拡大時 で Temporal へ移行する場合も、Workflow History が Temporal Server 側で保持されるため同等の永続性を維持する。

- Dapr Workflow（リリース時点）: Actor State を PostgreSQL バックエンドに置換し、Valkey 揮発のリスクを排除（既定は Valkey だが監査要件で PG に固定）。
- Reminder 再生成: Pod 再起動時に pending reminders を PG から読み直して Scheduler に再登録。
- Temporal（採用後の運用拡大時）: History Service 3 レプリカ、Persistence Cassandra/PostgreSQL 構成。
- 再開性検証: リリース時点 の DR 演習で「Workflow 中断 → クラスタ再起動 → 残ステップ実行」の一連を確認。
- 確定段階: リリース時点 Dapr Workflow 実装、採用後の運用拡大時 Temporal 切替。
...
- 確定段階: リリース時点で OTel Agent サイドカー標準化 / 負荷試験。
...
- 確定段階: リリース時点で 3 段フォールバック実装 / 演習検証。
...
- 年次監査: 外部委託でアクセシビリティ監査を 採用後の運用拡大時に実施。
- 確定段階: リリース時点 で Backstage 実装、採用後の運用拡大時 で外部監査。
...
- NFR-A-CONT/FT/DR/REC（可用性・DR 全 13 要件）
  - NFR-A-CONT-001 稼働率 99% SLA → DS-NFR-AVL-001
  - NFR-A-CONT-002 OpenBao 障害時の degrade 稼働 → DS-NFR-AVL-001
  - NFR-A-CONT-003 Valkey/Kafka/PostgreSQL 影響限定 → DS-NFR-AVL-008
  - NFR-A-FT-001 単一 Pod 障害時の自動復旧 → DS-NFR-AVL-002 / DS-NFR-AVL-003
  - NFR-A-FT-002 Kafka ブローカー 1 台障害時の継続性 → DS-NFR-AVL-009
  - NFR-A-FT-003 PostgreSQL フェイルオーバー → DS-NFR-AVL-007
  - NFR-A-FT-004 Istio Ambient ztunnel 障害時の Pod 通信 → DS-NFR-AVL-006
  - NFR-A-DR-001 RTO 4 時間（クラスタ全壊）→ DS-NFR-AVL-012
  - NFR-A-DR-002 RPO 数秒（PostgreSQL）→ DS-NFR-AVL-007
  - NFR-A-DR-003 RPO 24 時間（etcd）→ DS-NFR-AVL-016
  - NFR-A-DR-004 マルチリージョン DR（採用側の全社ロールアウト）→ DS-NFR-AVL-012
  - NFR-A-REC-001 バックアップ復旧訓練（年 2 回）→ DS-NFR-AVL-013
  - NFR-A-REC-002 Runbook 充足率 → DS-NFR-AVL-013
...
- NFR-A-DR-003 RPO 24 時間（etcd）→ DS-NFR-AVL-016
- NFR-A-DR-004 マルチリージョン DR（採用側の全社ロールアウト）→ DS-NFR-AVL-017
