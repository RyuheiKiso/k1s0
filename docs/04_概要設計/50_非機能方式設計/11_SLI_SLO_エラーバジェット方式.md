...
Google SRE 本の「エラーバジェットを全部使い切ってもいい、ただし使い切ったら新機能開発を止めて信頼性向上に回す」という契約文化を k1s0 でも採用する。これは開発と運用の衝突を数値で解決するメカニズムであり、採用側の小規模運用を守る命綱でもある。
...
- 確定段階: リリース時点で Envoy + 内部 / Synthetic 統合。
...
- 確定段階: リリース時点で 全 API 実装 / 初回 SLO レビュー。
...
- 残予算: 100% − Consumed。
- 確定段階: リリース時点 で recording rule 実装。
...
- 確定段階: リリース時点で overview / 詳細・月次レポート。
...
- SLA 99.0%: 外部コミット、採用側組織の顧客との契約。月間 7.2 時間までのダウンタイム許容。
...
- 確定段階: リリース時点 末の初回発行、以降毎年。

## SLI 計測方式の具体化

要件定義 NFR-I-SLI-001 は「全 tier1 公開 API で Availability / Latency / Freshness / Correctness の 4 SLI を Prometheus 互換メトリクスで計測（リリース時点 MUST）」を採番する。DS-NFR-SLO-001 では 3 種類（Availability / Latency / Quality）を定義したが、要件側は 4 SLI を明示しているため、Freshness と Correctness を独立した SLI として追加する。
...
- 確定段階: リリース時点で Availability / Latency / Freshness / Correctness。
...
- 確定段階: リリース時点で Log SLO（`k1s0.Log` が 採用初期 の唯一の tier1 Go 実装 API のため） / Telemetry SLO、採用後の運用拡大時 で残 9 API（Service Invoke / State / PubSub / Secrets / Binding / Workflow / Decision / Audit-Pii / Feature）の SLO。SLO 目標値自体は要件定義書 NFR-I-SLO-001〜011 で リリース時点に固定済みで、本確定段階は SLO の**運用開始時期**を指す。
...
- Kubernetes API Server (NFR-I-SLO-101): 99.95% 稼働、kubectl 応答 p99 < 500ms（リリース時点 は 99.9% 暫定 / で 99.95% 昇格）。
- Istio Ambient ztunnel (NFR-I-SLO-102): レイテンシ増加 p99 < 5ms（採用後の運用拡大時 SHOULD）。tier1 API p99 の積算モデル（DS-NFR-PERF-001）に収まる制約。
- Longhorn (NFR-I-SLO-103): IOPS 低下 < 20%、復旧 RTO < 10 分（リリース時点 MUST）。データ層の I/O 性能を保証。
- MetalLB (NFR-I-SLO-104): VIP フェイルオーバー < 30 秒（リリース時点 MUST）。外部入口の切替性能。
- Keycloak (NFR-I-SLO-105): 99.95% 稼働、トークン発行 p99 < 200ms（リリース時点 MUST）。認証は全 API の前段のため厳しめ。
- OpenBao (NFR-I-SLO-106): 99.9% 稼働、シークレット取得 p99 < 100ms（リリース時点 MUST）。
- Argo CD Sync (NFR-I-SLO-107): Git push から apply 完了まで p99 < 2 分（リリース時点 MUST、採用初期 の GitOps 導入に合わせる）。GitOps のリードタイム保証。要件定義書側の MUST 採番は リリース時点 だが、Argo CD 自体が [企画書.md](../../01_企画/企画書.md) の 採用初期（リリース時点）初投入のため、概要設計側は リリース時点 で SLO 運用を開始する（リリース時点.3 要件改訂時に要件定義側の 段階指定も再調整予定）。
- 個別ダッシュボード: Grafana `k1s0-infra-slo` で 7 基盤 OSS の SLO を統合表示。
- 確定段階: リリース時点で Argo CD / Longhorn / MetalLB（いずれも 採用初期 投入） / Keycloak HA / OpenBao（採用初期 で HA 化または初投入）、リリース時点 後半で k8s API Server SLO 昇格（99.9% → 99.95%）、採用後の運用拡大時 で Istio Ambient。
...
要件定義 NFR-I-EB-002 は「バジェット超過時のリリース凍結プロセスを Runbook で整備（リリース時点 MUST）」を要求する。DS-NFR-SLO-007 の自動化と合わせて、Runbook-SLO-FREEZE-001 を整備する。
...
- 確定段階: リリース時点 実装、運用蓄積後係数調整。

**設計項目 DS-NFR-SLO-021 月次マージン層別消費レビュー**

要件定義 NFR-I-EB-004 は「月次マージン（SLA − SLO）の層別消費実績をレビュー、閾値 70% 超の層は翌月の体質改善タスクを OKR に組込み（リリース時点 MUST）」を要求する。層別消費を可視化しないと、どこで予算を食ったかを特定できず改善が回らない。
...
- 確定段階: リリース時点 で月次レビュー開始、採用後の運用拡大時 で四半期追跡。

## SLA 契約条項の具体化

要件定義 NFR-I-SLA-001 は「対外 SLA は SLO より 0.9 ポイント緩い設定（tier1 全般 99%）で BC-LGL-005 に明文化、違反時ペナルティ条項を契約書に含める（リリース時点 契約締結時 MUST）」を採番する。DS-NFR-SLO-013 で役割分担は設計済みだが、契約書条項の具体化が不足する。
...
- 免責条項: 天災・採用側組織の上位 NW 障害・顧客起因の障害は除外。
- 契約書レビュー: リリース時点 の契約締結前に法務部門 + Product Council 承認、BC-LGL-005 と整合。
- 違反時の運用: SLA 違反発生時は Runbook-SLA-BREACH-001 に従い、クレジット自動発行 + 再発防止 Post-Mortem 公開。
- 確定段階: リリース時点 契約締結時 MUST。
...
- NFR-B-PERF-001（tier1 API p99 500ms）、NFR-B-PERF-003（State Get 10ms）、NFR-B-PERF-004（Decision 1ms）、NFR-B-PERF-005（PubSub Publish 50ms）の p99 性能 SLO → DS-NFR-SLO-003
- NFR-B-PERF-002（tier1 API スループット 150 RPS SLO）→ DS-NFR-SLO-003
