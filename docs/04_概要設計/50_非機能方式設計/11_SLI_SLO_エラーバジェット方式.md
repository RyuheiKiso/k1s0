...
本ファイルは要件定義書の SLO 関連要件（NFR-I 系 8 要件および NFR-A-001/002 と連携）を受けて、tier1 11 API の SLI 定義、SLO 目標、Error Budget Policy、Burn Rate Alert の方式を確定する。[04_監視と観測性方式.md](04_監視と観測性方式.md) が監視の**実装**を扱うのに対し、本章は「何を約束し、その約束を守るためにどう運用するか」の**契約層**を扱う。
...
- 確定フェーズ: Phase 1a で 7 API（Service Invoke / State / PubSub / Secrets / Log / Telemetry / Decision）、Phase 1b で残 4 API。
...
- Argo CD Sync (NFR-I-SLO-107): Git push から apply 完了まで p99 < 2 分（Phase 1a MUST）。GitOps のリードタイム保証。
- 個別ダッシュボード: Grafana `k1s0-infra-slo` で 7 基盤 OSS の SLO を統合表示。
- 確定フェーズ: Phase 1a Argo CD、Phase 1b Longhorn / MetalLB / OpenBao、Phase 1c k8s / Keycloak、Phase 2 Istio Ambient。
...
- NFR-I-001（可用性指標）→ DS-NFR-SLO-001〜003
- NFR-I-002（Latency SLO）→ DS-NFR-SLO-003
- NFR-I-003（測定点）→ DS-NFR-SLO-002
- NFR-I-004（Error Budget）→ DS-NFR-SLO-005〜007
- NFR-I-005（Burn Rate）→ DS-NFR-SLO-008 / DS-NFR-SLO-009
- NFR-I-006（SLO ダッシュボード）→ DS-NFR-SLO-010
- NFR-I-007（SLO レビュー）→ DS-NFR-SLO-011 / DS-NFR-SLO-012
- NFR-I-008（SLA 関係）→ DS-NFR-SLO-013 / DS-NFR-SLO-014
- NFR-A-001（SLA 99%）→ DS-NFR-SLO-013
- NFR-A-002（SLO 99.9%）→ DS-NFR-SLO-005 / DS-NFR-SLO-013
- NFR-B-001〜005（個別 API 性能）→ DS-NFR-SLO-003
