...
本設計は構想設計 ADR-MIG-001（バッチ + CDC 二段構え方針）と ADR-MIG-002（デュアルライト期間を許容する並行稼働）を前提として、Phase 1a（MVP-0）から Phase 2 にかけてどのデータをどの順序で、どの道具で、どういう検証を経て移すかを具体化する。要件定義書 MIG-DATA-001〜003 / MIG-SCHEMA-001〜002 / MIG-VALIDATE-001〜002 と NFR-D-001〜D-011 を対応先とする。
...
既存データを k1s0 側にそのまま入れても動作しないため、3 軸の変換を設計段階から盛り込む。第 1 軸は文字コード変換で、Shift_JIS / EUC-JP / CP932 を UTF-8（NFC 正規化）へ統一する。変換には ICU ライブラリを使い、機種依存文字（いわゆる IBM 拡張漢字）の扱いは Unicode CJK Compatibility Ideographs への写像で対応する。写像テーブルは [../../03_要件定義/00_要件定義方針/07_データ辞書.md](../../03_要件定義/00_要件定義方針/07_データ辞書.md) で管理する。
...
| --- | --- | --- | --- |
| DS-MIG-DATA-001 | バッチ + CDC 二段構え移行方針 | Phase 1a | MIG-DATA-001 / NFR-D-001 |
| DS-MIG-DATA-002 | RDB 3 系統（SQL Server / Oracle / MySQL）→ PostgreSQL | Phase 1a | MIG-DATA-002 / NFR-D-002 |
| DS-MIG-DATA-003 | ファイル系 → MinIO 移行（NFKC 正規化） | Phase 1b | MIG-DATA-003 / NFR-D-003 |
| DS-MIG-DATA-004 | メッセージ系 → Kafka 移行（Avro 変換） | Phase 1c | MIG-DATA-004 / NFR-D-004 |
| DS-MIG-DATA-005 | 文字コード変換（Shift_JIS → UTF-8 NFC） | Phase 1a | MIG-SCHEMA-001 / NFR-D-005 |
| DS-MIG-DATA-006 | タイムゾーン変換（JST → UTC） | Phase 1a | MIG-SCHEMA-001 / NFR-D-006 |
| DS-MIG-DATA-007 | スキーマ変換（列追加/削除/分割/結合） | Phase 1a | MIG-SCHEMA-002 / NFR-D-007 |
| DS-MIG-DATA-008 | マッピング定義 YAML 標準書式 | Phase 1a | MIG-SCHEMA-002 / NFR-D-008 |
| DS-MIG-DATA-009 | 3 段階検証（行数・チェックサム・サンプル） | Phase 1a | MIG-VALIDATE-001 / NFR-D-009 |
| DS-MIG-DATA-010 | デュアルライト方式（CDC + 参照切替） | Phase 1b | MIG-VALIDATE-002 / NFR-D-010 |
| DS-MIG-DATA-011 | Phase 2 Debezium アップグレード計画 | Phase 2 | MIG-DATA-001 / NFR-D-011 |
