...
- `DS-CTRL-RETRY-001`: リトライ対象の分類（HTTP / gRPC エラー判定）。確定フェーズ: Phase 1b。
- `DS-CTRL-RETRY-002`: 指数バックオフ + full jitter の数値仕様（base 100ms、cap 30s、attempts 5）。確定フェーズ: Phase 1b。
- `DS-CTRL-RETRY-003`: 最大試行回数のユースケース別デフォルト。確定フェーズ: Phase 1b。
- `DS-CTRL-RETRY-004`: 429 `Retry-After` ヘッダ尊重方式。確定フェーズ: Phase 1b。
...
- `DS-CTRL-RETRY-005`: サーキットブレーカー状態機械（Closed / Open / HalfOpen）。確定フェーズ: Phase 1b。
- `DS-CTRL-RETRY-006`: サーキットブレーカー閾値（失敗 10 回 / 1 分 / 失敗率 50%、Open 30 秒）。確定フェーズ: Phase 1b。
- `DS-CTRL-RETRY-007`: 実装ライブラリ選定（Go: `sony/gobreaker`、Rust: `failsafe` 互換自前実装）。確定フェーズ: Phase 1b。
...
- `DS-CTRL-RETRY-008`: Bulkhead パターンの依存先別プール分離。確定フェーズ: Phase 1b。
- `DS-CTRL-RETRY-009`: プール上限到達時の `429` 即時応答方式。確定フェーズ: Phase 1b。
...
- `DS-CTRL-RETRY-010`: Istio Ambient リトライ disable（tier1 との二重回避）。確定フェーズ: Phase 1b。
- `DS-CTRL-RETRY-011`: リトライ時の冪等キー維持ルール。確定フェーズ: Phase 1b。
...
- `DS-CTRL-RETRY-012`: Fallback 対応 API の選定基準と Feature Flag 連動。確定フェーズ: Phase 1b。
- `DS-CTRL-RETRY-013`: Fallback 応答の `X-Fallback-Reason` ヘッダ仕様。確定フェーズ: Phase 1b。
...
- `DS-CTRL-RETRY-014`: リトライ・サーキットブレーカー観測メトリクス。確定フェーズ: Phase 1b。
- `DS-CTRL-RETRY-015`: Grafana アラートルール。確定フェーズ: Phase 1b。
