...
ただし「2 エンジン併用」は運用工数の増加を招く。[../../01_企画/03_運用工数試算.md](../../01_企画/企画書.md) で合意した年 0.5 人月の増分を本章の運用設計で吸収する。これを超える複雑化は本章の判定ロジックを簡素化することで対処する。
...
具体的には、tier1 が提供する Workflow DSL（決定表による宣言的定義 + Activity 関数定義）を、Dapr Workflow / Temporal それぞれのランタイムに変換する内部トランスパイラを実装する。リリース時点 で Dapr Workflow のみ対応、採用後の運用拡大時 で Temporal 対応を追加する。
...
- `DS-CTRL-WF-001`: 振り分け判定軸（実行時間・スループット・ユース区分）。確定段階: リリース時点。
- `DS-CTRL-WF-002`: ZEN Engine JDM 形式の決定表による振り分けロジック。確定段階: リリース時点。
- `DS-CTRL-WF-003`: デフォルト Temporal とする判断根拠。確定段階: リリース時点。
- `DS-CTRL-WF-004`: 判定オーバーヘッド 1ms 以内の数値根拠。確定段階: リリース時点。
...
- ランタイム: Dapr Workflow（daprd に組込み、リリース時点 で stable 化想定）
...
- `DS-CTRL-WF-005`: Dapr Workflow 採用基準（5 分 / 100 RPS / tier1 内部）。確定段階: リリース時点。
- `DS-CTRL-WF-006`: Dapr Workflow の Valkey バックエンド構成と制約値。確定段階: リリース時点。
...
- **承認ワークフロー**: 申請・発注・契約承認など、人手介入 + 複数段階の状態遷移を持つフロー。数日〜数週間規模。
- **大規模バッチ処理**: 月次決算・年次レポート集計など、数万〜数百万レコードを段階的に処理するフロー。
- **複雑な Saga**: 注文処理（在庫 → 決済 → 出荷 → 配送 → 通知 → 返品対応）など 5 ステップ以上の補償を持つフロー。

### 実装構成

- ランタイム: Temporal Server（採用後の運用拡大時 確定、ADR-RULE-002）
- History Store: **PostgreSQL**（CloudNativePG クラスタ）
- Visibility Store: PostgreSQL（採用後の運用拡大時 初期）、採用側のマルチクラスタ移行時 で OpenSearch 移行検討
...
- `DS-CTRL-WF-007`: Temporal 採用基準（5 分超 / 1000 RPS / tier2 業務フロー）。確定段階: 採用後の運用拡大時。
- `DS-CTRL-WF-008`: Temporal の PostgreSQL バックエンド構成と制約値。確定段階: 採用後の運用拡大時。

## 2 エンジン併用のコスト

### 運用工数の見積

2 エンジン併用は運用工数を増加させる。企画書 [../../01_企画/03_運用工数試算.md](../../01_企画/企画書.md) では以下の増分を計上している。
...
この 0.5 人月を超える場合、本章の振り分けロジックを簡素化（例: 境界値を単純化、判定軸を削減）する。運用工数の増加は 採用側の小規模運用原則（原則 9）の破綻リスクに直結する。
...
- `DS-CTRL-WF-009`: 2 エンジン併用の運用工数見積（年 0.5 人月）。確定段階: 採用後の運用拡大時。
- `DS-CTRL-WF-010`: 障害切り分け Runbook。確定段階: 採用後の運用拡大時。
...
切替テストは 採用後の運用拡大時 の受け入れテストで検証する。同一業務フロー定義を Dapr Workflow / Temporal で実行し、結果の同一性を確認する。

### 設計 ID

- `DS-CTRL-WF-011`: Workflow DSL とバックエンド切替可能性。確定段階: 採用後の運用拡大時。
- `DS-CTRL-WF-012`: 既存インスタンス保持のままエンジン切替する段階的移行パターン。確定段階: 採用後の運用拡大時。

## 可視化（採用後の運用拡大時）

### Backstage プラグイン

採用後の運用拡大時 で両エンジン統合ビューを提供する Backstage プラグインを開発する。機能は以下 4 つ。
...
- `DS-CTRL-WF-013`: Backstage プラグインによる両エンジン統合ビュー。確定段階: 採用後の運用拡大時。
...
関連する構想設計 ADR は ADR-RULE-001（ZEN Engine 決定評価 p99 1ms）、ADR-RULE-002（Temporal 長期ワークフロー リリース時点 確定）、ADR-DATA-004（Valkey）。本章で採番した設計 ID は `DS-CTRL-WF-001`〜`DS-CTRL-WF-013` の 13 件。詳細な要件 ↔ 設計対応は [../80_トレーサビリティ/02_要件から設計へのマトリクス.md](../80_トレーサビリティ/02_要件から設計へのマトリクス.md) で管理する。
