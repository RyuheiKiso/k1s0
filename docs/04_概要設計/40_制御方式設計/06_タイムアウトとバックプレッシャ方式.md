...
- `DS-CTRL-TO-001`: タイムアウト階層構造（Client 10s → Gateway 8s → Istio 7s → tier1 6s → Dapr 5s → Backend 4s）。確定段階: リリース時点。
- `DS-CTRL-TO-002`: 層間タイムアウト差分 1〜2 秒の数値根拠。確定段階: リリース時点。
- `DS-CTRL-TO-003`: API 別個別タイムアウト一覧。確定段階: リリース時点。
...
- `DS-CTRL-TO-004`: gRPC Deadline Propagation の全 tier1 内部通信での必須化。確定段階: リリース時点。
- `DS-CTRL-TO-005`: Go `context.Context` / Rust `tokio::timeout` の受け渡し規約。確定段階: リリース時点。
- `DS-CTRL-TO-006`: CI による `ctx.Done()` 監視漏れの静的検証。確定段階: リリース時点。
...
- **Window size**: 64KB（gRPC デフォルト、採用後の運用拡大時 で調整可能）
...
- `DS-CTRL-TO-007`: gRPC Flow Control 設定値（window 64KB、streams 100、keepalive 30s）。確定段階: リリース時点。
- `DS-CTRL-TO-008`: Kafka Consumer Pause/Resume 閾値（pause 5000 / resume 1000）。確定段階: リリース時点。
- `DS-CTRL-TO-009`: Valkey 接続プール上限到達時の即時 429 応答。確定段階: リリース時点。
...
- デフォルト上限: **1000 RPS/tenant**（リリース時点 初期値）
...
- `DS-CTRL-TO-010`: Envoy Gateway による Rate Limit（デフォルト 1000 RPS/tenant、バースト 1.5 倍）。確定段階: リリース時点。
- `DS-CTRL-TO-011`: テナント別カスタム上限の Backstage 承認フロー。確定段階: 採用後の運用拡大時。
...
Stage 3 で P0 API のみに絞ってもレイテンシが回復しない場合、tier1 Pod の水平スケールアウトを KEDA 経由で自動発動する（採用後の運用拡大時）。
...
- `DS-CTRL-TO-012`: Load Shedding 過負荷検知閾値（p99 400ms 30s、CPU 80% 30s）。確定段階: 採用後の運用拡大時。
- `DS-CTRL-TO-013`: 優先度別 API 分類（P0 / P1 / P2）。確定段階: 採用後の運用拡大時。
- `DS-CTRL-TO-014`: 段階的 Shedding（Stage 1-3）の発動と解除ロジック。確定段階: 採用後の運用拡大時。
...
- `DS-CTRL-TO-015`: タイムアウト・バックプレッシャ観測メトリクス。確定段階: リリース時点。
- `DS-CTRL-TO-016`: Grafana アラートルール。確定段階: リリース時点。
