...
- **NFR-B-PERF-002**: PubSub Publish p99 50ms。API 別タイムアウト 1 秒上限。
- **NFR-B-PERF-003**: State Get p99 10ms。API 別タイムアウト 500ms 上限。
- **NFR-B-PERF-004**: Decision Evaluate p99 1ms。API 別タイムアウト 100ms 上限。
- **NFR-B-PERF-005**: テナント別性能分離。Envoy Rate Limit で実現。
- **NFR-B-PERF-006**: 過負荷時の秩序ある劣化。Load Shedding で優先度制御。
- **NFR-B-PERF-007**: スケールアウト連動。KEDA + Load Shedding で実現。
