...
- `DS-CTRL-SAGA-001`: State BulkTransaction の原子性保証方式（Valkey MULTI / PostgreSQL transaction）。確定フェーズ: Phase 1b。
- `DS-CTRL-SAGA-002`: 楽観的ロック（ETag）による競合検出方式。確定フェーズ: Phase 1b。
- `DS-CTRL-SAGA-003`: BulkTransaction 操作数上限 10 とタイムアウト 200ms の数値根拠。確定フェーズ: Phase 1b。
...
加えて、2PC はプリペア段階で全リソースのロック保持を要求する。1 つのリソースがネットワーク分断で応答不能になると、他の全リソースもロック保持のまま応答を待ち続け、可用性が連鎖崩壊する。JTC 情シス基盤としては稟議合意の SLA 99%（月 7.2h 許容）を破る構造的リスクがあり、2PC は採用不可と判断する。
...
長期業務フロー（数分〜数週間、例: 承認ワークフロー、定期バッチ連鎖）は **Temporal** で実装する。Temporal は Event Sourcing による Workflow 状態永続化・Activity 失敗時の自動リトライ・Compensation API による補償定義を備える。ADR-RULE-002 で Phase 1b 確定済み。
...
Choreography では全体フローの可視性が課題となる。これを補うため、各サービスは Saga 相関 ID（`saga_id`）をイベントヘッダに付与し、Backstage プラグインで相関 ID 検索により全体フローをタイムライン表示できるようにする（Phase 2）。
...
- `DS-CTRL-SAGA-004`: Saga パターン採用と 2PC 不採用の判断根拠。確定フェーズ: Phase 0。
- `DS-CTRL-SAGA-005`: Orchestration Saga（Temporal / Dapr Workflow）の実装方式。確定フェーズ: Phase 1b / Phase 2。
- `DS-CTRL-SAGA-006`: Choreography Saga（Kafka イベント連鎖）の実装方式。確定フェーズ: Phase 1b。
- `DS-CTRL-SAGA-007`: 補償アクションの業務逆操作ルール。確定フェーズ: Phase 1b。
- `DS-CTRL-SAGA-008`: Saga 相関 ID（UUIDv7）伝播方式。確定フェーズ: Phase 1b。
...
Backstage プラグインとして「Saga Inspector」を Phase 2 で提供する。停留中 Saga の一覧、各ステップの実行状態、補償試行履歴、エラーメッセージを表示し、運用者が以下 3 アクションを選択できるようにする。
...
- `DS-CTRL-SAGA-009`: Saga 停留検出と escalation 方式。確定フェーズ: Phase 1b。
- `DS-CTRL-SAGA-010`: Saga Inspector（Backstage プラグイン）の手動介入 UI。確定フェーズ: Phase 2。

## 具体例: 注文処理 Saga

### フロー概要

![](img/saga_compensation_flow.svg)
...
補償系では、在庫引当ステップは完了済みだが決済ステップが 5 回 retry を全失敗したケースを描いている。Temporal は失敗検出から 10 秒以内に Compensation フェーズに遷移し、既完了ステップを逆順（ここでは在庫引当 1 つ）で補償する。補償は業務イベントの逆操作（`InventoryReleased` → `OrderCancelled`）として発行し、Valkey の冪等キーストアで重複排除する。補償が 60 秒以内に完了すれば `compensated` ステータスに遷移し、Backstage Saga Inspector で可視化される。補償自体が 5 回全失敗した場合のみ DLQ + 人的介入 escalation（Slack / PagerDuty 1 分以内）へと分岐する。この 2 段構え（自動補償 → 人的介入）により、補償不能なビジネス状態が自動復旧を盲信する形で放置されることを構造的に防いでいる。
...
決済ステップで一時的エラー（ゲートウェイ 503）が発生した場合、Temporal の Activity 内でリトライポリシー（exponential backoff base 1s、max 30s、attempts 5）により自動再試行する。全回失敗した場合、Temporal は Saga を Compensation フェーズに遷移させ、既完了の「在庫引当」の補償（`InventoryReleased`）を実行する。補償自体が失敗した場合は 5 回再試行し、最終失敗で DLQ + アラート通知する。
...
- 補償フェーズ全体の目標完了時間: **60 秒以内**（業務データ復旧の許容時間）

### 設計 ID

- `DS-CTRL-SAGA-011`: 注文処理 Saga の参照実装仕様（tier2 開発者向けサンプル）。確定フェーズ: Phase 1b。
...
関連する構想設計 ADR は ADR-RULE-002（Temporal 長期ワークフロー Phase 1b 確定）、ADR-DATA-002（Kafka Strimzi、at-least-once）である。本章で採番した設計 ID は `DS-CTRL-SAGA-001`〜`DS-CTRL-SAGA-011` の 11 件。詳細な要件 ↔ 設計対応は [../80_トレーサビリティ/02_要件から設計へのマトリクス.md](../80_トレーサビリティ/02_要件から設計へのマトリクス.md) で管理する。
