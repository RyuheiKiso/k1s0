...
Runbook は「採用側の小規模運用で成立する」という企画コミットの中核資産である。起案者が不在の夜間・休日に協力者が単独で SEV1 に対応するには、判断要素を最小化した機械的手順が必要である。判断に頼る運用は属人化を招き、バス係数 2（ひとり抜けても業務継続可能）を破綻させる。
...
合計 Runbook 数 = リリース時点 時点で 15 本: 要件 NFR-A-REC-002 のコミット値。詳細目録は [09_Runbook目録方式.md](09_Runbook目録方式.md) で定義する。
...
PR レビューは 複数名承認を原則とする。1 名は起案者または協力者、もう 1 名は対象コンポーネントの担当者（例: DB 関連なら DB 担当）。ただし緊急対応で Runbook 追加が必要な場合は 1 名承認で許容し、事後に 代替担当の承認を追記する。
...
抜き打ちドリル: 日時を事前通知せず、オンコール当番に対して「今から障害を発生させる」と通知してからシナリオを走らせる。実戦さながらの対応力を検証する。運用蓄積後に導入。

自動化ドリル: Chaos Mesh（Kubernetes 向け Chaos Engineering OSS）で自動的に障害を発生させ、Runbook 実行を記録する。週次で軽微な障害（Pod 削除、ネットワーク遅延）を発生させ、自動復旧の挙動を確認する。採用後の運用拡大時に導入。
...
Temporal（長時間実行・ステートフルワークフロー）: 複数ステップのトランザクション的な自動化に用いる。データ移行、段階的リストア、複数コンポーネントの連携復旧に用いる。運用蓄積後に導入。
...
Runbook の品質は主観で評価せず、以下指標で定量的に計測する。運用蓄積後、月次で計測し Grafana ダッシュボードで可視化する。

Runbook カバー率: 全アラートルール数に対する Runbook 整備済みアラート数の比率。目標 90% 以上。リリース時点 時点の目標は 80% / で 90% 達成。
...
Runbook は新規協力者の学習教材としても機能する。リリース時点 で新規協力者を受け入れる際、最初の 2 週間で以下のオンボーディングを行う。
...
NFR-C-OPS-001（運用プロセスの基盤整備）への対応である。本章で前提とする Slack（通知）、PagerDuty（オンコール）、Backstage（ナレッジベース / FAQ / Runbook 閲覧）、Jira（チケット管理）、GitHub（Runbook PR レビュー）の 5 ツールを運用プロセス基盤として位置付け、それぞれの責務を明確化する。Slack は非同期通知・雑談型問合せ・インシデント宣言の場、PagerDuty は SEV1 時のコールアウト・エスカレーションポリシー・オンコールローテーション、Backstage は Runbook / FAQ / Service Catalog / SBOM を TechDocs で統合表示、Jira は問合せチケット・再発防止アクション・Sprint 計画、GitHub は Runbook / IaC / アプリコードの一元バージョン管理を担う。5 ツール間の相互連携は（a）Alertmanager → Slack / PagerDuty の Webhook、（b）PagerDuty → Jira チケット自動生成、（c）GitHub PR マージ → Backstage TechDocs 再ビルド、（d）Jira チケット → Slack スレッド自動通知、の 4 経路で自動化する。これにより「人が情報を転記する」工程をゼロに近づけ、採用側の小規模運用でも情報の整合性が維持される状態を作る。

## 設計 ID 一覧

| 設計 ID | 項目 | 対応要件 | 確定段階 |
| --- | --- | --- | --- |
| DS-OPS-RB-001 | Runbook 形式（Markdown / TechDocs） | NFR-A-REC-002 | リリース時点 |
| DS-OPS-RB-002 | 必須 8 セクション | NFR-A-REC-002 | リリース時点 |
| DS-OPS-RB-003 | 粒度設計（3 分 / 1 時間 / 15 本） | NFR-A-REC-002 | リリース時点 |
| DS-OPS-RB-004 | バージョン管理と変更プロセス | NFR-A-REC-002 | リリース時点 |
| DS-OPS-RB-005 | Chaos Drill 検証 | NFR-A-REC-002 | リリース時点 |
| DS-OPS-RB-006 | 自動化対象識別 | OR-INC-003 | 採用後の運用拡大時 |
| DS-OPS-RB-007 | Alert 連動 Runbook 提示 | OR-INC-001 | リリース時点 |
| DS-OPS-RB-008 | 品質指標計測 | NFR-A-REC-002 | リリース時点 |
| DS-OPS-RB-009 | 新規協力者オンボーディング | OR-SUP-005 | リリース時点 |
| DS-OPS-RB-010 | 監視基盤 LGTM スタック構成 | NFR-C-MON-001 | リリース時点 |
| DS-OPS-RB-011 | Kafka Consumer Lag と DLQ 監視 | NFR-C-MON-002 | リリース時点 |
| DS-OPS-RB-012 | Workflow 遅延・滞留監視 | NFR-C-MON-003 | リリース時点 |
| DS-OPS-RB-013 | 時刻同期（NTP 100ms 以内） | NFR-C-NOP-004 | リリース時点 |
| DS-OPS-RB-014 | 運用プロセス基盤 5 ツール統合 | NFR-C-OPS-001 | リリース時点 |
