...
なお、RB-AUD-001（監査証跡ハッシュチェーン整合性月次検証）は採番自体は AUD カテゴリの正式採番として確定済だが、現時点の runbook_catalog.svg には独立カテゴリとして描画されていない。SVG 上の既存カテゴリは「API / データ（DB・MSG・BKP）/ セキュリティ（SEC・AUTH・NET）/ 運用移行（OPS）/ 災害復旧（DR）」の 5 分類で、監査関連は RB-SEC-003（事後アラート起点の改ざん検知）がセキュリティカテゴリに含まれるに留まる。RB-AUD-001 は「事前検証起点（月次バッチでの能動的ハッシュチェーン再計算）」という異質の起点を持つため、Phase 1c の図版改訂時に「監査検証」カテゴリを新設し RB-AUD-001 を配置する。同時に、SLI/SLO 接続（紫破線）では既存の `sli_audit`（監査完整性・NFR-H-AUD-001）ノードへ RB-AUD-001 からの接続を追加する。改訂までの暫定運用として、本文（上記 RB-AUD-001 の節）と設計 ID 一覧（DS-OPS-RB-056）で紐付けを明示する。詳細手順書は Phase 1c で整備する。
...
| DS-OPS-RB-056 | RB-AUD-001 監査証跡ハッシュチェーン整合性月次検証 | NFR-H-COMP-004 / NFR-H-AUD-001 | Phase 1c |
