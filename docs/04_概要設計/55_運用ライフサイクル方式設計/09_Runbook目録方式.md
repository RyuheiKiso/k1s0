...
### RB-AUD-001（仮）: 監査証跡ハッシュチェーン整合性月次検証

対象事象: 月次バッチによるハッシュチェーン整合性検証で不整合を検出した場合、または内部監査・外部監査・通報等により改ざん疑義が提起された場合。想定 Severity: SEV1（法令違反リスク、稟議コミットである監査完整性の喪失）。想定復旧時間: 暫定対応（Compliance Officer へのエスカレーションと当該期間ログの隔離）1 時間、恒久対応（原因特定・監督官庁報告要否判定・ポストモーテム）48 時間。本 Runbook は RB-SEC-003（監査ログ改ざん検知対応）が「事後のアラート起点」であるのに対し、月次バッチで能動的にハッシュチェーンを再計算して改ざんを早期発見する「事前検証起点」として役割が異なる。
...
対応設計 ID: [../30_共通機能方式設計/04_監査証跡方式.md](../30_共通機能方式設計/04_監査証跡方式.md) の DS-CF-AUD-\*（ハッシュチェーン方式・WORM 保管方式・月次検証バッチ方式）と連携する。所有者: 起案者（SEV1 のため）。対応 FMEA: FMEA-009（監査ログ改ざん）と同事象系統。確定フェーズ: Phase 1c（月次バッチ実装と同時に本 Runbook を初版固定。Phase 1b 時点では仮採番として目録に明示し、本数カウント上は Phase 1c の追加 5 本（15 → 20 本）に含める）。
...
なお、本章で仮採番した RB-AUD-001（監査証跡ハッシュチェーン整合性月次検証）は Phase 1c で本採番される Runbook であり、現時点の runbook_catalog.svg には独立カテゴリとして描画されていない。SVG 上の既存カテゴリは「API / データ（DB・MSG・BKP）/ セキュリティ（SEC・AUTH・NET）/ 運用移行（OPS）/ 災害復旧（DR）」の 5 分類で、監査関連は RB-SEC-003（事後アラート起点の改ざん検知）がセキュリティカテゴリに含まれるに留まる。RB-AUD-001 は「事前検証起点（月次バッチでの能動的ハッシュチェーン再計算）」という異質の起点を持つため、Phase 1c の図版改訂時に「監査検証」カテゴリを新設し RB-AUD-001 を配置する。同時に、SLI/SLO 接続（紫破線）では既存の `sli_audit`（監査完整性・NFR-H-AUD-001）ノードへ RB-AUD-001 からの接続を追加する。改訂までの暫定運用として、本文（上記 RB-AUD-001 の節）と設計 ID 一覧（DS-OPS-RB-056）で紐付けを明示する。
...
| DS-OPS-RB-056 | RB-AUD-001 監査証跡ハッシュチェーン整合性月次検証（仮採番） | NFR-H-COMP-004 / NFR-H-AUD-001 | Phase 1c |
