...
RPO（Recovery Point Objective）は「障害発生から何秒/分/時間前までのデータを失う可能性があるか」、RTO（Recovery Time Objective）は「障害発生から何分/時間で復旧するか」を意味する。RPO/RTO の数値は要件定義書 NFR-A（CONT / FT / DR / REC の 13 要件全般）と整合させている。

各データ層の OSS 採用根拠は構想設計 ADR で確定済みだが、本表の読み解きを助けるため概要を添える。RDB 層（State Warm / Workflow / 監査）で PostgreSQL / CloudNativePG を採る根拠は ADR-DATA-001 が示すとおり、MVCC による SERIALIZABLE 強整合と WAL 連続バックアップによる秒オーダー PITR、採用側組織の既存 DBA 知見の流用可能性（年 2,000 万円のライセンス費削減）の 3 点で、商用 DB（Oracle / SQL Server）は年次費用と DBA 依存度の面で却下している。メッセージング層で Kafka（Strimzi）を採る根拠は ADR-DATA-002 のとおり、at-least-once 配送と順序保証付きパーティショニング、KRaft モードによる ZooKeeper 削減（運用コンポーネント数半減）が決め手で、NATS / RabbitMQ は順序保証とスケール容量で不足する。キャッシュ層で Valkey Cluster を採る根拠は ADR-DATA-003 のとおり、Redis プロトコル互換により tier2 アプリの既存ライブラリ資産がそのまま利用でき、Linux Foundation 傘下 BSD-3 への移行で将来のライセンス変更リスクを回避できる点。オブジェクト層で MinIO を採る根拠は ADR-DATA-004 と ADR-STOR-002 のとおり、S3 API 互換で既存 NAS 1.5TB と Object Lock Compliance モードによる WORM 要件（NFR-H-COMP-003）を単一 OSS で満たせる点。これらの採用根拠は 36 行目以降の各節で詳細化する。
...
- tier1 State API の内部で自動 Hot→Warm→Cold 階層化（採用後の運用拡大時 で実装、リリース時点は Hot のみ）
- バックアップ: 週次で別拠点 NAS への rsync レプリケーション

## PubSub（Kafka）方式

PubSub はイベント駆動アーキテクチャの中核で、tier2 間の非同期連携 / エッジ IoT イベント / 監査イベントの配送を担当する。リリース時点では ZooKeeper フリーの KRaft モードで 3 ブローカを立ち上げ、採用後の運用拡大時 で MirrorMaker2 による DC 間レプリケーションを追加する。
...
- トピック数上限目安: 100（リリース時点）、1,000（採用後の運用拡大時）
- パーティション数標準: 3（トピック作成時の Dapr デフォルト）
- 保持期間: デフォルト 7 日、最大 30 日（個別設定可）
- 最大スループット目標: 10,000 msg/sec / cluster（リリース時点）、50,000 msg/sec（採用後の運用拡大時）
...
- MirrorMaker2（採用後の運用拡大時）: 第 2 DC Kafka へのトピックレプリケーション、RPO 60 秒目標
...
- シャーディング: 単一クラスタ（リリース時点〜採用後の運用拡大時）、Enterprise Replication は使用しない
- 暗号化: AES-256-GCM（マスターキーは Shamir Secret Sharing で 5 分割、3 しきい値）
- アンシール方式: Auto Unseal は リリース時点で使わず、Manual Unseal（採用側の小規模運用で鍵を分担保管）
...
- 保持期間: リリース時点は 30 日、採用後の運用拡大時 で 365 日
...
- 保持期間: リリース時点は 30 日、採用後の運用拡大時 で 90 日
...
- 保持期間: リリース時点は 14 日、採用後の運用拡大時 で 30 日
- サンプリング: リリース時点 導入時は 100%、運用蓄積後は tail-based 10%（リリース時点 では Tempo 自体が未導入）
...
- ILM（Information Lifecycle Management）: 30 日で Standard → Glacier 風 tiering（採用後の運用拡大時）
...
- ローカル評価: 採用後の運用拡大時 で in-process モード（Provider 経由）検討

## バックアップ統合戦略

各データ層のバックアップは異なる戦略を取るが、**最終的な退避先は MinIO に集約** し、さらに月次で**既存 採用側組織のバックアップテープ装置**に複製する 2 段階方式を取る。これによりランサムウェア対策（イミュータブルなテープ保管）と、日常的な復旧のしやすさ（MinIO からの高速復元）を両立する。
...
- MinIO: リリース時点 24 時間 / 採用後の運用拡大時 レプリケーション後 1 時間 / RTO 4 時間（リリース時点 は別拠点 NAS への週次 rsync のみのため 24h が下限 / でサイト間非同期レプリケーションを有効化して 1h まで短縮する）
...
**設計項目 DS-SYS-DB-020 採用段階別導入**

- リリース時点（採用初期、VM 1 台、2.5 人月 / 10 週間、`k1s0.Log` のみ）: Keycloak 用 PostgreSQL single のみ導入。アプリのログは stdout 直出力のみで、専用データストアを持たない。Valkey / Kafka / Mimir / Loki / Tempo / OpenBao / MinIO / Longhorn / flagd は全て未導入。
- リリース時点（採用初期、kubeadm HA 3 台、3.4 か月）: PostgreSQL HA（CloudNativePG）、Prometheus / Mimir、Grafana、OTel Collector を追加（`k1s0.Telemetry` API サポート）。Loki / Tempo は未導入、Valkey / Kafka も未導入。
- リリース時点（採用初期、VM 3 台、2.2 か月）: 運用品質向上として Loki / MinIO（Loki バックエンド）/ OpenBao / Harbor / Longhorn を追加。監査専用 PostgreSQL は本段階 で導入せず（Audit-Pii API は 採用後の運用拡大時）。
- 採用後の運用拡大時: tier1 残 9 API の本格運用に合わせて Valkey Cluster / Kafka 3 ブローカ / Tempo / 監査専用 PostgreSQL / WORM バケット / ConfigMap ベース flagd を追加。DC 間 MirrorMaker2、MinIO site-to-site、PostgreSQL 遠隔 Standby、Temporal クラスタもここで整備。

## 対応要件一覧

本ファイルは以下の要件 ID を方式設計で充足する。

- **FR-T1-STATE-001〜005**（State API）: DS-SYS-DB-002 / DS-SYS-DB-003 / DS-SYS-DB-004
- **FR-T1-PUBSUB-001〜005**（PubSub API）: DS-SYS-DB-005 / DS-SYS-DB-006 / DS-SYS-DB-007
- **FR-T1-SECRETS-001〜004**（Secrets API）: DS-SYS-DB-008 / DS-SYS-DB-009
- **FR-T1-WORKFLOW-001〜005**（Workflow API）: DS-SYS-DB-003
- **FR-T1-AUDIT-001〜003 / FR-T1-PII-001〜002**（Audit-Pii API）: DS-SYS-DB-010 / DS-SYS-DB-011
- **FR-T1-LOG-001〜004**（Log API）: DS-SYS-DB-013
- **FR-T1-TELEMETRY-001〜004**（Telemetry API）: DS-SYS-DB-012 / DS-SYS-DB-014
- **FR-T1-FEATURE-001〜004**（Feature API）: DS-SYS-DB-016
- **NFR-A-CONT/FT/DR/REC**（可用性全 13 要件）: DS-SYS-DB-017 / DS-SYS-DB-018 / DS-SYS-DB-019
- **NFR-B-WL/PERF/RES/QA**（性能・拡張全 14 要件）: DS-SYS-DB-002 / DS-SYS-DB-005 / DS-SYS-DB-012
- **NFR-F-SYS/CHR/STD/FAC/ECO**（環境・エコロジー全 13 要件）: DS-SYS-DB-015 / DS-SYS-DB-020
- **C-INF-001**（既存 採用側組織の資産活用）: DS-SYS-DB-015 / DS-SYS-DB-017
