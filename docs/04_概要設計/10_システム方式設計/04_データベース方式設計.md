...
- サンプリング: Phase 1b 導入時は 100%、Phase 1c 以降は tail-based 10%（Phase 1a では Tempo 自体が未導入）
...
- Phase 1a（MVP-0、VM 1 台、2.5 人月 / 10 週間、`k1s0.Log` のみ）: Keycloak 用 PostgreSQL single のみ導入。アプリのログは stdout 直出力のみで、専用データストアを持たない。Valkey / Kafka / Mimir / Loki / Tempo / OpenBao / MinIO / Longhorn / flagd は全て未導入。
- Phase 1b（MVP-1a、kubeadm HA 3 台、3.4 か月）: PostgreSQL HA（CloudNativePG）、Prometheus / Mimir、Grafana、OTel Collector を追加（`k1s0.Telemetry` API サポート）。Loki / Tempo は未導入、Valkey / Kafka も未導入。
- Phase 1c（MVP-1b、VM 3 台、2.2 か月）: 運用品質向上として Loki / MinIO（Loki バックエンド）/ OpenBao / Harbor / Longhorn を追加。監査専用 PostgreSQL は本 Phase で導入せず（Audit-Pii API は Phase 2）。
- Phase 2: tier1 残 9 API の本格運用に合わせて Valkey Cluster / Kafka 3 ブローカ / Tempo / 監査専用 PostgreSQL / WORM バケット / ConfigMap ベース flagd を追加。DC 間 MirrorMaker2、MinIO site-to-site、PostgreSQL 遠隔 Standby、Temporal クラスタもここで整備。
