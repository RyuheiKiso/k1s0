...
導入フェーズは [01_MVPスコープ.md](../../01_企画/03_ロードマップと体制/01_MVPスコープ.md) と [02_開発工数試算.md](../../01_企画/04_定量試算/02_開発工数試算.md) の積算を正本とする。MVP-0（Phase 1a、1 FTE / 10 週間 / VM 1 台）で導入できる OSS は最小限に絞り、HA / 可観測性 / サプライチェーン保護は Phase 1b / 1c 以降に段階投入する。

| OSS | 担当層 | ADR | ライセンス | 導入フェーズ |
|-----|-------|-----|-----------|------------|
| Kubernetes（kubeadm 1 ノード） | infra | (既定) | Apache 2.0 | Phase 1a |
| containerd | infra | (既定) | Apache 2.0 | Phase 1a |
| Kustomize | operation | (既定) | Apache 2.0 | Phase 1a |
| Helm | operation | (既定) | Apache 2.0 | Phase 1a |
| Dapr Control Plane | tier1 | (既定) | Apache 2.0 | Phase 1a |
| PostgreSQL（単一インスタンス、Keycloak 用） | tier1 | ADR-DATA-001 | PostgreSQL License | Phase 1a |
| Keycloak（単一インスタンス） | operation | ADR-SEC-001 | Apache 2.0 | Phase 1a |
| Kubernetes（kubeadm 3 ノード HA） | infra | (既定) | Apache 2.0 | Phase 1b |
| kubespray | infra | (既定) | Apache 2.0 | Phase 1b |
| OpenTofu | operation | (既定) | MPL-2.0 | Phase 1b |
| Calico | infra | (既定) | Apache 2.0 | Phase 1b |
| MetalLB | infra | (既定) | Apache 2.0 | Phase 1b |
| kube-vip | infra | (既定) | Apache 2.0 | Phase 1b |
| CoreDNS | infra | (既定) | Apache 2.0 | Phase 1b |
| Longhorn | infra | ADR-STOR-001 | Apache 2.0 | Phase 1b |
| CloudNativePG（プライマリ + レプリカ） | tier1 | ADR-DATA-001 | Apache 2.0 | Phase 1b |
| CloudNativePG Pooler（PgBouncer） | tier1 | ADR-DATA-001 | Apache 2.0 | Phase 1b |
| Keycloak HA（2 レプリカ） | operation | ADR-SEC-001 | Apache 2.0 | Phase 1b |
| Argo CD | operation | ADR-CICD-002 | Apache 2.0 | Phase 1b |
| GHA self-hosted runner（actions-runner-controller） | operation | ADR-CICD-004 | Apache 2.0 | Phase 1b |
| Prometheus | operation | ADR-OBS-001 | Apache 2.0 | Phase 1b |
| Grafana | operation | ADR-OBS-001 | AGPL-3.0（隔離運用）| Phase 1b |
| OTel Collector（Agent / DaemonSet） | operation | ADR-OBS-002 | Apache 2.0 | Phase 1b |
| kube-state-metrics | operation | (既定) | Apache 2.0 | Phase 1b |
| cert-manager | infra | (既定) | Apache 2.0 | Phase 1b |
| Tilt | operation | (既定) | Apache 2.0 | Phase 1b |
| Renovate | operation | (既定) | AGPL-3.0（隔離運用）| Phase 1b |
| Testcontainers | operation | (既定) | MIT | Phase 1b |
| Buf | operation | (既定) | Apache 2.0 | Phase 1b |
| Kubeshark | operation | (既定) | Apache 2.0 | Phase 1b |
| Headlamp | operation | (既定) | Apache 2.0 | Phase 1b |
| Harbor | operation | ADR-CICD-001 | Apache 2.0 | Phase 1c |
| Trivy | operation | ADR-CICD-003 | Apache 2.0 | Phase 1c |
| Loki | operation | ADR-OBS-001 | AGPL-3.0（隔離運用）| Phase 1c |
| Sealed Secrets | operation | (既定) | Apache 2.0 | Phase 1c |
| OpenBao（HA 3 Pod Raft + KV Engine） | tier1 | ADR-SEC-002 | MPL-2.0 | Phase 1c |
| Kyverno | operation | (既定) | Apache 2.0 | Phase 1c |
| MinIO | infra | ADR-STOR-002 | AGPL-3.0（隔離運用）| Phase 1c |
| Velero | operation | (既定) | Apache 2.0 | Phase 1c |
| External Secrets Operator | operation | (既定) | Apache 2.0 | Phase 1c |
| Syft + Grype | operation | (既定) | Apache 2.0 | Phase 1c |
| Istio Ambient（ztunnel + waypoint proxy） | tier1 / infra | ADR-0001 | Apache 2.0 | Phase 2 |
| Envoy Gateway | tier1 | (既定) | Apache 2.0 | Phase 2 |
| Apache Kafka（Strimzi Operator） | tier1 | ADR-DATA-002 | Apache 2.0 | Phase 2 |
| Valkey | tier1 | ADR-DATA-003 | BSD 3-Clause | Phase 2 |
| ZEN Engine | tier1（自作 Rust 領域内） | ADR-RULE-001 | MIT | Phase 2 |
| flagd（OpenFeature） | tier1 / operation | ADR-FM-001 | Apache 2.0 | Phase 2 |
| Grafana Tempo | operation | ADR-OBS-001 | AGPL-3.0（隔離運用）| Phase 2 |
| Grafana Pyroscope | operation | ADR-OBS-001 | AGPL-3.0（隔離運用）| Phase 2 |
| Grafana Mimir | operation | ADR-OBS-001 | Apache 2.0 | Phase 2 |
| Backstage | operation | (既定) | Apache 2.0 | Phase 2 |
| Temporal | tier1 | (既定) | MIT | Phase 2 |
| SPIFFE/SPIRE | infra | ADR-SEC-003 | Apache 2.0 | Phase 2 |
| Cosign | operation | (既定) | Apache 2.0 | Phase 2 |
| Apicurio Registry | tier1 | (既定) | Apache 2.0 | Phase 2 |
| Argo Rollouts | operation | (既定) | Apache 2.0 | Phase 2 |
| Argo Events | operation | (既定) | Apache 2.0 | Phase 2 |
| KEDA | operation | (既定) | Apache 2.0 | Phase 2 |
| Litmus（Chaos Engineering） | operation | (既定) | Apache 2.0 | Phase 2 |

表中の AGPL-3.0 OSS（MinIO / Grafana / Loki / Tempo / Pyroscope / Renovate）は、原則 8「AGPL OSS は Phase 1b 以降で隔離運用」に従い、プロセス分離 / ライブラリリンクなし / 無改変利用 / 外部境界遮断 / 監査ログ可検証性 の 5 原則を満たす形で運用する。詳細は [../75_事業運用方式設計/07_OSSライセンス運用方式.md](../75_事業運用方式設計/07_OSSライセンス運用方式.md) で定義する。Phase 1a は AGPL OSS を完全不使用とすることで、デモ時点で「AGPL を開発者 PC に持ち込まずに価値を証明できる」ことを担保する。
...
5 層の保守責任は以下のチームに分割する。この分割は企画書と構想設計で確定しており、本ファイルで再確認する。Phase 1a（MVP-0）は 1 名体制（起案者のみ）で全層を兼務、Phase 1b 以降の MVP 期は 2 名体制となり 1 名が infra + tier1 + operation、もう 1 名が DevEx + ドキュメント + サポートを兼務する想定である。体制の Phase 依存は [../../01_企画/03_ロードマップと体制/02_体制と役割.md](../../01_企画/03_ロードマップと体制/02_体制と役割.md) の通り。
