...
Canary 中の業務トラフィック分割は Istio Ambient の TrafficSplit で実施する（Phase 2 から Istio Ambient 採用）。サービスメッシュの採用可否判定および Ambient / Sidecar モードの選定根拠は構想設計 ADR（[ADR-0001 Istio Ambient mesh 採用](../../02_構想設計/adr/ADR-0001-istio-ambient-vs-sidecar.md)）を参照する。
