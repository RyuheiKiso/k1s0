...
本ファイルは k1s0 の CI（Continuous Integration）と CD（Continuous Delivery）の構成方式を定める。対象は GitHub Actions（self-hosted runner）での PR / main ビルド、コンテナビルドと脆弱性スキャン、Harbor レジストリ運用、Argo CD による GitOps デリバリ、Argo Rollouts による Canary 配布（採用後の運用拡大時）、Cosign 署名検証（採用後の運用拡大時）、Renovate による依存更新の 7 領域である。
...
Unit Test は `cargo test --all-features`（Rust）、`go test -race -cover ./...`（Go、Race Detector ON）、`pnpm test -- --coverage`（TypeScript）を並列実行する。カバレッジ閾値は 80%（宣言的閾値設定）とし、下回るとジョブ失敗する。カバレッジレポートは Codecov 相当の自己ホスト UI（運用蓄積後は Backstage プラグイン）で集約する。

Protobuf Breaking Check は `buf breaking --against '.git#branch=main'` で main との breaking 差分を検出する。tier1 内部の .proto ファイル変更で後方互換を壊した場合に CI を失敗させ、安易な破壊変更を構造的に防止する。詳細は [../20_ソフトウェア方式設計/03_内部インタフェース方式設計/01_内部gRPC契約方式.md](../20_ソフトウェア方式設計/03_内部インタフェース方式設計/01_内部gRPC契約方式.md) で定義する。
...
## Argo Rollouts（採用後の運用拡大時）

採用初期 は Deployment による全 Pod 一斉置換だが、採用後の運用拡大時 から Argo Rollouts の Canary 戦略を導入する。Canary は 10% → 25% → 50% → 100% の 4 段階で、各段階で 5 分の待機と自動分析（Prometheus メトリクス）を挟む。分析失敗時は自動ロールバックする。
...
Canary 中の業務トラフィック分割は Istio Ambient の TrafficSplit で実施する（採用後の運用拡大時 から Istio Ambient 採用）。サービスメッシュの採用可否判定および Ambient / Sidecar モードの選定根拠は構想設計 ADR（[ADR-0001 Istio Ambient mesh 採用](../../02_構想設計/adr/ADR-0001-istio-ambient-vs-sidecar.md)）を参照する。

## 品質ゲートと署名検証

レジストリへの push とクラスタへの pull の両面で品質ゲートを設ける。Harbor 側では Project レベルの設定で「Severity CRITICAL のイメージは pull を拒否する」を有効化する（リリース時点）。これによりスキャン漏れのイメージが本番へ流れない。Harbor プロジェクトは tier1 / tier2 / tier3 の 3 階層で切り分け、それぞれ別の CRITICAL 閾値（tier1 は 0 件、tier2 は 0 件、tier3 は 3 件まで許容）とする。

Kubernetes 側では Kyverno の ClusterPolicy で 3 種類のルールを強制する。第 1 に「`harbor.k1s0.internal` 以外のレジストリからの pull を拒否」（外部レジストリ経由の攻撃防止）、第 2 に「`:latest` タグの使用を拒否」（タグ不動性の確保）、第 3 に「Cosign 署名付きイメージのみ pull 許可」（採用後の運用拡大時、サプライチェーン攻撃防止）、である。Kyverno ポリシーの詳細は [../50_非機能方式設計/05_セキュリティ方式設計.md](../50_非機能方式設計/05_セキュリティ方式設計.md) で扱う。

Cosign 署名は 採用後の運用拡大時 で導入する。main パイプラインの Harbor push 後に `cosign sign --key=<openbao-ref> <image>` で署名し、Kubernetes 側で `cosign verify --key=<public-key> <image>` を Kyverno が pull 時に強制する。署名鍵は OpenBao Transit エンジンで保護し、CI の runner Pod は短期トークンで OpenBao から署名権限を取得する。詳細は [../50_非機能方式設計/05_セキュリティ方式設計.md](../50_非機能方式設計/05_セキュリティ方式設計.md) を参照する。
...
| Argo Rollouts Canary（採用後の運用拡大時） | 30 分 | 45 分 | 分析指標の閾値見直し |
| GitOps total（commit → prod 着） | 60 分 | 90 分 | 段階別所要時間の計測 |

Lead Time 目標 1 時間（DX-MET-001）はこのパイプライン時間予算と、人間のレビュー時間（平均 15 分）の合計で達成する。PR 5 分 + レビュー 15 分 + main 10 分 + Argo CD 3 分 + Rollouts 30 分 = 63 分、予算超過 3 分は 採用後の運用拡大時 で Canary 戦略を短縮して吸収する。

## 設計 ID 一覧

| 設計 ID | 設計項目 | 確定段階 | 対応要件 |
| --- | --- | --- | --- |
| DS-DEVX-CICD-001 | GitHub Actions self-hosted runner 方式 | リリース時点 | DX-CICD-001 |
| DS-DEVX-CICD-002 | PR パイプライン 5 ジョブ（5 分以内） | リリース時点 | DX-CICD-002 |
| DS-DEVX-CICD-003 | main パイプライン 6 ジョブ（10 分以内） | リリース時点 | DX-CICD-002 |
| DS-DEVX-CICD-004 | Argo CD + ApplicationSet 構成 | リリース時点 | DX-CICD-003 |
| DS-DEVX-CICD-005 | Argo Rollouts Canary 戦略（採用後の運用拡大時） | 採用後の運用拡大時 | DX-CICD-004 |
| DS-DEVX-CICD-006 | Harbor + Trivy 品質ゲート | リリース時点 | DX-CICD-005 / NFR-SEC-004 |
| DS-DEVX-CICD-007 | Kyverno イメージポリシー | リリース時点 | NFR-SEC-004 |
| DS-DEVX-CICD-008 | Cosign 署名検証（採用後の運用拡大時） | 採用後の運用拡大時 | NFR-SUP-003 |
| DS-DEVX-CICD-009 | SBOM（Syft）生成とライセンスレポート | リリース時点 | NFR-SUP-003 |
| DS-DEVX-CICD-010 | Renovate 週次依存更新 | リリース時点 | DX-CICD-006 |
| DS-DEVX-CICD-011 | パイプライン時間予算 5/10/30 分 | リリース時点 | DX-MET-001 |
