...
プラットフォーム製品は「使われて初めて価値が生じる」。DevEx を計測せず改善サイクルを回さないと、tier2 / tier3 開発者が「k1s0 を使うより自前で作った方が早い」と判断して離脱し、基盤投資が無駄になる。一方で計測のための計測に陥ると、運用 採用側の小規模運用を圧迫する。本設計は「自動収集 + 週次レポート + 月次改善」という最小構成で改善サイクルを維持する。

本設計は構想設計 ADR-OBS-001（Grafana LGTM 採用）と ADR-CICD-002（Argo CD GitOps）を前提として、DORA 4 指標の目標値（Deployment Frequency 1 日 10 回以上 / Lead Time 1h 以内 / MTTR 4h 以内 / CFR 15% 以下）を企画コミットとして リリース時点 で達成可能な水準に維持する設計に落とす。対応要件は DX-MET-001〜006、DX-GP-003（10 分ルール）、NFR-I-SLI-001 / NFR-I-SLO-001〜011（SLI/SLO 運用）である。
...
| --- | --- | --- | --- | --- |
| Deployment Frequency | prod への本番デプロイ頻度 | 1 日 10 回以上（運用蓄積後） | 1 日複数回 | Argo CD History |
...
Deployment Frequency の 1 日 10 回は、10 サービス × 1 日 1 回 = 10 回、または 1 サービス × 1 日 10 回のいずれでも達成できる水準である。リリース時点（2 サービス × 5 回）で達成し、採用後の運用拡大時 で 5 サービス以上に拡大する計画である。業界 Elite 水準（1 日複数回）に合致する設計とする。
...
CFR の 15% は業界 Elite 水準（5%）には届かないが、リリース時点 着手時点での現実的な水準とする。採用後の運用拡大時 で Canary 戦略と Chaos テストを導入し / 末で 5% 到達を目標とする。

## SPACE Framework の補完

DORA 4 指標だけでは「開発者が実際に満足しているか」が捕捉できない。SPACE Framework（Satisfaction / Performance / Activity / Communication / Efficiency）で補完する。

Satisfaction（満足度）は四半期アンケートで計測する。「k1s0 基盤は業務で役立っているか」（5 段階）、「tier1 API は使いやすいか」（5 段階）、「Golden Path 10 分ルールは実現されているか」（はい / いいえ）、「改善してほしい点」（自由記述）の 4 項目を標準とする。目標は平均 3.5 以上、リリース時点 で 3.0 到達、採用後の運用拡大時 で 3.5 到達とする。

Performance（成果）は DORA の Lead Time と Deployment Frequency が該当するため、重複を避けて本次元は「開発者 1 人当たりのサービスリリース数」を計測する。目標は月 5 サービス以上（採用後の運用拡大時 時点、5 名チーム想定）である。
...
Efficiency（効率性）は中断（interruption）頻度、コンテキストスイッチの頻度を計測する。採用後の運用拡大時 で導入予定の「フォーカスタイム」指標（会議以外の連続 2 時間以上のブロック）を個人レベルで可視化し、深い作業時間を確保する。

## 計測基盤の設計

指標は全自動で収集することを必須とする。手動エクスポートや Excel 集計は運用 採用側の小規模運用を圧迫するため禁止する。
...
計測パイプラインは以下の通り。日次バッチで GitHub API / Argo CD API からイベントを取得し、PostgreSQL の `devex_metrics` テーブルに格納する。Grafana の Prometheus データソースを介して DORA 指標を集計する。Incident Log は PagerDuty 代替の自作システム（リリース時点）で管理し、障害時刻と復旧時刻をシステムが自動記録する。Backstage ダッシュボードで 4 指標 + 5 次元を統合表示する。
...
| --- | --- | --- | --- |
| DS-DEVX-MET-001 | DORA Four Keys の採用 | リリース時点 | DX-MET-001 |
| DS-DEVX-MET-002 | Deployment Frequency 目標（1 日 10 回以上） | リリース時点 | DX-MET-002 |
| DS-DEVX-MET-003 | Lead Time 目標（1h 以内） | リリース時点 | DX-MET-003 |
| DS-DEVX-MET-004 | MTTR 目標（4h 以内） | リリース時点 | DX-MET-004 |
| DS-DEVX-MET-005 | CFR 目標（15% 以下 → 5% 以下） | リリース時点 | DX-MET-005 |
| DS-DEVX-MET-006 | SPACE 5 次元補完 | 採用後の運用拡大時 | DX-MET-006 |
| DS-DEVX-MET-007 | 計測基盤（GitHub + Argo + Grafana） | リリース時点 | DX-MET-001 |
| DS-DEVX-MET-008 | Backstage ダッシュボード統合 | リリース時点 | DX-BS-003 |
| DS-DEVX-MET-009 | 週次自動レポート | リリース時点 | DX-MET-001 |
| DS-DEVX-MET-010 | 月次 Product Council レビュー | リリース時点 | DX-MET-001 |
| DS-DEVX-MET-011 | 4 粒度集計（全社 / チーム / サービス / 個人） | リリース時点 | DX-MET-006 |
| DS-DEVX-MET-012 | グッドハート化防止規範 | リリース時点 | DX-MET-006 |
