...
5 原則は前項まで散文で各原則の意図と検証手段を述べたが、稟議および法務監査の局面では「どの原則を、どのチームが、どの設計章で、いつまでに実装するか」を一覧で問われる。散文記述では即答できないため、5 原則を実装責務・確定設計章・法務確認タイミング・違反検出手段の 4 軸で集約する本マトリクスを規約化する。本マトリクスは AGPL 採用 ADR 起票時の必須記載事項とし、6 つの AGPL OSS（Grafana / Loki / Tempo / Pyroscope / MinIO / Renovate）すべてについて本マトリクスのチェックリストを充足することを採用条件とする。

このマトリクスが欠落していた場合、「5 原則は守られている」と散文で主張しても監査で「実装責務が誰にあるのか」「設計書のどこに固定化されているのか」「法務はいつ確認するのか」を即座に提示できず、Phase 1b 移行時の MinIO 投入が法務差し戻しで遅延する事故が起きる。本マトリクスは法務との合意済み（AGPL 採用 ADR-0003）の運用を概要設計レベルで凍結する役割を持つ。

| 原則 | 実装責務チーム | 確定設計章（設計 ID）| 法務確認タイミング | 違反検出手段 | 違反時の挙動 |
|---|---|---|---|---|---|
| 1. プロセス分離 | infra チーム（Kubernetes 設定担当） | [../10_システム方式設計/03_システム処理方式設計.md](../10_システム方式設計/03_システム処理方式設計.md)（DS-SYS-PROC-* で AGPL Pod の独立配置を確定）+ [../50_非機能方式設計/05_セキュリティ方式設計.md](../50_非機能方式設計/05_セキュリティ方式設計.md)（NetworkPolicy） | AGPL OSS 採用 ADR レビュー時 + Phase 1b 移行判定時 | CI で Kubernetes Pod Spec を検査し、AGPL コンテナと tier1 自作コンテナが同一 Pod に同居していないことを自動検出（`scripts/compliance/agpl_pod_isolation_check.py`）| PR ブロック + Slack `#k1s0-license-alert` 発報 |
...
| 3. 無改変利用 | infra チーム + tier1 API チーム | [../50_非機能方式設計/05_セキュリティ方式設計.md](../50_非機能方式設計/05_セキュリティ方式設計.md)（DS-NFR-SEC-* で Harbor の `official/` 配下隔離を確定） | AGPL OSS 採用 ADR レビュー時 + 四半期ライセンスレポート確認時 | CI で Harbor のイメージ起点を検査し、AGPL OSS のイメージタグが公式 registry（`docker.io/grafana/...` 等）から取得されていることを確認。`custom/` 配下への AGPL OSS 格納を自動検出 | PR ブロック + 既存 `custom/` 配下イメージは Phase 1b までに `official/` へ移送 |
| 4. 外部境界遮断 | infra チーム + Sec チーム | [../50_非機能方式設計/05_セキュリティ方式設計.md](../50_非機能方式設計/05_セキュリティ方式設計.md)（DS-NFR-SEC-* で Envoy Gateway + Keycloak OIDC 強制）+ [../30_共通機能方式設計/01_認証方式設計.md](../30_共通機能方式設計/01_認証方式設計.md)（DS-CF-AUTH-* で社外 IP 遮断）| AGPL OSS 採用 ADR レビュー時 + Phase 1b 移行判定時 + Phase 1c 移行判定時（外部公開境界の総点検）| Kyverno / OPA Gatekeeper で AGPL OSS Pod への Istio AuthorizationPolicy 設定 Drift を検出。社外 IP からのアクセスログを Loki で監視し、想定外アクセスを発報 | Drift 検出時は Kyverno が即座に拒否、想定外アクセス検出時は Sec チームが 2 時間以内にトリアージ |
| 5. 監査ログ可検証性 | tier1 API チーム + Sec チーム | [../30_共通機能方式設計/04_監査証跡方式.md](../30_共通機能方式設計/04_監査証跡方式.md)（DS-CF-AUD-* で AGPL OSS アクセス記録）+ [../50_非機能方式設計/08_データ保護とプライバシー方式.md](../50_非機能方式設計/08_データ保護とプライバシー方式.md)（DS-NFR-PRV-* でハッシュチェーン改ざん防止）| AGPL OSS 採用 ADR レビュー時 + Phase 1c 完了判定時（監査ログ集約の総点検）+ 四半期ライセンスレポート確認時 | 月次バッチで PostgreSQL の AGPL OSS アクセスログのハッシュチェーン整合性を全件検証。Envoy Gateway のアクセスログ取得率が 99.9% 以上であることを Prometheus で監視 | ハッシュチェーン破断検出時は Sec チームが SEV1 として 24 時間以内にトリアージ、改ざん事実確定時は監査委員会へ報告 |
