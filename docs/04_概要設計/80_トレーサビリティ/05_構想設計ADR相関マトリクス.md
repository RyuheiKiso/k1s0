...
tier1 プラットフォームの基盤となる 6 個の ADR は、概要設計書のほぼ全章に影響する。これらは Phase 0 稟議で最も重点的に審査される根幹判断である。
...
- **新規 ADR 起票時**: 構想設計側で新規 ADR が採択された場合、同じ PR で本マトリクスに行を追加する。影響する設計 ID 範囲が Phase 0 時点で未確定の場合は「Phase 1a 時点で確定」と注記する。
...
- **Phase 切替時**: Phase 1a / 1b / 1c / 2 の切替時、全 ADR と概要設計の整合を確認する。整合未確認のまま Phase 切替を承認することは禁止する。
...
| 日付 | 版 | 改訂内容 | 起票者 |
|---|---|---|---|
| 2026-04-20 | 0.1 | Phase 0 稟議向け初版。26 ADR × 概要設計の相関を整理。 | 概要設計チーム |
