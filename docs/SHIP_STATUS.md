...
| `src/tier1/rust/crates/{decision, audit, pii}` | Rust 側 3 Pod（DS-SW-COMP-008/007/009） | **同梱済**（in-memory backend） | 3 crate に gRPC handler 完全実装。decision: ZEN Engine 0.55+ JDM 評価 / 評価トレース / registry。audit: SHA-256 hash chain WORM in-memory store + Record / Query / Export(streaming) / VerifyChain（FR-T1-AUDIT-002 / NFR-H-INT-001）。pii: classify + mask（email / credit card / IPv4 / mynumber / phone）。全件 tonic server + reflection + graceful shutdown + テナント分離。**HealthService（Liveness / Readiness）も全 3 Pod で登録済**（共通 `crates/health` 経由、依存先 probe は本リリース時点 空）。Postgres / 外部 backend は post-MVP |
