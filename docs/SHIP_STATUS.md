...
| **H4e: P4 実機検証実績 + deferred items** | 7 イテレーションで `down → up --role full --mode strict` の rebuild を完走。**達成**: kind multi-node (4 nodes) 起動、Calico CNI、cert-manager + Gateway API CRDs、MetalLB (IPv4 dynamic pool)、Istio Ambient (4 helm release: base/cni/istiod/ztunnel)、Kyverno + 5 ClusterPolicy + drift policy Enforce、SPIRE (server + agent + CSI driver)、Dapr 1.17.5、flagd、gitea (PVC 永続化、admin user + repo bootstrap + git push)、registry (PVC)、Argo CD (4 AppProjects + 8 ApplicationSets + **22 Application 生成 / 20 Synced+Healthy**)、Argo Rollouts、Envoy Gateway、CNPG cluster `k1s0-postgres@cnpg-system`、Strimzi Kafka、Temporal、MinIO、OpenBao (dev mode)、observability (Loki / Tempo / Grafana / Prometheus / OTel Collector) 計 **20 helm releases + istio 4 = 24 canonical layers**。**Deferred** (本 PR 完結後の follow-up): (a) **valkey** / **keycloak** = Bitnami OCI chart の Docker Hub unauthenticated 100 pulls/6h 制限を踏み chart pull が 429 で失敗、6 時間後の再試行 or `docker login` 認証で解消、(b) **backstage** = chart 2.6.3 の Pod が `pod-security.kubernetes.io/enforce: restricted` 由来の securityContext 要件 (runAsNonRoot / drop ALL caps / seccompProfile=RuntimeDefault / allowPrivilegeEscalation=false) を満たさず admission denied、values.yaml に `backstage.podSecurityContext` + `backstage.containerSecurityContext` 追加で対処予定。Argo CD Application 22 件中 20 件が即時 Synced + Healthy、`infra-dev` (1) が OutOfSync Degraded、`ops-backstage` (1) が Unknown Healthy はいずれも上記 deferred 由来であり SoT 整合性は維持。 |
