...
| `tests/` | contract / integration / fuzz / golden / fixtures（e2e は刷新中で未配置） | **同梱済**（一部雛形） | 5 カテゴリすべてに README + 動作可能な最小骨格 + CI hook 連携の入口。**fuzz/go**: 4 fuzz target（State.Set / Audit.Record / PubSub.Publish / Workflow.Start の protojson decode、735k execs / 0 panic 検証済）。**fuzz/rust**: 2 fuzz target（prost::Message::decode 4 message + SHA-256 hash chain 3 連鎖、libfuzzer 経由 CI 実行可能）。**integration/go**: tier1-facade の binary レベル結合テスト 3 件（Pod 起動 + HTTP/JSON gateway round-trip + tenant 越境）。**contract/openapi-contract**: tier1-openapi-spec.yaml を物理コピー、生成ツール `tools/codegen/openapi/run.sh` で自動同期。**golden/scaffold-outputs**: 4 ServiceType の expected.tar.gz、`compare-outputs.sh` で drift 検証。**e2e**: テスト基盤刷新により本リリース時点では削除済、新 ADR と新基盤で再構築する |
...
| **H4e: P4 実機検証実績 + finishing 全件達成** | 7 イテレーションで `down → up --role full --mode strict` の rebuild を完走、その後 finishing カテゴリ A (4 件) + B (5 件) で実装の漏れと約束未達を全件解消。**最終達成**: kind multi-node (4 nodes) 起動、Calico CNI、cert-manager + Gateway API CRDs、MetalLB (IPv4 dynamic pool)、Istio Ambient (4 helm release: base/cni/istiod/ztunnel)、Kyverno + 5 ClusterPolicy + drift policy default Enforce (再 apply で revert しない逆設計)、SPIRE (server + agent + CSI driver)、Dapr 1.17.5、flagd、gitea (PVC 永続化、SQLite schema polling + admin user + user repo bootstrap + git push retry/wait 全自動)、registry (PVC)、Argo CD (4 AppProjects + 8 ApplicationSets multi-service 化 + **35 Application 全 Synced + Healthy = 100%**)、Argo Rollouts、Envoy Gateway、CNPG cluster `k1s0-postgres@cnpg-system` (CREATEDB 権限付与で backstage plugin 動的 DB 作成 OK)、Strimzi Kafka、Temporal、MinIO、OpenBao (dev mode)、Backstage (PodSecurity restricted 対応 + showcase image 起動 / 11 plugins 全初期化)、observability (Loki / Tempo / Grafana / Prometheus / OTel Collector) 計 **20 helm releases + istio 4 = 24 canonical layers + 業務サービス 27 Application**。tier2/tier3 業務サービス 8 件 (notification-hub / taxcalc / approvalflow / invoicegen / example-tier2-go / admin-bff / portal-bff / admin-web / portal-web) を multi-service ApplicationSet で個別 Application 化。infra-dev / ops-backstage は ADR-POL-002 SoT 整理で argocd 管理対象から除外（dual ownership 衝突回避）。`backup-cluster.sh` ⇄ `restore-cluster.sh` で down/up cycle が完全自動化。**Deferred (DH rate limit 由来のみ)**: valkey / keycloak は Bitnami OCI chart の Docker Hub unauthenticated 100 pulls/6h を踏み chart pull が 429 失敗、6h 後の retry or `docker login` 認証で解消する経路は確定済（values.yaml は global.imageRegistry: docker.io 設定済で mirror.gcr.io 経由 pull が効くようになっているため、rate limit cooldown 後は何もせず install OK）。 |
