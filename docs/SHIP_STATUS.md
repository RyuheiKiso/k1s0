...
| `src/contracts/internal/` proto | tier1 内部 gRPC（Go ↔ Rust core、ADR-TIER1-002 / ADR-TIER1-003） | **同梱済** | 4 proto ファイル / 3 service / 8 RPC を配置（DS-SW-IIF-004〜029 準拠）。`k1s0.internal.errors.v1`（ErrorDetail + 6 区分 ErrorCategory）/ `k1s0.internal.audit.v1.AuditService`（AppendHash + VerifyChain Server Streaming）/ `k1s0.internal.decision.v1.DecisionService`（EvaluateDecision、collect_trace 対応）/ `k1s0.internal.pii.v1.PiiMaskService`（MaskPii + MaskPiiBatch、p99<3ms 目標）。`buf lint` 通過、`buf format` 適用済、`buf generate --template buf.gen.internal.yaml` で Go（`src/tier1/go/internal/proto/v1/`）と Rust（`src/tier1/rust/crates/proto-gen/src/`）に再生成済。Rust 側 `proto-gen/src/lib.rs` を `k1s0::internal::<comp>::v1` 階層に再構成、Go 側は `tier1/go` の `go build ./...` 通過 |
...
| 領域 | docs 規定 | 実装ランク | 備考 |
|---|---|---|---|
| `src/tier2/templates/` + `src/tier3/templates/` | k1s0-scaffold が参照する型付き Backstage Software Template v1beta3（IMP-CODEGEN-SCF-031〜034） | **同梱済** | 4 テンプレート配置（ディレクトリ名 / template.yaml metadata.name）: `tier2/templates/go-service/` → `tier2-go-service`（Go SDK + HTTP /healthz /readyz、distroless multi-stage Dockerfile）/ `tier2/templates/dotnet-service/` → `tier2-dotnet-service`（ASP.NET Core minimal API + K1s0.Sdk.Grpc、`{{namespace}}` PascalCase 必須）/ `tier3/templates/bff/` → `tier3-bff`（Go GraphQL minimal /graphql endpoint）/ `tier3/templates/web/` → `tier3-web`（React 18 + Vite + TS strict、`pnpm dev` 起動）。各テンプレ配下は `template.yaml`（Backstage v1beta3）+ `skeleton/{{name}}/...`（Handlebars `*.hbs`）+ `README.md` 構成。`catalog-info.yaml.hbs` を全テンプレで必須化（IMP-CODEGEN-POL-006、`backstage.io/techdocs-ref: dir:.` 自動付与）。`tier` / `language` は `template.yaml` の固定 values で設定 |
...
| `infra/dapr/components/` + `infra/dapr/subscriptions/` | Dapr Component CRD（IMP-DIR-INFRA-074、04_Dapr_Component配置.md） | **雛形あり** | tier1 公開 API の backing store を Component CRD 7 件で宣言: `state/postgres.yaml`（state.postgresql、CNPG）/ `state/redis-cache.yaml`（state.valkey、TTL 5m 既定）/ `pubsub/kafka.yaml`（pubsub.kafka、mTLS、`consumerID: k1s0-{{.appId}}`）/ `secrets/vault.yaml`（secretstores.hashicorp.vault、OpenBao 経由・SPIRE SVID 認証）/ `binding/s3-inbound.yaml`（bindings.aws.s3、MinIO path-style、polling 30s）/ `binding/smtp-outbound.yaml`（bindings.smtp、STARTTLS）/ `configuration/default.yaml`（tracing 100% / mTLS 24h / accessControl deny-by-default + 5 appId 許可 / SchedulerReminders + AppHealthCheck features）。`auth.secretStore: vault` で全 Component の認証情報を OpenBao に統一。`scopes:` で参照可能 appId を tier1 facade に限定。Subscription 2 件: `audit-pii.yaml`（audit-events → /v1/audit/ingest）/ `feature.yaml`（feature-flag-updates → /v1/feature/refresh）+ DLQ 設定。各サブディレクトリと subscriptions に README 配置。`workflow/temporal.yaml` は採用後の運用拡大時 |
...
| `infra/environments/{dev, staging, prod}` | 環境別 overlay（IMP-DIR-INFRA-078 / DS-OPS-ENV-007〜011） | **雛形あり** | 3 環境 overlay を配置: `kustomization.yaml`（infra base の `*-cluster.yaml` / `peer-authentication.yaml` / `cluster-issuer.yaml` / `baseline-policies.yaml` / `flagd-deployment.yaml` / `namespaces.yaml` / `storage-classes.yaml` を参照）+ `patches/`（dev: CNPG instances 3→1 / Kafka broker 3→1+5Gi、staging: CNPG 3→2 / Kafka 100Gi→50Gi、prod: 空＝base がそのまま prod）+ `values/`（Helm overlay; dev は loki 7d / tempo 3d / cert-manager letsencrypt-staging、staging は letsencrypt-staging）+ `secrets/.gitkeep`（SOPS 暗号化済 secret は採用初期 で配置）+ 各 README。dapr-components-overlay は `infra/dapr/components/` 配置時に追記する設計コメント記載 |
...
| `tools/codegen/` | buf / openapi / grpc-docs 生成ラッパ（IMP-CODEGEN-002 / 006 / 007） | **同梱済** | 3 ラッパ script 配置済: `buf/run.sh`（94 行、`buf generate` + `buf lint` + `buf breaking` をサブコマンド化、`--update-deps` / `--check` / `--lint` / `--breaking` フラグ対応）/ `openapi/run.sh`（64 行、proto → OpenAPI v2 export、`--check` で diff 検知）/ `grpc-docs/run.sh`（61 行、proto → gRPC reference Markdown、`--check` で diff 検知）。各 script は `set -euo pipefail` + repo root 解決 + `--help` 対応 |
...
| `tools/ci/go-dep-check/` + `tools/ci/rust-dep-check/` | 依存方向 linter（IMP-DIR-ROOT-002、05_依存方向ルール.md） | **同梱済** | `tier3 → tier2 → sdk → tier1` 一方向ルールを Go / Rust 両側で `import` / `path` 依存レベルで強制する自作 linter。Go 実装（独立 go.mod、go-dep-check は go/parser でクリーン import を抽出、rust-dep-check は BurntSushi/toml で Cargo.toml の `[dependencies]` `[dev-dependencies]` `[build-dependencies]` `[workspace.dependencies]` の `path = "..."` を解決）。両ツールに `--root` / `--json` フラグ、tier 識別 + 禁止 prefix / 許容方向 写像を分離した rules.go、golden test。SDK 公開 `sdk-go/proto/v1/` および Rust SDK 配下の `*-proto` crate は contracts material として exempt（DS-SW-COMP-132 と整合）。`go test ./...` 通過、リポジトリ全走査で 0 件の違反確認 |
| `tests/` | e2e / contract / integration / fuzz / golden / fixtures（IMP-DIR / docs/05_実装/00_ディレクトリ設計/70_共通資産/02_tests配置.md） | **雛形あり** | 6 カテゴリすべてに README + 動作可能な最小骨格 + CI hook 連携の入口を配置。`e2e/`（独立 go.mod、tenant_onboarding シナリオ + helpers/cluster.go の skeleton、t.Skip で stub 化）/ `contract/`（Pact broker-config.yaml + consumers/{portal,admin}-bff + providers/{tier1-state,tier2-payroll} + openapi-contract README）/ `integration/`（Go 独立 go.mod + tier1-facade ServiceInvoke 統合テスト skeleton + compose/dapr-compose.yaml で testcontainers 起動雛形）/ `fuzz/`（cargo-fuzz Cargo.toml + proto_fuzz / crypto_fuzz fuzz_targets、Go 1.18+ 標準 fuzzing FuzzFacadeRequest）/ `golden/`（diff-tool/compare-outputs.sh で 4 ServiceType を expected.tar.gz と再帰 diff、scaffold-outputs/ は採用初期 で expected を埋める）/ `fixtures/`（seed-data / tls-certs / openapi-samples の subdir 配置）。`go build ./...` は e2e / integration / fuzz の 3 module で通過 |
