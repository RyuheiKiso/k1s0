---
id: arch.formal.counter_example_policy
axis: formal
phase: architecture
kind: policy
status: draft
...
## 至高路線における立ち位置
- counter-example の「軽微だから無視」を禁止する。必ず 4 close_kind のいずれかで closure。
- close_due_at の延長を禁止する。延長は新 close_kind に切り替えることでのみ表現可能。
- accepted_as_bug の cap 越えを禁止する（cap=10 件以内）。
- 文章のみで「修正完了」を禁止する。必ず再 verify の certificate hash 一致を CI で物理 enforce。

## counter-example の意味論
- **safety violation**: reachable state で invariant が破られる trace（state sequence + variable assignment）
- **liveness violation**: fair execution path 上に lasso cycle（無限 loop で goal state に到達不可）
- **refinement violation**: S_low の transition が S_high の transition に mapped できない反例
- **program correctness violation**: Hoare triple `{P} f {Q}` の post-condition が破られる input
- **runtime modelcheck violation**: bounded depth で memory safety / overflow / panic が起きる input + trace

## counter_example.lock.yaml schema
- 各 entry の field:
    ...
    - `close_due_at`: ISO8601 timestamp（high=14d / medium=30d / low=90d、open は close_kind が確定するまで）
...
### fixed_in_code
- 実装側を修正、新 statement_hash で再 verify、counter-example が再現しないことを確認
- PR に `regression_corpus_added` 注釈必須（test/15 軸の regression corpus に転写）
...
- cap: v1=10 件以内（`assumption.lock.yaml` の cap=20 件とは別）、超過は CI fail
...
## closure 物理 enforce
- [formal 強制機構](../../04_詳細設計/02_強制機構/10_formal強制機構.md) の Kyverno admission policy で:
...
- cron-based audit: daily で `counter_example.lock.yaml` の状態を `audit_event` subject に emit、close_due_at breach の事前 alert（7 day before）。

## counter-example trace の扱い
- trace は machine-readable 形式（Apalache `.json` / P `.json` / Kani `.json` / CBMC `.xml`）で artifact として保存、Ceph RGW Object Lock Compliance mode で retention。
- human-readable 形式（trace を可視化した sequence diagram、variable timeline）も併設、Perses dashboard で表示。
- reproducer: trace から bounded harness を自動生成、close 後の regression check に利用。

## test 軸との bind
- counter-example で発見された input は test/15 軸の regression corpus に物理転写、property test の seed pool に追加:
    - close_kind=fixed_in_code の場合、修正後の実装が counter-example input で必ず正常動作することを property test で継続 check（regression corpus として永久保存）
    - close_kind=fixed_in_spec の場合、spec 修正後の formal proof で counter-example が解消されることを確認、property test seed として追加
- test/15 の v1_property_axiom cell の seed pool drift は CI fail。

## workflow
- **detect**: tool 起動で counter-example 検出 → `counter_example.lock.yaml` に open entry 自動追加（generator が tool log から抽出）
- **triage**: drill_owner が severity 判定 + close_kind 候補判断、24 hour 以内に severity 決定（high severity は ChatOps + page）
- **close**: close_kind に応じた action:
...
- **verify_close**: closure 後に再 verify、certificate hash が新 expected と一致することを CI で確認
- **archive**: closed entry を `proof_event` subject（ClickHouse SoR）に emit、Object Lock で retention

## 採用しない設計
- counter-example の「軽微だから無視」: 禁止、必ず 4 close_kind のいずれかで closure。
- close_due_at の延長: 禁止、延長は新 close_kind に切り替えることでのみ表現可能。
- accepted_as_bug の cap 越え: 禁止、cap=10 件以内。
- 文章のみで「修正完了」: 禁止、必ず再 verify の certificate hash 一致を CI で物理 enforce。
- counter-example trace の deletion: 禁止、retention cap 内は Object Lock 物理 delete 不可。
