...
| 言語 | パス |
| --- | --- |
| Rust | `regions/system/server/rust/workflow/` |
...
全エンドポイントは [API設計.md](API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_WORKFLOW_` とする。
...
[helm設計.md](helm設計.md) のサーバー用 Helm Chart を使用する。workflow 固有の values は以下の通り。
...
- [system-workflow-server-実装設計.md](system-workflow-server-実装設計.md) -- 実装設計の詳細
- [system-workflow-server-デプロイ設計.md](system-workflow-server-デプロイ設計.md) -- デプロイ設計の詳細
...
- [system-notification-server設計.md](system-notification-server設計.md) -- 通知連携先
- [system-scheduler-server設計.md](system-scheduler-server設計.md) -- 期日監視ジョブ連携先
- [system-saga-server設計.md](system-saga-server設計.md) -- システム間分散トランザクション（saga-server との役割分担）
