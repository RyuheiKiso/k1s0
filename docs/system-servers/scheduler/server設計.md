...
system tier のスケジューラーサーバー設計を定義する。cron 式によるジョブ定義・分散実行を管理する。ジョブトリガー時は Kafka トピック `k1s0.system.scheduler.triggered.v1` を発行し、実行結果を PostgreSQL に記録する。
Rust での実装を定義する。
...
| 言語 | パス |
| --- | --- |
| Rust | `regions/system/server/rust/scheduler/` |
...
全エンドポイントは [API設計.md](../../api/gateway/API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_SCHED_` とする。
...
- [system-scheduler-server-実装設計.md](system-scheduler-server-実装設計.md) -- 実装設計の詳細
- [system-scheduler-server-デプロイ設計.md](system-scheduler-server-デプロイ設計.md) -- デプロイ設計の詳細
