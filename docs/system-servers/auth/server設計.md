...
全エンドポイントは [API設計.md](../../api/gateway/API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_AUTH_` とする。
...
proto ファイルは [API設計.md](../../api/gateway/API設計.md) D-009 の命名規則に従い、以下に配置する。
...
- [system-server-デプロイ設計.md](../common/デプロイ設計.md)
...
- [system-config-server設計.md](../config/server設計.md) -- 設定管理サーバー設計（同 tier の参考実装）
