...
system-config-server のキャッシュ戦略・DB マイグレーション・テスト・Dockerfile・Helm values を定義する。概要・API 定義・アーキテクチャは [system-config-server設計.md](server設計.md) を参照。
...
設定値と変更ログの2テーブルを PostgreSQL（config-db）に格納する。詳細なスキーマ定義と全マイグレーションファイルは [system-config-database設計.md](database設計.md) 参照。
...
[認証認可設計.md](../../auth/design/認証認可設計.md) の Kong ルーティング設計に従い、設定管理サーバーを Kong に登録する。
...
- [system-config-server設計.md](server設計.md) -- 概要・API 定義・アーキテクチャ
- [system-config-server-実装設計.md](実装設計.md) -- Rust 実装詳細
