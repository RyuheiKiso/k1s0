...
system-config-server の Rust 実装詳細を定義する。概要・API 定義・アーキテクチャは [system-config-server設計.md](server設計.md) を参照。
...
[config設計.md](../../cli/config/config設計.md) のスキーマを拡張した設定管理サーバー固有の設定。
