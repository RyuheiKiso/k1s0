...
| 言語 | パス |
| --- | --- |
| Rust | `regions/system/server/rust/saga/` |
...
全エンドポイントは [API設計.md](../../api/gateway/API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_SAGA_` とする。
...
- [メッセージング設計.md](../../architecture/messaging/メッセージング設計.md) — Saga パターンの基本方針・採用基準
- [system-server設計.md](system-server設計.md) — auth-server 設計（同一パターン）
- [テンプレート仕様-サーバー.md](../../templates/server/サーバー.md) — サーバーテンプレート仕様
- [API設計.md](../../api/gateway/API設計.md) — REST API・gRPC 設計ガイドライン
- [system-saga-database設計.md](database設計.md) — Saga データベーススキーマ・状態管理テーブル
