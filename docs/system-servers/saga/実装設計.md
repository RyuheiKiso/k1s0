...
system-saga-server（Sagaオーケストレーションサーバー）の Rust 実装詳細を定義する。概要・API 定義・アーキテクチャは [system-saga-server設計.md](server設計.md) を参照。
...
- [system-saga-server設計.md](server設計.md) -- 概要・API 定義・アーキテクチャ
- [system-server-実装設計.md](../common/実装設計.md) -- auth-server 実装設計（同一パターン）
- [system-saga-database設計.md](database設計.md) -- Saga データベーススキーマ
