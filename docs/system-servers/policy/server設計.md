...
| 言語 | パス |
| --- | --- |
| Rust | `regions/system/server/rust/policy/` |

---

## 設計方針

[認証認可設計.md](../../auth/design/認証認可設計.md) および [RBAC設計.md](../../auth/mechanisms/RBAC設計.md) に基づき、以下の方針で実装する。
...
全エンドポイントは [API設計.md](../../api/gateway/API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_POLICY_` とする。
...
- [system-policy-server-実装設計.md](system-policy-server-実装設計.md) -- 実装設計の詳細
- [system-policy-server-デプロイ設計.md](system-policy-server-デプロイ設計.md) -- デプロイ設計の詳細

---

## 関連ドキュメント

- [RBAC設計.md](../../auth/mechanisms/RBAC設計.md) -- RBAC ロールモデル
