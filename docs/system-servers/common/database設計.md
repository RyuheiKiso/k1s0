...
[tier-architecture.md](../../architecture/overview/tier-architecture.md) の設計原則に従い、auth-db へのアクセスは **system Tier のサーバーからのみ** 許可する。下位 Tier（business / service）がユーザー情報や権限情報を必要とする場合は、system Tier のサーバーが提供する API（gRPC 等）を経由する。
...
| セッション管理 | Redis Sentinel | BFF セッションストア（[認証認可設計](../../auth/design/認証認可設計.md) 参照） |
| ユーザープロフィール（アプリ固有） | auth-db | Keycloak の sub と紐づくアプリケーション固有データ |
| ロール・権限マッピング（アプリ固有） | auth-db | [認証認可設計](../../auth/design/認証認可設計.md) D-005 の細粒度 RBAC |
...
Keycloak の `sub` claim（UUID）と紐づくアプリケーション固有のユーザーデータを管理する。[認証認可設計](../../auth/design/認証認可設計.md) の JWT Claims 構造で定義された `sub` を `keycloak_sub` カラムで参照する。
...
[認証認可設計](../../auth/design/認証認可設計.md) D-005 で定義された Tier 別ロール（`sys_admin`, `biz_{domain}_admin`, `svc_{service}_admin` 等）を管理する。
...
リソースに対する操作権限を管理する。[認証認可設計](../../auth/design/認証認可設計.md) D-005 のパーミッションマトリクスに対応する。
...
ユーザーの操作履歴を記録する監査ログテーブル。OpenTelemetry の trace_id を記録し、[可観測性設計](../../observability/overview/可観測性設計.md) の分散トレーシングと連携する。月次パーティショニングにより大量データを効率的に管理する。
...
[認証認可設計](../../auth/design/認証認可設計.md) D-005 の Tier 別ロール定義に対応する初期ロールを投入する。
...
[認証認可設計](../../auth/design/認証認可設計.md) D-005 のパーミッションマトリクスに対応する初期権限を投入する。
...
[認証認可設計](../../auth/design/認証認可設計.md) D-005 の system Tier パーミッションマトリクスに対応する。
...
- [認証認可設計](../../auth/design/認証認可設計.md) — OAuth 2.0 / OIDC・RBAC・Vault シークレット管理
...
- [可観測性設計](../../observability/overview/可観測性設計.md) — OpenTelemetry トレース ID 連携
