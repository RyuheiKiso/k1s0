...
監査ログテーブルは PostgreSQL に格納する。ユーザー情報は Keycloak が管理するため、認証サーバーの DB には監査ログのみを格納する。詳細スキーマは [system-database設計.md](database設計.md) 参照。
...
[認証認可設計.md](../../auth/design/認証認可設計.md) の Kong ルーティング設計に従い、認証サーバーを Kong に登録する。
...
- [system-server-実装設計.md](実装設計.md) -- Rust 実装詳細
