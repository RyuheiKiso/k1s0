...
**配置先**: `regions/system/library/csharp/config/`

```
config/
├── src/
│   ├── Config.csproj
│   ├── AppConfig.cs           # Config レコード型定義
│   ├── ConfigLoader.cs        # YAML 読み込み・マージ
│   ├── ConfigValidator.cs     # バリデーション
│   ├── MergeHelper.cs         # YAML マージロジック
│   └── VaultHelper.cs         # Vault シークレットオーバーライド
├── tests/
│   ├── Config.Tests.csproj
│   ├── Unit/
...
**名前空間**: `K1s0.System.Config`

**主要クラス・インターフェース**:

| 型 | 種別 | 説明 |
|---|------|------|
| `AppConfig` | record | アプリケーション設定（App・Server・Database 等のネスト型） |
| `ConfigLoader` | static class | YAML 読み込み・マージ |
| `ConfigValidator` | static class | 設定値バリデーション |
| `MergeHelper` | internal static class | YAML ディープマージ |
| `VaultHelper` | static class | Vault シークレット上書き |
| `ConfigException` | class | config ライブラリの公開例外型 |
...
{
    /// <summary>
    /// YAML を読み込み AppConfig を返す。envPath があればマージする。
    /// </summary>
...
public static class VaultHelper
{
    ...
    /// </summary>
    public static AppConfig MergeSecrets(AppConfig config, IDictionary<string, string> secrets);
}
```
...
│       ├── models.py          # AppConfig 等の pydantic BaseModel 型定義
│       ├── loader.py          # load() — YAML 読み込み・環境別マージ・バリデーション
│       ├── merger.py          # deep_merge() — YAML ディープマージユーティリティ
│       ├── vault.py           # merge_vault_secrets() — Vault シークレット上書き
│       ├── exceptions.py      # ConfigError・ConfigErrorCodes
...
| 型 | 種別 | 説明 |
|---|------|------|
| `AppConfig` | BaseModel | アプリケーション設定全体（app・server・grpc・database・kafka・redis・observability・auth） |
| `AppSection` | BaseModel | アプリケーション基本設定（name・version・tier・environment） |
| `ServerSection` | BaseModel | HTTP サーバー設定（host・port・timeout） |
| `GrpcSection` | BaseModel | gRPC サーバー設定 |
| `DatabaseSection` | BaseModel | データベース接続設定（host・port・name・user・password・ssl_mode・pool） |
| `KafkaSection` | BaseModel | Kafka 接続設定（brokers・consumer_group・sasl・topics） |
| `RedisSection` | BaseModel | Redis 接続設定 |
| `ObservabilitySection` | BaseModel | 可観測性設定（log・trace・metrics） |
| `AuthSection` | BaseModel | 認証設定（jwt・oidc） |
| `ConfigError` | Exception | config ライブラリのエラー基底クラス（code・message・cause） |

### 使用例

```python
from pathlib import Path
from k1s0_config import load, merge_vault_secrets

# 設定読み込み（ベース + 環境別マージ）
config = load(
    base_path=Path("config.yaml"),
    env_path=Path("config.dev.yaml"),
)
print(config.app.name, config.server.port)

# Vault シークレットのマージ
config = merge_vault_secrets(config, {
    "database.password": "secret123",
    "kafka.sasl.password": "kafka-secret",
})

# pydantic バリデーションは load() 内で自動実行
# 不正値の場合 ConfigError が送出される
```
