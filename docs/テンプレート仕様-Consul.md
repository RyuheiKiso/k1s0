...
本ドキュメントは、k1s0 CLI の「ひな形生成」機能で生成される **Terraform Consul Backend 設定** および **Consul Agent ConfigMap** のテンプレート仕様を定義する。Terraform の State 管理に使用する Consul Backend の設定ファイルと、Consul Agent の基本設定を環境（`environment`）に応じて自動生成する。

Consul による State 管理の全体設計は [terraform設計](terraform設計.md) を、インフラ構成は [インフラ設計](インフラ設計.md) を参照。
...
Consul テンプレートで使用する変数を以下に示す。変数の定義と導出ルールの詳細は [テンプレートエンジン仕様](テンプレートエンジン仕様.md) を参照。
...
- [terraform設計](terraform設計.md) -- Terraform モジュール設計・State 管理・Consul Backend
- [インフラ設計](インフラ設計.md) -- オンプレミスインフラ構成・共有サービス（Consul）
...
- [可観測性設計](可観測性設計.md) -- Prometheus メトリクス収集（Consul telemetry 連携）
