...
**配置先**: `regions/system/library/csharp/search-client/`

```
search-client/
├── src/
│   ├── SearchClient.csproj
│   ├── ISearchClient.cs            # 検索・インデックス操作インターフェース
│   ├── GrpcSearchClient.cs         # gRPC 実装
│   ├── SearchQuery.cs              # SearchQuery・Filter・FacetBucket
│   ├── IndexDocument.cs            # IndexDocument・IndexResult・BulkResult・IndexMapping
│   └── SearchException.cs          # 公開例外型
├── tests/
│   ├── SearchClient.Tests.csproj
│   ├── Unit/
│   │   ├── SearchQueryTests.cs
│   │   └── IndexDocumentTests.cs
│   └── Integration/
│       └── GrpcSearchClientTests.cs
├── .editorconfig
└── README.md
```
...
**名前空間**: `K1s0.System.SearchClient`

**主要クラス・インターフェース**:

| 型 | 種別 | 説明 |
|---|------|------|
| `ISearchClient` | interface | 検索・インデックス操作の抽象インターフェース |
| `GrpcSearchClient` | class | gRPC 経由の search-server 接続実装 |
| `SearchQuery` | record | クエリ・フィルター・ファセット・ページネーション |
| `SearchResult<T>` | record | ヒット一覧・総件数・ファセット集計・処理時間 |
| `IndexDocument` | record | ドキュメント ID・フィールドマップ |
| `IndexResult` | record | インデックス済みドキュメント ID・バージョン |
| `BulkResult` | record | 成功件数・失敗件数・失敗詳細リスト |
| `IndexMapping` | class | フィールド定義マップ |
| `Filter` | record | フィールド・演算子・値 |
| `FacetBucket` | record | バケット値・ドキュメント件数 |
| `SearchException` | class | 検索エラーの公開例外型 |
...
public sealed class GrpcSearchClient : ISearchClient
{
    public GrpcSearchClient(string serverUrl);
...
public struct Filter: Sendable {
    ...
    public init(field: String, operator: String, value: any Sendable)
}
...
public protocol SearchClient: Sendable {
    ...
    func search<T: Decodable & Sendable>(index: String, query: SearchQuery) async throws -> SearchResult<T>
