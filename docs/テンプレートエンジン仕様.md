...
| `language` | String | 言語識別子 | `rust` / `typescript` / `dart` / `csharp` |
...
└── templates/
    ...
    │   └── swift/
    │       ├── Package.swift.tera
    │       ├── Sources/{module}/LibError.swift.tera
    │       ├── Sources/{module}/Config.swift.tera
    │       ├── Sources/{module}/Client.swift.tera
    │       ├── Tests/{module}_tests/ClientTests.swift.tera
    │       └── README.md.tera
    └── database/
        ├── postgresql/
...
### Swift ライブラリテンプレートの変数

Swift テンプレートは `CLI/crates/k1s0-cli/templates/library/swift/` に配置する。
テンプレート内で使用する主要変数は以下の通り。

| 変数名 | 型 | 説明 | 例 |
|---|---|---|---|
| `service_name` | String | ライブラリ名（kebab-case） | `k1s0-authlib` |
| `service_name_pascal` | String | ライブラリ名（PascalCase） | `K1s0Authlib` |
| `service_name_snake` | String | ライブラリ名（snake_case） | `k1s0_authlib` |

`{module}` プレースホルダはテンプレートファイルパス中で `service_name_pascal` に置換される。
...
| C#/.NET | `dotnet restore` | 依存解決 |
| Swift/SPM | `swift package resolve` | 依存解決 |
...
ネットワーク依存のコマンド（`go mod tidy`、`npm install` 等）は、一時的なエラー時に最大3回の指数バックオフリトライを行う。リトライの詳細仕様は [テンプレート仕様-コード生成パイプライン](テンプレート仕様-コード生成パイプライン.md) の「リトライ機構」セクションを参照。
...
- [テンプレート仕様-Observability](テンプレート仕様-Observability.md) --- Observability テンプレートの詳細
- [テンプレート仕様-Grafana](テンプレート仕様-Grafana.md) --- Grafana ダッシュボードテンプレートの詳細
- [テンプレート仕様-OpenTelemetry](テンプレート仕様-OpenTelemetry.md) --- OpenTelemetry Collector テンプレートの詳細
- [テンプレート仕様-Loki](テンプレート仕様-Loki.md) --- Loki/Promtail テンプレートの詳細
- [テンプレート仕様-Alertmanager](テンプレート仕様-Alertmanager.md) --- Alertmanager テンプレートの詳細
- [テンプレート仕様-Kafka](テンプレート仕様-Kafka.md) --- Kafka メッセージングテンプレートの詳細
- [テンプレート仕様-Vault](テンプレート仕様-Vault.md) --- Vault シークレット管理テンプレートの詳細
- [テンプレート仕様-Flagger](テンプレート仕様-Flagger.md) --- Flagger カナリアリリーステンプレートの詳細
- [テンプレート仕様-Consul](テンプレート仕様-Consul.md) --- Consul テンプレートの詳細
- [テンプレート仕様-Storage](テンプレート仕様-Storage.md) --- Storage テンプレートの詳細
- [テンプレート仕様-ServiceMesh](テンプレート仕様-ServiceMesh.md) --- ServiceMesh テンプレートの詳細
