...
`module_path` は以下の要素を結合して構成される。

```
module_path = "regions/{tier}/{service_name}/{kind}/{language}"
```

例:

| tier | service_name | kind | language | module_path |
|---|---|---|---|---|
| `service` | `order` | `server` | `go` | `regions/service/order/server/go` |
| `business` | `crm` | `client` | `react` | `regions/business/crm/client/react` |
| `system` | `auth` | `library` | `rust` | `regions/system/auth/library/rust` |
...
```
go_module = "github.com/org/k1s0/regions/service/order/server/go"
```
