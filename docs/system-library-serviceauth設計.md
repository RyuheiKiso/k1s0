...
**配置先**: `regions/system/library/python/service_auth/`
...
│       ├── client.py         # ServiceAuthClient ABC
│       ├── http_client.py    # HttpServiceAuthClient（httpx OAuth2 Client Credentials 実装）
│       ├── models.py         # ServiceToken, ServiceClaims, SpiffeId, ServiceAuthConfig
│       ├── exceptions.py     # ServiceAuthError, ServiceAuthErrorCodes
│       └── py.typed
└── tests/
    ├── test_client.py
    └── test_models.py
...
| 型 | 種別 | 説明 |
|---|------|------|
| `ServiceAuthClient` | ABC | サービス認証クライアント抽象基底クラス（`get_token`, `get_token_async`, `get_cached_token`, `get_cached_token_async`） |
| `HttpServiceAuthClient` | class | httpx を使った OAuth2 Client Credentials フロー実装（トークンキャッシュ+自動更新） |
| `ServiceToken` | dataclass | アクセストークン（access_token, token_type, expires_at, scope）、`is_expired()` メソッド付き |
| `ServiceClaims` | dataclass | サービス JWT クレーム（sub, iss, scope, extra） |
| `SpiffeId` | dataclass | SPIFFE URI パース・検証（trust_domain, namespace, service_account）、`parse()` / `to_uri()` メソッド付き |
| `ServiceAuthConfig` | dataclass | 認証設定（token_url, client_id, client_secret, scope, jwks_uri, timeout_seconds） |
| `ServiceAuthError` | Exception | エラー基底クラス（code, message, cause） |
| `ServiceAuthErrorCodes` | class | エラーコード定数（TOKEN_REQUEST_FAILED, TOKEN_VALIDATION_FAILED, UNAUTHORIZED） |

### 使用例

```python
import asyncio
from k1s0_service_auth import (
    HttpServiceAuthClient, ServiceAuthConfig, SpiffeId,
)

config = ServiceAuthConfig(
    token_url="https://auth.k1s0.internal/realms/k1s0/protocol/openid-connect/token",
...
# 非同期でトークン取得
async def get_token():
    token = await client.get_cached_token_async()
    return token

# SPIFFE ID パース
spiffe = SpiffeId.parse("spiffe://k1s0.internal/ns/system/sa/auth-service")
print(f"Trust Domain: {spiffe.trust_domain}")
print(f"URI: {spiffe.to_uri()}")
```
