...
パイプラインの実装は `CLI/src/commands/generate.rs` の以下の関数に対応する。
...
| `determine_post_commands()` | kind × language × api_style から実行コマンドリストを決定 |
...
コード生成は **Server** かつ対象の `api_style` が選択されている場合のみ実行される。
...
| GraphQL | — | （現状未実装 — 将来的に `go run github.com/99designs/gqlgen generate` を追加予定） |
...
後処理の第2フェーズ。API 定義ファイルからスタブコードや型定義を自動生成する。**Server のみ** で実行され、選択された `api_style` に応じてコマンドが決定される。
...
現状、コード生成パイプラインへの組み込みは未実装。将来的に以下のコマンドを追加予定。

- Go: `go run github.com/99designs/gqlgen generate`（`gqlgen.yml.tera` から生成された設定ファイルを使用）

テンプレート自体（`api/graphql/schema.graphql.tera`、`gqlgen.yml.tera`）は既に用意されており、手動でのコード生成は可能。
