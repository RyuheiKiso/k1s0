...
![トピック命名パターン分解](images/topic-naming-pattern.svg)

#### トピック完全一覧

> **DOC-HIGH-003 監査対応（2026-04-04）**: `infra/messaging/kafka/create-topics.sh` および `topics.yaml` との照合により、完全なトピック一覧を記載。
> 最新の正規定義は常に `infra/messaging/kafka/create-topics.sh` を参照すること。

##### System Tier（6 パーティション: 高優先度）

| トピック名 | 用途 | Producer |
| --- | --- | --- |
| `k1s0.system.auth.audit.v1` | 監査ログイベント | auth-server |
| `k1s0.system.config.changed.v1` | 設定変更通知 | config-server |
| `k1s0.system.auth.login.v1` | ログインイベント | auth-server |
| `k1s0.system.auth.permission_denied.v1` | パーミッション拒否イベント | auth-server |
| `k1s0.system.apiregistry.schema_updated.v1` | API スキーマ更新通知 | api-registry |
| `k1s0.system.featureflag.changed.v1` | フィーチャーフラグ変更通知 | featureflag-server |
| `k1s0.system.auth.token_validate.v1` | トークン検証イベント | auth-server |
| `k1s0.system.notification.requested.v1` | 通知リクエスト | notification-server |
| `k1s0.system.saga.state_changed.v1` | Saga 状態変更 | saga-server |

##### System Tier（3 パーティション: 低優先度）

| トピック名 | 用途 | Producer |
| --- | --- | --- |
| `k1s0.system.file.uploaded.v1` | ファイルアップロード | file-server |
| `k1s0.system.file.deleted.v1` | ファイル削除 | file-server |
| `k1s0.system.file.events.v1` | ファイル汎用イベント（event_type ヘッダーで種別区別） | file-rust |
| `k1s0.system.tenant.events.v1` | テナントイベント | tenant-server |
| `k1s0.system.vault.secret_rotated.v1` | シークレットローテーション通知 | vault-server |
| `k1s0.system.quota.exceeded.v1` | クォータ超過通知 | quota-server |
| `k1s0.system.mastermaintenance.data_changed.v1` | マスタデータ変更通知 | mastermaintenance-server |
| `k1s0.system.search.index.v1` | 検索インデックス更新 | search-rust |
| `k1s0.system.workflow.state.v1` | ワークフロー状態変更 | workflow-rust |
| `k1s0.system.scheduler.created.v1` | スケジュール作成イベント | scheduler-rust |
| `k1s0.system.scheduler.executed.v1` | スケジュール実行イベント | scheduler-rust |
| `k1s0.system.scheduler.triggered.v1` | スケジュールトリガーイベント | scheduler-rust |

##### Service Tier（3 パーティション）

| トピック名 | 用途 | Producer |
| --- | --- | --- |
| `k1s0.service.task.created.v1` | タスク作成イベント | task-server |
| `k1s0.service.task.updated.v1` | タスク更新イベント（旧 `task_status_changed_topic` から分割） | task-server |
| `k1s0.service.task.cancelled.v1` | タスクキャンセルイベント（旧 `task_status_changed_topic` から分割） | task-server |
| `k1s0.service.board.column_updated.v1` | ボードカラム更新イベント | board-server |
| `k1s0.service.activity.created.v1` | アクティビティ記録イベント | activity-server |
| `k1s0.service.activity.approved.v1` | アクティビティ承認イベント | activity-server |

##### Business Tier（3 パーティション）

| トピック名 | 用途 | Producer |
| --- | --- | --- |
| `k1s0.business.taskmanagement.projectmaster.project_type_changed.v1` | プロジェクトタイプ変更イベント | projectmaster-server |
| `k1s0.business.taskmanagement.projectmaster.status_definition_changed.v1` | ステータス定義変更イベント | projectmaster-server |

##### DLQ トピック

> 全メイントピックに対して `.dlq` サフィックスを付与した DLQ トピックが存在する（1 パーティション、保持期間 30 日）。
> 完全なリストは `infra/messaging/kafka/create-topics.sh` の DLQ セクションを参照。

#### 旧トピック命名例（後方互換参照用）

#### バージョン同期ルール（topic vN / proto vN / schema_version）

トピック末尾の `vN` は **スキーマのメジャーバージョン** として扱い、以下を必ず同期させる。

- Kafka トピック名の末尾 `.{version}`（例: `...v1`）
- 対応するイベント Proto のパッケージメジャー（例: `package k1s0.event.service.task.v1;`）およびディレクトリ `.../v1/`
- `EventMetadata.schema_version` の値（`N`）

このルールにより、`v2` 移行時に「トピック名」「Proto パッケージ/配置」「Schema Registry Subject」が分岐せず、一貫した運用ができる。

![スキーマバージョン同期ルール](images/messaging-schema-versioning.svg)
...
| `ProjectTypeChangedEvent` | プロジェクトタイプ変更イベント | taskmanagement_events.proto |
| `StatusDefinitionChangedEvent` | ステータス定義変更イベント | taskmanagement_events.proto |
| `TaskCreatedEvent` | タスク作成イベント | task_events.proto |
| `TaskUpdatedEvent` | タスク更新イベント（旧 `TaskStatusUpdatedEvent` から分割） | task_events.proto |
| `TaskCancelledEvent` | タスクキャンセルイベント（旧 `TaskStatusUpdatedEvent` から分割） | task_events.proto |
| `BoardColumnUpdatedEvent` | ボードカラム更新イベント | board_events.proto |
| `ActivityCreatedEvent` | アクティビティ記録イベント | activity_events.proto |
| `ActivityApprovedEvent` | アクティビティ承認イベント | activity_events.proto |
...
> **注記**: メッセージングライブラリの `TopicConfig` は `default_partitions_for_tier()` / `DefaultPartitionsForTier()` で tier 別のデフォルトパーティション数を提供する（system/business: 6、service: 3）。`with_tier_defaults()` / `WithTierDefaults()` でトピック名から自動設定が可能。
...
![Kafkaクラスタ環境別比較](images/kafka-cluster-comparison.svg)
...
| 消費者グループ名                  | サービス       | 用途                     |
| --------------------------------- | -------------- | ------------------------ |
| `task-server.board-check`         | task-server    | ボードWIP確認処理        |
| `board-server.task-assigned`      | board-server   | タスク割り当て処理       |
| `notification-server.task-event`  | notification-server | タスク通知          |
| `analytics-server.all-events`     | analytics-server    | 分析用イベント集約  |

![コンシューマグループインタラクション](images/consumer-group-interaction.svg)
...
![DLQ 再処理フロー](images/messaging-dlq-reprocessing.svg)
...
message EventMetadata {
  ...
  string event_type = 2;        // e.g., "task.created"
  string source = 3;            // e.g., "task-server"
...
```protobuf
// k1s0/event/service/task/v1/task_events.proto
syntax = "proto3";
...
![CQRS パターン](images/messaging-cqrs-pattern.svg)
...
```
Task Server            Board Server             Activity Server
     │                        │                        │
     │  TaskCreated           │                        │
     │───────────────────────▶│                        │
     │                        │  BoardReserved         │
...
![Saga パターンフロー](images/messaging-saga-flow.svg)
