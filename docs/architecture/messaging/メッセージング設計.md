...
> **ガイド**: 設計背景・選定理由・実装パターンは [メッセージング設計.guide.md](./メッセージング設計.guide.md) を参照。
...
- メッセージブローカーは **Apache Kafka 3.8** を採用する
...
スキーマは `api/proto/k1s0/event/system/auth/v1/auth_events.proto` で定義する。主要なメッセージ型:

| メッセージ型 | 用途 |
| --- | --- |
| `LoginEvent` | ログイン成功/失敗 |
| `TokenValidationEvent` | トークン検証結果 |
| `PermissionCheckEvent` | パーミッション確認結果 |
| `AuditLogRecordedEvent` | 監査ログ記録イベント |
...
Kafka クラスタの管理には **Strimzi Kafka Operator** を採用する（採用理由はガイドを参照）。
...
Schema Registry は共通基盤として `k1s0-system` Namespace に配置し、Confluent Schema Registry の Helm Chart でデプロイする（配置理由はガイドを参照）。
...
CQRS はトラフィック非対称性が大きいサービスに限定して適用する（詳細はガイドを参照）。
...
イベントソーシングは監査要件が厳しいドメインに限定して適用する（詳細はガイドを参照）。
...
分散トランザクションにはコレオグラフィ方式を基本とする（フロー図・選定理由はガイドを参照）。

#### Saga Orchestrator 実装パターン

5 ステップ以上のワークフローではオーケストレーション方式を検討する。Orchestrator は Rust で自前の軽量実装を構築する。詳細は [system-saga-server.md](../../servers/saga/server.md) を参照。
...
Go / Rust の Producer / Consumer 実装パターンはガイドを参照。
