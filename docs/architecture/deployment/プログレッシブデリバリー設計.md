...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: order-server
          image: harbor.k1s0.internal/k1s0/order-server:latest
          ports:
            - containerPort: 8080
              name: http
              protocol: TCP
  ...
  strategy:
    canary:
      canaryService: order-server-canary
      stableService: order-server-stable
      trafficRouting:
        istio:
          virtualServices:
            - name: order-server
...
```yaml
# infra/istio/virtualservices/order-server.yaml
...
```yaml
# infra/argo-rollouts/analysis-templates/order-server-business-analysis.yaml
apiVersion: argoproj.io/v1alpha1
kind: AnalysisTemplate
metadata:
  name: order-server-business-analysis
  namespace: k1s0-service
spec:
  args:
    - name: service-name
...
k1s0 generate rollout --name order-server --tier service --lang rust

# Go Gin サーバー向け Rollout テンプレート生成
k1s0 generate rollout --name bff-proxy --tier system --lang go

# カスタム AnalysisTemplate 生成
k1s0 generate analysis-template --name order-server --tier service
