...
| サービス名 | REST | gRPC | 役割 |
|-----------|------|------|------|
| domain-master | 8210 | 9210 | ドメインマスタ管理（会計） |

### Service Tier（3 サービス）

| サービス名 | REST | gRPC | 役割 |
|-----------|------|------|------|
| order | 8310 | 9310 | 注文管理 |
| inventory | 8320 | 9320 | 在庫管理 |
| payment | 8330 | 9330 | 決済管理 |
...
| Kafka | 3.8 (KRaft) | 非同期イベントバス | auth, config, saga, dlq-manager, notification, order, domain-master 等 |
...
| S3 | — | ファイルストレージ | file, app-registry |
...
```mermaid
flowchart LR
    SagaSvr[saga-server<br/>:8085] -->|gRPC| Inventory[inventory<br/>:9320]
    SagaSvr -->|gRPC| Payment[payment<br/>:9330]
    SagaSvr -->|Kafka| Kafka[(Kafka)]
    SagaSvr -->|PostgreSQL| SagaDB[(PostgreSQL<br/>saga_db)]

    Order[order-server<br/>:8310] -->|Kafka publish| Kafka
...
| saga | inventory (9320) | 在庫引当・取消 |
| saga | payment (9330) | 決済処理・取消 |
...
| `k1s0.service.order.created.v1` | order | saga | 注文作成イベント |
| `k1s0.service.order.updated.v1` | order | — | 注文更新イベント |
| `k1s0.service.order.cancelled.v1` | order | saga | 注文キャンセルイベント |
...
| **Kafka** | auth, config, saga, dlq-manager, order, domain-master 等 | 非同期イベント処理停止（DLQ に蓄積） | **SEV1** |
...
| **saga-server** | order, inventory, payment の整合性 | 分散トランザクションが宙に浮く | **SEV2** |
| **dlq-manager** | 各コンシューマー | DLQ メッセージの再処理不能（蓄積） | **SEV2** |
| **inventory-server** | saga（在庫引当） | Saga の在庫ステップ失敗 | **SEV2** |
| **payment-server** | saga（決済処理） | Saga の決済ステップ失敗 | **SEV2** |
