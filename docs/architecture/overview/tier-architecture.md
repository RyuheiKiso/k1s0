...
![Tier依存関係図 — 許可/禁止フロー](images/tier-dependency-flow.svg)
...
- **business tier に server/client を置く意義** — 同一業務領域内で複数サービスが共通のサーバー処理やクライアントコンポーネントを共有するケースに対応する。共通コードが library だけで足りる場合は library のみ配置すればよい。
...
- [APIゲートウェイ設計](../api/APIゲートウェイ設計.md)
- [API設計](../api/API設計.md)
