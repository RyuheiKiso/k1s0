...
- JWT 検証は **防御層**とし、API Gateway（Kong）でも検証するが、各サービスも JWKS により検証する
- アプリケーションレベルの認可は **RBAC** で制御する
- シークレット管理は **HashiCorp Vault** で一元化する

![認証認可アーキテクチャ全体像](images/auth-overview-architecture.svg)
...
> **D-004: Kong 認証フロー** は [APIゲートウェイ設計.md](../api/APIゲートウェイ設計.md)（D-117: Kong 構成管理）を参照。
