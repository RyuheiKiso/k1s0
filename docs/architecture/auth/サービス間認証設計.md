...
#### 適用範囲（どこまでが Istio mTLS の対象か）

Istio の mTLS（PeerAuthentication）は **メッシュ参加（サイドカー注入）しているワークロード**に対して適用される。外部入口（Nginx Ingress Controller 等）や、Kafka 等の基盤コンポーネントは要件によってメッシュ外で運用される場合があるため、PeerAuthentication の有無だけで暗号化の有無を判断しない。

| Namespace | 位置づけ | 取り扱い |
| --- | --- | --- |
| `ingress` | 外部入口 | 外部→入口は通常 Istio mTLS の範囲外。メッシュ内（Kong 以降）は STRICT を前提とする |
| `messaging` | 基盤（Kafka 等） | 原則 Istio mTLS の対象外（コンポーネント自身の TLS/SASL 等で保護する）。必要ならメッシュ参加させる |
...
![Tier間 mTLS アクセス制御](images/service-auth-mtls-tiers.svg)
...
---
# service tier: ingress と同一 Tier からのアクセスのみ許可
# 注意: 同一 Tier 内の通信を Namespace レベルで許可しているが、
# tier-architecture.md の「service tier の BFF 間の通信は原則禁止」ルールに基づき、
...
---
# service tier: BFF 間の直接通信を禁止
# tier-architecture.md の「service tier の BFF 間の通信は原則禁止」ルールを
