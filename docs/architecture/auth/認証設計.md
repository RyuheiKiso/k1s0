...
### auth-server の補助 API（API キー管理 / ナビゲーション設定）

認証・認可の中核機能に加え、`auth-server` は以下の補助 API を提供する。

- API キー管理: `POST /api/v1/api-keys`, `GET /api/v1/api-keys`, `GET /api/v1/api-keys/:id`, `DELETE /api/v1/api-keys/:id/revoke`
- ナビゲーション設定: `GET /api/v1/navigation`

これらは system tier の運用機能として扱い、詳細仕様は [system-auth-server 設計](../../servers/auth/server.md) を参照する。
