...
**外部監査 M-6（2026-03-25）追記**:

外部技術監査で「個別 Dockerfile 管理コスト」として M-6 指摘を受けた。具体的には C-3（`.dockerignore **/tests/` と `[[test]]` の競合によるビルド破壊）がその事例として挙げられている。C-3 は `.dockerignore` の修正で対応済みだが、テンプレート化されていれば 1 箇所の修正で済んだケースである。

モノレポのサービス数増加やビルドインフラの変更頻度が高まった場合、`docker buildx bake`（HCL）または `ARG PACKAGE_NAME` パターンへの移行を再評価すること。
