...
**却下理由**: 現在の設定管理（serde_yaml + 環境変数オーバーライド）で十分に機能しており、
figment への移行は全27サービスの大規模変更を伴うため費用対効果が低いと判断。
フェーズ1（シェル変数構文の排除）は既に完了しており、フェーズ2の figment 移行は不採用とする。
将来的な設定管理の見直しが必要な場合は、新たな ADR を作成すること。
...
| 2026-04-05 | ステータスを「却下」に更新。現在の設定管理で十分として figment 移行を不採用とする（MED-014 監査対応） | @team |
