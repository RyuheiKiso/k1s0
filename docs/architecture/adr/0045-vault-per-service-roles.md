...
H-04 対応（2026-03-30）にて、`auth.tf` の旧 `vault_kubernetes_auth_backend_role "system"` リソースブロックを削除し、全サービスの個別ロールへの移行が完了した。
...
| 2026-03-30 | auth.tf の旧モノリシック `vault_kubernetes_auth_backend_role "system"` リソースブロックを削除（H-04 対応完了）。全サービスが個別ロールに完全移行済み。 | - |
