...
### Phase 4: business tier / service tier への展開（CRIT-01 / HIGH-06 対応済み）

system tier での確立パターンを business tier および service tier のサーバーに横展開する。

**2026-03-25 対応済み**: 以下のサーバーにスタブリポジトリを使った HTTP 統合テストを追加済み。

| サーバー | テストファイル | テスト数 |
| --- | --- | --- |
| service/task | `regions/service/task/server/rust/task/tests/integration_test.rs` | 7 |
| service/board | `regions/service/board/server/rust/board/tests/integration_test.rs` | 6 |
| service/activity | `regions/service/activity/server/rust/activity/tests/integration_test.rs` | 7 |
| business/project-master | `regions/business/taskmanagement/server/rust/project-master/tests/integration_test.rs` | 7 |

各テストは `auth_state: None` の認証なしモードで AppState を構築し、HTTP レイヤーを検証する。
実 DB を使ったテストは `#[cfg(feature = "db-tests")]` で区分けしており、Phase 4 の実 DB テスト導入時に解除する予定。

CI の `integration-test.yaml` では `tests/integration_test.rs` が存在するサーバーを自動検出するため、
上記ファイルを追加することで service tier・business tier の統合テストジョブが自動的に有効化される。
...
## 実装マイルストーン（H-5 外部監査対応）

| マイルストーン | 対象サーバー | 期限 | 状態 |
| --- | --- | --- | --- |
| Phase 1 完了 | auth-server の `#[ignore]` 解除 + CI 組み込み | 2026-04-14 | 未着手 |
| Phase 2 完了 | config-server, saga-server の実 DB テスト実装 | 2026-04-28 | 未着手 |
| Phase 3 完了 | session-server, tenant-server | 2026-05-12 | 未着手 |
| Phase 4 完了 | business/service tier への横展開 | 2026-06-02 | 一部完了（スタブ HTTP テストのみ） |
...
| 2026-03-25 | 1.1 | service tier 3 サービス（task/board/activity）および business tier project-master に統合テスト追加（CRIT-01 / HIGH-06 対応） |
| 2026-03-25 | 1.2 | 外部監査 H-5 対応: 実装マイルストーンと期限を具体化 |
