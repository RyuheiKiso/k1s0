...
agent:
  ...
  system_prompt: |
    ...
    タスク情報の変更やアクティビティ処理は、必ず人間の承認を得てから実行してください。
...
```yaml
# tools/search-tasks.yaml
tool:
  name: search_tasks
  description: "タスクID、担当者名、日付範囲でタスクを検索します"
  source:
    type: openapi
    service: task-server
...
test_suite:
  ...
  cases:
    - name: task_status_inquiry
      description: "タスク状況の問い合わせに正しく回答できること"
      input: "タスクID TASK-12345 の進捗状況を教えてください"
      expected:
        contains:
          - "TASK-12345"
          - "進捗"
    ...
    - name: activity_request_requires_approval
      description: "アクティビティリクエストで Human-in-the-Loop が発動すること"
      input: "タスク TASK-67890 のアクティビティを処理してほしい"
      expected:
...
feature_set:
  ...
  features:
    - name: total_tasks_30d
      type: int64
      description: "過去30日間のタスク数"
      source:
        type: sql
        query: "SELECT COUNT(*) FROM tasks WHERE user_id = :entity_id AND created_at > NOW() - INTERVAL '30 days'"
      freshness: 1h

    - name: avg_task_duration_90d
      type: float64
      description: "過去90日間の平均タスク所要時間"
      source:
        type: sql
        query: "SELECT AVG(duration) FROM tasks WHERE user_id = :entity_id AND created_at > NOW() - INTERVAL '90 days'"
      freshness: 6h
...
| Feature Store (オフライン) | PostgreSQL + ローカルファイルシステム（PV） | 既存 DB 活用。Parquet 保存で分析に適合 |
