...
### エラーコード体系図 & API バージョニングタイムライン

![エラーコード体系図とAPIバージョニングタイムライン](images/error-code-system.svg)
...
| `BIZ_`         | business | `BIZ_PRJMST_NOT_FOUND`      |
| `SVC_`         | service  | `SVC_TASK_NOT_FOUND`        |
...
| --------------- | ---------------------------------- | ------------------------------- |
| 400             | バリデーションエラー               | `SVC_TASK_VALIDATION_FAILED`    |
| 401             | 認証エラー（トークン無効・期限切れ）| `SYS_AUTH_TOKEN_EXPIRED`        |
| 403             | 認可エラー（権限不足）             | `SYS_AUTH_FORBIDDEN`            |
| 404             | リソースが見つからない             | `SVC_TASK_NOT_FOUND`            |
| 409             | 競合（楽観ロック等）               | `SVC_TASK_VERSION_CONFLICT`     |
| 422             | ビジネスルール違反                 | `SVC_BOARD_WIP_LIMIT_EXCEEDED`  |
...
{
  "error": {
    "code": "SVC_TASK_VALIDATION_FAILED",
...
services:
  ...
  - name: task-v2
    url: http://task-server-v2.k1s0-service.svc.cluster.local:80
    routes:
      - name: task-v2-route
        paths:
          - /api/v2/tasks
...
Link: <https://api.k1s0.internal.example.com/api/v2/tasks>; rel="successor-version"
```
...
#[derive(OpenApi)]
#[openapi(
    paths(handler::create_task, handler::get_task, handler::list_tasks),
    components(schemas(CreateTaskInput, Task, TaskStatus, ErrorResponse))
)]
struct ApiDoc;
...
dependencies:
  task_client:
    ...
    # git:
    #   url: https://git.internal.example.com/k1s0/task-client-dart.git
    #   ref: v1.0.0
```
