...
### `k1s0.business.taskmanagement.project-master.v1` — ProjectMasterService

タスク管理ドメインのプロジェクトマスタデータ管理サービス。
...
| `GetActivity` | アクティビティ取得 |
| `DeleteActivity` | アクティビティ削除 |
...
| パッケージ | 説明 |
|-----------|------|
| `k1s0.event.service.task.v1` | タスクドメインイベント（TaskCreated, TaskUpdated 等） |
| `k1s0.event.service.board.v1` | ボードドメインイベント（BoardReserved, BoardReleased 等） |
| `k1s0.event.service.activity.v1` | アクティビティドメインイベント（ActivityCreated, ActivityDeleted 等） |
