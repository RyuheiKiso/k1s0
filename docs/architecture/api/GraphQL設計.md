...
| 画面 | 集約対象サービス例 | 理由 |
| --- | --- | --- |
| ダッシュボード画面 | タスク、在庫、会計、ユーザーなど | 複数ドメインの集約データを一覧表示する |
| レポート画面 | 売上、在庫推移、顧客分析など | 複数サービスの分析データを統合表示する |
| ユーザープロフィール画面 | ユーザー情報、タスク履歴、通知など | ユーザーに紐づく複数サービスのデータを表示する |
...
type Resolver struct {
    taskClient  pb.TaskServiceClient    // gRPC クライアント
    authClient   pb.AuthServiceClient
}

// internal/adapter/graphql/task.resolvers.go（生成テンプレートから手動実装）
func (r *queryResolver) Task(ctx context.Context, id string) (*model.Task, error) {
    resp, err := r.taskClient.GetTask(ctx, &pb.GetTaskRequest{TaskId: id})
    if err != nil {
        return nil, err
    }
...
func (r *queryResolver) Tasks(ctx context.Context, first *int, after *string) (*model.TaskConnection, error) {
    ...
    return toTaskConnection(resp), nil
}
```
...
impl QueryRoot {
    async fn task(&self, ctx: &Context<'_>, id: ID) -> Result<Option<Task>> {
        let client = ctx.data::<TaskServiceClient>()?;
        let resp = client
...
**N+1 クエリ問題**が発生する。100件のタスクを返す場合、各タスクのチェックリストを1件ずつ取得すると101回のクエリが発生する。
...
query {
  tasks {       # 1回のDB呼び出し
    id
    checklists {      # N件 × 1回 = N回のDB呼び出し（N+1問題）
      title
    }
  }
}
...
```
リクエスト受信
  ↓ リゾルバー1: checklists(task_id=1) → DataLoader にキューイング
  ↓ リゾルバー2: checklists(task_id=2) → DataLoader にキューイング
  ↓ ...（N件分）
  ↓ イベントループ終端で DataLoader が flush
  ↓ SELECT * FROM task_checklist_items WHERE task_id IN (1, 2, ..., N) ← 1回のクエリ
  ↓ 結果を各リゾルバーに配布
```
...
```go
// graph/dataloader/task_checklist_loader.go
import "github.com/vikstrous/dataloadgen"

type TaskChecklistLoader struct {
    repo TaskChecklistRepository
}

// BatchFn は task_id のバッチを受け取り、対応する checklists を返す
func (l *TaskChecklistLoader) BatchFn(ctx context.Context, keys []string) ([][]*model.TaskChecklistItem, []error) {
    items, err := l.repo.FindByTaskIDs(ctx, keys)
    if err != nil {
    ...
    // キーごとに結果をグループ化して返す
    result := make([][]*model.TaskChecklistItem, len(keys))
...
// リゾルバー側での使用
func (r *taskResolver) Checklists(ctx context.Context, obj *model.Task) ([]*model.TaskChecklistItem, error) {
    return dataloader.For(ctx).TaskChecklists.Load(ctx, obj.ID)
}
```
...
```rust
// src/graphql/dataloader/task_checklist_loader.rs
use async_graphql::dataloader::Loader;

pub struct TaskChecklistLoader {
    repo: Arc<dyn TaskChecklistRepository>,
}

#[async_trait::async_trait]
impl Loader<String> for TaskChecklistLoader {
    type Value = Vec<TaskChecklistItem>;
    type Error = Arc<anyhow::Error>;

    async fn load(&self, keys: &[String]) -> HashMap<String, Self::Value> {
        self.repo.find_by_task_ids(keys).await
            .unwrap_or_default()
    }
}
...
let schema = Schema::build(QueryRoot, MutationRoot, EmptySubscription)
    .data(DataLoader::new(TaskChecklistLoader::new(repo), tokio::spawn))
    .finish();

// リゾルバー側での使用
#[graphql(name = "checklists")]
async fn checklists(&self, ctx: &Context<'_>) -> async_graphql::Result<Vec<TaskChecklistItem>> {
    ctx.data_unchecked::<DataLoader<TaskChecklistLoader>>()
