...
**配置先**: `regions/system/library/csharp/featureflag/`

```
featureflag/
├── src/
│   ├── FeatureFlag.csproj
│   ├── IFeatureFlagClient.cs        # フラグ評価インターフェース
│   ├── GrpcFeatureFlagClient.cs     # gRPC 実装（MemoryCache 付き）
│   ├── EvaluationContext.cs         # 評価コンテキスト
│   ├── FeatureFlag.cs               # フラグ定義モデル
│   ├── FeatureFlagConfig.cs         # gRPC エンドポイント・キャッシュ TTL 設定
│   └── FeatureFlagException.cs      # 公開例外型
├── tests/
│   ├── FeatureFlag.Tests.csproj
│   ├── Unit/
│   │   ├── EvaluationContextTests.cs
│   │   └── FeatureFlagTests.cs
│   └── Integration/
│       └── GrpcFeatureFlagClientTests.cs
├── .editorconfig
└── README.md
```
...
| Microsoft.Extensions.Caching.Memory | ローカルキャッシュ |

**名前空間**: `K1s0.System.FeatureFlag`

**主要クラス・インターフェース**:

| 型 | 種別 | 説明 |
|---|------|------|
| `IFeatureFlagClient` | interface | フラグ評価・取得の抽象インターフェース |
| `GrpcFeatureFlagClient` | class | featureflag-server への gRPC 実装（MemoryCache 付き） |
| `EvaluationContext` | record | 評価コンテキスト（Environment・UserId・ServiceName） |
| `FeatureFlag` | record | フラグ定義（Key・Enabled・RolloutPercentage・TargetEnvironments） |
| `FeatureFlagConfig` | record | gRPC エンドポイント・キャッシュ TTL 設定 |
| `FeatureFlagException` | class | featureflag ライブラリの公開例外型 |
...
public protocol FeatureFlagClient: Sendable {
    func evaluate(key: String, context: EvaluationContext) async throws -> Bool
    func getFlag(key: String) async throws -> FeatureFlag?
}
...
public struct FeatureFlag: Sendable {
    ...
    public let targetEnvironments: [String]
...
public enum FeatureFlagError: Error, Sendable {
    ...
    case evaluationFailed(key: String, reason: String)
...
**配置先**: `regions/system/library/python/featureflag/`
...
│       ├── client.py         # FeatureFlagClient ABC, GrpcFeatureFlagClient
│       ├── models.py         # EvaluationContext, FeatureFlag, FeatureFlagConfig
│       ├── cache.py          # ローカルキャッシュ管理
│       ├── exceptions.py     # FeatureFlagError
│       └── py.typed
└── tests/
    ├── test_client.py
    ├── test_cache.py
...
| 型 | 種別 | 説明 |
|---|------|------|
| `FeatureFlagClient` | ABC | フラグ評価抽象基底クラス |
| `GrpcFeatureFlagClient` | class | gRPC 実装（ローカルキャッシュ付き） |
| `EvaluationContext` | dataclass | 評価コンテキスト（environment, user_id, service_name） |
| `FeatureFlag` | dataclass | フラグ定義（key, enabled, rollout_percentage） |
| `FeatureFlagConfig` | dataclass | gRPC エンドポイント・キャッシュ TTL |
| `FeatureFlagError` | Exception | エラー基底クラス |
...
from k1s0_featureflag import (
    GrpcFeatureFlagClient, FeatureFlagConfig, EvaluationContext,
)

config = FeatureFlagConfig(
    endpoint="featureflag-service:50051",
    cache_ttl=60.0,
    environment="production",
)
client = GrpcFeatureFlagClient(config)
...
async def check_feature():
    ...
    enabled = await client.evaluate("new-checkout-flow", ctx)
...
async def get_flag_detail():
    flag = await client.get_flag("new-checkout-flow")
    if flag:
...
| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| grpcio | >=1.69 | gRPC クライアント |
| cachetools | >=5.5 | ローカルキャッシュ |
