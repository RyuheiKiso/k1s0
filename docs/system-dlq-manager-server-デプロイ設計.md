...
system-dlq-manager-server の Dockerfile・テスト・CI/CD パイプライン・設定ファイル・Helm values を定義する。概要・API 定義・アーキテクチャは [system-dlq-manager-server設計.md](system-dlq-manager-server設計.md) を参照。
...
[認証認可設計.md](認証認可設計.md) の Kong ルーティング設計に従い、DLQ Manager を Kong に登録する。
...
- [system-dlq-manager-server-実装設計.md](system-dlq-manager-server-実装設計.md) -- Rust 実装詳細
