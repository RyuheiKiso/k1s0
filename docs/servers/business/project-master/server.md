...
business tier の標準ロールと taskmanagement 固有ロールの両方をサポートする。

| ロール | read | write | admin |
|--------|------|-------|-------|
| `sys_admin` | ✅ | ✅ | ✅ |
| `biz_admin` | ✅ | ✅ | ✅ |
| `biz_taskmanagement_admin` | ✅ | ✅ | ✅ |
| `biz_operator` | ✅ | ✅ | ❌ |
| `biz_taskmanagement_manager` | ✅ | ✅ | ❌ |
| `biz_auditor` | ✅ | ❌ | ❌ |
| `biz_taskmanagement_viewer` | ✅ | ❌ | ❌ |

| アクション | 対象エンドポイント |
|-----------|-----------------|
| `read` | GET 全エンドポイント（一覧・単体取得） |
| `write` | POST / PUT（作成・更新・テナント拡張 upsert） |
| `admin` | DELETE（プロジェクトタイプ・ステータス定義・テナント拡張の削除） |

実装: `adapter/middleware/rbac.rs` の `check_taskmanagement_permission` 関数。認証は `k1s0-server-common` の `auth_middleware` + `AuthState` （JWKS ベース JWT 検証）を使用。
...
| ポート | 8211（REST）/ 50051（gRPC 予定: 現在は未起動） |
...
| レイヤー | 責務 | コンポーネント |
| --- | --- | --- |
| API Layer | REST エンドポイント（gRPC は proto 定義済み・サーバー未起動） | Project Master Server (axum + tonic) |
...
### gRPC サービス定義（Proto 定義済み・サーバー未起動）

> **CRIT-008 監査対応**: gRPC の Proto は定義済み（`ProjectMasterGrpcService` 構造体も実装済み）だが、
> `startup.rs` で tonic サーバーが起動されていない。現在の通信は REST のみ。
> gRPC を有効化する際は `infrastructure/startup.rs` に tonic::transport::Server を追加すること。
...
| カテゴリ | フィールド | 説明 |
| --- | --- | --- |
| server | `port` / `grpc_port` / `environment` | REST 8211 / gRPC 50051（CRIT-008: 現在の実装では gRPC サーバーは未起動） |
...
```
1. JWT から user_id + realm_roles を取得
2. adapter/middleware/auth.rs の auth_middleware で Bearer トークンを検証し Claims をリクエスト拡張に注入
3. adapter/middleware/rbac.rs の require_permission(resource, action) でアクセス制御
   - read  → GET エンドポイント（svc_viewer/biz_auditor/biz_taskmanagement_viewer 以上）
   - write → POST/PUT エンドポイント（biz_operator/biz_taskmanagement_manager 以上）
   - admin → DELETE エンドポイント（biz_admin/biz_taskmanagement_admin/sys_admin のみ）
4. 操作が許可されていない場合は 403 Forbidden (BIZ_AUTH_PERMISSION_DENIED) を返却
5. /healthz・/readyz・/metrics は認証除外
```
