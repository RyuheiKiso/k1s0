...
| name | VARCHAR(100) | UNIQUE NOT NULL | ロール名（例: sys_admin, svc_task_user） |
...
| resource | VARCHAR(255) | NOT NULL | 操作対象リソース（例: tasks, users, config） |
...
命名規則は [テンプレート仕様-データベース](../../templates/data/データベース.md) に準拠。
...
audit_logs テーブルに月次レンジパーティショニングを適用。

監査ログは書き込みが多く長期保存が必要なため、パーティショニングにより以下の効果を得る。
...
[config設計](../../cli/config/config設計.md) の database セクション準拠。
