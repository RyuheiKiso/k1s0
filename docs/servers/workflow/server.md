...
| DELETE | `/api/v1/workflows/:id` | ワークフロー定義削除 | `workflows/write` 権限（`sys_operator` 以上） |
| POST | `/api/v1/workflows/:id/execute` | インスタンス起動（実行開始） | `sys_operator` 以上 |
| GET | `/api/v1/workflows/:id/status` | ワークフロー実行ステータス取得（`:id` はワークフローインスタンス ID） | `sys_auditor` 以上 |
...
| POST | `/internal/tasks/check-overdue` | 期日超過タスク検出と通知発行 | `sys_operator` 以上（`workflows/write`） |
