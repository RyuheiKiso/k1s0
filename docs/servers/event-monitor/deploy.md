# system-event-monitor-server デプロイ設計

system-event-monitor-server の Dockerfile・テスト・CI/CD パイプライン・設定ファイル・Helm values を定義する。概要・API 定義・アーキテクチャは [system-event-monitor-server.md](server.md) を参照。
...
RUN cargo chef cook --release --recipe-path recipe.json -p k1s0-event-monitor-server
...
RUN if [ -n "${CARGO_FEATURES}" ]; then \
      cargo build --release -p k1s0-event-monitor-server --features "${CARGO_FEATURES}"; \
    else \
      cargo build --release -p k1s0-event-monitor-server; \
    fi
...
COPY --from=builder /app/target/release/k1s0-event-monitor-server /k1s0-event-monitor-server
COPY --from=busybox /bin/busybox /busybox

USER nonroot:nonroot
EXPOSE 8112 50051

ENTRYPOINT ["/k1s0-event-monitor-server"]
```
...
| ビルドコマンド | `cargo build --release -p k1s0-event-monitor-server`（ワークスペースから特定パッケージを指定） |
| ビルドコンテキスト | `regions/system`（`COPY . .` でシステム全体のライブラリ依存を含める） |
| 公開ポート | 8112（REST API）、50051（gRPC） |
...
開発環境用の設定ファイル。`regions/system/server/rust/event-monitor/config/config.yaml` に配置。
...
### CI（`.github/workflows/event-monitor-ci.yaml`）

PR 時に `regions/system/server/rust/event-monitor/**` の変更を検出してトリガーする。
...
### CD（`.github/workflows/event-monitor-deploy.yaml`）
...
**レジストリ**: `harbor.internal.example.com/k1s0-system/event-monitor`
...
| --- | --- | --- | --- |
| docker-compose | 8112 | 8112 | REST API |
| docker-compose | 50051 | 50051 | gRPC |
| Kubernetes | 80（Service） | 8112（Pod） | REST API |
...
| サービス | 用途 | 必須 |
| --- | --- | --- |
| PostgreSQL | イベントフロー定義・KPI データの永続化 | Yes |
| Kafka | 業務イベントの購読（マルチトピック） | Yes |
| event-store-server | イベントソースデータの取得 | Yes |
| notification-server | SLO 違反時のアラート通知 | No（オプション） |
| dlq-manager-server | 障害イベントの選択的リプレイ連携 | No（オプション） |

---

## Helm Chart

Helm Chart は `infra/helm/services/system/event-monitor/` に配置。k1s0-common チャート（v0.1.0）に依存。

### Chart 構成

```
infra/helm/services/system/event-monitor/
  Chart.yaml
  Chart.lock
...
```yaml
services:
  - name: event-monitor-v1
    url: http://event-monitor.k1s0-system.svc.cluster.local:80
    routes:
      - name: event-monitor-v1-events-route
        paths:
          - /api/v1/events
        strip_path: false
      - name: event-monitor-v1-flows-route
        paths:
          - /api/v1/flows
        strip_path: false
    plugins:
...
- [system-event-monitor-server.md](server.md) -- 概要・API 定義・アーキテクチャ
- [event-monitor データベース設計](database.md) -- データベーススキーマ・マイグレーション
