# system-policy-server デプロイ設計

system-policy-server の Dockerfile・Helm values・デプロイ設定を定義する。概要・API 定義・アーキテクチャは [system-policy-server.md](server.md) を参照。
...
| 項目 | 値 |
| --- | --- |
| サーバー名 | `k1s0-policy-server` |
| Tier | system |
| 実装言語 | Rust |
| REST ポート | 8096（ホスト側）/ 8080（コンテナ内） |
| gRPC ポート | 50051 |
| DB | PostgreSQL（`policy` スキーマ） |
| Kafka | プロデューサー（`k1s0.system.policy.updated.v1`） |
| OPA | HTTP API 連携（`http://opa:8181`） |
| キャッシュ | moka（評価結果 TTL 30 秒、最大 50,000 エントリ） |
| イメージレジストリ | `harbor.internal.example.com/k1s0-system/policy` |
| ビルドコンテキスト | `regions/system` |

policy-server は OPA 連携の動的ポリシー評価サーバーである。Rego ポリシーの管理・バンドル管理・評価キャッシュを提供し、ポリシー変更時に Kafka で変更通知を配信する。
...
| ビルドコマンド | `cargo build --release -p k1s0-policy-server` |
...
COPY . .
RUN cargo build --release -p k1s0-policy-server
...
policy-server は `config.yaml` による設定を基本とし、環境変数でのオーバーライドをサポートする。

| 環境変数 | 説明 | デフォルト |
| --- | --- | --- |
| `APP_NAME` | アプリケーション名 | `policy` |
| `APP_ENVIRONMENT` | 実行環境（`dev` / `staging` / `production`） | `dev` |
| `SERVER_HOST` | バインドアドレス | `0.0.0.0` |
| `SERVER_PORT` | REST API ポート | `8096` |
...
| `OPA_URL` | OPA HTTP API エンドポイント | - |
| `OPA_TIMEOUT_MS` | OPA タイムアウト（ミリ秒） | `2000` |
| `KAFKA_BROKERS` | Kafka ブローカーアドレス（カンマ区切り） | - |
| `KAFKA_SECURITY_PROTOCOL` | Kafka セキュリティプロトコル | `PLAINTEXT` |
| `KAFKA_TOPIC` | ポリシー変更通知トピック | `k1s0.system.policy.updated.v1` |
| `CACHE_MAX_ENTRIES` | キャッシュ最大エントリ数 | `50000` |
| `CACHE_TTL_SECONDS` | キャッシュ TTL（秒） | `30` |
...
ポリシーデータは PostgreSQL の `policy` スキーマに格納する。policies テーブルと policy_bundles テーブルで構成される。詳細スキーマは [database.md](database.md) 参照。
...
| Readiness | `/readyz` | 8080 | DB・OPA 接続確認を含むトラフィック受付可否 |
...
# Readiness Probe: DB・OPA 接続を含むトラフィック受付可否の確認
readinessProbe:
  httpGet:
    path: /readyz
    port: 8080
...
> policy-server は moka キャッシュ（最大 50,000 エントリ）を持つため、メモリを多めに確保する。キャッシュサイズに応じて limits の調整が必要。
...
> ポリシー評価はキャッシュにより高スループットを実現するため、maxReplicas は 5 に設定。各インスタンスが独自のキャッシュを持つことに注意。

---

## Helm Chart

Helm Chart は `infra/helm/services/system/policy/` に配置。k1s0-common チャート（v0.1.0）に依存。
...
| --- | --- | --- | --- |
| docker-compose | 8096 | 8080 | REST API |
...
| サービス | 用途 | 必須 |
| --- | --- | --- |
| PostgreSQL | ポリシー・バンドルの永続化 | Yes |
| OPA | Rego ポリシー評価エンジン | No（未設定時はフォールバック評価） |
| Kafka | ポリシー変更通知 | No（未設定時は Noop publisher） |
| Keycloak | JWT 認証 | Yes |
...
services:
  - name: policy-v1
    url: http://policy-server.k1s0-system.svc.cluster.local:80
    routes:
      - name: policy-v1-route
        paths:
          - /api/v1/policies
        strip_path: false
      - name: policy-v1-bundles-route
        paths:
          - /api/v1/bundles
        strip_path: false
    plugins:
...
### CI（`.github/workflows/policy-ci.yaml`）

PR 時に `regions/system/server/rust/policy/**` の変更を検出してトリガーする。
...
### CD（`.github/workflows/policy-deploy.yaml`）
...
| infrastructure/opa | 統合テスト | `wiremock`（OPA HTTP API モック） |

---

## 関連ドキュメント

- [system-policy-server.md](server.md) -- 概要・API 定義・アーキテクチャ
