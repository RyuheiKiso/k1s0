...
| ロール名 | リソース/アクション |
|---------|-----------------|
| sys_auditor 以上 | events/read |
| sys_operator 以上 | events/write |
| sys_admin のみ | events/admin |
...
| 認可 | 参照は `event-store/read`、追記・スナップショット作成は `event-store/write`、ストリーム削除は `event-store/admin` |
| ポート | ホスト側 8099（内部 8099）、gRPC 50051 |
...
| --- | --- | --- | --- |
| POST | `/api/v1/events` | イベント追記（`stream_id` はリクエストボディで指定） | `event-store/write` |
| GET | `/api/v1/events` | 全イベント一覧取得 | `event-store/read` |
| GET | `/api/v1/events/:stream_id` | ストリーム別イベント一覧取得 | `event-store/read` |
| GET | `/api/v1/streams/:stream_id/events/:sequence` | シーケンス指定の単一イベント取得 | `event-store/read` |
| GET | `/api/v1/streams` | ストリーム一覧取得 | `event-store/read` |
| POST | `/api/v1/streams/:stream_id/snapshot` | スナップショット作成 | `event-store/write` |
| GET | `/api/v1/streams/:stream_id/snapshot` | 最新スナップショット取得 | `event-store/read` |
| DELETE | `/api/v1/streams/:stream_id` | ストリーム削除（監査・テスト用途に限定） | `event-store/admin` |
...
| フィールド | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `stream_id` | string | Yes | イベントを追記するストリーム ID |
| `aggregate_type` | string | No | 集約種別（新規ストリーム作成時に使用） |
...
> `state` の型: REST は JSON object、gRPC (`CreateSnapshotRequest.state`) は `bytes`。
...
Prometheus 形式のメトリクスを返す。認証不要。`Content-Type: text/plain; version=0.0.4; charset=utf-8`。
...
- ListStreams, AppendEvents, ReadEvents, ReadEventBySequence, CreateSnapshot, GetLatestSnapshot
