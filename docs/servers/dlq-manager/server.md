...
配置: `regions/system/server/rust/dlq-manager/`（[Tier別配置パス参照](../../templates/server/サーバー.md#tier-別配置パス)）

---

## 設計方針

[メッセージング設計.md](../../architecture/messaging/メッセージング設計.md) の DLQ パターンに基づき、以下の方針で実装する。
...
全エンドポイントは [API設計.md](../../architecture/api/API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_DLQ_` とする。
...
> JSON 例は [server.guide.md](./server.guide.md#get-apiv1dlqtopic) を参照。

#### GET /api/v1/dlq/messages/:id

ID 指定で DLQ メッセージの詳細を取得する。

> JSON 例は [server.guide.md](./server.guide.md#get-apiv1dlqmessagesid) を参照。
...
> JSON 例は [server.guide.md](./server.guide.md#post-apiv1dlqmessagesidretry) を参照。

#### DELETE /api/v1/dlq/messages/:id

DLQ メッセージを削除する。

> JSON 例は [server.guide.md](./server.guide.md#delete-apiv1dlqmessagesid) を参照。
...
> JSON 例は [server.guide.md](./server.guide.md#post-apiv1dlqtopicretry-all) を参照。
...
> メッセージ取り込みフローは [server.guide.md](./server.guide.md#kafka-コンシューマー設計) を参照。
...
| adapter/grpc | gRPC ハンドラー | プロトコル変換（tonic） |
...
> 統合テスト一覧は [server.guide.md](./server.guide.md#統合テスト一覧) を参照。
