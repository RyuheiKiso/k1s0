...
| `versions[].breaking_change_details` | BreakingChange[] | 破壊的変更の詳細 |
...
| `breaking_change_details` | BreakingChange[] | 破壊的変更の詳細 |
...
| `non_breaking_changes` | BreakingChange[] | 非破壊的変更のリスト |
...
| `breaking_changes` | bool | 破壊的変更フラグ |
| `diff.added` | BreakingChange[] | 追加された要素 |
| `diff.modified` | BreakingChange[] | 変更された要素 |
| `diff.removed` | BreakingChange[] | 削除された要素 |
...
message ApiSchemaVersionProto {
  ...
  repeated BreakingChange breaking_change_details = 9;
}
...
message CompatibilityResultProto {
  ...
  repeated BreakingChange non_breaking_changes = 3;
}
...
| `non_breaking_changes` | Vec\<BreakingChange\> | 非破壊的変更のリスト |
...
- `ApiSchemaVersionProto.breaking_change_details` は `repeated BreakingChange`。
- 追加メッセージ: `BreakingChange`, `SchemaDiffProto`, `DiffEntryProto`, `DiffModifiedEntryProto`。
