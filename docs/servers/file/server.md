...
ファイルメタデータ一覧をページネーション付きで取得する。テナント ID・MIME type でフィルタリングできる（`tag` は未実装）。
...
| `uploaded_by` | string | No | - | アップロード実行ユーザー ID でフィルタ |
| `mime_type` | string | No | - | MIME type でフィルタ（例: `image/`） |
| `tag` | string | No | - | 現在は未実装（指定しても無視される） |
| `page` | int | No | 1 | ページ番号 |
...
`expires_in_seconds` の指定は現状未対応で、有効期限は **3600 秒固定**。
...
現在の実装では usecase 層のイベント発行処理は未実装であり、以下は設計上のイベントフォーマット（TODO）を示す。
