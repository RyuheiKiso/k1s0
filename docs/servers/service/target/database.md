...
target-db は service Tier に属する PostgreSQL 17 データベースであり、OKR 目標（Objective と Key Results）と進捗情報を管理する。Outbox pattern によるイベント配信の信頼性を確保するための outbox_events テーブルも備える。

[tier-architecture.md](../../architecture/overview/tier-architecture.md) の設計原則に従い、target-db へのアクセスは **target-server からのみ** 許可する。
...
| データベース | `k1s0_service` |
| スキーマ | `target_service` |
| マイグレーションパス | `regions/service/target/database/postgres/migrations/` |
...
```sql
CREATE TABLE target_service.board_columns (
...
CREATE INDEX IF NOT EXISTS idx_board_columns_project_id ON target_service.board_columns (project_id);
CREATE INDEX IF NOT EXISTS idx_board_columns_project_status ON target_service.board_columns (project_id, status_code);
```
...
```sql
CREATE TABLE target_service.outbox_events (
    id             UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_board_outbox_unpublished
    ON target_service.outbox_events (created_at)
...
FROM target_service.board_columns
...
UPDATE target_service.board_columns
...
```sql
INSERT INTO target_service.board_columns (id, project_id, status_code, task_count, wip_limit, version)
VALUES ($1, $2, $3, 0, 0, 1)
...
配置先: `regions/service/target/database/postgres/migrations/`

```
migrations/
├── 001_create_board_columns.up.sql     # target_service スキーマ・board_columns テーブル・インデックス・制約
...
| 番号 | ファイル名 | 説明 |
|------|-----------|------|
| 001 | create_board_columns | `target_service` スキーマ・`board_columns` テーブル・UNIQUE 制約・CHECK 制約・インデックス |
| 002 | create_outbox | `outbox_events` テーブル・未配信イベント用部分インデックス |

### 001_create_board_columns.up.sql

```sql
CREATE SCHEMA IF NOT EXISTS target_service;

SET search_path TO target_service;
...
```sql
CREATE OR REPLACE FUNCTION target_service.update_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = now();
    RETURN NEW;
...
CREATE TRIGGER trg_board_columns_updated_at
    BEFORE UPDATE ON target_service.board_columns
    FOR EACH ROW EXECUTE FUNCTION target_service.update_updated_at();
```
...
| 動的クレデンシャル（読み書き） | `database/creds/target-server-rw` | TTL: 24時間 |
