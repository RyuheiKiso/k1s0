# service-board-server デプロイ設計

service-board-server の Dockerfile・Helm values・デプロイ設定を定義する。概要・API 定義・アーキテクチャは [server.md](server.md) を参照。
...
| 項目 | 値 |
| --- | --- |
| サーバー名 | `k1s0-board-server` |
| Tier | service |
| 実装言語 | Rust |
| REST ポート | 8080（コンテナ内） |
| gRPC ポート | 50051（コンテナ内） |
| DB | PostgreSQL（`board_service` スキーマ） |
| Kafka | プロデューサー（`k1s0.service.board.column_updated.v1`） |
| イメージレジストリ | `harbor.internal.example.com/k1s0-service/board` |
...
| ビルドコマンド | `cargo build --release -p k1s0-board-server` |
...
RUN cargo chef cook --release --recipe-path recipe.json -p k1s0-board-server

FROM cook AS builder
COPY . .
RUN cargo build --release -p k1s0-board-server
...
COPY --from=builder /app/target/release/k1s0-board-server /k1s0-board-server
COPY --from=busybox /bin/busybox /busybox

USER nonroot:nonroot
EXPOSE 8080 50051

ENTRYPOINT ["/k1s0-board-server"]
```
...
| シークレット | パス |
| --- | --- |
| DB パスワード | `secret/data/k1s0/service/board/database` |
...
ボードデータは PostgreSQL の `board_service` スキーマに格納する。詳細スキーマは [database.md](database.md) 参照。

マイグレーションは `sqlx migrate run` で実行する。Advisory Lock ID: `1000000011`。

Docker Compose 起動時のスキーマ初期化: `infra/docker/init-db/16-board-schema.sql`
...
vault:
  ...
  role: "service"
  secrets:
    - path: "secret/data/k1s0/service/board/database"
      key: "password"
...
board-rust:
  build:
    context: .
    dockerfile: ./regions/service/board/server/rust/board/Dockerfile
  profiles: [service]
  ports:
    - "${BOARD_REST_HOST_PORT:-8321}:8080"
    - "${BOARD_GRPC_HOST_PORT:-9321}:50051"
  environment:
  ...
  depends_on:
    ...
    kafka:
      condition: service_healthy
  volumes:
    - ./regions/service/board/server/rust/board/config:/app/config
```
...
| Kafka | ボードカラム更新イベント配信 | No（未設定時は NoopBoardEventPublisher） |
| Keycloak | JWT 認証 | Yes |
...
### CI（`.github/workflows/board-ci.yaml`）

変更トリガーパス:
- `regions/service/board/server/**`
- `regions/service/board/client/**`
- `regions/service/board/database/**`

```
detect-changes → lint-rust → test-rust → build-rust → security-scan
```

### CD（`.github/workflows/board-deploy.yaml`）
