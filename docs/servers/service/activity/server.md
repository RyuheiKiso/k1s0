...
| リソース/アクション | 対応ロール |
|-----------------|---------|
| `activities/read` | svc_viewer / svc_operator / svc_admin / sys_admin |
| `activities/write` | svc_operator / svc_admin / sys_admin |
| `activities/admin` | svc_admin / sys_admin |
...
| アクション | 対象エンドポイント |
|-----------|-----------------|
| `activities/read` | GET（アクティビティ一覧・詳細取得） |
| `activities/write` | POST / PUT（作成・提出） |
| `activities/admin` | PUT（承認・却下：`/approve`・`/reject`） |
...
<!-- DOCS-CRIT-002 対応: 実装（adapter/handler/mod.rs）に合わせて HTTP メソッドと認可スコープを修正 -->
<!-- submit/approve/reject は PUT、認可スコープは activities/{write,admin} を使用する -->
| Method | Path | Description | 認可 |
| --- | --- | --- | --- |
| POST | `/api/v1/activities` | アクティビティ作成 | `activities:write` |
| GET | `/api/v1/activities` | アクティビティ一覧取得 | `activities:read` |
| GET | `/api/v1/activities/{activity_id}` | アクティビティ詳細取得 | `activities:read` |
| PUT | `/api/v1/activities/{activity_id}/submit` | 提出（承認待ち） | `activities:write` |
| PUT | `/api/v1/activities/{activity_id}/approve` | 承認 | `activities:admin` |
| PUT | `/api/v1/activities/{activity_id}/reject` | 却下 | `activities:admin` |
...
| `activity_created_topic` | string | `k1s0.service.activity.created.v1` | アクティビティ作成イベントのトピック名 |
| `activity_approved_topic` | string | `k1s0.service.activity.approved.v1` | アクティビティ承認イベントのトピック名 |
...
旧来のフォールバック（"system" テナントへのサイレント降格）は廃止済み。

```
x-tenant-id ヘッダー → 取得成功 → UseCase 実行（tenant_id スコープ）
                       取得失敗 → UNAUTHENTICATED エラー返却（認証ミドルウェアの設定漏れを即検出）
```
