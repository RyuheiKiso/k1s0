...
| DELETE | `/api/v1/services/:id` | サービス削除 | `services` | `admin` |
...
| GET | `/api/v1/teams/:id/services` | チーム別サービス取得 | `teams` | `read` |
...
| `DELETE /api/v1/services/:id` | `services` | `admin` |
| `GET /api/v1/teams/**` | `teams` | `read` |

> **権限表記**: RBAC は `resource/action` で表記する（例: `services/read`, `services/write`, `services/admin`）。
