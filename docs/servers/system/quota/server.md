...
| ロール名 | リソース/アクション |
|---------|-----------------|
| sys_auditor 以上 | quotas/read |
| sys_operator 以上 | quotas/write |
| sys_admin のみ | quotas/admin |
...
| POST | `/api/v1/quotas/{id}/check` | クォータチェック（残量判定） | `sys_auditor` 以上 |
...
指定したクォータポリシーを更新する（全フィールド指定）。

**リクエスト例**

```json
{
  "name": "スタンダードプランAPIクォータ(v2)",
  "subject_type": "tenant",
  "subject_id": "tenant-abc",
  "limit": 15000,
  "period": "daily",
...
#### POST /api/v1/quotas/{id}/check

指定したクォータの残量を判定する（read-only）。レスポンス形式は `GET /api/v1/quotas/{id}/usage` と同一。
...
REST では `quota_id` は Path パラメータ（`{id}`）で指定し、gRPC では `ResetQuotaUsageRequest.quota_id` の request body フィールドで指定する。
...
Proto の `ResetQuotaUsageResponse` は `QuotaUsage usage = 1` を返す。REST も同じ形式で返す。
...
```json
{
  ...
  "period_end": "2026-02-23T23:59:59.999+00:00",
  "reset_at": "2026-02-24T00:00:00.000+00:00",
  "retrieved_at": "2026-02-23T15:00:00.000+00:00"
}
```
...
| `retrieved_at` | DateTime\<Utc\> | 取得日時（REST レスポンス用、Proto には含まない） |
...
```
                    ...
                    │  │  CheckQuota / IncrementQuotaUsage /      │   │
                    │  │  ResetQuotaUsage                         │   │
                    │  └──────────────────────┬───────────────────┘   │
                    └─────────────────────────┼───────────────────────┘
                                              │
