...
`workflow_postgres.rs` が参照するワークフロー定義永続化テーブル。Saga 実行時の最新定義を保持する。

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| name | VARCHAR(255) | PK | ワークフロー名 |
| version | INT | NOT NULL DEFAULT 1 | 定義バージョン |
| definition | JSONB | NOT NULL | ワークフロー定義本体 |
...
├── 006_create_workflow_definitions.up.sql      # workflow_definitions テーブル
├── 006_create_workflow_definitions.down.sql
...
#### 006_create_workflow_definitions.up.sql
...
-- saga_states テーブルに tenant_id カラムを追加する
...
-- スーパーユーザー・オーナーロールも RLS の適用対象とする
...
| workflow_definitions | idx_workflow_definitions_enabled | enabled | B-tree | 有効フラグでのフィルタリング |
