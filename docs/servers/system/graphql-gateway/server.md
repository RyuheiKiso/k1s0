...
| バックエンド通信 | tonic gRPC クライアントで各バックエンドサービスを呼び出す（service-catalog は reqwest REST クライアント、ADR-0039 参照） |
...
| ポート | ホスト側 8092（内部 8080）<!-- H-18 監査対応: ポート番号を docker-compose 実態に合わせて修正 --> |
...
| `service_catalog.address` | string | サービスカタログサービス REST エンドポイント（reqwest HTTP クライアント、ADR-0039 参照） |
...
graphql-gateway は以下の system tier サービスをバックエンドとして利用する（service-catalog のみ REST、他は gRPC）。

| バックエンドサービス | 用途 | 通信方式 | タイムアウト |
| --- | --- | --- | --- |
| auth | ユーザー・ロール・監査ログ管理 | gRPC | 2000ms |
| session | セッション CRUD | gRPC | 2000ms |
| tenant | テナント管理 | gRPC | 3000ms |
| featureflag | フィーチャーフラグ管理 | gRPC | 3000ms |
| config | 設定値管理 | gRPC | 3000ms |
| navigation | ナビゲーション定義 | gRPC | 3000ms |
| service-catalog | サービスカタログ | REST（reqwest、ADR-0039） | 3000ms |
| vault | シークレット管理 | gRPC | 5000ms |
| workflow | ワークフロー管理 | gRPC | 5000ms |
| scheduler | ジョブスケジューリング | gRPC | 2000ms |
| notification | 通知管理 | gRPC | 2000ms |
| ratelimit | レート制限（gRPC 連携） | gRPC | 設定による |
