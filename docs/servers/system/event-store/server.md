...
| 認可 | 参照は `events/read`、追記・スナップショット作成は `events/write`、ストリーム削除は `events/admin` |
...
| --- | --- | --- | --- |
| POST | `/api/v1/events` | イベント追記（`stream_id` はリクエストボディで指定） | `events/write` |
| GET | `/api/v1/events` | 全イベント一覧取得 | `events/read` |
| GET | `/api/v1/events/{stream_id}` | ストリーム別イベント一覧取得 | `events/read` |
| GET | `/api/v1/streams/{stream_id}/events/{sequence}` | シーケンス指定の単一イベント取得 | `events/read` |
| GET | `/api/v1/streams` | ストリーム一覧取得 | `events/read` |
| POST | `/api/v1/streams/{stream_id}/snapshot` | スナップショット作成 | `events/write` |
| GET | `/api/v1/streams/{stream_id}/snapshot` | 最新スナップショット取得 | `events/read` |
| DELETE | `/api/v1/streams/{stream_id}` | ストリーム削除（監査・テスト用途に限定） | `events/admin` |
