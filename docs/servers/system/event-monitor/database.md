...
**UNIQUE インデックス:** `idx_flow_definitions_tenant_name` `(tenant_id, name)` — テナントスコープでのフロー名一意性
...
**Row Level Security:** `ENABLE / FORCE / AS RESTRICTIVE / WITH CHECK` — `event_monitor.event_records` に設定済み
...
**Row Level Security:** `ENABLE / FORCE / AS RESTRICTIVE / WITH CHECK` — `event_monitor.flow_instances` に設定済み

---

## マルチテナント対応（CRITICAL-DB-001）

全テーブルに `tenant_id TEXT NOT NULL` カラムと RLS ポリシーを追加（migration 003）。

- `flow_definitions`: UNIQUE(name) を UNIQUE(tenant_id, name) に変更
- `event_records` / `flow_instances`: テナントスコープで RLS 適用
