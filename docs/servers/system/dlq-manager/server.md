...
| `SYS_DLQ_PROCESS_FAILED` | 500 | 再処理実行の失敗 |
...
| `payload` | bytes（JSON UTF-8） | メッセージペイロード。保存・伝送時は bytes、REST 表示時は JSON object |
| `status` | DlqStatus | メッセージステータス（PENDING / RETRYING / RESOLVED / DEAD） |
| `created_at` | DateTime\<Utc\> | 作成日時 |
| `updated_at` | DateTime\<Utc\> | 更新日時 |
| `last_retry_at` | Option\<DateTime\<Utc\>\> | 最終リトライ日時 |

> `payload` は proto では `bytes`。REST API では可読性のため UTF-8 デコード後の JSON object として返却する。
