...
| --- | --- | --- | --- |
| GET | `/api/v1/schemas` | スキーマ一覧取得 | `schemas/read` |
| POST | `/api/v1/schemas` | スキーマ登録（初回バージョン） | `schemas/write` |
| GET | `/api/v1/schemas/{name}` | スキーマ取得（最新バージョン） | `schemas/read` |
| GET | `/api/v1/schemas/{name}/versions` | バージョン一覧取得 | `schemas/read` |
| GET | `/api/v1/schemas/{name}/versions/{version}` | 特定バージョン取得 | `schemas/read` |
| POST | `/api/v1/schemas/{name}/versions` | 新バージョン登録 | `schemas/write` |
| DELETE | `/api/v1/schemas/{name}/versions/{version}` | バージョン削除 | `schemas/admin` |
| POST | `/api/v1/schemas/{name}/compatibility` | 互換性チェック（破壊的変更検出） | `schemas/write` |
| GET | `/api/v1/schemas/{name}/diff` | バージョン間差分取得 | `schemas/read` |
...
| `versions[].breaking_change_details` | SchemaChange[] | 破壊的変更の詳細 |
...
| `breaking_change_details` | SchemaChange[] | 破壊的変更の詳細 |
...
| `breaking_changes` | SchemaChange[] | 破壊的変更のリスト |
| `non_breaking_changes` | SchemaChange[] | 非破壊的変更のリスト |
...
| `breaking_changes` | bool | 破壊的変更フラグ |
| `diff.added` | SchemaChange[] | 追加された要素 |
| `diff.modified` | SchemaChange[] | 変更された要素 |
| `diff.removed` | SchemaChange[] | 削除された要素 |
...
| `breaking_change_details` | Vec\<SchemaChange\> | 破壊的変更の詳細リスト |
...
| `breaking_changes` | Vec\<SchemaChange\> | 破壊的変更のリスト |
| `non_breaking_changes` | Vec\<SchemaChange\> | 非破壊的変更のリスト |

#### SchemaChange（破壊的変更の種別）
...
| Kafka | プロデューサー（`k1s0.system.apiregistry.schema_updated.v1`）。設定キーは `topic`（旧 alias: `schema_updated_topic`） |
| 認証 | JWT による認可。権限は resource/action 形式（`schemas/read`, `schemas/write`, `schemas/admin`） |
...
kafka:
  ...
  schema_updated_topic: "k1s0.system.apiregistry.schema_updated.v1"  # 互換 alias
...
- `ApiSchemaVersionProto.breaking_change_details` は `repeated SchemaChange`。
- 追加メッセージ: `SchemaChange`, `SchemaDiffProto`, `DiffEntryProto`, `DiffModifiedEntryProto`。
