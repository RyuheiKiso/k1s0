...
| `breaking_change_details` | JSONB | NOT NULL | '[]' | 破壊的変更点の詳細配列 |
...
| `005_add_breaking_change_details.up.sql` | `api_schema_versions.breaking_change_details` カラム追加 |
| `006_add_updated_at_trigger.up.sql` | `api_schemas` テーブルに `updated_at` 自動更新トリガー関数およびトリガーを追加 |
| `007_add_tenant_id_rls.up.sql` | `api_schemas` / `api_schema_versions` に `tenant_id TEXT NOT NULL` と RLS ポリシー（AS RESTRICTIVE / WITH CHECK）を追加（CRITICAL-DB-001 対応） |
