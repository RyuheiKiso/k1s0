...
| リソース/アクション | 対応ロール |
|-----------------|---------|
| `workflows/read` | sys_auditor 以上 |
| `workflows/write` | sys_operator 以上 |
| `workflows/admin` | sys_admin のみ |
...
| --- | --- |
| 実装言語 | Rust |
| ワークフロー定義形式 | JSON/YAML で記述したステップ定義（`step_type: human_task` / `step_type: automated`）をDBに保存 |
...
| ポート | 8100（REST）/ 50051（gRPC） |
| 自動マイグレーション | 起動時に `sqlx::migrate!()` で `workflow-db/migrations` を自動適用（C-01 対応） |
...
| DELETE | `/api/v1/workflows/{id}` | ワークフロー定義削除 | `workflows/admin` 権限（`sys_admin` のみ） |
...
| GET | `/api/v1/instances/{id}/status` | インスタンス実行ステータス取得 | `sys_auditor` 以上 |
| POST | `/api/v1/instances/{id}/cancel` | インスタンスキャンセル | `workflows/admin` 権限（`sys_admin` のみ） |
...
| POST | `/internal/tasks/check-overdue` | 期日超過タスク検出と通知発行 | 認証不要（内部エンドポイント） |
...
新しいワークフロー定義を作成する。`steps` はステップ定義の配列で、各ステップは `step_type: human_task` または `step_type: automated` を指定する。
...
期日超過タスクを検出し、notification-server 向けイベントを発行する内部エンドポイント。scheduler-server から定期実行される。
  ...
  "tasks": [
    {
      ...
      "assignee_id": "usr_01JABCDEF1234567890",
      "due_at": "2026-02-23T10:00:00.000+00:00"
    }
  ]
}
```
...
| `SYS_WORKFLOW_DISABLED` | 409 | ワークフロー定義が無効化されている |
| `SYS_WORKFLOW_NO_STEPS` | 400 | ステップ定義が空 |
| `SYS_WORKFLOW_DEFINITION_NOT_FOUND` | 404 | ワークフロー定義が存在しない |
| `SYS_WORKFLOW_INSTANCE_INVALID_STATUS` | 409 | インスタンス状態が操作要件を満たさない |
| `SYS_WORKFLOW_TASK_INVALID_STATUS` | 409 | タスク状態が操作要件を満たさない |
| `SYS_WORKFLOW_TASK_REASSIGN_INVALID_STATUS` | 409 | 再割当可能なタスク状態ではない |
...
タスク期日超過の監視は scheduler-server にジョブを登録して行う（15分ごとに `http://workflow.k1s0-system.svc.cluster.local:8100/internal/tasks/check-overdue` を呼び出し）。期日超過タスクが検出された場合は notification-server の Kafka トピック `k1s0.system.notification.requested.v1` へメッセージを送信する。
...
> `CheckOverdueTasksUsecase` は `notification_request_publisher` を利用し、期日超過タスクを `k1s0.system.notification.requested.v1` へ発行する。
...
{
  "error": {
    "code": "SYS_WORKFLOW_INSTANCE_INVALID_STATUS",
...
```json
{
  ...
  "timezone": "UTC",
  "target_type": "http",
  "target": "http://workflow.k1s0-system.svc.cluster.local:8100/internal/tasks/check-overdue",
  "payload": {}
}
```
