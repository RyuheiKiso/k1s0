...
#### テナント分離（RUST-CRIT-001 対応）

マルチテナント環境でのデータ漏洩を防ぐため、全リクエストにテナント境界を適用する。

**JWT Claims からの tenant_id 抽出**（REST）

```rust
// HTTP ハンドラーでは Option<Extension<Claims>> を受け取り tenant_id を取得する
fn tenant_id_from_claims(claims: Option<&Claims>) -> String {
    claims.map(|c| c.tenant_id().to_string())
          .unwrap_or_else(|| "system".to_string())
}
```

**gRPC リクエスト Extensions からの tenant_id 抽出**

`GrpcAuthLayer` がリクエスト Extensions に `Claims` を挿入し、`tonic_service.rs` で `request.extensions().get::<Claims>()` を使って取得する。

**リポジトリへの伝播**

全ユースケース Input 構造体に `tenant_id: String` フィールドを持ち、リポジトリメソッド呼び出し時に渡す。PostgreSQL リポジトリ実装では `SET LOCAL app.current_tenant_id = $1` を実行して RLS を有効化する。
