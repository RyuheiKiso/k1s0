...
| 認証 | JWTによる認可。管理系エンドポイントは `scheduler/read`, `scheduler/write`, `scheduler/admin` を使用 |
| ポート | REST 8095 / gRPC 50051 |
| 自動マイグレーション | 起動時に `sqlx::migrate!()` で `scheduler-db/migrations` を自動適用（C-01 対応） |
...
| --- | --- | --- | --- |
| GET | `/api/v1/jobs` | ジョブ一覧取得 | `scheduler/read` |
| GET | `/api/v1/jobs/{id}` | ジョブ詳細取得 | `scheduler/read` |
| POST | `/api/v1/jobs` | ジョブ作成 | `scheduler/write` |
| PUT | `/api/v1/jobs/{id}` | ジョブ更新 | `scheduler/write` |
| DELETE | `/api/v1/jobs/{id}` | ジョブ削除 | `scheduler/admin` |
| POST | `/api/v1/jobs/{id}/trigger` | ジョブ手動実行 | `scheduler/write` |
| PUT | `/api/v1/jobs/{id}/pause` | ジョブ一時停止 | `scheduler/write` |
| PUT | `/api/v1/jobs/{id}/resume` | ジョブ再開 | `scheduler/write` |
| GET | `/api/v1/jobs/{id}/executions` | 実行履歴一覧 | `scheduler/read` |
...
| `name_prefix` | string | No | - | ジョブ名の前方一致フィルタ |
| `page` | int | No | 1 | ページ番号 |
...
| `SYS_SCHED_INVALID_TIMEZONE` | 400 | タイムゾーンが不正 |
...
| `SYS_SCHED_INVALID_CRON` / `SYS_SCHED_INVALID_TIMEZONE` / `SYS_SCHED_VALIDATION_ERROR` | `InvalidArgument` | 入力不正 |
...
| レイヤー | モジュール | 責務 |
| --- | --- | --- |
| domain/entity | `SchedulerJob`, `SchedulerExecution` | エンティティ定義 |
| domain/repository | `SchedulerJobRepository`, `SchedulerExecutionRepository` | リポジトリトレイト |
...
| infrastructure/persistence | `SchedulerJobPostgresRepository`, `SchedulerExecutionPostgresRepository` | PostgreSQL リポジトリ実装 |
...
> `duration_ms` はエンティティには格納せず、ハンドラーで `finished_at - started_at`（ミリ秒）から算出する計算フィールドである。REST / gRPC レスポンスには含まれるが、永続化対象ではない。
