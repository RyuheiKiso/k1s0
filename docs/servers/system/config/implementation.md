...
│   ├── main.rs                          # エントリポイント
│   ├── lib.rs                           # ライブラリルート
│   ├── proto/                           # tonic-build 生成ファイル
│   │   ├── mod.rs
│   │   ├── k1s0.system.config.v1.rs
...
│   │   ├── grpc/
│   │   │   ├── mod.rs
│   │   │   ├── config_grpc.rs           # gRPC ハンドラー（設定値）
│   │   │   ├── tonic_service.rs         # tonic サービス登録
│   │   │   └── watch_stream.rs          # WatchConfig ストリーム実装
│   │   ├── repository/
│   │   │   ├── mod.rs
│   │   │   ├── cached_config_repository.rs    # キャッシュ付きリポジトリ
│   │   │   ├── config_postgres.rs             # ConfigRepository PostgreSQL 実装
│   │   │   ├── config_change_log_postgres.rs  # ConfigChangeLogRepository PostgreSQL 実装
│   │   │   └── config_schema_postgres.rs      # ConfigSchemaRepository PostgreSQL 実装
│   │   └── middleware/
│   │       ├── mod.rs
...
│       ├── database.rs                  # PostgreSQL 接続管理
│       ├── cache.rs                     # インメモリキャッシュ（moka）
...
| `config_server.audit` | `enabled` | bool | `true` | 監査ログ記録の有効化 |
| `config_server.audit` | `kafka_enabled` | bool | `false` | Kafka への非同期配信を有効化 |
...
| `config_server.namespace` | `default_prefix` | string | `"system"` | namespace のデフォルト接頭辞 |
