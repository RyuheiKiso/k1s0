...
| POST | `/api/v1/ratelimit/reset` | カウンターリセット | `ratelimit/admin`（`sys_admin`） |
...
`window` は Optional。未指定時はルール定義の `window_seconds` を使用する。
...
```json
{
  "rules": [
    ...
    },
    {
      ...
      "name": "service-task-global",
      "scope": "service",
      "identifier_pattern": "task-server",
      "limit": 1000,
      "window_seconds": 60,
      "enabled": true,
      "created_at": "2026-02-20T10:00:00.000+00:00",
...
新しいレートリミットルールを作成する。
`name` は `{scope}:{identifier_pattern}` 形式で自動生成される。
...
{
  "success": true,
  "message": "rate limit reset for user:user-001"
}
```
...
| スコープ | キーフォーマット | 例 |
| --- | --- | --- |
| service | `ratelimit:service:{service_name}:{window}` | `ratelimit:service:task-server:60` |
...
| レイヤー | モジュール | 責務 |
| --- | --- | --- |
| domain/entity | `RateLimitRule`, `RateLimitDecision` | エンティティ定義 |
| domain/repository | `RateLimitRuleRepository`（PostgreSQL）, `RateLimitStateRepository`（Redis） | リポジトリトレイト |
| domain/service | `RateLimitDomainService` | トークンバケット判定ロジック、scope/identifier バリデーション、ルール入力バリデーション、effective_limit_and_window 計算、アルゴリズム選択、fail-open 判定 |
| usecase | `CheckRateLimitUsecase`, `GetUsageUsecase`, `ResetLimitUsecase`, `CreateRuleUsecase`, `UpdateRuleUsecase`, `DeleteRuleUsecase`, `GetRuleUsecase`, `ListRulesUsecase` | ユースケース |
...
| `name` | String | ルール名（`{scope}:{identifier_pattern}` から自動生成） |
| `limit` | u32 | ウィンドウあたりの最大リクエスト数 |
| `window_seconds` | u32 | ウィンドウサイズ（秒） |
| `algorithm` | Algorithm | `TokenBucket` / `SlidingWindow` / `FixedWindow` / `LeakyBucket` |
| `enabled` | bool | ルールの有効/無効 |
| `created_at` | DateTime\<Utc\> | 作成日時 |
| `updated_at` | DateTime\<Utc\> | 更新日時 |

> **注**: ドメインモデルでは `limit` / `window_seconds` を `u32` として扱い負数を禁止する。gRPC/proto では後方互換のため `int64` を維持し、ハンドラーで正の `u32` に変換して検証する。
...
| `limit` | i64 | 適用ルールの上限 |
| `used` | i64 | 現在ウィンドウでの使用数 |
| `remaining` | i64 | 残余リクエスト数 |
| `reset_at` | DateTime\<Utc\> | ウィンドウリセット時刻 |
| `rule_id` | UUID | 適用されたルールの ID |
| `reason` | String | 判定理由（"allowed", "rate_limited" 等） |
...
```
    ...
    │  RateLimitDecision│            │ (PostgreSQL trait)         │   │
