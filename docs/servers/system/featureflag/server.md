...
| リソース/アクション | 対応ロール |
|-----------------|---------|
| `flags/read` | sys_auditor 以上 |
| `flags/write` | sys_operator 以上 |
| `flags/admin` | sys_admin のみ |
  ...
  "flags": [
    {
      ...
      "rules": [
        { "attribute": "tenant_id", "operator": "eq", "value": "tenant-001", "variant": "on" }
      ],
...
既存のフィーチャーフラグを更新する。`enabled` / `description` / `variants` / `rules` を更新可能。REST API では部分更新（省略フィールドは変更なし）。gRPC では `UpdateFlagRequest` の optional フィールドと repeated フィールドを使って更新する。
...
```json
{
  ...
  "variants": [
    { "name": "on", "value": "true", "weight": 90 },
    { "name": "off", "value": "false", "weight": 10 }
  ],
  "rules": [
    { "attribute": "tenant_id", "operator": "eq", "value": "tenant-001", "variant": "on" }
  ]
...
フラグを評価し、指定されたコンテキスト（ユーザー・テナント・属性）に基づいて有効/無効とバリアントを判定する。内部サービス用のエンドポイント。認証ミドルウェアが有効な場合は `flags/write` 権限が必要、無効な場合は認証不要。`context` は省略可能で、省略時は空コンテキストとして扱う。`context.attributes` 省略時は空マップ。
...
3. フラグが enabled=true の場合
   3a. ルール評価（match_rule_variant）
       → rules を順に評価し、最初にマッチしたルールの variant を返す
       → マッチしない場合はステップ 3b へ
   3b. 重み付きバリアント選択（select_weighted_variant）
       → weight > 0 のバリアントをフィルタ
       → user_id（なければ tenant_id、なければ "anonymous"）のハッシュで決定的バケット配置
       → 累積重みに基づいて variant を選択
       → 重み付きバリアントがない場合のみ先頭バリアントにフォールバック
   → EvaluationResult { enabled: true, variant: Some(selected), reason: "flag is enabled" }
```

#### ルール評価（match_rule_variant）

ルールごとに以下の手順で属性マッチングを行う。

1. **属性解決**: `rule.attribute` に応じてコンテキストから値を取得
