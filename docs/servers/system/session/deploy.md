# system-session-server デプロイ設計

system-session-server の Dockerfile・Helm values・デプロイ設定を定義する。概要・API 定義・アーキテクチャは [system-session-server.md](server.md) を参照。
...
| 項目 | 値 |
| --- | --- |
| サーバー名 | `k1s0-session-server` |
| Tier | system |
| 実装言語 | Rust |
| REST ポート | 8102 |
| gRPC ポート | 50051 |
| DB | PostgreSQL（`session` スキーマ）+ Redis 7（セッションデータ本体） |
| Kafka | コンシューマー（`k1s0.system.session.revoke_all.v1`）+ プロデューサー（`k1s0.system.session.created.v1`, `k1s0.system.session.revoked.v1`） |
| イメージレジストリ | `harbor.internal.example.com/k1s0-system/session` |
| ビルドコンテキスト | `regions/system` |

session-server は Redis によるステートフルセッション管理サービスである。マルチデバイス対応のセッション作成・取得・更新・失効を提供し、PostgreSQL にセッションメタデータを記録する。
...
| ビルドコマンド | `cargo build --release -p k1s0-session-server` |
| 公開ポート | 8102（REST API）、50051（gRPC） |
...
RUN cargo chef cook --release --recipe-path recipe.json -p k1s0-session-server

# builder ステージ: ソースコードのビルド
FROM cook AS builder
COPY . .
RUN cargo build --release -p k1s0-session-server
...
COPY --from=builder /app/target/release/k1s0-session-server /k1s0-session-server
COPY --from=busybox /bin/busybox /busybox

USER nonroot:nonroot
EXPOSE 8102 50051

ENTRYPOINT ["/k1s0-session-server"]
```
...
session-server は `config.yaml` による設定を基本とし、環境変数でのオーバーライドをサポートする。

| 環境変数 | 説明 | デフォルト |
| --- | --- | --- |
| `APP_NAME` | アプリケーション名 | `k1s0-session-server` |
| `APP_ENVIRONMENT` | 実行環境（`dev` / `staging` / `production`） | `dev` |
| `SERVER_HOST` | バインドアドレス | `0.0.0.0` |
| `SERVER_PORT` | REST API ポート | `8102` |
...
| `SESSION_DEFAULT_TTL_SECONDS` | セッションデフォルト TTL（秒） | `3600` |
| `SESSION_MAX_TTL_SECONDS` | セッション最大 TTL（秒） | `86400` |
| `KAFKA_BROKERS` | Kafka ブローカーアドレス（カンマ区切り） | - |
| `KAFKA_CONSUMER_GROUP` | Kafka コンシューマーグループ | `session-server.default` |
...
| シークレット | パス |
| --- | --- |
| DB パスワード | `secret/data/k1s0/system/session/database` |
| Redis パスワード | `secret/data/k1s0/system/session/redis` |
...
セッションメタデータ（デバイス情報・作成日時）は PostgreSQL の `session` スキーマに格納する。セッションデータ本体は Redis に TTL 付きで保存する。詳細スキーマは [database.md](database.md) 参照。
...
| エンドポイント | パス | ポート | 用途 |
| --- | --- | --- | --- |
| Liveness | `/healthz` | 8102 | プロセス生存確認 |
| Readiness | `/readyz` | 8102 | Redis・PostgreSQL・Kafka 接続確認を含むトラフィック受付可否 |
| Metrics | `/metrics` | 8102 | Prometheus メトリクス |
...
# Readiness Probe: Redis・PostgreSQL・Kafka 接続を含むトラフィック受付可否の確認
readinessProbe:
  httpGet:
    path: /readyz
    port: 8102
...
> session-server は Redis 接続プールと Kafka コンシューマー/プロデューサーを持つため、メモリを多めに確保する。
...
> セッション管理はレイテンシに敏感であるため、十分なレプリカ数を確保する。Kafka コンシューマーグループのリバランスも考慮して調整する。

---

## Helm Chart

Helm Chart は `infra/helm/services/system/session/` に配置。k1s0-common チャート（v0.1.0）に依存。
...
vault:
  ...
  secrets:
    ...
    - path: "secret/data/k1s0/system/session/redis"
      key: "password"
...
| --- | --- | --- | --- |
| docker-compose | 8102 | 8102 | REST API |
| docker-compose | 50051 | 50051 | gRPC |
| Kubernetes | 80（Service） | 8102（Pod） | REST API |
...
| サービス | 用途 | 必須 |
| --- | --- | --- |
| Redis 7 | セッションデータ本体の TTL 付き保存 | Yes |
| PostgreSQL | セッションメタデータ（デバイス情報等）の永続化 | Yes |
| Kafka | セッション作成/失効イベント配信・一括失効トリガー | No（未設定時は Noop publisher） |
| Keycloak | JWT 認証 | Yes |
...
services:
  - name: session-v1
    url: http://session-server.k1s0-system.svc.cluster.local:80
    routes:
      - name: session-v1-sessions-route
        paths:
          - /api/v1/sessions
        strip_path: false
      - name: session-v1-users-sessions-route
        paths:
          - /api/v1/users
        strip_path: false
    plugins:
...
### CI（`.github/workflows/session-ci.yaml`）

PR 時に `regions/system/server/rust/session/**` の変更を検出してトリガーする。
...
### CD（`.github/workflows/session-deploy.yaml`）
...
- [system-session-server.md](server.md) -- 概要・API 定義・アーキテクチャ
