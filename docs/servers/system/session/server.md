...
| ポート | HTTP 8106（docker-compose で 8106 にマップ、内部ポート 8080）<!-- H-19 監査対応: ポート番号を docker-compose 実態に合わせて修正 --> |
...
| `last_accessed_at` | Option\<DateTime\<Utc\>\> | 最終アクセス日時 |
| `revoked` | bool | 失効フラグ |
...
server:
  host: "0.0.0.0"
  port: 8080  # H-19 監査対応: コンテナ内部ポート。docker-compose ホスト側は 8106 にマップされる
  grpc_port: 50051
