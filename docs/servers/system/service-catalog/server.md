...
| キャッシュ | k1s0-cache（Redis）※将来実装予定（現在未実装） |
| イベント発行 | k1s0-kafka ※将来実装予定（現在未実装） |
...
全エンドポイントは [API設計.md](../../architecture/api/API設計.md) D-007 の統一エラーレスポンスに従う。エラーコードのプレフィックスは `SYS_SCAT_` とする。REST API を提供する。
...
| `tier` | string | No | - | ティアでフィルター（critical/standard/internal） |
...
| `services[].tier` | string | ティア（critical/standard/internal） |
...
| `tier` | string | Yes | ティア（critical/standard/internal） |
...
PostgreSQL への接続を確認する（Redis は将来実装予定のため現在は確認対象外）。
...
| `checks.cache` | string | Redis 接続状態（将来実装予定） |
