# プロジェクトマスタ管理 (Project Master Management)

## 概要

master-maintenance サーバーに `domain_scope` を導入し、業務領域（taskmanagement, fa 等）ごとに固有のマスタデータをスキーマ分離して管理する機能。
...
- NULL = システム共通（既存動作維持）
- 値あり = ドメイン固有（例: "taskmanagement", "fa"）
...
- domain_scope が設定されている場合、プロジェクト別トピックにも発行
