...
| ポート | サービス内部ポートは YAML 設定ファイル（`config.yaml` の `server.port`）で決まり、デフォルト **8080**。`BFF_PROXY_HOST_PORT` 環境変数は Docker ホスト側のポートマッピング（`docker-compose.yaml` の `127.0.0.1:${BFF_PROXY_HOST_PORT:-8082}:8080`）にのみ使用され、サービス内部の動作ポートには影響しない — M-016 監査対応 |
...
| POST | `/auth/logout` | Yes | Yes（設定時） | セッション削除・IdP ログアウト（HIGH-006: CSRF 保護追加済み） |
