...
auth-server が Keycloak Admin API を使用するには、以下の設定が必要。

### auth-rust-admin サービスアカウントの realm-management ロール付与

auth-server は定期的に Keycloak Admin API（`/admin/realms/k1s0/roles`）を呼び出してロール定義を取得する。
このため、`auth-rust-admin` Keycloak クライアントのサービスアカウントに以下のロールが必要（DOCKER-HIGH-001 監査対応）:

| ロール | 用途 |
|--------|------|
| `realm-management/view-users` | ユーザー一覧取得 |
| `realm-management/query-groups` | グループクエリ |
| `realm-management/view-clients` | クライアント一覧取得 |
| `realm-management/view-realm` | レルム情報取得 |

`infra/keycloak/realm-k1s0.json` の `users` セクションにこれらのロールマッピングが定義されている。
