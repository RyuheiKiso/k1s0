...
| リソース/アクション | 対応ロール |
|-----------------|---------|
| `auth/read` | sys_auditor 以上 |
| `auth/write` | sys_operator 以上 |
| `auth/admin` | sys_admin のみ |
...
ナビゲーション API は `navigation server`（`docs/servers/system/navigation/server.md`）が正規オーナー。

#### 保護エンドポイント（Bearer トークン必要）

##### Bearer トークン認証のみ（RBAC 不要）

> SEC-001（2026-03-20）: `POST /api/v1/api-keys/validate` は公開エンドポイントから保護エンドポイントに移動。Bearer トークン認証を必須とするが、サービス間検証用途のため RBAC チェックは不要。

| Method | Path | Description |
| --- | --- | --- |
| POST | `/api/v1/api-keys/validate` | API キー検証 |
...
| コード | HTTP Status | 説明 |
| --- | --- | --- |
| `SYS_AUTH_MISSING_TOKEN` | 401 | Authorization ヘッダーが存在しない |
| `SYS_AUTH_MISSING_CLAIMS` | 401 | 認証コンテキストに claims が存在しない |
| `SYS_AUTH_PERMISSION_DENIED` | 403 | 要求された resource/action の権限がない |
| `SYS_AUTH_AUDIT_VALIDATION` | 400 | 監査ログ入力のバリデーションエラー |
| `SYS_AUTH_INTERNAL_ERROR` | 500 | 認証・認可処理の内部エラー |
...
| `claims.typ` | string | トークン種別 |
| `claims.azp` | string | 認可されたクライアント ID |
...
API キーを検証し、有効/無効を返却する。無効（`invalid`/`revoked`/`expired`）時も HTTP 200 で `valid: false` を返す。

**リクエストフィールド**

| フィールド | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `api_key` | string | Yes | 検証対象の API キー（平文） |

**レスポンスフィールド（200 OK）**

| フィールド | 型 | 説明 |
| --- | --- | --- |
| `valid` | bool | API キーの有効性 |
| `tenant_id` | string? | 有効時のテナント ID |
| `name` | string? | 有効時の API キー名 |
| `scopes` | string[] | 有効時のスコープ（無効時は空配列） |
| `reason` | string? | 無効時の理由（`invalid` / `revoked` / `expired`） |

**エラーレスポンス（500）**: `SYS_AUTH_INTERNAL_ERROR`
...
| `search` | string | No | - | `username` / `email` の部分一致検索 |
| `enabled` | bool | No | - | 有効フラグでの絞り込み |
...
| `pagination.total_count` | int64 (REST) / int32 (gRPC PaginationResult) | 総件数 |
...
| フィールド | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `user_id` | string | No | ユーザー ID（指定時はユーザー由来ロールで判定） |
...
| --- | --- | --- |
| `allowed` | bool | 許可フラグ |
| `reason` | string | 拒否時の理由（`allowed=true` の場合はレスポンスから省略） |
...
**エラーレスポンス（400）**: `SYS_AUTH_AUDIT_VALIDATION`
...
**エラーレスポンス（400）**: `SYS_AUTH_API_KEY_VALIDATION`
...
> **権限表記**: RBAC は `resource/action` で表記する（例: `auth/read`, `auth/write`, `auth/admin`）。
...
| `security_protocol` | string | セキュリティプロトコル（例: `PLAINTEXT`, `SASL_SSL`） |
| `sasl` | object | SASL 設定 |
| `consumer_group` | string | コンシューマーグループ名 |
| `topics.publish` | string[] | 発行トピック一覧 |
| `topics.subscribe` | string[] | 購読トピック一覧 |
...
`user_cache` は現状コード上の固定値で運用する。

| フィールド | 型 | 値 | 説明 |
| --- | --- | --- | --- |
| `max_entries` | int | `5000` | ユーザーキャッシュの最大件数 |
| `ttl_secs` | int | `300` | ユーザーキャッシュ TTL（秒） |
...
| レイヤー | パッケージ / モジュール | 責務 |
| --- | --- | --- |
| domain/entity | `Claims`, `User`, `UserRoles`, `Role`, `AuditLog`, `ApiKey` | エンティティ定義 |
| domain/repository | `UserRepository`, `AuditLogRepository`, `ApiKeyRepository` | リポジトリインターフェース / トレイト |
| usecase | `ValidateTokenUseCase`, `GetUserUseCase`, `ListUsersUseCase`, `GetUserRolesUseCase`, `CheckPermissionUseCase`, `RecordAuditLogUseCase`, `SearchAuditLogsUseCase`, `CreateApiKeyUseCase`, `ListApiKeysUseCase`, `GetApiKeyUseCase`, `RevokeApiKeyUseCase`, `ValidateApiKeyUseCase` | ユースケース |
| adapter/handler | REST ハンドラー（`auth_handler`, `audit_handler`, `api_key_handler`, `jwks_handler`） | HTTP プロトコル変換 |
