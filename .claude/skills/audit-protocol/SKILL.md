...
#### orphan 検出（2 系統: code-orphan / docs-orphan）

orphan は**発生源と影響範囲が異なる 2 系統**で別々に集計する（合算しない）。判定基準の正典は `audit_criteria.md` §A 軸 orphan 定義。

| 系統 | 走査範囲 | 影響 | 出力ファイル |
|---|---|---|---|
| **code-orphan** | `src/infra/deploy/tools/examples` | コードが無効な ADR ID を参照、実装と設計の乖離 | `audit-evidence/<date>/orphans-adr.txt` |
| **docs-orphan** | `docs/` 全体 | 採用検討者が docs を読んで存在しない ADR を探す事故、統合 / 改名 / 削除の追跡漏れ | `audit-evidence/<date>/docs-orphans-adr.txt` |

両系統とも `tools/audit/lib/coverage.sh` の ADR セクションで自動化。手順:

1. ADR ファイル ID 集合: `ls docs/02_構想設計/adr/ | grep -oE 'ADR-([0-9]{4}|[A-Z][A-Z0-9]*-[0-9]+)' | sort -u`（旧形式 4 桁通し番号 + 新形式カテゴリ別の両対応）
2. コード参照 ID 集合: `grep -rohE 'ADR-([0-9]{4}|[A-Z][A-Z0-9]*-[0-9]+)' src/ infra/ deploy/ tools/ examples/ --exclude-dir=audit`（self-detection 防止に audit dir 除外必須）
3. docs 参照 ID 集合: `grep -rohE 'ADR-([0-9]{4}|[A-Z][A-Z0-9]*-[0-9]+)' docs/`
4. code-orphan = (2) − (1) → `orphans-adr.txt`
5. docs-orphan = (3) − (1) → `docs-orphans-adr.txt`
6. AUDIT.md に両系統の件数を併記
