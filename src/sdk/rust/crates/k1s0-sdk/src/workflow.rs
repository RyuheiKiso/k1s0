...
use crate::proto::k1s0::tier1::workflow::v1::{
    workflow_service_client::WorkflowServiceClient, CancelRequest, GetStatusRequest,
    GetStatusResponse, QueryRequest, SignalRequest, StartRequest, TerminateRequest,
};
...
impl WorkflowFacade {
    ...
    pub async fn start(
        ...
        let resp = self.raw.start(StartRequest {
    ...
    pub async fn query(
        ...
        let resp = self.raw.query(QueryRequest {
            workflow_id: workflow_id.to_string(),
    ...
    pub async fn terminate(&mut self, workflow_id: &str, reason: &str) -> Result<(), Status> {
        self.raw.terminate(TerminateRequest {
            workflow_id: workflow_id.to_string(),
            reason: reason.to_string(),
            context: Some(self.client.tenant_context()),
        }).await?;
    ...
    pub async fn get_status(&mut self, workflow_id: &str) -> Result<GetStatusResponse, Status> {
        Ok(self.raw.get_status(GetStatusRequest {
            workflow_id: workflow_id.to_string(),
            context: Some(self.client.tenant_context()),
        }).await?.into_inner())
    }
