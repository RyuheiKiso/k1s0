...
use crate::proto::k1s0::tier1::pii::v1::{
    pii_service_client::PiiServiceClient, ClassifyRequest, MaskRequest, PiiFinding,
};
...
impl PiiFacade {
    ...
    pub async fn classify(&mut self, text: &str) -> Result<(Vec<PiiFinding>, bool), Status> {
        let resp = self.raw.classify(ClassifyRequest {
            text: text.to_string(),
    ...
    pub async fn mask(&mut self, text: &str) -> Result<(String, Vec<PiiFinding>), Status> {
        let resp = self.raw.mask(MaskRequest {
            text: text.to_string(),
            context: Some(self.client.tenant_context()),
        }).await?.into_inner();
