...
// Config は notification-hub のトップレベル設定。
type Config struct {
	// サービスバージョン。
...
// BindingsConfig は通知チャネルごとに使う Dapr Binding Component 名を保持する。
type BindingsConfig struct {
	// email チャネル用 Binding Component 名（例: "smtp-outbound"）。
	Email string
	// slack チャネル用 Binding Component 名（例: "slack-webhook"）。
	Slack string
	// webhook チャネル用 Binding Component 名（汎用 HTTP 出力、例: "http-outbound"）。
	Webhook string
}
...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		K1s0: K1s0Config{
			...
			// 主体識別子。
			Subject: getenvDefault("K1S0_SUBJECT", "tier2/notification-hub"),
			// TLS 利用フラグ。
		...
		Bindings: BindingsConfig{
			// email チャネル。
			Email: getenvDefault("BINDING_EMAIL", "smtp-outbound"),
			// slack チャネル。
			Slack: getenvDefault("BINDING_SLACK", "slack-webhook"),
			// webhook チャネル（汎用）。
			Webhook: getenvDefault("BINDING_WEBHOOK", "http-outbound"),
		},
	}
	// 必須項目を検証する。
