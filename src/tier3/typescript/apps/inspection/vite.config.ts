// k1s0 検査 tier3 SPA Vite 設定
// 検査ドメイン UI（品質検査 / 不良票 / 図面 review / 在庫）向けのビルド設定を定義する
...
export default defineConfig({
  ...
  // 開発サーバー設定
  server: {
    // ポートを 5175 に固定する（inspection 固有ポート）
    port: 5175,
    // BFF へのプロキシ設定（開発時に CORS を回避する）
    proxy: {
      // /auth/ 配下のリクエストを inspection BFF（:8082）にプロキシする
      "/auth": {
        target: "http://localhost:8082",
        // リバースプロキシ時に Host ヘッダーを書き換える
        changeOrigin: true,
      },
      // /api/ 配下のリクエストを inspection BFF（:8082）にプロキシする
      "/api": {
        target: "http://localhost:8082",
        // リバースプロキシ時に Host ヘッダーを書き換える
        changeOrigin: true,
