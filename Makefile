...
.DEFAULT_GOAL := help
.PHONY: help codegen codegen-check openapi openapi-check grpc-docs grpc-docs-check lint pre-commit lint-proto verify verify-quick verify-cones verify-e2e verify-conformance verify-portability doctor clean
...
verify-e2e: ## E2E テスト実行（事前に tools/local-stack/up.sh --role tier1-go-dev で kind 起動が必要、未起動なら個別 test が Skip）
	@cd tests/e2e && go test ./scenarios/... -v -timeout=10m
