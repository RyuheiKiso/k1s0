...
## scope（リリース時点 minimum、最小完動）

リリース時点では以下を実装済 (`go run ./cmd/example-facade` で起動可能):

1. `go.mod` (k1s0 monorepo path-style: `github.com/k1s0/k1s0/examples/tier1-go-facade`)
2. `cmd/example-facade/main.go` — gRPC server + 標準 health protocol + reflection +
   SIGINT/SIGTERM graceful shutdown
3. `Dockerfile` (Linux multi-stage、distroless / nonroot)
4. `.dockerignore`

**未実装 (採用初期に拡張予定):**

- proto 定義 → handler 登録の完動例 (現状は health protocol のみ登録)
- Dapr Go SDK adapter (`internal/adapter/dapr/`) 経由の State / PubSub / Secrets 呼出例
- OTel interceptor 接続例 (trace / metrics / logger)
