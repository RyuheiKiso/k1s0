...
| 段階 | 提供範囲 |
|---|---|
| リリース時点 | 最小完動: `cmd/example-service/main.go` (HTTP + JWT 認証 + tier1 SDK State.Save) + `Dockerfile` + `catalog-info.yaml` + `go.mod` |
| 採用初期 | `internal/{domain,application,infrastructure}` 4 層分離 + Pact 契約テスト |
| 採用後の運用拡大時 | OutBox / Saga / Idempotency Key の典型例 |

`go run ./cmd/example-service -listen :8080 -tier1-target localhost:50001` で起動できる。
