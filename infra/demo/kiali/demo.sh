...
scenario_fault() {
  ...
  echo "  - Error Rate panel should rise for order-bff -> order-server"
  echo "  - P99 Latency should spike because of the 500ms delay"
  echo "  - Response Codes pie should include 503"
  echo ""
  echo "  [Kiali Guide]"
  echo "  - Graph > edges to order-server turn red/orange (errors)"
  echo "  - Click order-server > Health > compare error spikes with Grafana"
}
...
scenario_logs() {
  ...
  echo "  - Logs include trace_id, span_id, service, and request_id labels"
  echo "  - Click a trace_id derived field to jump to Jaeger"
  echo ""
  echo "  [Dashboards]"
...
scenario_kafka() {
  ...
  echo ""
  echo "  [Grafana Guide] http://localhost:3200"
  echo "  - Open 'k1s0 Demo / k1s0 Kafka Overview'"
  echo "  - Confirm topic partitions, consumer lag, and order-events throughput"
  echo ""
  echo "  [Kiali Guide]"
