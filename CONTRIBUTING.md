...
| 接頭辞 | 用途 | 例 |
| --- | --- | --- |
| `feat/` | 新しい機能の追加 | `feat/order/add-status-filter` |
| `fix/` | バグの修正 | `fix/order/total-calculation-error` |
| `chore/` | 依存ライブラリの更新、CI設定変更など | `chore/update-dependencies` |
| `docs/` | ドキュメントの追加・修正 | `docs/order/update-api-spec` |
| `refactor/` | 動作を変えないコード改善 | `refactor/order/extract-usecase` |
| `test/` | テストの追加・修正 | `test/order/add-integration-tests` |
...
| 種類 | 意味 | 例 |
| --- | --- | --- |
| `feat` | 新機能の追加 | `feat(order): 注文ステータス更新APIを追加` |
| `fix` | バグ修正 | `fix(order): ステータス遷移の検証を強化` |
| `chore` | 雑務（依存更新、設定変更等） | `chore: sqlx を 0.8.1 に更新` |
| `docs` | ドキュメントの変更 | `docs: APIエンドポイントの説明を追加` |
| `refactor` | 動作を変えないコード改善 | `refactor(order): カテゴリリポジトリのクエリを整理` |
| `test` | テストの追加・修正 | `test(order): 注文作成ユースケースの単体テストを追加` |
