...
fn test_library_swift_file_list() {
    ...
    assert!(
        names.iter().any(|n| n == "Package.swift"),
        "Package.swift missing"
    );
    assert!(
        names
            .iter()
            .any(|n| n == "Sources/shared_utils/Client.swift"),
        "Client.swift missing"
    );
    assert!(
        names
            .iter()
            .any(|n| n == "Sources/shared_utils/LibError.swift"),
        "LibError.swift missing"
    );
    assert!(
        names
            .iter()
            .any(|n| n == "Sources/shared_utils/Config.swift"),
        "Config.swift missing"
    );
    assert!(
        names
            .iter()
            .any(|n| n == "Tests/shared_utils_tests/ClientTests.swift"),
...
fn test_library_swift_has_lib_error() {
    ...
    assert!(
        content.contains("LibError"),
        "Swift library should contain LibError"
    );
    assert!(
        content.contains("enum LibError"),
        "Swift LibError should be an enum"
    );
}
...
fn test_library_swift_has_validate() {
    ...
    assert!(
        content.contains("validate"),
        "Swift Config should have validate() method"
    );
}
