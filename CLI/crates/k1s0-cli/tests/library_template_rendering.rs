...
fn test_library_python_file_list() {
    ...
    assert!(names.iter().any(|n| n.contains("pyproject.toml")), "pyproject.toml missing");
    assert!(names.iter().any(|n| n.contains("README.md")), "README.md missing");
    assert!(names.iter().any(|n| n.contains("__init__.py")), "__init__.py missing");
    assert!(names.iter().any(|n| n.contains("exceptions.py")), "exceptions.py missing");
    assert!(names.iter().any(|n| n.contains("test_")), "test file missing");
}
...
fn test_library_python_service_name_substitution() {
    ...
    assert!(content.contains("shared-utils") || content.contains("shared_utils"), "Service name not substituted in pyproject.toml");
}
...
fn test_library_python_pyproject_content() {
    let (tmp, _) = render_library("python");
    let content = read_output(&tmp, "pyproject.toml");
    assert!(content.contains("[project]"), "pyproject.toml should have [project] section");
    assert!(content.contains("python"), "pyproject.toml should reference python version");
}
...
fn test_library_python_has_error_class() {
    ...
    assert!(content.contains("Error"), "exceptions.py should contain Error class");
    assert!(content.contains("code"), "exceptions.py should have error code");
}
...
fn test_library_python_has_validate() {
    ...
    assert!(content.contains("validate"), "Python library should have validate method");
}
