...
pub fn run() -> Result<()> {
    // LOW-001 技術監査対応: 現状 k1s0 generate は TTY が必須のインタラクティブコマンドであり、
    // CI/CD パイプラインからの利用が不可能。
    // TODO(LOW-001): 将来的に --config-file オプションまたは設定ファイルベースの非インタラクティブモードを
    // 実装し、CI/CD パイプラインから k1s0 generate を利用可能にすること。
    // 参照: 報告書.md LOW-001、設定ファイル例: k1s0-generate-config.yaml
