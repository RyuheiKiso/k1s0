...
pub fn execute_init_at(config: &InitConfig, base_dir: &Path) -> Result<PathBuf> {
    // プロジェクト名に '..' が含まれる場合はパストラバーサルを防止する（Defense in Depth）
    if config.project_name.contains("..") {
        bail!("プロジェクト名に '..' を含めることはできません");
    }
    ...
    // CLI-HIGH-003 監査対応: resolve_scaffold_root() が Result を返すようになったため、
    // エラーを上流に伝播させる。
    ...
    // LOW-004 監査対応: k1s0 doctor / 各コマンドが k1s0.yaml を要求するため init 時に生成する。
    // k1s0.yaml はプロジェクトのメタ情報を保持する設定ファイル。
    fs::write(
        base.join("k1s0.yaml"),
        generate_k1s0_yaml(&config.project_name),
    )?;
...
/// # Errors
///
/// いずれの方法でもスキャフォールドルートが見つからない場合にエラーを返す。
fn resolve_scaffold_root() -> Result<PathBuf> {
    ...
    // CLI-HIGH-003 監査対応: cargo install でのバイナリ配布時に CARGO_MANIFEST_DIR が
    // 実行環境に存在しないため panic する問題を修正する。
    ...
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "スキャフォールドルートが見つかりません。K1S0_SCAFFOLD_ROOT 環境変数を設定してください。\n\
                 例: export K1S0_SCAFFOLD_ROOT=/path/to/k1s0-workspace"
            )
        })
}
...
// LOW-004 監査対応: k1s0 init 時に k1s0.yaml を自動生成する。
// k1s0 doctor / config コマンドがプロジェクトルートの k1s0.yaml を必要とするため。
// MED-019: config-schema-schema.json に合わせて version を整数（1）に修正し、
// 必須フィールド（service, namespace_prefix, categories）を追加する。
// 旧: version: "0.1.0"（文字列）は config-schema の JSON Schema が integer を要求するため不正。
fn generate_k1s0_yaml(project_name: &str) -> String {
    format!(
        "# k1s0 プロジェクト設定ファイル（k1s0 init で自動生成）\n\
# config-schema-schema.json に準拠したスキーマバージョン（integer 型）\n\
project:\n\
  name: \"{project_name}\"\n\
  # version は整数型（u32）。config-schema の JSON Schema 検証で integer が要求される\n\
  version: 1\n\
  # Kubernetes namespace のプレフィックス（例: myproject.io）\n\
  namespace_prefix: \"{project_name}.io\"\n\
  # サービスの識別名（DNS ラベル形式: 小文字英数字とハイフンのみ）\n\
  service: \"{project_name}\"\n\
\n\
# 設定カテゴリ一覧（config-schema の required フィールド）\n\
# 各カテゴリに id・label・namespaces・fields を定義すること\n\
categories: []\n\
\n\
tiers:\n\
  - system\n\
